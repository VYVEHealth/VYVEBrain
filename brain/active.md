...
**Last verified:** 13 May 2026 PM-87 (PF-10 SHIPPED — wellbeing + monthly check-in Dexie-first reads). PM-78 shipped PF-1/2/3; PM-79 shipped PF-4/5; build chat shipped PF-6 (commit `48c4d17e`), PF-7 (commit `97863198`), PF-8 (commit `d2fc1e89`), PF-9 (commit `197a0d89`), and PF-10 (commit `9813e800`) directly to `local-first-spike`. PM-83/84/85 are scope-chat brain-only writes for PF-13/PF-24/PF-25/PF-30 — no build state change. PM-86 = PF-9 brain restore, PM-86.1 = base64 double-encoding repair (see §23 candidate).

| What | Value |
|---|---|
| vyve-site main HEAD | `ff3e0e0f` (PM-76 perf.js v2 promotion — unchanged; PF-1/2/3 on feature branch only) |
| vyve-site `local-first-spike` HEAD | `9813e800` (PF-1 through PF-10 — Dexie schema + hydrate + shadow outbound queue + since-cursor delta-pull + habits + workouts + nutrition + cardio + wellbeing-checkin + monthly-checkin Dexie-first reads, spike gated by `localStorage.vyve_lf_spike`; 10 commits ahead of `main`) |
| VYVEBrain main HEAD | this commit (PM-87 — PF-10 ship narrative; active.md + campaign.md advanced to PF-10 SHIPPED state; weekly_scores schema-extension queued as PF-30 companion candidate; auth.js relative-path pre-existing bug logged) |
| SW cache key (production) | `vyve-cache-v2026-05-12-pm76-perf-promote-a` (unchanged; spike branch cache key is `vyve-cache-v2026-05-13-pm78-pf10-checkins-a`) |
...
| Last shipped vyve-site commits | main unchanged: `fc8232bb` (PM-74), `5cef00a2` (PM-75), `ff3e0e0f` (PM-76). Spike branch: `8d07d26b` (PF-1), `e8f02742` (PF-2/3), `903127d6` (PF-4), `fa116ef2` (PF-5), `48c4d17e` (PF-6), `97863198` (PF-7), `d2fc1e89` (PF-8), `197a0d89` (PF-9), `9813e800` (PF-10). |
...
**Status at PM-87 commit:** PF-1 through PF-10 SHIPPED to `local-first-spike` branch (`9813e800`), 10 commits ahead of main, awaiting Dean's verification + merge. Spike covers Dexie schema (~27 tables), hydrate-on-login, shadow outbound queue (mirrors `writeQueued` into Dexie `_sync_queue` and runs parallel drainer), proper since-cursor delta-pull on visibilitychange, habits.html Dexie-first reads (3 sites flipped — member_habits join, daily_habits today, daily_habits 365-day dates), workouts surfaces Dexie-first reads with PM-77.3 thumbnail prefetch (4 sites in workouts-programme.js + 1 in workouts-session.js, three-way branch: Dexie when enabled+non-empty, Supabase fallthrough otherwise), nutrition surfaces Dexie-first reads (4 sites in nutrition.html + log-food.html: weight_logs, today's nutrition_logs, recent-foods nutrition_logs history, nutrition_my_foods), cardio.html Dexie-first reads (3 sites in `fetchWeek` + `fetchHistory` + per-page optimistic `VYVELocalDB.cardio.upsert` at POST-success for the PF-4b Part 2 direct-fetch-bypass case), and wellbeing-checkin + monthly-checkin Dexie-first reads (11 sites total: wellbeing-checkin fetchMemberData 5-of-6 + fetchThisWeekCheckin + optimistic wellbeing_checkins.upsert; monthly-checkin loadRecap all-6). `members` reads deliberately left on Supabase across all pages per PF-8 PF-4b Part 1 carve-out. `weekly_scores` left on Supabase in wellbeing-checkin: NOT in Dexie schema — extending the schema is queued as a PF-30 companion candidate (~30-45 min, only if telemetry shows the read is hot). AI-moment EF submits (wellbeing-checkin EF v29, monthly-checkin EF v18) stay on the wire as the deliberate server-compute carve-out — Anthropic API calls + persona-prompt rendering are not persistable client-side. Realtime cross-device merge (PF-5b) deferred until post-launch — single-device-per-user assumption (§0) makes this acceptable. Next: PF-11 (home dashboard, most visible win, hits index.html which currently dominates first-paint).
...
2.11b. **PF-11 through PF-20** — remaining page refactors + iOS hardening + nav restructure. Sequenced in playbook. PF-11 (home dashboard) is the most visible win and should land next.
