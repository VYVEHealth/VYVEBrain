...
- vyve-site main: `0022dc8e` (PM-58 ship — Layer 4 cardio canonical, 11 May 2026; **Layer 2 complete, Layer 3 complete, Layer 4 in-progress (1/8 surfaces), Layer 5 clock running**)
...
- Last shipped: `vyve-cache-v2026-05-11-pm58-layer4-cardio-a` (Layer 4 cardio canonical envelopes; bus.js v3 → v4)
...
**Audit-count baseline (publishing-surface call sites at HEAD `0022dc8e`, PM-58 Layer 4 cardio canonical — bus.js v3 → v4, +3556 chars; `recordCanonical` + 10s suppression map drops own-write Realtime echoes for canonical publishes; first per-surface Layer 4 wiring on cardio.html):**
...
- `VYVEBus.publish(`: 25 (was 23 in the PM-55 narrative — live whole-tree audit at PM-58 found 24 at HEAD `5de6b6f5` including a `workout:shared` publisher in workouts-programme.js that the PM-55 narrative undercounted; PM-58 adds 1 new `cardio:failed` publisher in cardio.html. Corrected baseline.)
- `VYVEBus.subscribe(`: 35 (was 33 at PM-57; PM-58 adds 2 new — cardio.html `cardio:failed` (revert subscriber, calls VYVEHomeState.revertPatch) and index.html `cardio:failed` (belt-and-braces cache invalidate). Distribution at HEAD `0022dc8e`: index.html ×16, engagement.html ×8, workouts.html ×3, monthly-checkin.html ×2, cardio.html ×2, plus one each in certificates, habits, nutrition, wellbeing-checkin.)
- `VYVEBus.recordWrite(`: 15 (UNCHANGED). **`VYVEBus.recordCanonical(`: 1 (NEW PM-58, cardio.html only — Layer 4 sibling discipline; mirrors recordWrite but with 10s TTL and dedicated map. Grows by 1 per Layer 4 surface migration.)**
...
- bus.js version: **v4 (PM-58)** — Layer 4 canonical-publish suppression. New API: `recordCanonical(table, pk)` mirrors `recordWrite` with 10s TTL on a dedicated `recentCanonicals` map. `handleRealtimeRow` checks both maps and drops echoes for own canonical writes (after the existing `recordWrite` check). New `__inspect()` field: `recent_canonicals`. No new test harness — existing `__mockRealtimeFire` covers the suppression check, future Layer 4 self-tests slot into the existing test page. Envelope origins unchanged at 4 (`local`, `remote`, `realtime`, `realtime-resync`); kind:`canonical` is a new payload-level discriminator, not a new origin.
...
**Layer 4 is in progress (1/8 surfaces).** PM-58 shipped cardio.html — first per-surface Layer 4 wiring. Design call documented in changelog PM-58: chose option (c) direct client-side cache-patching over option (b) `evaluate_only`-round-trip. Rationale: every ms of network latency on the optimistic path degrades the premium-feel contract; the home_state math is bounded (four columns) and self-correcting on the next member-dashboard fetch. Remaining 7 surfaces: habits.html (3 publish sites incl. autotick), workouts.html / workouts-session.js / workouts-programme.js / workouts-builder.js (the workout family), log-food.html (3 publish sites, dual-op food:logged + food:deleted), nutrition.html (weight:logged), wellbeing-checkin.html (2 publish sites — live + flush), monthly-checkin.html (1 publish site), tracking.js (session_views/replay_views). Layer 5 telemetry clock still running (target 18 May 2026); Layer 6 SPA-shell still gated on that data.
...
- 🛠 **NEXT P0 — Layer 4 surface migrations (7 remaining).** Suggested order by complexity/risk: (1) **habits.html** — 3 publish sites (live tick, untick, autotick), `daily_habits` table, simplest writeQueued surface. (2) **nutrition.html** — 1 publish site, `weight_logs`, dual-op surface (PM-51 INSERT+UPDATE) so the revert math is straightforward. (3) **tracking.js** — `session_views`/`replay_views`, INSERT-only writes with heartbeat PATCH downstream. (4) **workouts family** — workouts-session.js + workouts-programme.js + workouts-builder.js + movement.html, 8 publish sites total. Likely 2 sessions to fully wire. (5) **log-food.html** — 3 publish sites, dual-op `food:logged` + `food:deleted` (revert path for deletes is the cleanest place to validate the failed-publish + revert pattern symmetrically). (6) **wellbeing-checkin.html** — 2 publish sites, server-side EF writer. (7) **monthly-checkin.html** — 1 publish site, server-side EF writer. writeQueued return-shape extension (4xx eager / 5xx defer-to-outbox-dead) lands on first writeQueued surface migration. Per-surface migration; 1 session per surface or 1-2 surfaces per session as scope allows.
