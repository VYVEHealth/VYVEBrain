...
**Last verified:** 13 May 2026 PM-86 (PF-9 SHIPPED retroactively + brain restore after PM-83/84/85 scope-chat drift). PM-78 shipped PF-1/2/3; PM-79 shipped PF-4/5; build chat shipped PF-6 (commit `48c4d17e`), PF-7 (commit `97863198`), PF-8 (commit `d2fc1e89`), and PF-9 (commit `197a0d89`) directly to `local-first-spike`. PM-83/84/85 are scope-chat brain-only writes for PF-13/PF-24/PF-25/PF-30 — no build state change.

| What | Value |
|---|---|
| vyve-site main HEAD | `ff3e0e0f` (PM-76 perf.js v2 promotion — unchanged; PF-1/2/3 on feature branch only) |
| vyve-site `local-first-spike` HEAD | `197a0d89` (PF-1 through PF-9 — Dexie schema + hydrate + shadow outbound queue + since-cursor delta-pull + habits + workouts + nutrition + cardio Dexie-first reads, spike gated by `localStorage.vyve_lf_spike`; 9 commits ahead of `main`) |
| VYVEBrain main HEAD | this commit (PM-86 — brain restore: PF-9 ship narrative recorded retroactively, active.md + campaign.md advanced to PF-9 SHIPPED state; PM-83/84/85 scope writes preserved unchanged) |
| SW cache key (production) | `vyve-cache-v2026-05-12-pm76-perf-promote-a` (unchanged; spike branch cache key is `vyve-cache-v2026-05-13-pm78-pf9-cardio-a`) |
...
| Last shipped vyve-site commits | main unchanged: `fc8232bb` (PM-74), `5cef00a2` (PM-75), `ff3e0e0f` (PM-76). Spike branch: `8d07d26b` (PF-1), `e8f02742` (PF-2/3), `903127d6` (PF-4), `fa116ef2` (PF-5), `48c4d17e` (PF-6), `97863198` (PF-7), `d2fc1e89` (PF-8), `197a0d89` (PF-9). |
...
**Status at PM-86 commit:** PF-1 through PF-9 SHIPPED to `local-first-spike` branch (`197a0d89`), 9 commits ahead of main, awaiting Dean's verification + merge. Spike covers Dexie schema (~27 tables), hydrate-on-login, shadow outbound queue (mirrors `writeQueued` into Dexie `_sync_queue` and runs parallel drainer), proper since-cursor delta-pull on visibilitychange, habits.html Dexie-first reads (3 sites flipped — member_habits join, daily_habits today, daily_habits 365-day dates), workouts surfaces Dexie-first reads with PM-77.3 thumbnail prefetch (4 sites in workouts-programme.js + 1 in workouts-session.js, three-way branch: Dexie when enabled+non-empty, Supabase fallthrough otherwise), nutrition surfaces Dexie-first reads (4 sites in nutrition.html + log-food.html: weight_logs, today's nutrition_logs, recent-foods nutrition_logs history, nutrition_my_foods), and cardio.html Dexie-first reads (3 sites in `fetchWeek` + `fetchHistory`: weekly_goals cardio-week count, workouts week count, cardio history slice 10) plus per-page optimistic `VYVELocalDB.cardio.upsert` at POST-success to cover the PF-4b Part 2 direct-fetch-bypass case. `members` reads deliberately left on Supabase in PF-8 due to read-after-write hazard from optimistic-write-to-Dexie gap (see PF-4b below). Realtime cross-device merge (PF-5b) deferred until post-launch — single-device-per-user assumption (§0) makes this acceptable. Next: PF-10 (weekly-checkin + monthly-checkin Dexie-first reads, with the AI-moment EF submit deliberately staying on the wire as server-compute carve-out).
