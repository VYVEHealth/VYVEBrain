...
**SESSION HANDOFF — 2026-05-16 (PM-150):** Page-by-page paint audit, index.html. Shipped PM-150 — session_views storage/cap decoupled + 60s dwell threshold. Migration applied (rerouting `cap_*` triggers dropped, redundant `counter_sessions` dropped, `cert_sessions_count` single-writer + backfilled, 2/day cap preserved in `update_cert_sessions_count` only). tracking.js v9 shipped `9a95ab5c` (vyve-site main) — 60s cumulative dwell threshold before a session view inserts; build banner Update 15, sw key `pm150-session-threshold-a`. Device-verified end-to-end. New rule §23.34. Two feature backlog items added (`page_visits` owned analytics; `session_schedule` + live-session minute-windowing) + two hardening items (cross-visit dwell accumulation; tracking.js outbox). Full detail: changelog PM-150, master §19 PM-150, backlog PM-150 section.

**⚠️ ACTIVE.MD IS STALE — REBUILD NEEDED.** This file was last rebuilt PM-122 (15 May) and predates PM-128–150 (the local-first paint sessions, the PM-149 incident, and PM-150). §2 live-state snapshot below is a session behind — trust master.md §19 PM-150/PM-149/PM-148b/PM-147 over §2 here. Next session should rebuild active.md against current master before relying on it.
