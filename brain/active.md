...
| VYVEBrain main HEAD | this commit (14 May 2026 daytime — PM-104: §2 snapshot rebuilt to reflect today's three ships (PM-97 reset-rehydrate fix, PF-34 certs wire, PF-34 movement wire), three audit-only outcomes (running-plan, sessions, leaderboard), the PF-14c offline cold-boot diagnostic, and the SW-cache-key drift between brain and live noticed at session-start). |
| SW cache key (production) | `vyve-cache-v2026-05-14-pf34-movement-a` (PF-34 sweep ship 2/3 — movement.html Dexie-first wire + 4 optimistic upserts. Bumped today through pm97-reset-rehydrate-a → pf34-certs-a → pf34-movement-a. NOTE: a pre-today live key `pm101-ship-a` existed on `main` that this file did not reflect — PM-98..PM-101 shipped without active.md §2 refresh. State now realigned.) |
