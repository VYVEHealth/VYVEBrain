...
**PM-187 Connect Phase 2 — steps 1-6 SHIPPED (21 May 2026)** — `d439477f` on vyve-site. Hub + 3 sub-pages (`connect-checkin.html`, `connect-feed.html`, `connect-challenge.html`) live; both EFs (`connect-challenge-summary`, `connect-feed-counts`) deployed v1 ACTIVE; client EF wiring with 60s `_kv` cache lifecycle landed. Only step 7 (sub-page audit on sessions.html + leaderboard.html) remaining. Recap entries in changelog at session top.
