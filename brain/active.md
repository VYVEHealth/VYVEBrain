...
**Last verified:** 13 May 2026 PM-89 (PF-11b SHIPPED — index.html client-side member_home_state computation; PF-11 complete). PM-78 shipped PF-1/2/3; PM-79 shipped PF-4/5; build chat shipped PF-6 (commit `48c4d17e`), PF-7 (commit `97863198`), PF-8 (commit `d2fc1e89`), PF-9 (commit `197a0d89`), PF-10 (commit `9813e800`), PF-11a (commit `8b79c54d`), and PF-11b (commit `a248bfef`) directly to `local-first-spike`. PM-83/84/85 are scope-chat brain-only writes for PF-13/PF-24/PF-25/PF-30 — no build state change. PM-86 = PF-9 brain restore, PM-86.1 = base64 double-encoding repair (see §23 candidate). PM-88 split PF-11; PM-89 completes it.

| What | Value |
|---|---|
| vyve-site main HEAD | `ff3e0e0f` (PM-76 perf.js v2 promotion — unchanged; PF-1/2/3 on feature branch only) |
| vyve-site `local-first-spike` HEAD | `a248bfef` (PF-1 through PF-11 — Dexie schema + hydrate + shadow outbound queue + since-cursor delta-pull + habits + workouts + nutrition + cardio + wellbeing-checkin + monthly-checkin Dexie-first reads + index.html full Dexie-derived dashboard via new home-state-local.js module porting refresh_member_home_state SQL function, spike gated by `localStorage.vyve_lf_spike`; 12 commits ahead of `main`) |
| VYVEBrain main HEAD | this commit (PM-89 — PF-11b ship narrative; active.md + campaign.md advanced to PF-11 COMPLETE state; new home-state-local.js module ports refresh_member_home_state + compute_engagement_components SQL functions to JS; engagement score + streaks + recent counts now computed client-side from Dexie) |
| SW cache key (production) | `vyve-cache-v2026-05-12-pm76-perf-promote-a` (unchanged; spike branch cache key is `vyve-cache-v2026-05-13-pm78-pf11b-home-b`) |
...
| Last shipped vyve-site commits | main unchanged: `fc8232bb` (PM-74), `5cef00a2` (PM-75), `ff3e0e0f` (PM-76). Spike branch: `8d07d26b` (PF-1), `e8f02742` (PF-2/3), `903127d6` (PF-4), `fa116ef2` (PF-5), `48c4d17e` (PF-6), `97863198` (PF-7), `d2fc1e89` (PF-8), `197a0d89` (PF-9), `9813e800` (PF-10), `8b79c54d` (PF-11a), `a248bfef` (PF-11b). |
...
**Status at PM-89 commit:** PF-1 through PF-11 COMPLETE on `local-first-spike` branch (`a248bfef`), 12 commits ahead of main, awaiting Dean's verification + merge. Spike covers Dexie schema (~27 tables), hydrate-on-login, shadow outbound queue (mirrors `writeQueued` into Dexie `_sync_queue` and runs parallel drainer), proper since-cursor delta-pull on visibilitychange, habits.html Dexie-first reads (3 sites flipped), workouts surfaces Dexie-first reads with PM-77.3 thumbnail prefetch (4+1 sites), nutrition surfaces Dexie-first reads (4 sites), cardio.html Dexie-first reads (3 sites + per-page optimistic `VYVELocalDB.cardio.upsert` at POST-success), wellbeing-checkin + monthly-checkin Dexie-first reads (11 sites total + optimistic wellbeing_checkins.upsert at submit), index.html cold-start Dexie-derived paint (PF-11a), and **PF-11b — index.html client-side member_home_state computation** via new `home-state-local.js` module porting `refresh_member_home_state` + `compute_engagement_components` SQL functions to JS over Dexie data. Engagement score, all 4 components, all 6 streaks (overall + 5 types), recent_30d counts, weekly goal progress — all now Dexie-derived on cold-start. `members` reads still on Supabase across all pages per PF-8 PF-4b Part 1 carve-out. `weekly_scores` not in Dexie schema (PF-30 companion candidate). AI-moment EF submits stay on the wire. `member-dashboard` EF still always fires post-Dexie-paint to upgrade `charity_total` (cross-member), `achievements`, `certificates`, and HK connection state — these stay server-only. A TTL-cache that skips the EF call when local data is fresh enough is the natural post-launch iteration. Realtime cross-device merge (PF-5b) deferred until post-launch. Next: PF-12 (settings + remaining surfaces, ~2 hours) on page-refactor track, OR PF-30 (telemetry redirect + observability stack — pre-launch recommended).
