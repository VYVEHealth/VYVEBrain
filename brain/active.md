...
0. **PF-40 — LOCAL-FIRST CONSOLIDATION CAMPAIGN (active meta-frame, supersedes per-page wire work).** 12 sub-items (PF-40.1 audit through PF-40.12 cleanup), ~13-16 Claude-assisted sessions total. **PF-40.1 audit SHIPPED PM-107 (2026-05-14)** — 321 call sites enumerated and classified; `audit/pf-40-1-callsites.json` + `playbooks/pf-40-local-first-consolidation.md` are the canonical artefacts. **PF-40.2 Part A SHIPPED PM-110 (2026-05-14 late night, commit `3d9abe44`)** — `?debug=hydrate` probe on `habits.html` + SW cache key bump. Drops planned `member_achievements` sync.js edit: live verification (`grep` + `GITHUB_LIST_COMMITS`) showed the pull entry has been in `sync.js` plan() L268-276 since `e8f02742` (13 May, PF-2). PM-108 Q8 lock and PM-109 brief both wrong; §23.14 fired as designed. **PF-40.2 Part B is the next ship after Dean walks the probe on `test1@test.com?debug=hydrate&spike=1` and pastes console output back.** Part B scope is conditional on what the probe reveals — Part A intentionally ships zero structural change. — 10 decisions locked PM-108 (`playbooks/pf-40-local-first-consolidation.md` §4.6 DEAN_DECISIONS_LOCKED) so scope is fixed. Fixes the Habits "undefined" canary at the root. PF-14d, PF-14e, PF-15.write-optimistic, PF-31, PF-32, PF-33, PF-34 partial, PF-34b, PF-35, PF-36 all fold into PF-40 as symptoms. PF-14b stays separate. See `tasks/backlog.md` PF-40 section for full sub-item list + dependency graph + device-requirement table.
