...
| Active campaign | **Premium Feel Migration (local-first via Dexie)** — **PF-40 Local-First Consolidation** sub-phase. PF-40.1 audit SHIPPED PM-107. PF-40.2 unblocked by **10 Dean decisions locked PM-108** (see `playbooks/pf-40-local-first-consolidation.md` §4.6 DEAN_DECISIONS_LOCKED). PF-40.2 (fat-row member-scoped hydrate) is the next ship. |
...
0. **PF-40 — LOCAL-FIRST CONSOLIDATION CAMPAIGN (active meta-frame, supersedes per-page wire work).** 12 sub-items (PF-40.1 audit through PF-40.12 cleanup), ~13-16 Claude-assisted sessions total. **PF-40.1 audit SHIPPED PM-107 (2026-05-14)** — 321 call sites enumerated and classified; `audit/pf-40-1-callsites.json` + `playbooks/pf-40-local-first-consolidation.md` are the canonical artefacts. **PF-40.2 (fat-row member-scoped hydrate) is the next ship** — 10 decisions locked PM-108 (`playbooks/pf-40-local-first-consolidation.md` §4.6 DEAN_DECISIONS_LOCKED) so scope is fixed. Fixes the Habits "undefined" canary at the root. PF-14d, PF-14e, PF-15.write-optimistic, PF-31, PF-32, PF-33, PF-34 partial, PF-34b, PF-35, PF-36 all fold into PF-40 as symptoms. PF-14b stays separate. See `tasks/backlog.md` PF-40 section for full sub-item list + dependency graph + device-requirement table.
