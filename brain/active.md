...
**Last verified:** 2026-05-14 late-night session PM-111 — session close after device walk on `test1@test.com` exposed the actual architecture problem. Two vyve-site ships this session (PM-110 `3d9abe44` probe + PM-110-followup `83521f27` spike-on-default). Device walk findings: (1) **PM-106 canary explained** — "undefined" cards are a transient cache-first-paint state caused by render-template/cache-writer shape mismatch in habits.html (cache flat, template nested). Not a Dexie hydrate gap. Three sessions misdiagnosed this. (2) **Spike-on default works** — `VYVELocalDB.isEnabled()` now returns true for every member, but exposed that `sync.js` mass-hydrate takes **81 seconds for a near-empty test account** because it pulls 23 tables in 6 sequential batches of 4 with no per-request timeout, plus 4 tables fail (schema drift since PF-2 on 13 May). (3) **The PF-40 campaign as scoped (13-16 sessions, 321 call sites, fat-row hydrate, write API, catalogue residency) was the wrong scaffolding.** The actual problem is "first paint must be fast and instant-from-Dexie thereafter" — Dean's original brief. That needs a thin first-paint hydrate (4-5 critical tables per page, parallel, 5s timeout), not a structural campaign. **Next ship is `firstPaintHydrate.js` exporting `criticalHydrate(pageName)` — see PM-111 changelog for full scope.** Spike-on default stays live; rolling back would lose the local-first architectural commitment. The 81s mass-hydrate is the lazy background path; after `firstPaintHydrate` lands, it's invisible to the member because the page is already painted from the thin hydrate.

**Last verified:** 2026-05-14 late-night session PM-110 — PF-40.2 Part A SHIPPED. vyve-site commit `3d9abe44`: query-param-gated diagnostic probe in `habits.html` (`?debug=hydrate`) + SW cache key bump (`pf14c-precache-a` → `pf40-2-probe-a`). Probe captures the full `hydrate()` → Dexie chain for the PM-106 Habits "undefined" canary: localStorage `vyve_lf_spike` raw value, `VYVESync.hydrate()` resolution time + return value, full first-row dump of `member_habits` Dexie rows post-await pre-render (with field-by-field `=== undefined` check on the five `habit_library` join cols), plus same row re-fetched at +3s and +10s. All output prefixed `[VYVE-PROBE]` for iOS Safari console grep. Inert in production. **Critical live-state surprise:** PM-108 Q8 lock + PM-109 brief both said "add `member_achievements` to sync.js plan()" — live state shows it has been there since `e8f02742` (13 May, PF-2). Verified via `grep` on live `sync.js` + `db.js` + `GITHUB_LIST_COMMITS`. Part 2 of the brief dropped (no-op). The Q8 audit finding was an unverified assertion propagated PM-108 → PM-109 → PM-110 brief without anyone reading the live file. §23.14 fired as designed. **Test walk:** Dean opens `habits.html?debug=hydrate` on iPhone with `test1@test.com`, spike ON, pastes console output. Part B (structural fix) lands with ground truth.

**Last verified:** 2026-05-14 night session PM-109 — PF-40.2 diagnostic dive + brain bookkeeping. No vyve-site ships. Brain commit only: master.md §23.14 (parallel-session collision discipline) added, changelog PM-109 prepended, active.md §2 + §5 patched. Key findings carried forward into the next session: (1) live Supabase schema fetched for all 22 member-scoped + catalogue tables — see PM-109 changelog entry; (2) `member_habits` is the ONLY genuine join in scope for PF-40.2 fat-row hydrate (the rest were JSONB-blob field accesses, not joins); (3) the PM-106 Habits "undefined" canary cannot be root-caused without instrumentation — the full chain (pull query, FK declared, replaceForMember denormalisation, db.js merge override) all check out on inspection, yet PM-106 saw `undefined` cards. (4) DEAN_DECISION Q3 implies engagement cache via `_kv` lands in PF-40.2 alongside the fat-row work, not PF-40.3. **The next ship is PF-40.2 Part A: ship the diagnostic probe on habits.html (`?debug=hydrate` query-param gated logger), bump SW cache key, add `member_achievements` to sync.js plan() (Q8 lock).** Solo, no device required. Part B is Dean walking `test1@test.com?debug=hydrate&spike=1` on iPhone and pasting console output back, after which PF-40.2's structural fix lands with ground truth.

**Last verified:** 2026-05-14 session PM-107 — PF-40.1 audit shipped. Read-only ship; brain-only commit; no vyve-site changes. Audit enumerated 321 unique network / local-DB call sites across 78 HTML+JS files (40,400 lines) at vyve-site main `66f02b84`. Classification: 73 R_MEMBER + 4 R_CATALOGUE + 60 LOCAL_READ (133 read sites for PF-40.5 to migrate); 21 LOCAL_UPSERT + 13 W_QUEUED + 21 W_MEMBER (55 write sites for PF-40.4); 36 §23.10 NET_BOUND carve-outs for PF-40.11 offline UX; 5 dead-code candidates (4 log-perf EF, 1 push_subscriptions/vapid.js) flagged for PF-40.12. **Key finding:** `window.VYVEData` already exists in `vyve-offline.js` with `cacheGet/cacheSet/fetchCached/writeQueued/outboxFlush` — PF-40.4/40.5 is evolving the existing API, not building from scratch. Deliverables committed: `audit/pf-40-1-callsites.json` (129KB structured map keyed by file:line) + `playbooks/pf-40-local-first-consolidation.md` (21KB living campaign reference). PF-40.2 is the next ship — split into Part A (diagnostic probe + sync.js member_achievements entry, solo) and Part B (structural fat-row fix after Dean walks the probe on test1@test.com) — fixes the Habits "undefined" canary at the root.

**Prior verified (PM-106):** 14 May 2026 evening session PM-106 — verification walk on `test1@test.com` (fresh-onboarding canary) followed by an architectural conversation that re-shaped the campaign. **No vyve-site ships this session.** PM-97 reset-rehydrate verified inertly (spike was OFF at first visit; toast read "Local-first OFF · Nothing to reset" per the spike-gate check); flipped spike ON via 7-tap on version footer; navigated home → exercise → habits. Exercise cold-load ~15-20s consistent with §23.5.1 EF backend latency, not a regression. **Habits surfaced the campaign canary: page paints from Dexie immediately (per §23.7.1) with all habit cards rendering `undefined / undefined / undefined`; ~10s later EF backfill returns denormalised data and page re-renders correctly.** Root cause: `db.js pullOneTable('member_habits')` pulls the column subset native to `member_habits`; denormalised join columns owned by `habit_library` (name, description, category, difficulty) are NOT in the hydrate. Page reads find rows but missing join columns → template prints `undefined`. PF-14c airplane-mode test deferred — same hydrate gap would have surfaced offline and given a misleading signal. The Habits canary is the read-path equivalent of the §23.7.5 partial-upsert landmine and the §23.7.8 reset-rehydrate rule — same family of bug from a different direction. **The conversation that followed re-shaped the campaign:** Dean stated the contract he wants (Dexie as complete reading source, offline-forever for non-network-bound surfaces, tiered asset strategy, single-device-per-user through 31 May launch). Three pushbacks raised (catalogue staleness, first-login hydrate cost, multi-device sync), all surmountable. Contract locked, §3 of this file replaced with the strengthened paragraph, three new §23 hard rules added (§23.11 hydrate completeness, §23.12 no page-level fetches, §23.13 tiered assets), and **PF-40 Local-First Consolidation Campaign scoped end-to-end as the meta-frame that supersedes per-page wire work.** 12 sub-items, ~13-16 Claude-assisted sessions total, PF-40.1 audit is the next ship (read-only, solo daytime, no device required). PF-14c (done), PF-14d, PF-14e, PF-15.write-optimistic, PF-31, PF-32, PF-33, PF-34 partial, PF-34b, PF-35, PF-36 all fold into PF-40 as symptoms. PF-14b stays separate (same review cycle, its own commit). See PM-106 changelog entry for full session narrative. Daytime PM-104/PM-105 ship narrative preserved below for context.

**Earlier today (PM-104 + PM-105):** three production ships landed sequentially during Dean's workday (read/plan/diagnostic + low-risk only, device verification deferred to evening). **PM-97 reset-rehydrate fix SHIPPED** (`361b44dc`, settings.html `resetLocalCache()` flipped from `hydrate()` to per-table `hydrateTable()` for `members`/`member_habits`/`workout_plan_cache` — bypasses the `hydratePromise` session-memoisation that made the original `await hydrate()` a no-op; failure now BLOCKS reload per §23.7.8). **PF-34 (1/3) certificates.html Dexie-first wire SHIPPED** (`be690345`, member-dashboard EF response shape built locally from `certificates` + raw activity tables + `members.first_name`; EF still fires in background for authoritative cert numbering + charity_total). **PF-34 (2/3) movement.html Dexie-first wire + 4 optimistic upserts SHIPPED** (`f8bc15cf`, fetchPlan read flips + per-page upserts on markDone workouts/wpc and logMovement walk-cardio/non-walk-workouts paths per PF-4b Part 2). **PF-34 (3/3) running-plan.html → audit-only outcome** — `member_running_plans` not in Dexie schema, `running_plan_cache` only exists in db.js as a future-use `_kv` mention (cross-member shared cache by `cache_key`, not member-scoped); proper wire needs schema work + cross-member sync rule, deferred as PF-34b. **sessions.html audit** — fully static catalogue, zero network calls, no wire needed. **leaderboard.html audit** — single `leaderboard` EF call computing cross-member aggregates server-side; cannot be local-first by nature; existing localStorage offline-cache pattern is the right shape. **PF-14c offline cold-boot fix SHIPPED** (`66f02b84`, PM-105 same-day follow-up). Causes A+B addressed: vendored `/dexie.min.js` (94KB from jsDelivr, dexie@4.0.10 verified) into the repo and changed `db.js` `DEXIE_CDN` constant to `/dexie.min.js`; added `/supabase.min.js`, `/dexie.min.js`, `/achievements.js` to `sw.js urlsToCache`. sw.js cache key: `pf34-movement-a` → `pf14c-precache-a`. Cause C (Promise.race timeout on `vyveLoadSupabaseSDK`) deferred as belt-and-braces — ship in a follow-up only if device verification reveals further hangs. Shipped without device verification: the change is offline-only-affecting (asset cache populates from precache list instead of from-first-fetch, but content identical), online happy path is byte-equivalent, so the ship can't make anything worse — if A+B are sufficient, offline cold-boot resolves; if there's a Cause D we missed, offline cold-boot still fails the same way. Verification: Dean airplane-mode-off iPhone, kill app, re-open offline tonight. **Brain drift surfaced**: pre-session SW cache key in this file said `pm97-pf15-merge-upsert-a` but live `sw.js` was actually on `pm101-ship-a` — PM-98..PM-101 shipped on main without active.md §2 being refreshed afterward. PM-103 entry below preserved for canary-walk findings context.

| What | Value |
|---|---|
| vyve-site main HEAD | `83521f27` (PM-110 followup: spike-on by default + habit_title undefined guards). Prior: `3d9abe44` (PF-40.2 Part A probe). Prior: `66f02b84` (PF-14c offline cold-boot precache + Dexie vendoring). Today's ships: `361b44dc` PM-97 reset-rehydrate fix → `be690345` PF-34 1/3 certificates Dexie wire → `f8bc15cf` PF-34 2/3 movement Dexie wire → `66f02b84` PF-14c precache fix. Earlier landings still on main: `ddc13271` PM-97 PF-15 partial-upsert seal, `433d0650` exercise.html PM-96 reference, spike merge `c469e504`. Note: the live SW cache key reflected `pm101-ship-a` at session-start, meaning at least one additional ship landed between PM-97 partial-upsert (`ddc13271`) and this morning that active.md did not record. Brain realigned to live state. |
...
| VYVEBrain main HEAD | this commit (2026-05-14 — PM-111: late-night session close. Reframes PF-40 around thin first-paint hydrate after device walk on test1 exposed the 81s mass-hydrate problem. State table refreshed. §3 active campaign rewritten. Next ship is firstPaintHydrate.js). Prior commit PM-110: PF-40.2 Part A ship narrative + Q8 verification gap finding + §23.14 strengthened with first concrete example). Prior commit PM-109 (PF-40.2 diagnostic dive). Prior PM-108 (10 decisions locked). Prior PM-107: PF-40.1 audit ship. New artefacts: `audit/pf-40-1-callsites.json` (321 call sites, classified) + `playbooks/pf-40-local-first-consolidation.md` (campaign reference). active.md §2/§5 patched; changelog.md PM-107 prepended; tasks/backlog.md PF-40.1 marked SHIPPED, PF-40.2 promoted to next-ship). Prior commit PM-106 (14 May 2026 evening) — contract codification + PF-40 scope. |
| SW cache key (production) | `vyve-cache-v2026-05-14-pf40-2-spike-default-a` (PM-110 followup — spike-on default). Prior: `pf40-2-probe-a` (PF-40.2 Part A probe). Prior: `pf14c-precache-a` (PF-14c — offline cold-boot precache fix. Bumped today through pm97-reset-rehydrate-a → pf34-certs-a → pf34-movement-a → pf14c-precache-a. NOTE: pre-session live key was `pm101-ship-a`, meaning PM-98..PM-101 shipped without active.md §2 refresh — realigned in PM-104 brain commit.) |
...
| Active campaign | **Premium Feel Migration (local-first via Dexie)** — **PF-40 Local-First Consolidation** sub-phase. PF-40.1 audit SHIPPED PM-107. PF-40.2 unblocked by **10 Dean decisions locked PM-108** (see `playbooks/pf-40-local-first-consolidation.md` §4.6 DEAN_DECISIONS_LOCKED). PF-40.2 is the next ship — split into Part A (diagnostic probe + sync.js member_achievements entry, solo) and Part B (structural fat-row fix after Dean walks the probe on test1@test.com). |
