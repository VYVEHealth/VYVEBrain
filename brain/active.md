...
**Last verified:** 13 May 2026 PM-88 (PF-11a SHIPPED — index.html cold-start Dexie-derived paint). PM-78 shipped PF-1/2/3; PM-79 shipped PF-4/5; build chat shipped PF-6 (commit `48c4d17e`), PF-7 (commit `97863198`), PF-8 (commit `d2fc1e89`), PF-9 (commit `197a0d89`), PF-10 (commit `9813e800`), and PF-11a (commit `8b79c54d`) directly to `local-first-spike`. PM-83/84/85 are scope-chat brain-only writes for PF-13/PF-24/PF-25/PF-30 — no build state change. PM-86 = PF-9 brain restore, PM-86.1 = base64 double-encoding repair (see §23 candidate). PM-88 splits PF-11 into PF-11a (cold-start Dexie paint, shipped) + PF-11b (client-side member_home_state, queued).

| What | Value |
|---|---|
| vyve-site main HEAD | `ff3e0e0f` (PM-76 perf.js v2 promotion — unchanged; PF-1/2/3 on feature branch only) |
| vyve-site `local-first-spike` HEAD | `8b79c54d` (PF-1 through PF-10 + PF-11a — Dexie schema + hydrate + shadow outbound queue + since-cursor delta-pull + habits + workouts + nutrition + cardio + wellbeing-checkin + monthly-checkin Dexie-first reads + index.html cold-start Dexie-derived paint, spike gated by `localStorage.vyve_lf_spike`; 11 commits ahead of `main`) |
| VYVEBrain main HEAD | this commit (PM-88 — PF-11a ship narrative; active.md + campaign.md advanced to PF-11a SHIPPED state; PF-11 split into PF-11a + PF-11b; PF-11b queued as the member_home_state client-side computation that PF-11a deferred) |
| SW cache key (production) | `vyve-cache-v2026-05-12-pm76-perf-promote-a` (unchanged; spike branch cache key is `vyve-cache-v2026-05-13-pm78-pf11a-home-a`) |
...
| Last shipped vyve-site commits | main unchanged: `fc8232bb` (PM-74), `5cef00a2` (PM-75), `ff3e0e0f` (PM-76). Spike branch: `8d07d26b` (PF-1), `e8f02742` (PF-2/3), `903127d6` (PF-4), `fa116ef2` (PF-5), `48c4d17e` (PF-6), `97863198` (PF-7), `d2fc1e89` (PF-8), `197a0d89` (PF-9), `9813e800` (PF-10), `8b79c54d` (PF-11a). |
...
**Status at PM-88 commit:** PF-1 through PF-10 + PF-11a SHIPPED to `local-first-spike` branch (`8b79c54d`), 11 commits ahead of main, awaiting Dean's verification + merge. Spike covers Dexie schema (~27 tables), hydrate-on-login, shadow outbound queue (mirrors `writeQueued` into Dexie `_sync_queue` and runs parallel drainer), proper since-cursor delta-pull on visibilitychange, habits.html Dexie-first reads (3 sites flipped), workouts surfaces Dexie-first reads with PM-77.3 thumbnail prefetch (4 sites in workouts-programme.js + 1 in workouts-session.js), nutrition surfaces Dexie-first reads (4 sites), cardio.html Dexie-first reads (3 sites + per-page optimistic `VYVELocalDB.cardio.upsert` at POST-success), wellbeing-checkin + monthly-checkin Dexie-first reads (11 sites total + optimistic wellbeing_checkins.upsert at submit), and **PF-11a — index.html cold-start Dexie-derived paint** via new `buildHomeFromDexie()` function. When `vyve_home_v3_<email>` localStorage cache is empty (cold-start with no prior visit on this device), Dexie tables synthesise a partial member-dashboard payload (progress counts, activity_log, weekly_goals + progress, habitDatesThisWeek, wellbeing.current_score, member, habit assignments) and render that immediately. `loadDashboard()` always fires to upgrade engagement.score + streaks + charity_total + achievements from the EF. Net win: cold-start paint goes from skeleton-for-7-17s to real-dashboard-with-actual-counts. `members` reads deliberately left on Supabase across all pages per PF-8 PF-4b Part 1 carve-out. `weekly_scores` not in Dexie schema (PF-30 companion candidate). AI-moment EF submits (wellbeing-checkin EF v29, monthly-checkin EF v18) stay on the wire as deliberate server-compute carve-outs. The member-dashboard EF call on index.html also stays — PF-11b (client-side member_home_state computation, ~2-3 hours) is queued to replace or make it optional. Realtime cross-device merge (PF-5b) deferred until post-launch — single-device-per-user assumption (§0) makes this acceptable. Next: PF-11b (member_home_state client-side) OR PF-12 (settings + remaining surfaces) depending on bandwidth.
...
2.11c. **PF-11b — index.html client-side member_home_state computation** queued. Replaces (or makes optional / TTL-cached) the member-dashboard EF call. Includes engagement_score formula port + streak computation + recent counts. ~2-3 hours. Pre-launch if bandwidth, post-launch otherwise.
2.11d. **PF-12 through PF-20** — settings + remaining surfaces + iOS hardening + nav restructure. Sequenced in playbook.
