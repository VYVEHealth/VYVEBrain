...
**Last verified:** 13 May 2026 PM-96 (PF-14 device verification COMPLETE on Dean's iPhone 17 + PF-15 part 2 fix shipped — local-first architecture VALIDATED on iOS Capacitor WKWebView; spike merged to main; exercise.html now Dexie-wired; six remaining unwired pages logged for PF-15 sweep; ITP risk locked in as launch blocker for PF-14b). PM-78..PM-93 history preserved above; tonight added PM-95 parts 1-4 (`82524093`, `fa3bd6e6`, `244d96fc`, `a3c74734`) + the spike→main merge (`c469e504`). PF-1..PF-13 scaffolding + PF-30 are now LIVE ON MAIN behind the spike-gate, dormant for non-spike members.

| What | Value |
|---|---|
| vyve-site main HEAD | `433d0650` (PM-96 PF-15 part 2 — exercise.html Dexie wiring. Earlier: `4ffe3d72` PM-96 diagnostic ship; `67711c4e` PF-14 part 6 hydrate-await; `a3c74734` PF-14 part 4 habits-cache; spike merge `c469e504`. Spike branch still redundant, slated for PF-19 cleanup deletion) |
...
| VYVEBrain main HEAD | this commit (PM-93 — verification-mode audit repair; brain advanced from PM-90 stale state to live PM-92 reality; PF-12 + PF-13 scaffolding narratives spliced into §5 + playbook; playbook PF-30 'identity pending' language stripped; new §4 hard rule on plain-UTF-8 in upserts content) |
| SW cache key (production) | `vyve-cache-v2026-05-13-pm96-pf15-exercise-dexie-a` (PM-96 PF-15 part 2 — exercise.html Dexie wiring; bumped through pm96-pf15-diag-a for the diagnostic ship) |
...
2.11g. **PF-15 — Hardening (in progress)**. Part 1 (PM-96, 13 May 2026): diagnostic IIFE added to dexie-source-indicator.js → device walk → root cause confirmed = page-side Dexie wiring gap (NOT hydrate timing). Part 2 (PM-96, 13 May 2026): `exercise.html` Dexie-wired (`433d0650`). Remaining P0 items, Sunday: (a) wire `movement.html` to Dexie (currently 0 VYVELocalDB refs, low-stakes empty-state page — bundle with PF-15 sweep), (b) wire `sessions.html` / `leaderboard.html` / `running-plan.html` / `certificates.html` / `engagement.html` to Dexie where they paint member data (per-page audit needed first; some may be legit REST-pass-through), (c) PF-12 partial-upsert merge fix in db.js (`member_habits.upsert` override should merge by default rather than overwrite), (d) localStorage shape-cache audit (`vyve_home_v3`, `vyve_programme_cache`, `vyve_wt_cache`, `vyve_food_diary`, `vyve_exercise_cache_v2` — all need version-bump or replacement audit). Plus original PF-15 scope: PM-77.1 mitigations A/B/C codification, force-resync escape hatch, queue drain batching, storage quota handling. Sunday session — likely 2 sittings.
