...
**PM-434 SHIPPED (30 May 2026) — Apple Health re-prompt now server-authoritative.** vyve-site main HEAD `64b1aabaf3536a88e5c0158904302103f06a72f6`. Returning users (re-install / consent never granted — Owen, already-had-old-app) were never re-prompted: the onboarding HK prompt lives in consent-gate.html (only brand-new members with no terms_accepted_at reach it), and the home-screen fallback banner (`index.html initHbReprompt`) (a) could run before bridge hydration and bail permanently, and (b) suppressed on the stale local `vyve_healthkit_has_connected` flag that `hydrateFromDashboard` leaves set even when the server says not-connected. Fix: banner now `await ensureHydrated()` then decides off server-authoritative `__VYVE_HEALTH_CONNECTION_STATE__` ('none' → re-prompt regardless of stale flag; undefined → fall back, no regression). 7-day dismiss cooldown unchanged; native-iOS only. consent-gate/vyveCheckConsent untouched (launch-critical path). sw `pm434-hk-reprompt-server-state-a`, vbb 313→314. NOT launch-blocking (wipe → all re-onboard fresh); matters post-launch for re-installs. Web → next OTA; needs on-device banner check at bundle cut.

**PM-433 SHIPPED (30 May 2026) — HealthKit cross-app cardio duplicate guard (server-side).** Migration `pm433_cardio_workouts_healthkit_dedupe_guard` + `sync-health-data` EF v17 (was v16). Root cause of the cardio "double information": HK workouts were promoted via plain INSERT with no semantic dedupe; the same physical workout reported by two HK source apps (Owen: Strava + Garmin Connect) arrives under two native_uuids → two cardio rows. Fixed by (a) removing 48 existing dupes (cardio 287→239; Owen 97→49), (b) unique indexes on workout identity — cardio `(member_email, source, cardio_type, logged_at)`, workouts `(member_email, source, logged_at)`, (c) EF promotion switched to upsert `ignoreDuplicates`. Verified: 0 dupes remain, guard rejects re-inserts. **Server-side — reaches all members immediately, no OTA.** Ties to the Owen returning-user thread (his manual HK accept triggered the doubling). New HK-pipeline invariant for master §6/§7. PAT-direct + Supabase MCP.

**PM-432 SHIPPED (30 May 2026) — light-mode mood panel contrast fix.** vyve-site main HEAD `4dd7b23a2f42ca8076bf88080c92f41d8a1e02fd`. `index.html` PM-285 mood panel: the `[data-theme="light"]` override flipped only the panel container to near-white; child text/icons stayed white-on-dark → invisible / sub-AA in light mode (the prominent home-screen "How are you feeling today?" surface). Added child light-overrides via semantic tokens (`--text`/`--text-muted`/`--text-faint`/`--teal`). Repo-wide audit of all 74 pages confirmed it was the ONLY white-on-light bug; everything else (accent buttons, photo/gradient heroes, `.calendar-tile` gradients, always-dark screens incl. `.barcode-overlay` #000) is correct. Two-file atomic, byte-verified at SHA: index.html (mood block + vbb 312→313) + sw.js → `pm432-lightmode-mood-a`. CSS-only. PAT-direct path (Composio still down).

**⚠ active.md §2 DRIFT (flagged PM-432) — needs rebuild.** The §2 narrative entries below top out at PM-409 (26 May), but live shipped through PM-432. PM-410..431 (the Movement V2 polish run, etc.) are all in `brain/changelog.md` but were never spliced into this working-set snapshot. Do NOT trust §2 narrative as complete between PM-409 and PM-432 — grep changelog for that range. Backfill/rebuild of §2 deferred to a dedicated brain-sync task.

**PM-409 SHIPPED (26 May 2026) — pre-bundle debug surface gating.** vyve-site `050db258`. Four-file atomic commit. `session-live.js` renderDebugStrip + PM-328 postMessage listener gated behind `localStorage.vyve_dev_panel_unlocked` (was always-on across all 8 live shells); `settings.html` "Reset achievements (debug)" → "Reset achievements", row hidden by default, revealed when dev panel unlocked; tap-5x-in-3s gesture on Build row added (mirrors settings-account.html App-version gesture). One flag now gates three surfaces: renderDebugStrip, Reset achievements row, settings-account Developer Tools. New §23.75 hard rule earned — pre-bundle debug surface gating discipline. sw cache `pm408-analytics-taxonomy-a` → `pm409-debug-gating-a`, vbb-marker 286 → 287 on both index.html and settings.html. Class B orphan debug pages (hk-diagnostic, perf-test, reset-cache, achievements-mockup-c/pathb) left in repo per audit — no nav links inward, bundling cost trivial, removal cost N commits each. `reset-cache.html` now functionally redundant with Force Refresh row — flagged for post-launch tidy. Class C console noise (212 warn / 91 error / 34 log) left as diagnostic trail per §23.75 exemption clause.

**PM-188 nav fix SHIPPED (21 May 2026, late in same session)** — vyve-site main HEAD `b9d625381080a880e707166b3f3fefe9260b3ef8`. Bottom nav slot 4 swapped from Sessions to Connect across `nav.js` (mobile + desktop). Connect sub-pages had duplicate `<div class="topbar">` blocks stacking with nav.js's mobile header — stripped on all three. Live Sessions accessible via More menu (was "Catch-Up Replays"). **Real gap caught:** Connect was built end-to-end across PM-186/187/188 but never wired into the bottom nav — members would have had no way to find it from the home page. Six-file atomic commit. The §23.48 work earlier this session was correct but tangential to the actual launch-blocker.

**PM-188 Connect Phase 2 — Step 7 SHIPPED (21 May 2026)** — vyve-site `0622db8e0d3622dccdabe81f8628e2f847af591a`. Sub-page audit on sessions.html + leaderboard.html landed. New §23 hard rule **§23.48 — Connect freshness model (four patterns)** codified. Phase 2 build queue closed.

**PM-187 Connect Phase 2 — steps 1-6 SHIPPED (21 May 2026)** — `d439477f` on vyve-site. Hub + 3 sub-pages (`connect-checkin.html`, `connect-feed.html`, `connect-challenge.html`) live; both EFs (`connect-challenge-summary`, `connect-feed-counts`) deployed v1 ACTIVE; client EF wiring with 60s `_kv` cache lifecycle landed.


**SESSION HANDOFF — 2026-05-21 (PM-188 nav fix: bottom nav Sessions → Connect, sub-page topbar duplication stripped).** Dean caught a gap I missed in step 7 scope. The §23.48 audit on sessions.html and leaderboard.html was real work, but the *launch-blocking* gap was that **Connect existed and was never findable from the bottom nav**. Step 7's sub-page audit framing focused me on the data-freshness patterns inside those two pages; it should have included nav surface integration as part of closing Phase 2. Lesson: a "Phase X is CLOSED" assertion is only true if a fresh member can navigate to every surface in Phase X via the canonical nav. **Connect Phase 2 only actually closes at this commit, not the previous one.** Updating the closure assertion accordingly.

**Shipped this nav fix commit** — six files atomic via Git Data API, §23.45 PAT-direct path (Composio still dead from morning's security incident):
- **`nav.js`** — `getActiveTab()` recognises `/connect*` paths and returns `'connect'` (added before sessions check so legacy `-live.html`/`-rp.html` still maps to `sessions` for full-screen pages); `hubPaths` gains `/connect.html`; `hubLabels` gains `connect: 'Connect'`; `subPageLabels` gains `'connect-checkin' / 'connect-feed' / 'connect-challenge'` with their proper page-header labels; `desktopLinks` slot 4 swapped Sessions → Connect; `navItems` slot 4 swapped with a new two-overlapping-silhouettes Connect icon (community signal at bottom-nav small size); More menu items retitled `Catch-Up Replays` → `Live Sessions` on both desktop dropdown and mobile bottom sheet.
- **`connect-checkin.html`, `connect-feed.html`, `connect-challenge.html`** — each had a hardcoded `<div class="topbar">` block with `topbar-back` button to `/connect.html` left over from sub-page shape lift. nav.js was injecting its own `.mobile-page-header` with `.mph-back-btn` at the same time → stacked double back buttons on mobile. Stripped the page-local topbar `<div>` from all three; nav.js is now the canonical mobile header source, matching journal.html / affirmations.html / breathwork.html which never had their own topbars. CSS class definitions retained in case other markup leans on them.
- **`sw.js`** — cache `pm188-step7-a` → `pm188-navfix-a`.
- **`index.html`** — vbb-marker Update 57 → 58.

**Production state at this commit.** vyve-site main HEAD `b9d625381080a880e707166b3f3fefe9260b3ef8`. Bundled production iOS 1.3 / Android 1.0.3 still at SHA `83874dd5` — unchanged. Dean's dev iPhone via `server.url` dev-loop picks up new bottom nav on next WKWebView cache cycle (2-15 min per §23.29; `?debug=build` to confirm). Connect EFs still v1 ACTIVE.

**Open considerations parked, not blocked on:**
- nav.js back-button parent-page awareness. nav.js's `.mph-back-btn` uses `history.back()` with `/index.html` fallback. The page-local topbars I stripped previously hardcoded `/connect.html` as fallback — semantically richer (a deep-link / hard-refresh hit on a connect sub-page should fall back to `/connect.html`, not `/index.html`). 99% of nav is tap-from-hub so history works fine; deep-link edge case still safe-but-non-ideal. Real fix candidate: nav.js reads `<meta name="vyve-parent-page" content="/connect.html">` for fallback. Parked — not launch-blocking. Earnable as §23.49 if implemented.

---

**Earlier in this same session — PM-188 Connect Phase 2 step 7 (§23.48 codification + sessions.html / leaderboard.html audit):**

**SESSION HANDOFF — 2026-05-21 (PM-188: §23.48 codified + step 7 SHIPPED — Phase 2 CLOSED).** Talk-first opening: Dean asked for a coherent freshness strategy across Connect before touching code for step 7. The right answer turned out to be the spec had already locked it — value of the discussion was to *name* the four patterns so step 7 + future phases quote them rather than re-derive. Earned §23.48 as a hard rule. Mid-discussion Dean surfaced the session content management surface (Command Centre editor) — a real need but post-launch; parked in this active.md §5 and in `tasks/backlog.md` for coordinated build when he's ready to extend the Command Centre.

**Decision points resolved this session:**
- Sessions.html scope: **Option A** (hardcoded SESSIONS array remains, surgical §23.48 pattern 3 hygiene only). Rationale: full catalogue swap requires schema migration + Command Centre editor to consume the new columns; doing the migration without the editor leaves columns nullable and empty for weeks. Better to ship migration + editor + sessions.html catalogue swap as one coordinated campaign post-launch.
- Leaderboard.html scope: **Option A** (keep localStorage cache, surgical pattern 4 hygiene only). The cache mechanism works fine — `_kv` migration would be housework without UX gain.

**Shipped earlier in this session — two atomic commits:**

(1) **VYVEBrain `154d474548b1133bdc8df21f8a5fc120212102a6` (PM-188 brain commit).** Three files atomic via Git Data API. `brain/master.md` §23.48 inserted (~9.6KB, between §23.47 and §24). `tasks/backlog.md` Session content management surface added (post-Phase-2 parallel-to-Phase-1 entry). `brain/changelog.md` PM-188 entry prepended.

(2) **vyve-site `0622db8e0d3622dccdabe81f8628e2f847af591a` (PM-188 step 7).** Four files atomic via Git Data API. **sessions.html**: removed `—` and `--:--:--` skeletons from #nextSession/#nextTime (cleaned per §23.46); replaced `setInterval(tick, 1000)` with `scheduleNextTick()`/`startTicker()`/`stopTicker()` cluster — adaptive cadence (1s when any session is live OR nearest is ≤5 min out, 30s otherwise), `visibilitychange→hidden` pauses, `→visible` immediately re-runs tick + reschedules, `pageshow` covers bfcache restore. **leaderboard.html**: removed §23.46-violating `Loading leaderboard…` flash on tab-switch when cache exists (new `_showSkeletonIfNoCache(range, scope)` helper checks localStorage cache before painting skeleton); added `_maybeRefreshOnFocus()` triggered on `visibilitychange` + `pageshow` with 60s staleness threshold matching the Connect EF `_kv` cadence; cache stays visible regardless of refresh state. **sw.js**: cache `pm187-ef-wiring-a` → `pm188-step7-a`. **index.html**: vbb-marker Update 56 → 57. All three inline JS blocks per file node --check clean. Post-commit byte-equal verified at commit SHA on all 4 files.

**Build queue advance.** Phase 2 Connect section build: **CLOSED at PM-188 nav fix commit** (not at the step 7 commit — Connect wasn't navigable until the bottom nav was wired). All 7 build-queue items + nav integration shipped across PM-186 → PM-188. Next: **Phase 1 — Body section consolidation.**

**New §23 hard rule earned: §23.48 — Connect freshness model (four patterns).** Pattern 1 (local member data, bus-driven, no timers). Pattern 2 (catalogue data, fan-out-on-focus, no live updates). Pattern 3 (time-derived state from catalogue, page-visible ticker paused on hidden, immediate re-eval on `→visible` — the NEW pattern codified here). Pattern 4 (honestly-network-bound aggregates, 60s `_kv` cache, fetch-on-focus — proven by PM-187.3, now canonical). Includes decision tree, audit signals per pattern, and explicit application matrix for existing and future surfaces. Future specs reference §23.48 when wiring a surface, the way PM-187 spec referenced §23.46 for paint and §23.39 for writes.


**Composio incident — still dead.** Both this session's commits used the §23.45 PAT-direct path end-to-end. PAT fetched once from Supabase Vault via `execute_sql` at session start; no retries against Composio. Working as designed.


**PM-187 Connect Phase 2 — steps 1-6 SHIPPED (21 May 2026)** — earlier this day. Hub + 3 sub-pages (`connect-checkin.html`, `connect-feed.html`, `connect-challenge.html`) live; both EFs (`connect-challenge-summary`, `connect-feed-counts`) deployed v1 ACTIVE; client EF wiring with 60s `_kv` cache lifecycle landed. Only step 7 (sub-page audit on sessions.html + leaderboard.html) remaining — SHIPPED PM-188 above.


**SESSION HANDOFF — 2026-05-21 (PM-187: Connect Phase 2 step 1+2 SHIPPED — db.js + sync.js wiring + connect.html hub).** Fresh "Load VYVE brain and run build" session. Composio creds still dead from morning's security incident; §23.45 fallback exercised end-to-end (Supabase MCP `execute_sql` for PAT fetch from Vault, bash curl direct against api.github.com, Git Data API for two multi-file atomic commits to vyve-site).

**Shipped to vyve-site main.** (1) `597851534a9c83296c95f57ba789a6bf5e54268e` — five files atomic. **db.js** SCHEMA_V8 + db.version(8) chained with 5 new stores (`connect_checkins`, `checkin_reactions` compound-PK, `weekly_challenges` catalogue, `weekly_challenge_participation` compound-PK, `daily_checkin_prompts` catalogue). Three standard `makeTable` wirings plus two compound-PK `replaceForMember` overrides (generic `makeTable` keys stale-sweep on `row.id`; these tables key on `[checkin_id, member_email]` / `[challenge_id, member_email]` arrays). **sync.js** 5 PULLABLE entries — 3 member-scoped (connect_checkins 30-day lookback, reactions full per-member, participation full per-member) + 2 catalogue (weekly_challenges last 26 weeks, daily_checkin_prompts all 30 rows). **connect.html** NEW (~40KB) — mirrors mind.html outer shape, §23.46 paint pattern verbatim (counter defaults `0` in markup, no skeleton chars, no localStorage snapshot, hidden states for genuinely-unresolved remote data). Sections: page header → "X checked in today" feed banner (`_kv.connect_feed_counts_v1`, hidden until populated) → TODAY'S CHECK-IN hero (djb2 daily prompt rotation read from Dexie `daily_checkin_prompts`, with `active_from`/`active_until`/`weight` honoured client-side) → YOUR MOMENTUM (streak ring with `--ring-deg` conic-gradient fill, elite progress bar — union across `connect_checkins`/`mind_activities`/`body_activities`/`daily_habits`, last 30 days, cap 30) → LIVE THIS WEEK (service_catalogue `type='live_session'`) → THIS WEEK'S CHALLENGE → RECENT CHECK-INS (top 3 own rows v1) → LATEST FROM VYVE (service_catalogue `type='replay'`). Bus subscribers wired for `connect:checkin:logged|failed`, `connect:reaction:logged|cleared`, `connect:challenge:progress`, `connect:hydrated`, `mind:logged`, `body:logged`. Hub is read-only — no bus publishes. **sw.js** cache `pm183-7-devpanel-a` → `pm187-connect-hub-a`; `/connect.html` added to precache. **index.html** vbb-marker 52 → 53. node --check clean on db.js / sync.js / sw.js / both inline blocks in connect.html. Post-commit byte-equal verified at commit SHA on all 5 files.

(2) `a7123667d2c13c003b314b23e5022b099919d5ef` — PM-187.1 hygiene follow-up. Initial `<div id="checkin-prompt">Loading today's prompt…</div>` was a skeleton dressed as text, same anti-pattern §23.46 banned for counters. Replaced with real fallback question ("What will you commit to today?") so first frame is honest even before Dexie reads. sw.js `pm187-connect-hub-a` → `pm187-connect-hub-b`. vbb-marker 53 → 54.

**New §23 hard rule earned: §23.47 — Specs cross-checked against live schema before lock.** PM-186 connect-spec.md assumed `service_catalogue.kind` / `is_live=true` / `published_at` — none exist. Live schema has `type` / `active` / `created_at`. Hub reads off the live schema; functionally identical to what the spec asked for, but the spec language was wrong. Codified — any spec referencing columns on existing tables must include an `information_schema.columns` cross-check at lock time.

**Hub Recent Check-Ins is own-rows v1.** sync.js only hydrates own-member rows per PULLABLE entries; cross-member feed reads require a server-side REST scoped read that connect-feed.html will own (step 3 of build queue). On connect-feed.html ship, the hub Recent Check-Ins block re-wires to pull from a feed-scope cache rather than `allFor(memberEmail)`. Flagged in build queue notes for step 3.

**Production state of vyve-site at this commit.** Main HEAD `a7123667d2c13c003b314b23e5022b099919d5ef` (PM-187.1). Bundled-mode production still at `83874dd5` per §23.42 — members do not see Connect until next OTA. Dean's iPhone via dev-loop sees it immediately, subject to §23.29 WKWebView cache.

**Build queue advance.** Steps 1 (db.js + sync.js wiring) + 2 (connect.html hub) SHIPPED. **Step 3 next: connect-checkin.html.** Single-purpose write surface, lifts breathwork.html / journal.html / affirmations.html sub-page shape, `<textarea maxlength=60>`, 5 focus chips, post button gated ≥3 chars, §23.39 optimistic-first write skeleton with already-posted-today guard.
...
10. **Sub-page audit pass.** ✅ **SHIPPED PM-188 step 7** (vyve-site `0622db8e0d3622dccdabe81f8628e2f847af591a`). sessions.html: §23.48 pattern 3 ticker (adaptive cadence 1s near-live / 30s otherwise, `visibilitychange` pause/resume, immediate re-eval on visible, pageshow handler for bfcache), §23.46 skeleton removal on #nextSession/#nextTime. leaderboard.html: §23.48 pattern 4 hygiene — `_showSkeletonIfNoCache(range, scope)` checks cache before painting skeleton on tab switch, `_maybeRefreshOnFocus()` triggers fetch on visibility/pageshow with 60s staleness threshold matching Connect EF cadence. Sessions catalogue swap + Command Centre editor deferred to post-launch coordinated campaign (see Phase 6 / post-launch P1 below).
...
- **Profile identity system (avatar + display-name privacy).** PM-188 deferred. Standalone post-launch campaign. Three-tier avatar (curated library default → upload own → no nag), four-way display name (Full / First / Initials / Anonymous), Anonymous coerces generic avatar, single setting drives every community-facing surface including consolidating the existing leaderboard opt-in toggle. Schema: 4 columns on `members` + new `member-avatars` Storage bucket. Single `identity.js` helper consumed by every feed/leaderboard/challenge surface — avoids per-surface divergence. GDPR Article 17 path adds bucket file deletion. Estimated 2-3 sessions (4 if avatar SVG design takes its own). Full spec at `tasks/backlog.md` ("Added 21 May 2026 — Profile identity system"). **Important context:** the single biggest visual gap between the Connect design mockup and the live build is avatar absence — every feed card / leaderboard row / recent-checkin currently shows a teal initials badge that reads as email, not community. Visual polish to the cards/hub should ship FIRST (per Dean), identity SECOND, but if the soft-launch trial expands beyond initial 15-20 the priority may flip.
- **Session content management surface + sessions.html catalogue swap (coordinated campaign).** PM-188 deferred. Three-piece campaign to be done together so the schema migration ships with the editor that uses it AND the page that reads from it: (a) `service_catalogue` schema migration — add `description`, `short_description`, `image_url`, `host_avatar_url` (verify live columns first per §23.47); seed/backfill the 8 wellness sessions currently hardcoded in sessions.html into rows with the new shape. (b) Command Centre editor at `admin.vyvehealth.co.uk` (repo `VYVEHealth/vyve-command-centre`) — list view + edit modal for `type='live_session'`/`'replay'` rows, Supabase Storage uploader for image (new `session-images` bucket, public-read admin-write), admin role gate. (c) sessions.html catalogue swap — remove the hardcoded SESSIONS array, read from `VYVELocalDB.service_catalogue`, repaint via §23.48 pattern 2 (fan-out-on-focus catalogue refresh) + pattern 3 (existing 30s ticker, now driven by catalogue rows). Member-side propagation needs zero new architecture beyond what's already in sync.js. Realtime broadcast on catalogue changes deferred unless empirical need surfaces. Estimated 3-4 sessions. Lewis owns: copy review on the 8 seed rows, who gets admin write access beyond him (likely Calum + Phil), session imagery sourcing. Full v1 scope captured in `tasks/backlog.md` ("Added 21 May 2026 — Session content management surface").
