...
**Last verified:** 13 May 2026 PM-80 (this commit). PM-78 shipped PF-1/2/3; PM-79 shipped PF-4/5; PM-80 shipped PF-6.

| What | Value |
|---|---|
| vyve-site main HEAD | `ff3e0e0f` (PM-76 perf.js v2 promotion — unchanged; PF-1/2/3 on feature branch only) |
| vyve-site `local-first-spike` HEAD | `48c4d17e` (PF-1 through PF-6 — Dexie schema + hydrate + shadow outbound queue + delta-pull + habits.html Dexie-first reads, spike gated by `localStorage.vyve_lf_spike`) |
| VYVEBrain main HEAD | this commit (PM-80 — PF-6 ship: habits.html Dexie-first reads) |
...
| Last shipped vyve-site commits | main unchanged: `fc8232bb` (PM-74), `5cef00a2` (PM-75), `ff3e0e0f` (PM-76). Spike branch: `8d07d26b` (PF-1), `e8f02742` (PF-2/3), `903127d6` (PF-4), `fa116ef2` (PF-5), `48c4d17e` (PF-6). |
...
**Status at PM-80 commit:** PF-1 through PF-6 SHIPPED to `local-first-spike` branch (`48c4d17e`), 6 commits ahead of main, awaiting Dean's verification + merge. Spike covers Dexie schema (~27 tables), hydrate-on-login, shadow outbound queue, since-cursor delta-pull, and the first Dexie-first read-path page (habits.html). Realtime cross-device merge (PF-5b) deferred until post-launch — single-device-per-user assumption (§0) makes this acceptable. Next: PF-7 (workouts.html + workouts-session.js + workouts-programme.js + thumbnail prefetch) — same Dexie-first-read template as PF-6, ~3 hours.
...
2.85. **PF-7 — workouts.html + workouts-session.js + workouts-programme.js Dexie-first reads** next task. Same template as PF-6 plus thumbnail prefetch (PM-77.3 note). ~3 hours, self-contained build.
2.9. **PF-8 through PF-20** — remaining page refactors + iOS hardening + nav restructure. Sequenced in playbook.
...
- **Last full rebuild:** 13 May 2026 PM-77. Latest patches: PM-78 (PF-1/2/3 + spike-gate rule), PM-79 (PF-4/5 + delta-pull cursor semantics), PM-80 (PF-6 + habits Dexie-first read pattern).
