...
**Last verified:** 13 May 2026 PM-81 (this commit). PM-78 shipped PF-1/2/3; PM-79 shipped PF-4/5; PM-80 shipped PF-6; PM-81 shipped PF-7.

| What | Value |
|---|---|
| vyve-site main HEAD | `ff3e0e0f` (PM-76 perf.js v2 promotion — unchanged; PF-1/2/3 on feature branch only) |
| vyve-site `local-first-spike` HEAD | `97863198` (PF-1 through PF-7 — Dexie schema + hydrate + shadow outbound queue + delta-pull + habits.html + workouts surfaces Dexie-first reads + thumbnail prefetch, spike gated by `localStorage.vyve_lf_spike`) |
| VYVEBrain main HEAD | this commit (PM-81 — PF-7 ship: workouts surfaces Dexie-first reads + thumbnail prefetch + PM-80 changelog restored after PM-79.1 drift) |
...
**Status at PM-81 commit:** PF-1 through PF-7 SHIPPED to `local-first-spike` branch (`97863198`), 7 commits ahead of main, awaiting Dean's verification + merge. Spike covers Dexie schema (~27 tables), hydrate-on-login, shadow outbound queue, since-cursor delta-pull, and Dexie-first read paths on habits.html (PF-6) + the four workouts surfaces (PF-7 — workout_plan_cache, workout_plans catalogue, exercise_logs, custom_workouts) with thumbnail prefetch. Realtime cross-device merge (PF-5b) deferred until post-launch — single-device-per-user assumption (§0) makes this acceptable. Next: PF-8 (nutrition.html + log-food.html) — same template, ~2-3 hours.
...
2.8. **PF-6 — habits.html refactor (Dexie-first reads)** SHIPPED to `local-first-spike` (PM-80, commit `48c4d17e`). Three Supabase reads replaced with Dexie `member_habits.allFor` / `daily_habits.todayFor` / `daily_habits.allDatesFor`; autotick HealthKit metadata (`fetchDashboardHabits`) preserved on the wire as live evaluator output. Spike-gated with fall-through to Supabase on shim/no-hydrate. Awaiting Dean offline-mode verification.
2.85. **PF-7 — workouts surfaces Dexie-first reads + thumbnail prefetch** SHIPPED to `local-first-spike` (PM-81, commit `97863198`). Four read sites flipped in workouts-programme.js (workout_plan_cache, workout_plans catalogue, exercise_logs, custom_workouts) + one in workouts-session.js (getTotalWorkoutCount). Non-empty-gate three-way branch: spike-on hit → Dexie, spike-on empty → Supabase fall-through, spike-off → legacy. Thumbnail prefetch fires {mode:'no-cors'} for every distinct thumbnail URL in the active programme after load.
2.87. **PF-8 — nutrition.html + log-food.html Dexie-first reads** next task. weight_logs, nutrition_logs, nutrition_my_foods; TDEE/macros render. Open Food Facts via off-proxy stays on the wire. ~2-3 hours.
2.9. **PF-9 through PF-20** — remaining page refactors + iOS hardening + nav restructure. Sequenced in playbook.
...
- **Last full rebuild:** 13 May 2026 PM-77. Latest patches: PM-78 (PF-1/2/3 + spike-gate rule), PM-79 (PF-4/5 + delta-pull cursor semantics), PM-79.1 (PF-23..27 added — interactive tutorial + 4 polish tasks), PM-80 (PF-6 + habits Dexie-first read pattern + autotick carve-out principle), PM-81 (PF-7 + workouts surfaces + thumbnail prefetch + brain restore after PM-79.1 drift).
