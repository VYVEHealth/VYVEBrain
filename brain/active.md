...
**Last verified:** 13 May 2026 PM-95 (PF-14 device verification on Dean's iPhone 17 — local-first architecture VALIDATED on iOS Capacitor WKWebView; spike merged to main; eight findings logged for PF-15; ITP risk locked in as launch blocker for PF-14b). PM-78..PM-93 history preserved above; tonight added PM-95 parts 1-4 (`82524093`, `fa3bd6e6`, `244d96fc`, `a3c74734`) + the spike→main merge (`c469e504`). PF-1..PF-13 scaffolding + PF-30 are now LIVE ON MAIN behind the spike-gate, dormant for non-spike members.

| What | Value |
|---|---|
| vyve-site main HEAD | `a3c74734` (PM-95 PF-14 parts 1-4 + spike→main merge `c469e504`; spike branch now redundant, can be deleted in PF-15 cleanup) |
...
| SW cache key (production) | `vyve-cache-v2026-05-13-pm95-pf14-indicator-d` (PM-95 PF-14 part 4 — bumped through indicator-a/b/c during tonight's device verification cycle) |
...
2.11f. **PF-14** — Device verification PARTIAL (PM-95, 13 May 2026). Dexie indicator + settings toggle shipped (`82524093`, `fa3bd6e6`); spike merged to main (`c469e504`); paint-grace indicator semantics shipped (`244d96fc`); stale habits-cache fix shipped (`a3c74734`). Dean's iPhone walk: architecture validated, eight findings logged for PF-15. Cross-sync test (iPhone+Android), Capacitor scheme readout (DONE: remote-origin confirmed), and full flow re-walk after PF-15 hydration fix all still QUEUED.
...
2.11g. **PF-15 — Hardening (post-PF-14 findings)**. New P0 items from tonight: (a) Dexie hydration coverage gap diagnosis + fix (cardio/exercise_logs/wellbeing_checkins/monthly_checkins/member_achievements not arriving on hydrate for Dean), (b) PF-12 partial-upsert merge fix in db.js (`member_habits.upsert` override should merge by default rather than overwrite), (c) localStorage shape-cache audit (vyve_home_v3, vyve_programme_cache, vyve_wt_cache, vyve_food_diary — all need version-bump or replacement). Plus original PF-15 scope: PM-77.1 mitigations A/B/C codification, force-resync escape hatch, queue drain batching, storage quota handling. Sunday session.
