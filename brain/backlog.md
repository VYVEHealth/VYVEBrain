...
> Updated: 28 April 2026 (Native Push Notifications Foundation SHIPPED 27 April PM — APNs end-to-end live; iOS 1.2(1) submitted to App Store Connect 28 April 00:36 UTC, status Waiting for Review. Next phase = notification triggers/copy/cadence design, decoupled from binary releases.)
...
1. ~~**Native Push Notifications (2-3 sessions)** — APNs (iOS) + FCM (Android) via Capacitor plugins.~~ **iOS Foundation SHIPPED 27 April 2026 PM** — APNs end-to-end live: AppDelegate.swift bridge methods in place, `register-push-token` v1 EF + `push-send-native` v5 EF deployed, NATIVE_PUSH_ALLOWLIST fail-closed in dev, ES256 JWT via Web Crypto, smoke test verified on Dean's iPhone 15 Pro Max, PUSH_ENV flipped to production via [22939ac](https://github.com/VYVEHealth/vyve-site/commit/22939ac7034a64245cc486ba32f256f18cc61284). iOS 1.2(1) submitted to App Store Connect 28 April 00:36 UTC awaiting review. **Next phase (decoupled from binary releases — deployable via web pushes + EFs without App Store cycle):** notification triggers + copy + cadence design — daily habit reminders local time, streak-risk alerts, achievement-tier-earned celebrations, live session start, weekly/monthly check-in nudges, re-engagement after 7 days inactive. Each is an EF + cron + Lewis copy + frequency cap. **Android FCM still pending** — backlog item, requires Capacitor/FCM setup + Google Play binary.
...
- **[P0] APNs auth key rotation — KEY_ID `2MWXR57BU4`** — `.p8` PEM contents pasted in chat 27 April PM, treat as exposed. Generate replacement APNs key in Apple Developer portal (`developer.apple.com/account/resources/authkeys`), update Supabase secrets `APNS_AUTH_KEY` + `APNS_KEY_ID`, retire old key. Test push smoke before retiring. Calendar: this week.
- **[P1] AppDelegate.swift archive checklist** — Recurring rule (also in master §23). Before every iOS archive: open `~/Projects/vyve-capacitor/ios/App/App/AppDelegate.swift`, cross-check against every Capacitor plugin's iOS setup section in their README. Installing the npm package is necessary but not sufficient — many plugins require manual `application(_:...)` bridge methods. Codified after AppDelegate gap consumed half a session diagnosing why `getState()` reported plugin-found-and-permission-granted while no token row was being written.
- **[P2] `capacitor-native-biometric` decision** — Currently installed (4.2.2) but unused. Defensive `NSFaceIDUsageDescription` shipped in 1.2(1) to pass App Store review. Two options: (a) wire it up properly (FaceID quick-resign on app open after backgrounding); (b) remove from package.json to silence SPM warning + reduce binary size. Lewis to weigh in on user-facing biometric value.
- **[P2] iOS 1.2(1) review monitoring** — Daily check on App Store Connect until status moves Waiting for Review → In Review → Approved. Auto-release on approval is enabled, so once approved the binary goes live without further action. Expected timeline: 24-48h based on prior 1.0/1.1 cadence.
