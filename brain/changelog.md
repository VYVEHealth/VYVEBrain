**PM-956 (2026-09-02, morning session): COACHING FIRST-LOGIN GATE + APPLY PIPELINE SHIPPED — the one unbuilt door in PM-955's platform is now closed; a coach can provision a client and everything lands in the member's real app surfaces the moment they walk the gate.** Dean's three calls locked at session start: check-in photo bucket pulled forward into this build; re-assignment pushes apply AUTOMATICALLY on portal save; coaches author their own terms with a VYVE example template as the starting point. **DB (migration pm956_coach_gate_foundation):** member_habits.assigned_by CHECK extended (`OR LIKE 'coach:%'`); members.macro_override jsonb added (coach explicit macros — protein/fat/carbs/hydration + source tag — beside the TDEE machinery, nutrition_guidance column reused for coach notes); storage bucket **coach-checkins** (private, 50MB, image/jpeg|png|webp|heic + video/mp4|quicktime|webm) with policies cchk_member_ins/read (own email prefix), cchk_coach_read (is_coach_of on first path segment), cchk_admin_all; **RPC coach_apply_assignments(partner_id, member_email)** SECURITY DEFINER, EXECUTE revoked from public/anon/authenticated — the single materialisation implementation both EFs call. Habits: template titles upserted into **habit_library as active=false rows, created_by='coach:<pid>'** — the load-bearing trick: member_habits' embedded PostgREST join (and the Dexie member-habits hydrate) ignores library `active`, so coach habits render + log + autotick like any other, while active=false keeps them OUT of every member's Dexie catalogue hydrate (`active=eq.true&select=*`) and the settings custom-habit picker (`or=(created_by.is.null,created_by.eq.<email>)` can't match) — zero global pollution, dedupe by (created_by, habit_title), habit_pot='movement' (CHECK vocabulary has no 'coach'; §23.179 catch), removed-from-template coach rows deactivated, member/AI rows never touched. Workout: coach template transformed in plpgsql to the EXACT live programme_json contract (surface='workouts', weeks 1-8 repeating, sessions w/ session_name/session_number/warm_up+cool_down strings/estimated_duration_mins, exercises w/ exercise_name/sets int/reps string/rest_seconds/notes/letter_badge A-F — sampled from a live AI row; exercise.html hero + workouts-programme.js consume it unmodified, name-matched demo videos free); prior surface='workouts' actives deactivated, insert source='coach'/source_id=template — AI regen never clobbers. Nutrition: tdee_target=calories + macro_override + guidance. Supplements: replace-set into coach_supplements. Returns counts jsonb; **proven live + idempotent on deanonbrown12** (3 habits, 8-week Upper/Lower Starter, 2200kcal/180p, 2 supps — re-run stable). **EF coach-gate v1** (verify_jwt, member JWT self-resolved): status (coach identity + latest active coach_terms + assigned onboarding form + stamp state — resume-safe), accept (vyve_terms→members terms/privacy/health_data_consent stamps; coach_terms→version+accepted_at; consent→consent_accepted_at + invited→active — arms is_coach_of), submit (consent belt-and-braces, coach_form_responses upsert on form/member/week_start ISO-Monday, onboarding_complete=true, then apply RPC; returns applied summary). **EF coach-provision-client v5:** update_assignments action — validates final-keyed ids against the coach's live library, saves, auto-applies when status='active' AND consented. **vyve-site d40b5876 (vbb 565, md5×6):** NEW coach-welcome.html — standalone 4-step gate (VYVE terms → coach terms w/ mdLite render, step self-skips if coach has none → data-sharing consent w/ explicit shared-list + amber "your coach can NEVER see" card naming wellbeing/mental-health/AI as database-enforced → coach questionnaire rendering all 8 portal question types incl. photo/video uploads to coach-checkins at <email>/onboarding/<qid>-<ts>.<ext>) → done screen listing what the coach set up → index.html; steps drop out when already stamped (resume-safe), onboarding_complete → straight to app. auth.js: BOTH gates branch on billing_tier='coaching' — consent gate (which fires first) routes coaching members to coach-welcome instead of consent-gate, onboarding gate routes them instead of onboarding.html; coach-welcome in both skip lists. nutrition.html: macro_override.source='coach' branch — explicit coach macros replace the weight-derived split (kcal per macro computed 4/9/4), "Targets set by your coach · XL water daily" line, guidance card + supplement-stack card (member-read RLS), recalculator HIDDEN (member recalc would silently clobber coach targets). **CC 8e77ea60 (md5):** client-detail assignment saves route via update_assignments EF w/ applied-summary feedback ("Saved — pushed to their app (programme, 3 habits)" / "applies when they accept your terms"); Settings gains the coaching-terms editor — versioned publish (retire active, insert max+1, fresh max at save), example template button prefilling the 5-section agreement. **LIVE FIXTURE for device test:** deanonbrown12 assigned 5 real templates + terms v1 under the Dean Brown test coach (status still invited/unconsented — logging in walks the full gate; materialised data already visible pre-gate from the RPC proof run, harmless). [LEWIS COPY PASS non-gating] on all gate copy. New rule §23.182. Remaining from sprint order: Calum provisioning only. **ADDENDUM PM-956b (same day, autonomous stretch while Dean at work): TRIAL-WALL £10 ROUTING + WORKOUT-COMPLETION SUMMARIES SHIPPED.** **Routing (server-only — the Apple de-steer stands, the wall itself still sells nothing):** stripe-webhook v14 (deploy counter 16) adds a COACHING RAIL ahead of the three attribution rails — the pre-existing partner_memberships row (account_type='coaching', trialing, value 0 from provisioning) is now UPDATEd at customer.subscription.created (subscription_value=net paid → 10, status active, stripe ids); previously the v13 upsert's ignoreDuplicates meant the coaching row NEVER updated — value stayed 0 and the coach's £5/mo never reached run_partner_payouts. No partner-status gate on this rail (relationship is explicit in coach_clients; coach partners legitimately sit in 'onboarding' — the test coach does). trial-end-email v2: run+resend select billing_tier; coaching members get coach-framed copy ("everything <coach> set up is saved… £10/month keeps you connected", coach name via signup_campaign_code=coach slug, same continue.html?cid= link, tag 'coaching') [LEWIS COPY PASS non-gating]. Signature-gated webhook is sandbox-uninvokable — first real/test £10 conversion is the live verification; deanonbrown12's row sits at 0/trialing ready to flip. **Workout summaries (Kahunas parity):** migration pm956_workout_coach_feedback — workouts.difficulty_rating int CHECK 1-5 + member_note text (rides the row coach_read already covers; workouts_own_data is ALL so the member PATCHes own row). vyve-site c659d006 (vbb 566, md5×5): completion screen gains a coaching-only block (Easy/Fine/Solid/Hard/Brutal + optional note + Send to coach) — shown only when an active coach_clients row exists (member-read RLS, sessionStorage-cached per member); submit PATCHes /workouts by client_id+member_email with ONE 3s retry because the workout POST itself fires un-awaited in the background (PM-148 optimistic-first) and can race the PATCH; return=representation rowcount distinguishes miss from success; failure copy honest ("your workout is saved either way"). CC 3dd0a537 (md5): client-detail timeline shows workout completions w/ felt-rating + quoted note (esc'd at render); **LATENT BUG FIXED — the portal's 30d-workouts and 7d-habits header stats queried completed_at / completed_date, columns that DON'T EXIST (real: logged_at / activity_date) — both queries 400'd into their .catch and the stats have silently read 0 since PM-955.** Calum provisioning still parked on Dean's explicit go (touches a real inbox).

**PM-955 (2026-09-01, evening session — NUMBER NOTE: built in parallel with the PM-954 store-submission session; all six DB migrations are named `pm954*` and deployed EF header comments say PM-954 a–i — those artefacts are immutable, this entry is the canonical number): COACHING PLATFORM SHIPPED — Kahunas-parity PT client management, live end-to-end on real hardware.** Coaches are partners (`partner_type='coach'`, constraint extended; Calum's live row + Dean's test row flagged). Commercial model (Dean locked): client pays coach directly outside VYVE; client pays VYVE £10/mo after 7-day trial (`members.billing_tier='coaching'`); coach earns £5/mo for life of membership via existing `run_partner_payouts()` (rewritten: counts `account_type IN ('b2c','coaching')`; `partner_memberships` constraints extended +'coaching'/+'trialing'). No AI onboarding for coaching clients — coach's own questionnaire + T&Cs + data-sharing consent instead (gate UNBUILT, next session). **DB (migrations pm954_foundation/_read/c/d/h/i):** `coach_clients` (status invited→active→archived, `assignments` jsonb of 6 slot ids, `invite_code` 12-char unique), `coach_terms` (versioned), `coach_forms` (onboarding|checkin, MULTIPLE per kind — one-active index dropped on Dean's call), `coach_form_responses` (unique form/member/week), `coach_templates` (habits|workout|nutrition|supplements, UNIQUE partner+kind+name, soft-delete), `coach_supplements`. **RLS model:** `is_coach_of(email)` SECURITY DEFINER — true only for status='active' AND consent stamped; coach_read policies on exactly members, workouts, exercise_logs, exercise_swaps, custom_workouts, daily_habits, member_habits, nutrition_logs, weight_logs, cardio, workout_plan_cache, weekly_goals. **HARD BOUNDARY: never wellbeing_checkins / weekly_scores / ai_interactions / Mind pillar — keeps Phil out of the coaching critical path.** Isolation proven live with rolled-back fixtures (0-client coach sees 0 rows; consented-client coach sees exactly 1 member, 0 wellbeing/AI rows); zero pre-existing partner grants on member data — Postgres-enforced isolation is the Sage-DPO answer. **EFs:** `coach-provision-client` v4 (verify_jwt on; actions create|bulk≤100|libraries|resend|copy_link|archive|reactivate; creates member trial/coaching + auth user + coach_clients + partner_memberships referred/coaching/trialing + Brevo invite; assignment resolution by exact template name or id; guards: coached-elsewhere 409, existing uncoached member → support; v3 hotfix was `attribution_source` CHECK 23514 — 'admin' used). `coach-join` v2 (verify_jwt FALSE by design): permanent pretty links **www.vyvehealth.co.uk/start/<invite_code>** → Test-Site 404.html catch-all (`e712b31f`, /start matcher above /join+/trial; bare apex works too) → EF mints a FRESH **type='recovery'** link per click → set-password form. v1 used magiclink — WRONG: set-password.html's SIGNED_IN branch (no type=recovery in hash) paints "You're all set" assuming a pre-set password; recovery fires PASSWORD_RECOVERY → the form. Pretty links never expire, survive double-taps + mail-scanner prefetch, reuse the same code across resends, and go DEAD once activated (status gate — no coach back door into live accounts). **COACH PORTAL — NEW standalone admin.vyvehealth.co.uk/coach-portal.html** (CC repo; Dean call: separate surface from partner-portal, which was reverted byte-for-byte; commits 14931299→ac2cea0e, all md5-verified): roster w/ status badges, Add Client (6 assignment dropdowns from live library), Resend/Copy-link/Archive/Reactivate; **bulk onboarding via per-coach generated .xlsx** (ExcelJS in-browser template w/ hidden Lists sheet driving data-validation dropdowns of the coach's actual saved plans, 200 rows; upload parses xlsx or quoted CSV, preview, per-row failure reasons; Brevo free tier 300/day noted); **My Plans builders** for all 6 kinds (forms: 8 question types incl photo/video, new check-ins pre-seed 5 Kahunas defaults; workouts: sessions w/ warmup/cooldown groups + exercise datalist from workout_plans — name match = demo videos in app; nutrition macros; habits/supplements editors; soft-delete); **client detail page** (header stats, per-client assignments editor live any status, activity timeline, latest check-in answers). **LIVE TEST (Dean, iPhone):** full chain proven — portal create (after 23514 hotfix) → www.vyvehealth.co.uk/start/11b9543a2d2b → recovery link → password set → app login as deanonbrown12@hotmail.com (trial ends 08/09; deanonbrown2 portal password reset to vyve2026 via crypt()). First tap raced the Pages deploy by 2 seconds (build 22:25:30Z vs tap 22:25:28Z) — retap worked. Kahunas "1.6M foods/800k barcodes" already matched by off-proxy + log-food barcode scan — Lewis positioning line, not a build item. **UNBUILT (= next-session spec, top of backlog):** first-login gate (VYVE terms + coach_terms + consent + onboarding questionnaire), assignments→member-tables apply pipeline, trial-wall £10 routing, workout-completion summaries to coach, coach-checkins storage bucket, Calum provisioning (2-min job). New rules §23.179–181. **ADDENDUM PM-955j (same night, CC `f01bc4a4`): coach portal restructured to a Kahunas-style LEFT SIDEBAR** (Dean spec, exact order): Profile / Messages / Notifications / Dashboard / Clients / Nutrition / Workouts / Calendar / Content Library / Automations / Forms / Settings. New views: Dashboard (default landing — client-count tiles + recent clients), Profile (partner name/status/email/client count; editing deferred to the client-facing coach card), Settings (theme + sign-out). Forms/Workouts/Nutrition reuse the existing six-kind builders via section-filtered kind tabs (forms→questionnaires+check-ins+habits; nutrition→nutrition+supplements). Messages / Notifications / Calendar / Content Library / Automations ship as honest coming-soon views with SOON pills, each naming its build (messages = in-portal↔in-app client messaging; automations = triggered emails e.g. workout-updated; content = coach-only unlisted video library; calendar = booking-engine wiring; notifications = client-activity feed). Mobile: slide-in drawer + overlay. All roster/builder/detail logic untouched; both inline scripts node --check clean; md5-verified.**

**PM-954 (2026-09-01): iOS 2.0 (5) + ANDROID 1.0.9 (55) BOTH SUBMITTED — first store release since July, carrying vbb 564 (Lewis videos bundled, video-first onboarding, header fixes, PM-943 s3 hardening, everything since 1.9's snapshot).** Bundle: fresh vyve-site clone rsync'd into www/ (--delete, --exclude videos/ + .git), 7 bucket videos pulled into www/videos/ per the §23.175 store-build checklist (first execution — checklist works as written), `Update 564` verified in the bundle, cap sync both platforms clean. **iOS:** archive succeeded first try; distribution hit TWO account-level blockers, both cleared same night: (1) **Apple pushed a PLA update — Account Holder must accept before ANY upload/cert issuance account-wide** (Dean got in and accepted it himself); (2) **both existing Apple Distribution certs are keyless on Dean's Mac** (created on Lewis's machine, "Not in Keychain") — NEW Apple Distribution cert minted 01/09 via Xcode Settings→Accounts→Manage Certificates→+ (key lives on Dean's Mac; the two Lewis certs left alone, under the 2-cert cap only after Apple auto-expired one). codesign keychain prompt = Mac login password, Always Allow. Uploaded to Apple, build attached, **submitted — Waiting for Review**. Review notes kept minimal by Dean's call: existing on-file demo creds (deanonbrown@hotmail.com, fully onboarded member — reviewer skips onboarding); deanonbrown2@gmail.com password reset to VyveTest2026! via SQL as the internal test account either way. **Android:** `./gradlew bundleRelease` on the fresh clone failed three times, each a gitignored-artefact gap now §23.178: system Java 26 → "Unsupported class file major version 70" (fix: JAVA_HOME=Android Studio JBR); missing local.properties (echo sdk.dir); signReleaseBundle NullPointerException = missing keystore config (keystore.properties + app/keystore/vyve-release-key.jks copied from the old folder — storeFile path is relative to the app/ module). BUILD SUCCESSFUL → 75MB AAB → Play Console 55 (1.0.9) → full rollout **in review**. Play's 3 warnings (size / no deobfuscation / no native symbols) are advisory and expected — minify off, videos deliberate. Release notes both stores: "Improvements to onboarding, new intro videos, performance and stability fixes." **§23.106 stays CLOSED (PM-701) — Dean re-confirmed in-session he has pushed OTAs end-to-end in production already; Claude's compacted session state was stale on this and wrongly proposed a TestFlight canary. Capawesome subscription confirmed ACTIVE.** Parked by Dean (backlogged, not submission gates): Android assetlinks.json (Play Console app-signing SHA-256 page unfindable post-reorg; pull SHA from a Play-installed APK later), iOS Associated Domains entitlement (AASA web-side live+verified on all 3 domains + Apple CDN, inert without the entitlement, harmless), Cloudflare-in-front-of-portal purge-on-deploy (~2-15min dev loop → ~1-3min), header fine-tune verification on 564 (Dean's last device check likely predated propagation; OTA fixes it if needed).**

**PM-947–PM-953 (2026-09-01): ONBOARDING HEADER SAGA — six commits of fighting a phantom, root cause was a MOBILE MEDIA QUERY the whole time (§23.177 banked) — vyve-site `649fd05`→`2389d69`, vbb 558→564, sw v2026-09-01e→k.** **PM-947 (`649fd05`, 558):** intro-video "Tap for sound" chip when iOS refuses sound autoplay and the muted fallback fires — bottom-right of the slot, one tap unmutes, volumechange listener removes it; sound-first behaviour unchanged (Low Power Mode blocks play entirely; the physical silent switch mutes regardless — OS territory, not ours). **PM-948 (`d21b327`, 559):** header scaled to app chrome — bar 46→40px, logo 36→28px, toggle/label shrunk, side padding 32→16, shadow dropped. **PM-949 (`a72f44b`, 560):** +8px status-bar clearance; progress label "N of 10" → percentage (`Math.round(section/TOTAL_SECTIONS*100)+'%'` at the progressLabel writer ~line 1657) — doubles as an on-device version marker. **PM-950 (`7c17f2c`, 561):** misread of Dean's intent — transparent floating chrome with blur fade; rejected, solid band wanted back. **PM-951 (`6ee979d`, 562):** logo 28→34px, toggle/label full size, row at safe+14 matching certificates.html. **PM-952 (`a2b02ab`, 563):** solid band per Dean's explicit spec — contents down 5px, band ends 5px below the 34px logo (padding calc(safe+19px) 16px 5px, height calc(58px+safe)). **PM-953 (`2389d69`, 564): ROOT CAUSE — `@media (max-width:520px)` (~line 864) contained `.progress-wrap { padding: 0 16px; }`, zeroing vertical padding ON PHONES ONLY, collapsing the row upward inside the fixed-height band and silently defeating every base-rule fix above (the whole "still huge / half empty / band not visible" loop).** Line removed. Dean called time on further verification ("looks fucked but can't waste more time" — his last check likely predated 564 propagation); header ships as-is in the 2.0/1.0.9 binaries, OTA-fixable. **§23.177 HARD RULE: any phone-only CSS discrepancy → grep the media queries for the selector FIRST, before touching base rules.**

**PM-944–PM-946 (2026-09-01, earlier sessions): STORE PREP — AASA live on all domains, s1 Mobile field cut, s0 polish.** **PM-944 (vyve-site `b0e9e13a` + Test-Site `eed81a6f`+`2f60bdec` + capacitor `2fb5948`):** Apple webcredentials AASA at /.well-known/apple-app-site-association (VPW62W696B.co.uk.vyvehealth.app) on online + www + apex — **Test-Site gotcha: Jekyll drops dot-directories from Pages builds; `_config.yml` `include: [.well-known]` required** (www 404'd until then). Live-verified HTTP 200 on all three + Apple's CDN (app-site-association.cdn-apple.com) already serving it; content-type application/octet-stream is fine (Apple accepts). Inert until the Associated Domains entitlement ships in a future binary (parked). Capacitor commit = the 2.0(5)/1.0.9(55) version bumps. **PM-945 (`49040db9`):** Mobile field removed from onboarding s1 (Dean call — not needed); payload keeps phone:'' so the EF v120 contract is untouched. **PM-946 (`c0b5e43e`):** s0 polish off Dean device feedback — big base64 logo above Welcome removed (-142KB), header 58→46px + container tightened. **Store-version reconcile:** the brain's "1.8/1.0.7 in review" state was long stale — reality: iOS 1.9 (build 4) and Android 1.0.8 (code 54) were approved and LIVE (Play shows 54 since 4 Jul); tonight's 2.0(5)/55 supersede. Canonical native checkout is now the fresh clone **`~/vyve-capacitor-git`** (npm install clean, SPM warning on capacitor-native-biometric pre-existing/harmless); the old loose `~/Projects/vyve-capacitor` folder is RETIRED — keystore.properties + vyve-release-key.jks copied out of it into the clone (gitignored, stays Mac-local; 1Password co-location item still open). The recurring "vyve-capacitor git init" P0 is CLOSED — the repo existed and was current all along.**

**PM-943 (2026-09-01): IN-APP ONBOARDING AUDIT + S3 HARDENING SHIPPED — vyve-site `fc477994`, vbb 554, sw v2026-09-01a.** Dean session: full audit of onboarding.html (in-app questionnaire, s0–s10) plus member feedback that people can't self-assess "rate your physical exercise". **SHIPPED (all s3): (1) The ≥8 skip prompt on recapPhysical was a plan-killer** — `skipSection(3)` bypassed the stream picker and all workout follow-ups, so members rating physical satisfaction 8+ submitted `exerciseStream:'workouts'` (the silent default) with zero gym inputs and got an 8-week programme generated from nothing; prompt removed, replaced with a keep-going line ("the questions below still shape your workout plan"). (2) **validateSection s3 gate** — previously only s1 validated anything; s3 was the sole unguarded gate into programme generation. Now: a stream card must be selected; workouts stream additionally requires location-train + gymExperience + trainDays; new `err-stream`/`err-workouts` field-errors with scrollIntoView. Dev-skip (PM-941) unaffected — it routes via retrySubmit. (3) **activityLevel bare labels → life-card pattern** with descriptive sub-lines, mirroring the tdeeActivity cards ("Desk job, driving, little daily movement" etc.) — the self-assessment feedback fix; also improves calcTDEE's activityLevel fallback accuracy; the activityLevel→tdeeActivity mirror-sync script keys on `#activityLevel .choice` and still matches. (4) **q-hint self-assessment guidance** under the physical/sleep/diet/social/motivation satisfaction sliders (gut-feel framing, "no wrong answer, we'll track how this changes") [LEWIS COPY PASS, non-gating]. All inline scripts node --check clean; 4 files md5-perfect at `fc477994`. **AUDIT FINDINGS NOT YET FIXED (backlogged, next onboarding session): (a) Quick Start + skipped sections write fake 5/10 baselines** — submit reads every slider whether seen or not, polluting weekly-trend baselines; payload should null unseen scores (+ `path` flag) with an EF-side change. **(b) Consent PATCH race** — `health_data_consent` fires with `window._vyveJWT` set async in boot; fast tap → Bearer undefined → RLS reject → swallowed; belt-and-braces: carry consent in submit payload, EF stamps it. **(c) Boot-failure dead end** — session boot fails open, hidden email field fails validation invisibly, Continue silently does nothing. **(d) Sliders min=0 but labels claim 1–10** (a 0 is submittable). (e) DOB validation rejects age>80 with a misleading "must be 16+" message. (f) `smartwatch`/`bedtime` read in payload but no such questions exist (dead fields). (g) "Prefer not to say" gender silently disables TDEE with no explanation. (h) Draft-restore drops sectionHistory — Back from a restored section jumps to s0. (i) Quick-path progress bar 20%→100%. **RECONCILE NOTE: PM-942 (`b9a121ce`, vbb 553, video-first step-zero flow off Dean's cycle-one feedback) shipped 18 Aug with NO brain entry** — this stub records its existence; full reconcile if details ever matter.**

**PM-941 (2026-08-17): TEST-ACCOUNT ONBOARDING SKIP + RESET LOOP SHIPPED (vbb 552, `546e1c47`) + TRIAL PAYWALL PERMANENTLY LIFTED FROM ALL TEST/STAFF ACCOUNTS.** Dean can now run the full onboarding experience end-to-end, repeatedly, without typing a single answer. **Skip:** onboarding.html — 1.2s long-press on the step-zero heading (deanonbrown2@gmail.com only, armed at fire-time against `window._vyveEmail`) fills a canned answer set (mirrors Dean's real July answers: workouts stream, advanced, 3d/wk, full gym, calm-neutral tone) and submits through the REAL onboarding EF by setting `window._lastSubmitData` and calling the existing `retrySubmit()` — zero duplicated submit logic, real persona assignment, real 8-week programme generation, real reveal flow (persona → welcome video → three-cards → Health prompt → landing). **Reset:** settings.html — 1.2s long-press on the App version row (same allowlist) → confirm → NEW `dev-reset-onboarding` EF v1 (verify_jwt:true; validates the CALLER'S OWN JWT email against a hard allowlist, then rpc's SQL fn `dev_reset_onboarding(p_email, p_dry_run)` — SECURITY DEFINER with EXECUTE revoked from public/anon/authenticated, so only service role reaches it, and the fn re-checks its own allowlist: three independent gates). Server sweeps EVERY member-scoped base table dynamically (information_schema `member_email` columns) **excluding `admin_audit_log` (audit trail immutable) and `partner_%` (partner attribution fixtures survive resets — dry-run surfaced 5 partner_memberships rows that would otherwise be silently destroyed)**, then resets the members row in place (onboarding/persona/recs/certs/milestones/baselines/member_state/consent cleared; identity + auth preserved). Client then clears `vyve_hubvid_*` (hub/check-in videos re-fire), `vyve_onb_*`, the auth.js gate sessionStorage cache, and signs out — next login lands on fresh step-zero. Dry-run verified (18 tables), 403 verified against a valid-but-wrong JWT, real reset executed server-side same session (19 tables incl. 1,052 HealthKit samples). **PAYWALL EXEMPTION (Dean directive: test accounts + main account NEVER see the 7-day wall):** first live cycle hit the lock-out — deanonbrown2's July trial had expired and the PM-438 gate walls BEFORE Settings is reachable, making the in-app reset unreachable. Root-cause fix is data-level: PM-438 never gates `account_type='comp'` (the existing Kelly/Callum mechanism), the trial-clock trigger only stamps when account_type='trial', and onboarding EF v120's writeMember NEVER writes account_type — so comp is permanent and survives every rerun. **12 accounts flipped to comp/active/no-clock:** deanonbrown@hotmail.com (main), deanonbrown2, deanonbrown1, gpv1 (Gary), test@test.com–test5, kelly/cbudzski confirmed. dev_reset_onboarding corrected in the same pass — it was re-stamping `account_type='trial'` on every reset, which would have re-armed the clock each cycle; now sets 'comp'. §23.176 banked. Known trade-offs by design: reset clears health_data_consent (consent tick exercised every cycle — full step-zero fidelity) and the canned submit still sits through the real ~30–90s generation loading screen (genuine pipeline, not a shortcut). Recurrence path if a NON-allowlisted flow ever walls a needed account: one-liner `SELECT dev_reset_onboarding(email,false)` server-side, or extend comp.**

**PM-937–PM-940 (2026-08-17): LEWIS VIDEOS — SCRIPTED, FILMED, INGESTED, LIVE IN EVERY SLOT + LOCAL-FIRST PLAYBACK SHIPPED.** Filming pack delivered as docx (7 scripts after Dean merged then re-split the check-in pair — final set: questionnaire intro / after-questionnaire welcome / Mind / Body / Connect / weekly check-in / monthly check-in / Give Back; Mind closes on Marcus Aurelius 'You have power over your mind — not outside events', public domain; Mind script deliberately everyday-tools only, clear of the §23.84 Phil gate). **Corrections en route:** the QUICK questionnaire path EXISTS LIVE (onboarding.html step-zero Full ~5min gold-Recommended card vs Quick Start card, quick routes s1→s2→s10) — brain never recorded it and Claude wrongly flagged it missing from stale state; intro video renders on that same screen directly under the path cards. Lewis filming-day script edits: 'one of the founders', 'short option / full onboarding questionnaire' (never 'version' — implies two apps), welcome video must NOT mention Give Back yet. **PM-937 (vbb 548, `f1950f04`+fix`3cfa7ce3`): weekly check-in intro slot** — hub-videos.js MAP + `checkin_intro` seeded in app_videos. **PM-938 (vbb 550, `7f23384d`+fix`54cc882b`): monthly check-in intro slot** — MAP + `monthly_checkin_intro`; card anchor falls back to `.page-wrap` (monthly-checkin.html has no `<main>`). **PM-939 (no repo commit): storage-ingest EF v1** (secret-guarded generic URL→bucket puller, KEEP — reusable for future media drops) + Lewis's 8 mp4s (1024×576 h264/aac 33–50s, remuxed +faststart) transferred container→scratch-git-branch→EF→`app-videos` bucket (container egress can't reach supabase.co; pg_net invoked the EF; byte-perfect verified; scratch branch deleted). All 7 slot urls SET + active — members see Lewis from next page load, no OTA. `welcome_videp.mp4` was a byte-identical dupe of the questionnaire video (md5-proven, ignored; Dean confirmed). Brief false-alarm: welcome slot darkened then re-lit same session — after_questionnaire.mp4 IS the distinct welcome take. **`give_back.mp4` PARKED in bucket, no in-app slot by design** (Lewis cut Give Back from onboarding; placement = Lewis call: give-back page / demos / social). **PM-940 (vbb 551, `fa1ad7db`): LOCAL-FIRST PLAYBACK** — Dean call: videos must be INSIDE the binary so a no-Wi-Fi fresh install gets the full welcome flow. Native builds now resolve `/videos/<key>.mp4` from the bundled web root, error-fallback to remote app_videos.url (web PWA, server.url dev loop, and post-OTA all fall through to remote — zero behaviour change until a binary bundles them). hub-videos.js cards + both onboarding slots. **§23.175 HARD RULE: OTA bundle zips must EXCLUDE videos/** — Capawesome swaps the whole web root, so bundled videos ride the STORE BINARY only; store-build step = drop the 7 bucket files into www/videos/ named by key (~70MB binary cost, OTA stays slim; post-OTA devices stream, which is fine — receiving an OTA proves connectivity). **UX decision (Dean confirmed): NO forced walkthrough tour** — the reveal screen (welcome video + three-cards overview) then first-visit-triggered cards per area is the standing model. **§23.174 banked: PM-XXX placeholder sweeps hit twice on case** (`pmXXX` in sw CACHE_NAME survived uppercase-only sed in BOTH PM-937 and PM-938 → correction commits b) — sweeps now case-insensitive with a post-sweep regex assert. Filming-day commercial notes for Lewis's court: niche-expert partner map (addiction/suicide-prevention/weight/fertility ×2/diabetes/sleep/performance+everyday nutrition), tiered influencer rev-share (anchor 50%, recruits 40% + 10% override to anchor), podcast-alumni outreach prioritising employer-embedded guests, employer-referral rev share UNDEFINED, summer street-interview activation idea.**

**PM-935 (2026-08-11): partner portal image gate 15MB → 40MB — CC `e82b4c8e`.** Real-partner friction: Vanessa's 17.5MB cover photo was rejected at the file-pick gate. The gate is a browser-memory guard only — every image goes through the PM-860 cropper which exports a 1200×400 (cover) / 512² (avatar) canvas JPEG at q0.9, so the UPLOADED file is tiny regardless of source; storage, member-app payloads and the 8MB post-crop check are all unaffected. Modern phone photos routinely exceed 15MB (48MP iPhones), so the old cap was rejecting normal camera-roll images for zero benefit. Vanessa can retry as-is once Cloudflare deploys.**

**PM-934 (2026-08-11): HUMAN GUEST LINKS — `admin.vyvehealth.co.uk/m/<CODE>` — CC `d1b5d12e`+`a833f8e1`, meeting-manage v3, meeting-guest v6.** Dean: 'the link is terrible' (uuid+token soup; his pasted link was also the OLD EF format — cached meetings page, hard-refresh mints new). **Model:** client meetings get an 8-char Crockford-base32 short_code (unique column, collision-retry on insert); the guest link is now just `/m/<code>` — the code IS the secret (~40 bits behind the guest EF's 60/hr/IP limit + 24h expiry + knocking lobby: worst brute-force outcome is a stranger in a lobby the host never admits). Cloudflare `_redirects` rewrites `/m/:code → /meet?c=:code` — **gotcha banked: the target MUST be extensionless; a `.html` target gets 308-normalised by Pages and DROPS the query string** (live-observed). meet.html accepts code via query or path with legacy m+g fallback; meeting-guest v6 POST accepts {code} or the legacy pair (format-validated, invalid → 404). **Live-proven:** info-by-code returns the title, bad code 404s, `/m/TESTCD9Q` serves the join page directly HTTP 200; synthetic row cleaned. Both Vanessa meetings had already ended during testing — Dean creates a fresh client meeting and gets the short link natively. **When Dean attaches meet.vyvehealth.co.uk to the CC Pages project (backlogged 2-min Cloudflare step), the SAME links work as meet.vyvehealth.co.uk/m/<CODE> automatically.****

**PM-933 (2026-08-11): CLIENT GUEST PAGE MOVED TO OUR DOMAIN — platform-level gotcha discovered — CC `eabafcfb`+`1729eedd`, meeting-guest EF v4→v5.** Dean created 'Catchup with Vanessa' and the guest link rendered RAW HTML SOURCE. Diagnosis chain: source code correct in v1/v2/v3 (text/html set) yet live GET returned `content-type: text/plain` + `x-content-type-options: nosniff`; a 3-line ct-probe EF proved it: **the Supabase functions gateway (sb-gateway-version 1) now forcibly rewrites ALL Edge Function responses on the default *.supabase.co/functions/ domain to sandboxed text/plain + CSP `default-src 'none'; sandbox` — an anti-phishing platform policy. HTML can NEVER be served from the default functions domain. §23 HARD RULE: EFs are JSON APIs; pages live on our domains.** (The original device check passed because Dean joined from inside the CC — the guest page had never been browser-opened.) **Fix:** standalone `meet.html` at the CC repo root → served as **https://admin.vyvehealth.co.uk/meet** (Cloudflare pretty-URL; page reads m/g params, POST action:'info' shows the real meeting title pre-join, POST join mints the Daily token); meeting-guest v5 = pure JSON API (CORS admin origin) whose **GET 302-redirects to /meet preserving m/g — every previously issued guest link keeps working**; meetings.html now mints /meet links directly. **Full chain live-verified**: old EF link → 302 → /meet renders join page; info API validated the real Vanessa token (correctly reports meeting_over — that meeting already ended; Dean should mint a fresh client meeting for the real Vanessa call). Cleanup owed: delete ct-probe EF (one-shot) on the next EF-pruning pass. Later vanity: meet.vyvehealth.co.uk as an added custom domain on the same Pages project.**

**PM-932 (2026-08-11): Inbound → 'Applications' under Run the Business + instant arrival bells — CC `3e35b679`, inbound-intake EF v2.** Dean corrections after first use: wrong section (it sat next to Complaints under Members — 'it's not members') and he wants a notification on arrival, not digest-only. **Moved:** sidebar item now 'Applications' in Run the Business (after Podcast), slug 'inbound' unchanged so routes/deep-links stay stable; **untagged like Meetings** (no area key — visible to all CC users; the page itself still role-gates via getCurrentAdmin + PAGE_POLICY 'team'). Hero retitled Applications. **Instant bells:** intake EF v2 notifies Dean/Lewis/Alan on every stored submission ('New application: <name> — <role>', route #/inbound, dedupe inbound:<row_id>:<email>, never blocks the submission) — live-verified 3/3 bells then test row cleaned. Model now: instant CC bell per item + 07:45 daily digest bell + (pending Brevo IP fix) one daily email. Dean's real Non-Exec-HR-Director test application is the only cc_inbound row.**

**PM-929/930/931 (2026-08-11): WEBSITE INBOUND → COMMAND CENTRE + DAILY DIGEST (Dean: applications and everything else emailed should land in the CC, one daily overview instead of per-item emails).** **CRITICAL DISCOVERY EN ROUTE: contact.html AND contact-individual.html's handleSubmit sent NOTHING ANYWHERE** — preventDefault → 'Message received! We'll be in contact within 24 hours' → data discarded. Enterprise demo requests have been silently dropped for as long as those pages have been live. **Lewis needs to know.** Careers Apply was bare mailto (zero capture); podcast guest form leaked via Formspree. **Shipped: (1)** `cc_inbound` table (kinds role_application/contact/podcast_guest/other; status new/seen/actioned/archived; RLS is_admin_or_team read/update, service-only insert). **(2)** `inbound-intake` EF v1 (public, CORS www+apex, honeypot 'website' field pretends success, per-IP 10/hr rate limit, live-tested 4/4: store/honeypot/bad-kind/bad-email). **(3)** Marketing site `a613a210`: careers.html in-page application modal (name/email/CV link/message → payload.role); both contact pages now actually submit (success card kept, team@ alert fallback on failure); podcast form off Formspree. **(4)** CC `ecb30c5e`: pages/inbound.html triage (kind/status filters, detail modal with reply-by-email + clickable CV links, open=seen, actioned/archived; PM-923 mobile rules), sidebar next to Complaints, PAGE_POLICY 'inbound':'team'. **(5)** `inbound-digest` EF v3 + pg_cron 07:45 UTC daily: counts last-24h inbound → CC bells for Dean/Lewis/Alan deep-linked #/inbound (dedupe inbounddigest:date:email) + ONE team@ email. **Bells live-proven; the EMAIL leg is blocked: Brevo API rejects EF egress (account has Authorised-IPs restriction; EF IPs rotate) and send-email EF's internal auth shape couldn't be determined tonight (service key, apikey combo, VYVE_INTERNAL_KEY in three shapes all Unauthorized — how DO its callers authenticate? read a caller EF's source next session). LEWIS 1-MIN FIX: Brevo → Security → Authorised IPs → disable, then the digest's direct Brevo send just works (v3 already implements it and reports emailError).** Test rows cleaned; cc_inbound starts empty. NOT touched by design: crisis + manual-onboard alert emails stay immediate (never digest-gated); mirroring them into cc_inbound as records is backlogged.**

**PM-928 (2026-08-11, demo row): 'Demo — Dean & Lewis platform catch-up' meeting created in prod for Dean to show Lewis the full pipeline** — synthetic transcript, summarised live by the worker path: clean sectioned summary, 2/2 proposals correctly matched (Lewis→guest-page copy pass — a REAL standing gate; Dean→meet.vyvehealth.co.uk vanity domain), group-therapy idea + Sage timing correctly left as open questions, ready-to-approve bells delivered to BOTH Dean and Lewis with #/meetings deep links. Left pending approval deliberately so Lewis can tap bell → approve live. Keep or delete the meeting row after the show-and-tell — approving its two proposals is genuinely useful either way.**

**PM-926 + PM-927 (2026-08-11): partner-management port QA fixes — CC `bd6fc5dd` + `c0a1182a`. Port now DATA-LIVE and gesture-free on mobile.** **PM-926 (blank KPIs, Dean screenshots):** showApp() wrote the signed-in name/avatar into #sf-nm/#sf-av — side-foot elements the port deleted — null deref threw BEFORE loadAll(), so the app rendered with zero data and unhydrated data-i icon squares. Hidden compat stubs restore both ids; loadAll runs. cm-player 'missing' was a false positive (self-creating overlay). **TRANSFORM RULE for Phase 3: diff the script's getElementById refs against the surviving DOM — now mandatory.** Dean confirmed fixed. **PM-927 (his next report: 'slider nav at the top doesn't slide'):** ROOT CAUSE IS GLOBAL — base `.tab-bar{width:fit-content}` means the strip sizes to its buttons and never overflows internally, so PM-923's overflow-x:auto had NOTHING to scroll on every tab-bar page (app-health/intel/sessions/settings/strategy/usage/meetings). `≤900px .tab-bar{max-width:100%}` makes swipe real everywhere. partner-management's viewbar goes one better per Dean's 'better way / only me, Alan and Lewis use it': it WRAPS on mobile — all 9 view pills visible, zero gestures (page-scoped override). §23 lesson pair: (a) removing DOM during a port requires a JS-ref diff; (b) overflow-x:auto is inert on fit-content containers — clamp width when making anything swipeable.**

**PM-926 (2026-08-11): partner-management port fix — blank KPIs root-caused — CC `bd6fc5dd`.** Dean's QA screenshots: page renders, tabs work, every KPI blank, icon squares unhydrated, cards stretched. Root cause: `showApp()` writes the signed-in name/avatar into `#sf-nm`/`#sf-av` — the old sidebar-footer elements the port deleted — so the null deref threw AFTER making the app visible but BEFORE `loadAll()` and the invite-button listener. Fix: hidden compat stubs restore both ids inside the page div. `cm-player` also flagged missing but is self-creating (false positive). Card stretching is expected to resolve once data + icons hydrate — if not, next Dean report. **Transform rule added for Phase 3 (in the PM-924 commit message + here): after any port, diff the script's getElementById references against the surviving DOM — the sf- class of bug is mechanical to catch.** Dean re-QA: hard-refresh → Partner Management → numbers should populate; then the full 8-tab checklist stands.**

**PM-925 (2026-08-11, decision — no build): PARTNER PORTAL IS OUT OF CC APP-WRAP SCOPE.** Dean: the partner-facing portal won't sit in the CC app — long-term home is the customer-facing app ("eventually"), with view-access from the CC kept. **Status quo already satisfies this:** partners reach partner-portal.html directly (their own password sign-in, PORTAL_BRAND_URL pending the partners.vyvehealth.co.uk custom domain), the CC sidebar Partner Portal link opens the real live portal (same origin — works inside the future app webview), and PM-924's ported Partner Management keeps the Preview button (in-CC phone mock of a partner's member-facing space). **The eventual migration is architecture-aligned by design:** PM-859's one-identity model means a partner's app login and portal login are already the same auth user — porting portal features (content upload/push, diary, bank details, agreement, support inbox) into the native app is additive surface, not an auth rebuild. Sequenced: after the CC wrap ships and no earlier than the bookings-through-VYVE work (shared partner-money surfaces). Backlog line added; partner-portal.html excluded from the wrap-readiness gate.**

**PM-924 (2026-08-11): STREAMLINE PHASE 2 SHIPPED — partners.html ported into the SPA — CC `07338aab`. DEAN DEVICE QA GATES COMPLETION.** Mechanical machine transform of the 165KB monolith, not a rewrite — every behaviour preserved byte-close: body extracted under `#page-partner-management`; **internal sidebar dissolved into the shared .tab-bar** (8 views — Overview/Pipeline/Partners/Content/Onboarding/Support/Analytics/Revenue — + Preview portal; swipes on mobile via PM-923); **all 23KB of CSS machine-scoped under the page id** (a real scoper: body/:root/* → page scope, `html[data-theme=x] sel` → `html[data-theme=x] #page sel`, @keyframes verbatim, @media recursed) — kills the body/table/*/a global leaks that would have restyled the whole shell; **structural class renames** (main/topbar/content/crumb/spacer → pt-*) neutralise shell collisions, clashing `#app` id → `#pt-app` (index.html owns #app); **main 112KB script IIFE-wrapped** (§23.169 SPA re-entry: top-level const/let redeclare class) with **75 top-level functions window-exported** (superset of the 34 inline-onclick handlers incl. dynamically rendered ones); its PM-762 theme boot DROPPED (shell owns theme, html[data-theme] selectors still bind), PM-888 gate-flash-kill kept, supabase CDN tag dropped (shell loads the UMD; router preserves the kept qrcode src tag), **auth untouched** — same 'vyve-cc-supabase-auth' storage key means the shared session auto-hides its gate (second GoTrue instance accepted for v1). Sidebar href→slug swap + PAGE_POLICY 'partner-management':'lead'. Both inline scripts node --check clean; zero remaining `.nav-item` selector refs asserted. **ROLLBACK = revert one sidebar line; /partners.html untouched and directly reachable (soft-kill).** **DEAN QA CHECKLIST:** open Partner Management from the sidebar → all 8 tabs switch; open a partner detail (row click — the PM-840 navByView fallback must still fire); Profile tab phone preview; Documents; mint-invite modal; moderation watch-in-place; Support inbox reply; runPayouts view renders; then the same on iPhone (tab strip swipes). Any breakage: report the view name — or flip the sidebar line back and it's pre-port instantly.**

**PM-923 (2026-08-11): FULL CC MOBILE AUDIT (Dean: every page, app by end of week) — systemic fixes shipped CC `488c16d6`, complete findings in NEW `playbooks/cc-mobile-audit.md`.** Whole-repo scan (46 pages + 3 monoliths + css): the inconsistency Dean saw is a STALLED CAMPAIGN — PM-764's M1 mobile primitives (.m-scroll, mobile modals) exist but adoption stopped at 4/18 table pages. **Four systemic failures fixed globally, zero markup churn:** (1) 21 naked tables across 14 pages → `≤900px .page table{display:block;overflow-x:auto}` — every table self-scrolls (escape hatch table.no-mscroll); (2) shared .tab-bar (6 pages — Dean's Overview/Cron/Pipelines strip) now swipes; (3) tasks .tk-board rigid 4-col grid → 80vw snap-scroll columns (components.css already HAD a correct .kanban scroller — tasks rolled its own grid: the audit's poster child); (4) media max-width clamp. Fixed-width sampling: overwhelmingly benign max-widths; newer pages carry own media rules, older don't — covered by the globals. **REMAINING (in the playbook + backlog, all riding Phase 2/3 THIS WEEK as the wrap gate):** partners.html/admin-console/partner-portal are standalone documents OUTSIDE the .page rules — worst surfaces until ported; calendar month-view mobile treatment; chart canvas device sweep; full sidebar walk on Dean's iPhone. Conventions banked in the playbook (tab-bar always, .kanban always, max-width always, type-scoped input styling per PM-921, test at 393pt). App-wrap readiness checklist added — wrap = Capacitor pointing at admin.vyvehealth.co.uk, no sw/vbb ritual.**

**PM-922 (2026-08-11): READY-TO-APPROVE BELLS + DEEP LINK — meeting-summary EF v6 + CC `2275f371`.** Dean: "me and Lewis should be sent a notification that they're ready to approve and clicking the notification should take you where you need to approve." When proposals land, the EF now inserts PM-913 bell rows for the APPROVERS list (Dean + Lewis — constant next to NAME_MAP, extend as needed; dedupe meetproposals:<meeting_id>:<email>), route `#/meetings/<id>`. The meetings page handles the deep link on boot AND hashchange — opens that meeting's transcript with the amber approval box directly. Bell rows BACKFILLED for the live pending test meeting so both can tap-through immediately — Lewis will find it on his next login regardless. Full loop now: meeting → transcript → summary → proposals → bell → tap → approve → tasks + assignee notifications.**

**PM-921 (2026-08-11): invisible assignee names in the task modal — CC `2f2b39c7`.** Dean opened an auto-created task on his phone: checkbox column, no names. Root cause in components.css: the generic `.form-group input` rule (width:100%, padding 10/14, surface background) also catches checkboxes/radios — each becomes a full-width padded block that consumes its flex row and crushes the name span (min-width:0 + overflow:hidden) to zero. Latent since the rule was written; surfaced tonight only because cc_tasks got its FIRST EVER rows from the meetings pipeline. Fix: scoped `.form-group input[type=checkbox|radio]` override (16px, no padding, flex:0 0 auto, accent-colour), additive. §23 class to remember: broad element-selector form styling silently eats checkboxes — scope new input rules by type. Note to self recorded: Dean looked for task APPROVAL inside the Tasks edit modal — the approval surface lives on the MEETING (View transcript → amber proposals box); if that proves unintuitive again, add an approve deep-link into the summary card or a bell notification pointing at the meeting.**

**PM-920 (2026-08-11): CC MOBILE FIT — the dark right-strip on every phone page diagnosed and fixed — CC `c03700a8`.** Dean device report + declared direction: the CC will become a phone app for account management, but pages don't fit on mobile, "mainly because of the top nav". **Root cause from the shell CSS/markup: at ≤900px the tabs hide but .topnav-tools keeps EVERYTHING** — theme toggle, role pill, view-as select, bell, print, help, search, avatar — which cannot fit a 393pt viewport, so the document lays out wider than the screen and the overflowing shell background renders as the dark strip over the right of every page. **Fix, additive CSS only (end-of-file @media block, zero desktop change):** ≤900px keeps bell + search icon + avatar (theme/print/help/role hidden), ≤640px hides the view-as select, html/body overflow-x clamped as backstop. **Also: meetings rows now wrap properly** (mt-when/mt-main/mt-actions classes, full-width stacking ≤640px) — kills the one-word-per-line titles in Dean's screenshot. **DIRECTION BANKED: CC → native phone app** — the existing 'full CC native app wrap' backlog line is now Dean-reaffirmed with mobile-fit as the explicit precondition; a proper page-by-page mobile audit rides CC Streamline Phase 2/3 (partners.html + admin-console ports), since those two monoliths will need the same treatment.**

**PM-919 (2026-08-10): TASK APPROVAL GATE — extraction now PROPOSES; nothing hits the board or pings anyone until a human approves — meeting-summary EF v5 + CC `10aabb11`.** Dean, minutes after PM-918: "handy if I was able to sign them off as a go to before automatically assigning." **Model change:** the worker's summarise path stores extracted actions in transcript_meta.proposed_tasks (task/owner/email pre-matched) and the summary carries "Proposed tasks awaiting approval: N"; NEW `action:'approve'` (admin|team JWT ONLY — **the worker path returns 403 human_approval_required by design, live-verified**) takes selected indices → creates cc_tasks + PM-913 notifications, stamps tasks_created + reviewed_by, rewrites the summary line; empty selection = reviewed-and-dismissed. Both directions idempotent (already_reviewed 409). **CC transcript modal:** amber Proposed-tasks checklist (all pre-ticked, unmatched owners flagged "no CC account — will be unassigned") + Approve selected / Dismiss all; row chip shows "N tasks to approve". **Server side live-proven on a synthetic transcript: 2/2 proposals correct** (Calum→workout engine v2 inputs, Phil→HAVEN sign-off review — both, pleasingly, REAL standing gates), parked idea correctly not a task, zero rows created pre-approval, worker-approve 403. **DEAN DEVICE CHECK OPEN: the 'APPROVAL GATE TEST — DELETE ME' meeting is left in Past meetings on purpose** — open View transcript, approve or dismiss the two proposals with his own login (the human half of the proof), then delete the meeting row. PM-918's auto-create behaviour is SUPERSEDED — approval gate is the standing model. Cowork question answered en route: subscription and API are separate billing routes; Cowork can't host an unattended pipeline; automation stays API at ~3-4p/meeting; interactive transcript digging in Claude/Cowork = free on his plan.**

**PM-918 (2026-08-10): MEETINGS → TASKS — Claude now extracts actions from every transcript and puts them ON THE BOARD, assigned, with PM-913 notifications — meeting-summary EF v4.** Dean's ask: "give us tasks to do and automatically update the tasks in the app and assign them to the right people." Same single Claude call now returns the human summary PLUS a TASKS_JSON machine block of concrete committed actions with named owners. Owners matched first-name → email via an in-EF NAME_MAP roster (lewis/dean/alan/cole/calum/phil/heidi/ryan — admin|team only, partners never assignable; **update the map in the EF when the team changes**; Vicki has no CC account yet so lands unassigned-with-name-in-notes). Matched tasks → cc_tasks (stage todo, priority medium, pillar business, created_by 'meeting-summary', provenance in notes) + cc_notifications 'You've been assigned a task' rows with taskassign: dedupe keys — exactly the PM-913 shape the tasks page writes, so the bell just works. Discussion points and vague intentions are explicitly NOT tasks; unowned items stay in Open questions. **Idempotent:** transcript_meta.tasks_created guards — live-verified rerun created 0. **LIVE-PROVEN on a synthetic transcript: 3/3 tasks correct** (Dean→rotate Daily key; Lewis→file Daily DPA; Lewis→Hetzner on register), notifications fired, unowned vanity-domain item correctly left as an open question. **The three test tasks were kept on the board deliberately — they are the real open items from tonight's build** (notes rewritten to honest provenance; synthetic meeting row deleted). Lewis will find two assigned tasks + bell notifications on his next CC login. Summary spend: ~3-4p per hour-long meeting (Sonnet 4.5), ai_interactions-logged.**

**PM-917 (2026-08-10): AI MEETING SUMMARIES — auto-generated the moment a transcript completes — CC `67baf15d` + meeting-summary EF v3 + cc_meetings.summary + worker hook (installer `a4725dc5`, box synced).** Dean: transcription must be automatic (it is) and every completed meeting gets a summary. **Flow:** worker finishes a transcript → POSTs meeting-summary with its service key → EF calls Claude (**claude-sonnet-4-5** — v1 shipped the dated claude-sonnet-4-20250514 string and 404'd; PM-666 already banked this migration, missed on first write) → plain-text Summary / Key decisions / Actions / Open questions, transcript-only grounding (never invent owners) → cc_meetings.summary + ai_interactions spend row (triggered_by 'manual') → modal shows AI summary above the full transcript, Copy grabs both. **TWO REAL FIXES BANKED:** (1) **EF worker auth is a CAPABILITY probe, not string equality** — the box's extracted service key is valid but NOT byte-identical to the EF's injected SUPABASE_SERVICE_ROLE_KEY (403 on exact-match); v2+ verifies the presented token can read transcribe_heartbeat (RLS, zero policies — only service-grade tokens see the row; anon/user JWTs get []). Corollary gotcha: the installer's key 'validation' (HTTP 200 on that table) passes an ANON key too, since RLS-filtered PostgREST returns 200-empty — any future key validation must check for the ROW, not the status. (2) **File-transfer integrity:** first worker push to the box went via hand-reconstructed base64 in a tool call → md5 lineage mismatch vs the committed source; refused, and re-delivered by having the BOX extract worker.py from the served installer (git → EF → box), md5-matched against the same extraction locally (`a5c14d34`). Rule of thumb: never hand-carry file bytes between sandboxes — always pull from the committed artefact. **PROVEN:** summary generated for Dean's real test meeting via the box's own credentials (200, sensible text). Worker latency answer banked: RTF 0.38 → 60-min meeting ≈ 23 min, 30-min ≈ 11 min, background + automatic; WHISPER_MODEL=base halves it, API flip ≈ 30s at ~£10/mo if ever wanted.**

**PM-916 (2026-08-10): TRANSCRIPTION PIPELINE LIVE + END-TO-END PROVEN — MEETINGS WORKSTREAM COMPLETE AT £0/MO.** Sessions 2–3 collapsed into one. **Shipped:** `transcribe_heartbeat` table (RLS, no policies — runner_commands posture); **`scripts/vyve-transcribe-install.sh` in VYVEBrain** (`93d51619`, embedded worker.py — installer precedent PM-836: idempotent, service-key auto-extract from the box's env files with /dev/tty+API-validate fallback, benchmark-gated enable); **serve-transcribe-installer EF v1** (verify_jwt:false by design; serves the script FROM VYVEBrain@main via the GITHUB_PAT secret, 5-min cache — script stays versioned/md5-verifiable in git instead of an EF blob); **`vyve-transcribe` systemd unit LIVE on the Hetzner box** (Restart=always, RestartSec=10, Nice=19, MemoryHigh=2300M — install driven by Claude over SSH from the Composio sandbox, ZERO Dean pastes). **Worker model:** polls cc_meetings 15s; live+transcribe → mints owner token, joins as visible **'VYVE Notetaker'** (daily-python, mixed-room 16k mono VirtualSpeakerDevice → WAV parts); ends on DB status ended/cancelled or room-empty-60s (covers Leave-without-End); finalize marks the meeting ended if needed, queues cc_transcription_jobs; CAS claim via conditional PATCH (status=eq.queued + return=representation); faster-whisper small int8 2-thread, vad_filter, [mm:ss] paragraphing; 3 attempts then failed + report-error critical (PM-902 wire — capture failures also beacon); audio deleted on success, 7-day sweep otherwise; worker restarts mid-meeting rejoin as partN → 'incomplete' honesty; ONE capture at a time by design (global virtual speaker) — concurrent second meeting stays 'waiting'. **PROOF:** Dean's 23:40 test — Notetaker joined, 17s of speech, transcript ready seconds after end, opens with his actual words. **Real benchmark: RTF 0.38 on 2 vCPU → a 60-min meeting ≈ 23 min to transcript** (installer's synthetic-audio bench was meaningless — speech fetch 403'd, VAD skipped noise; the live run is the number). Pre-worker 23:29 test had no audio — stale 'waiting' chip cleared to 'none'. **REMAINING (all optional):** meet.vyvehealth.co.uk vanity guest domain; Daily key rotation (transcripts, carded account) at desktop; register additions Daily + Hetzner (Lewis); Lewis copy pass on guest-page transcription notice; later — per-participant tracks for speaker labels, 'Ask Claude about this meeting'.**

**PM-915b (2026-08-10): MEETINGS V1 DEVICE-PROVEN + DAILY KEY VAULTED.** Dean created the Daily account (subdomain **vyvehealth.daily.co**; card now REQUIRED for API access on new Daily accounts — $15 dev credit granted, free 10K participant-min tier unchanged) and pasted the API key in chat — so instead of the dashboard-secret route: key stored in **Vault as `DAILY_API_KEY`** + new **`get_daily_api_key()` RPC** (SECURITY DEFINER, empty search_path, REVOKEd from anon/authenticated, EXECUTE service_role only — deliberately name-fixed, no generic secret reader) + **meeting-manage v2 / meeting-guest v2** read vault-first with Deno.env override, module-cached. Key live-validated via Composio sandbox curl (200, zero rooms). **DEVICE CHECK PASSED:** Dean created + joined an internal meeting on iPhone Safari — camera live in the CC overlay, End-for-everyone/Leave present. **ROTATION DEBT +1: the Daily key sits in two chat transcripts on a carded account** — desktop task: Create API key in Daily → UPDATE vault secret → delete old key (joins the PM-714/836 service_role rotation item). HIPAA banner on Daily dashboard = US framing, ignored; our posture is the register + Daily DPA (Lewis).**

**PM-915 (2026-08-10): MEETINGS V1 — CC-NATIVE VIDEO MEETINGS + CLIENT GUEST DOOR — CC `7e6c7f82` + cc_meetings migration + meeting-manage EF v1 + meeting-guest EF v1.** Dean brought a ChatGPT-drafted meetings/transcription proposal with an explicit challenge-this brief; direction locked after discussion. **Three calls that changed the proposal:** (1) **Daily Prebuilt embedded in the CC** kept (10K free participant-min/mo covers current volume; Jitsi self-host rejected — a 10-person SFU won't live on the 2-vCPU box; LiveKit only wins at ~50× our volume). (2) **The Mac worker is DEAD — the Hetzner box is the transcription worker** (always-on, EU/Germany, self-healing, reuses the vyve-agent/runner_commands queue pattern; box checked live via the agent queue: 22G disk free, 3.1G RAM available, faster-whisper small int8 fits nice-19 under the near-idle stream-copy runner). (3) **NO Daily recording fees — Dean pushed to £0 total:** recording at $0.01349/min was the only real cost, so a daily-python bot on the box will join each meeting as a silent participant and capture audio itself (bot burns free-tier participant-minutes; bonus: per-participant track subscription = real speaker labels later). **Shipped tonight:** cc_meetings / cc_meeting_participants / cc_transcription_jobs (CC RLS pattern — is_admin_or_team SELECT, EF-only lifecycle writes, SKIP-LOCKED-ready job shape); meeting-manage EF v1 (verify_jwt:true explicit; caller must be active admin_users role admin|team — PM-884 lesson applied, partner rows excluded; actions create/join/end/cancel; private Daily rooms, 24h exp, owner tokens for internal, client rooms get enable_knocking lobby + one-time guest token stored as SHA-256 hash only); meeting-guest EF v1 (verify_jwt:false BY DESIGN — guests have no account; GET serves a branded standalone join page, POST validates token hash + expiry + status and mints a non-owner Daily token; 60/hr/IP; page carries 'may be transcribed' notice [LEWIS COPY PASS, non-gating]); pages/meetings.html + sidebar Meetings entry (RTB, deliberately untagged — visible to every CC user incl. per-area team) — create form (title/type/schedule/transcribe toggle), live+upcoming/past lists, full-screen Daily Prebuilt overlay (lazy daily-js load, Leave vs End-for-everyone), guest link shown ONCE at create, transcript status chips + viewer modal wired ahead of the pipeline. **Parallel-session note:** CC PM-911–914 (calendar/notifications) landed mid-session; §23.26 fresh-fetch caught my stale sidebar copy before commit (the calendar anchor line had literal \\u2014 escapes — patch re-applied against raw bytes on fresh HEAD, no clobber). **DEAN LOOP OPEN:** create the Daily account (team@vyvehealth.co.uk), set DAILY_API_KEY in EF secrets, then device-check admin.vyvehealth.co.uk #/meetings — create + join an internal meeting. **GDPR for the register:** Daily = NEW sub-processor (US; no EU-only media residency; SCCs via Daily DPA — Lewis to file) and Hetzner is ABSENT from the register despite hosting the live runner — add both. Sessions 2–3 backlogged: bot capture + whisper worker (benchmark rides the installer, one Dean paste), hardening, optional meet. vanity domain.**

**PM-904 (2026-08-10): CC member preview + PORTAL AGREEMENT RE-SIGN — CC `42f5a23a` (both files) + partner-agreement EF v1.** Two Dean asks off the back of the April incident.

**(a) Member preview on the CC Profile tab.** Admins were editing a partner's member-facing profile through a bare form while the partner saw a live phone mock in their portal. Same mini-phone markup/classes ported to the CC (circle avatar per PM-860, cover banner, About / Reason / Community-feel), live-updating on every keystroke via a delegated input listener. One truth on both sides.

**(b) Partner agreement signable FROM THE PORTAL (the gap April fell through).** The agreement was only ever written by the wizard's `submit` action — so April, who signed on screen, was 429'd out of submit (PM-903), and has since been advanced to ONBOARDING with portal access, had **no route to complete it and no agreement on file**. NEW `partner-agreement` EF v1 (verify_jwt:true; partner self-resolved via get_my_partner_id — nobody signs for anyone else; 5 signs/day): `status` + `sign` (legal name, typed signature, explicit rev-share acceptance). **DB write happens FIRST** — the signature is captured even if the copy generation or email later fails (the PM-847 lesson applied deliberately) — then steps.agreement_signed is stamped, the signed copy is rendered from the LIVE wizard contract pane (single source of truth), stored to partner-docs and emailed. Portal Profile tab gains a "Your partner agreement" card: amber sign form when unsigned, quiet signed state (name, date, %) once done. Doubles as the re-sign path if terms ever change. **April is provisioned and will see this card on her first login.**

**Parallel-session note:** a concurrent session committed PM-907 (per-person areas + admin view-as) to BOTH the same files minutes earlier; my base was md5-identical to their content, verified post-commit against `c0a7587e` — **no clobber in either direction**, my work sits on top of theirs.**

**PM-903 (2026-08-10): OUR ABUSE GUARD LOCKED OUT A REAL APPLICANT — partner-onboarding v17 (platform 24) + live data fix.** April Rosson (april.rosson@gmail.com, draft `1f7aaeb7`, arrived via a Facebook share link) hit "We couldn't submit your application". **Root cause from edge logs: a solid wall of 429s.** Her POSTs succeeded until 19:30:26, then EVERY request — including submit — returned 429 while she pressed Try again every ~15s for eight minutes. `ef_rate_limits` held **175 hits against the 120/hr `rl:ip` ceiling**. The wizard autosaves ~every 15s, so a careful applicant who spends an hour on the form generates ~240 req/hr and walks into a limit sized for bots. **Worst failure class we have: silent to us, terminal to her, and it punishes the applicants who care most.** **IMMEDIATE:** her IP window deleted — she can submit now (draft intact at pct 38, answers were held client-side; the error card's "nothing is lost" promise held). **v17 FIX, three parts:** (1) **submit is NEVER IP-rate-limited** — it's the one irreplaceable action in the funnel and the per-draft guards + uq_partner_applied_email already bound what a submit can create; **live-verified against a saturated 9999-hit window: submit reached the logic (404 draft-not-found) instead of 429**; (2) global IP ceiling 120 → 600/hr and `save` moved to its own per-draft bucket (400/hr) so one long application can't exhaust the shared allowance; (3) every 429 now carries `retry_after_seconds` + human copy ("wait a few minutes and your work will pick up where it left off"). Draft-scoped abuse limits (start 5/hr/IP, assessment 15, upload 30, verify 5+2) UNCHANGED. **Also fixed in v17: `https://vyvehealth.co.uk` (bare domain) added to ALLOWED_ORIGINS** — her link was a Facebook share and a bare-domain share would have CORS-failed every call. **PM-902 note: the beacon didn't catch this** — the wizard isn't wired to reportError yet (already backlogged); the sweep would have caught the stalled draft tomorrow morning, but a 429 wall deserves same-minute alerting. Wizard beacon promoted to next-session priority.**

**PM-902 (2026-08-10): KNOW BEFORE THE CUSTOMER TELLS US — client error beacon + nightly partner health sweep — CC `9e2f939c` + report-error EF v1 + partner-health-sweep EF v2 + client_errors table + partner_health_findings() RPC + cron.** Dean, after the Vanessa incident: "we need to know about these kind of issues that are failing before the customer tells us." He's right — **every failure that day (cover 413, dropped video, phantom path:null metadata) happened in a browser and reached us days later, from the partner.** Two layers, both live:

**Layer 1 — the browser tells us at the moment it breaks.** NEW `client_errors` table (surface/context/severity/message/detail jsonb/actor/partner/user-agent/resolved_at; admin-read RLS, EF-only writes) + `report-error` EF v1 (**verify_jwt:false ON PURPOSE** — the wizard has no login and pre-auth funnel breakage is exactly what we most need to see; abuse ceiling is 20/hr/IP + hard truncation). severity='critical' ALSO emails team@ + deanonbrown@ and fires a native push within seconds, **deduped per context+actor+hour** so a retry loop can't spam Dean's phone. Portal wired at every upload failure path (video: progress % + MB + type + welcome flag; cover/avatar: size + 413 detection; oversize pick-time rejection as 'warn'). **User-facing copy fixed at the same points** — a 413 now says "too large — 8MB max", a dropped upload says "interrupted at N% — usually a connection drop, try again on wifi". Live-verified end to end: simulated 47% failure → row + email + push queued.

**Layer 2 — the nightly sweep for failures no browser sees.** `partner-health-sweep` EF v2, cron `vyve-partner-health-sweep` 07:45 UTC (ahead of the 08:05 daily report). Six fingerprints, all from real incidents: (1) `credentials.documents.*` with path:null (Vanessa's cover — client recorded it after storage refused it); (2) `_system.agreement_copy_error` set (PM-847 has been writing this since July; **nothing had ever read it**); (3) provisioned but never signed in >72h (welcome email in spam); (4) LIVE partner with no approved welcome video (empty front door); (5) unresolved client_errors in 24h; (6) support threads open >48h. **Emails ONLY when something is found — a clean sweep sends nothing, so the email's arrival is itself the signal.** Findings 3–6 come from a purpose-built read-only `partner_health_findings()` RPC — deliberately NOT a generic SQL executor (that would be a service-role RCE surface); REVOKEd from anon/authenticated. **FIRST RUN FOUND 11 THINGS** — eleven quiet failures nobody had reported.**

**PM-901 (2026-08-10, minutes after PM-900 — Dean on the Riverside call with Lewis + Alan): PUSH MODEL — CC `21334583` + partner-content-upload v10 + RLS re-rewrite.** Dean: "once the content is in the Your Content area they should have the option to push it to their area." Better model than PM-900's auto-live, so PM-900's visibility semantics are SUPERSEDED same-night: **approval = VYVE's gate; going live in the space = the partner's explicit act.** RLS member_read back to `approved AND publish_at NOT NULL AND publish_at <= now()` (the pre-900 shape — publish_at NULL now means "approved, in the library, private"). Portal: approved+dateless renders "In your library — push it whenever you're ready" with a **Push to my space** button → v10 `publish` action (approved+dateless only; sets publish_at=now; 409 for scheduled/live/in-review; guarded update so a race can't double-fire). **Welcome videos AUTO-PUSH:** v10 commit stamps publish_at=now on is_welcome items — the front door goes live the moment moderation approves, no push step. Scheduled items unchanged (48h lead, date passes → live). Upload hint + welcome toast rewritten to the model. **Net lifecycle: upload (no date) → in_review → approved (library, private) → partner pushes → live | upload (dated) → approved → live at date | welcome → approved → live instantly.** App-side eq.approved filter from PM-900 stays correct — RLS is the visibility authority.**

**PM-900 (2026-08-10): DATELESS = EVERGREEN + WELCOME-VIDEO ASK + the dead member pipe — CC `8864a9ed` (portal) + vyve-site `93ddec46` (vbb 545) + partner-content-upload v9 + migration.** Dean's two asks plus one uncovered bug. **(1) Uploads without a date:** portal go-live field now optional — blank = library content, visible the moment VYVE approves. partner-content-upload v9: null publish_at accepted at commit, publish_at:null clears a date at update, 48h lead enforced only when a date IS chosen. **RLS member_read policy REWRITTEN: approved AND (publish_at IS NULL OR publish_at <= now())** — the old policy demanded a non-null date, so dateless approved content was invisible forever. Portal lifecycle: approved+dateless = "Live — In your library". **(2) Welcome-video ask:** the wizard's starter video is skippable (Vanessa proved it), so My Content now opens with an amber "Your welcome video" card until one exists — button pre-fills the title + arms welcome mode, commit stamps NEW `is_welcome` column, card tracks in-review/returned/live and stops nagging once acted on. **(3) DEAD PIPE FOUND + FIXED: the member app's partner-profile.html filtered `moderation_status=eq.published` — a status that exists NOWHERE** (moderation writes approved/returned) — no partner content was member-visible at all, masked by nobody being live yet. Now eq.approved with RLS as the visibility authority. vbb 544→545 + sw cache v2026-08-10h-pm900 atomic (§23.21). **NUMBERING: EF v9 header says PM-894** — claimed before the live re-fetch showed the parallel session at PM-899; header corrected at next functional deploy. Bucket note: partner-docs cap raised to 30MiB in PM-893.**

**PM-893 (2026-08-10): Vanessa's upload failures diagnosed + partner-docs limit raised (config, no deploy).** Her message via Lewis: video upload error, can't get back, "has it submitted?", cover image "disappeared". **DB truth: submission LANDED at 17:04** (agreement + copy emailed, photo ID, qualifications, profile photo all in storage) — only the two failed uploads missing. **Cover image root cause: her PNG is 16.5MB vs the partner-docs 15MiB bucket cap** — storage 413'd the PUT, wizard recorded metadata client-side (path:null, bucket:null in credentials.documents.coverPhoto) and told her nothing useful. **FIX: partner-docs file_size_limit raised 15MiB→30MiB** (bucket config, verified; design-tool PNG exports routinely exceed 15MB). **Welcome video: no object ever landed in partner-content** (bucket takes mp4/mov/webm to 2GiB — her client-side error unknowable from here); steps.starter_videos_uploaded=false, zero partner_content_items. **Resolution path: the PORTAL, not the wizard** — she's provisioned (PM-892) and the welcome video is literally step 2 of her provisioning email; reply drafted for Lewis (submission confirmed, limit raised, portal is the home for video + cover, go-live = upload + tell us + link switches on automatically). **BACKLOG: wizard upload error copy must surface 'file too large (max 30MB)' instead of a silent metadata-only record** — second silent-upload-failure class after PM-836.**

**PM-898 + PM-899 (2026-08-10): PERKS DEBUG LOOP SOLVED — SW PRECACHE WAS FASTLY-STALE — vyve-site `c1c41d74` (543) + `878b6cf2` (544) + NEW §23.173.** PM-898 (543): 'perks':'Discounts' added to nav.js subPageLabels (the top-right mph-page-label map — the header slot was blank because the stem was never registered); error card made diagnostic (real error text + HTTP status/body head, working Try again button replacing the phantom pull-to-refresh copy, catch wired to client-error-report → platform_alerts). **Dean's 543 screenshot was the forensic gift: PM-898's header label rendered NEXT TO pre-898 card copy — nav.js fresh, perks.html stale → the sw precache had stored a Fastly-stale page, and therefore PM-897's auth-race fix NEVER EXECUTED on his device.** The three-deploy 'still broken' loop was one undelivered fix, not three failed ones. Mechanism: PM-220.6's `cache:'reload'` precache bypasses only the LOCAL HTTP cache; the GitHub Pages Fastly edge (§23.162's known ritual) still serves its own TTL copy, and a 5-deploy hour leaves edges holding MIXED file generations — install faithfully stores whatever generation each edge coughs up. **PM-899 (544): install fetches append `?swv=<CACHE_NAME>`** (unique per deploy; response stored under the CLEAN url so fetch-handler lookups are untouched; static host ignores the query) — origin-fresh precache every deploy, staleness class dead platform-wide, §23.162's manual ?z= ritual no longer has an sw-shaped blind spot. Also 544: doubled 'Discounts' fixed — in-page h1 removed now the header carries the label (challenges-pattern pages keep theirs; perks follows Dean's call). Expected on device at 544: precache pulls fresh perks.html → PM-897 auth-ready fix finally runs → Eat Clean renders. If the diagnostic card still appears, its grey line + platform_alerts now carry the real error.**

**PM-897 (2026-08-10): PERKS EMPTY PAGE ON DEVICE (Dean, build 541) — auth-ready race — vyve-site `a0ea2207` vbb 542 + NEW §23.172.** Server side verified CLEAN first (simulated authenticated SELECT returns the Eat Clean row; grants + policy correct). Root cause client-side: **auth.js creates `vyveSupabase` ASYNCHRONOUSLY** — it script-injects supabase.min.js (vyveLoadSupabaseSDK) and only then createClient, so the global is STILL NULL at DOMContentLoaded. perks.html called `window.vyveSupabase.auth.getSession()` at boot → TypeError swallowed by the load try/catch → permanent "Couldn't load discounts" card. The correct primitive existed all along: **`window.VYVE_AUTH_READY`** (resolves {user, supabase} once SDK + client + session settle) — vyveToken now awaits it and null-guards the client; cache paint stays synchronous. **Why the help.html template hid this:** help.html's first client call is USER-TRIGGERED (form submit / category change), long after auth settles — the race exists there too but structurally never loses; any page cloned from it that fetches at boot inherits the bug (PM-824's SDK lesson, async flavour). help.html left as-is (its shape is safe by construction), pattern banked as the rule instead.**

**PM-896 (2026-08-10): FIRST PERK SEEDED — Eat Clean, 10% off — vyve-site `31f6efc6` vbb 541 + one INSERT.** Logo path settled: sandbox has NO storage-write route (no service_role key in vault, supabase.co not in egress whitelist), so **perk logos are repo-hosted at NEW `/assets/perks/` (PM-744 partner-asset convention)**, referenced from `member_perks.logo_url` by absolute `https://online.vyvehealth.co.uk/...` URL (PM-873 cross-host precedent — safe on the bundled capacitor://localhost fleet). The `perk-logos` bucket stays dormant for a future CC upload flow. Dean's AVIF wordmark (349×175) converted to a padded 512px square white-canvas PNG — the card's 52px object-fit:cover slot would centre-crop a landscape mark; padding-to-square = no crop by construction (bank as the pattern for future wordmark-shaped logos). Row: Eat Clean / '10% off' / Food & Drink / Newcastle / show_app (Dean gave no code, till-show is the default mechanic) / id 3cd0a06d. Row is LIVE to members now — no deploy needed, the PM-893 promise holding on its first use. Website URL left null (not provided, never guessed). Remaining Newcastle businesses still awaited from Dean.**

**PM-895 (2026-08-10): PERKS 'NEAR YOU' AREA PICKER (Dean: discounts local to yourself) — vyve-site `c2d3af6a` vbb 540.** Mechanism call: NOT geolocation — native GPS needs a permission prompt + NSLocationWhenInUseUsageDescription the shipped binary doesn't carry (new binary, not OTA-able), and one-city density doesn't justify it. Instead: pin-labelled 'Near you' chip row above the category chips, areas auto-derived from `member_perks.area` (row hides itself until a SECOND area exists in the data — appears automatically as the catalogue grows, zero further build); member's pick persisted on-device (`vyve_perks_area_v1` localStorage, no PII/no members-table column — deliberate GDPR-minimal) and defaulted on return. `area` NULL/'Online'/'Everywhere' rows are included in EVERY area view (an online code is local to everyone). Category chips + empty states are now area-scoped. JS syntax-checked; all 4 files md5-perfect. GPS distance-sort = future-binary option if density ever warrants; noted, not backlogged.**

**PM-894 (2026-08-10): perks.html header consistency (Dean device feedback) — vyve-site `dd3aef96` vbb 539.** PM-893 shipped the help/certificates oversized hero (uppercase eyebrow + DM Serif clamp-52px title); Dean flagged it inconsistent with the sibling More pages. Replaced with the compact challenges.html pattern: `.page-title` (var(--font-head), 800, 30px) + `.page-sub` (text-sm muted) inside a 640px/18px wrap with the fadeUp entrance — title "Discounts", sub unchanged. CSS+markup only, all logic untouched. NOTE for future page builds: the app has two header lineages — big-hero (help, certificates) vs compact page-title/page-sub (challenges, leaderboard) — default NEW More-menu feature pages to the compact challenges shape.**

**PM-893 (2026-08-10): MEMBER DISCOUNTS AREA LIVE IN MORE (Dean ask, one session) — vyve-site `ab3c484c` vbb 538 + migration `member_perks_feature_pm_xxx` (named before the PM claim — cosmetic).** Supabase-driven local-business discounts for members (first cohort: Newcastle businesses already agreed, seeding pending Dean's list). **DB:** `member_perks` catalogue (business_name/headline/description/category/area default 'Newcastle'/logo_url/website_url/redemption_type CHECK code|show_app|link/code/instructions/active/sort_order/valid_from/valid_until) — RLS: authenticated SELECT only where active AND inside the validity window (date gating enforced server-side, not client), is_admin() ALL. **Adding a business = one INSERT, live in the app on next page load — zero deploy, zero OTA** (the page reads live via PostgREST). `perk_events` usage tracking (member_email/perk_id/event CHECK view|reveal|link_click) — member-insert-own RLS, is_admin() read for stats; **registered in gdpr_table_policy as purge at build time** (the PM-707 challenge-photos lesson applied forward). New public bucket `perk-logos`. **PAGE:** NEW `perks.html` — help.html-pattern shell (theme.css dual-theme, CSP, auth.js/bus/analytics/nav/offline-manager), hero + category chips (auto-derived, hidden under 2 categories), card grid (logo w/ initials fallback, business, teal headline, category+area tags, description). **Redemption = tracking mechanic:** discount hidden behind "Show discount" → branded teal reveal panel (VYVE member offer + headline + today's date + business — till-trust framing); 'code' type adds a dashed tap-to-copy code, 'link' type goes straight out logging reveal+link_click. Every reveal/view/link_click POSTs perk_events via PostgREST **apikey + session bearer (§23.166 honoured)**, fire-and-forget — tracking never blocks the member. Per-business reveal counts = the usage story for the businesses ("N members pulled up your offer this month"). localStorage cache for instant paint, live refresh behind it; view events deduped per page load. Inline handlers hardened data-attribute/textContent style (no quoted-literal injection from admin-entered categories/codes). **NAV:** Discounts entry (Lucide tag) in desktop More dropdown Tools section + mobile More sheet after Replays. sw CACHE_NAME → v2026-08-10a-pm893-perks, perks.html precached. All 5 files md5-perfect at ab3c484c. **OPEN:** seed the Newcastle businesses (need name + discount + redemption type + optional logo/site per business); [LEWIS COPY PASS] on member-facing copy incl. the third-party-offers disclaimer line (shipped placeholder-honest); optional fast-follows backlogged (CC perks admin surface, per-business stats line).**

**PM-892 (2026-08-10): PORTAL-ACCESS VISIBILITY on the partner detail — CC `b5e6142f` + admin_audit_log read policy.** Dean advanced Vanessa applied→onboarding and asked whether login details were sent. **CONFIRMED via audit trail:** Gate-A provisioning fired at 17:23 (his advance) — new auth account + temp password created, admin_users role 'partner' granted, and the welcome/next-steps email **sent successfully** (partner-provision v5's email_sent flag; she hasn't signed in yet). The 6 Aug audit row with null fields is the v2 broken-run artefact from the PM-859 bug history. **SHIPPED:** the partner detail hero now shows an access line — green "Portal access active — login email sent ✓ (new account + temp password | existing VYVE login) · when by whom", red callout if the audit says the email FAILED (PM-859's tell-them-manually case), amber "No portal access yet — granted automatically when they advance to onboarding", amber warning when contact_email is missing. **Enabling migration:** admin_audit_log had NO authenticated SELECT policy (EF writes are service-role) — admins could never read their own audit trail; now is_admin() SELECT. Loader hooked after the hero paints (sync element lookup — caught in review before commit).**

**PM-891 (2026-08-10): DECLINED RESUBMISSION RETURNS TO THE BOARD (Dean: "Vanessa applied, notifications fired, Pipeline shows nothing") — partner-onboarding v16 + data fix.** Root cause: Vanessa Sturman (real applicant, declined ~Jul cohort) resubmitted TODAY (17:04, status_changed_at) via her resume link. v15 deliberately allows re-application after decline — but `submit` never flipped partner status (drafts are born 'applied', so it never needed to), so the "ready for review in Pipeline & Approvals" notifications fired while the board correctly hid her 'declined' row. **DATA FIX:** her status flipped declined→'applied' (verified) — she's now in the Applied column awaiting Dean's review; NOTE she WAS declined in July, so if that decline was deliberate and final, decline again from the panel. **EF FIX (v16, platform version 23, verify_jwt:false):** getDraft selects status; submit flips 'declined'→'applied' + stamps `_system.resubmitted_after_decline_at` for the record; ONLY declined flips — live/onboarding partners replaying an old resume link are never demoted.**

**PM-890 (2026-08-08): TWO-WAY SUPPORT THREADING — CC `e07833e3` (partners.html + partner-portal.html) + partner-support v2 + partner_support_replies.** Completes Dean's support loop: admins reply IN the CC inbox, partners get an inbox IN the portal. **Table:** partner_support_replies (message_id FK, sender_role admin|partner, body 1–4000) — RLS: partner SELECTs replies on own threads via message join, admin ALL, **deliberately NO authenticated INSERT** (all writes through the EF so emails + limits can't be bypassed). **partner-support v2:** role-aware `reply` — admin (admin_users active check, 60/day) → stored as VYVE, thread → 'replied', **partner emailed the reply body** + portal pointer [LEWIS COPY PASS]; partner (own-thread 403-enforced, 10/day) → thread → 'open' (back in the CC badge), team@ emailed reply-to-partner. **CC:** threads render inline (VYVE replies indented teal, partner replies plain), per-card reply box → EF with honest emailed/failed toast; manual "Mark replied" RETIRED — a real reply IS the reply (Close/Reopen stay). **Portal:** "Your messages" card on Help (hidden until threads exist) — expandable threads, You/VYVE bubbles, status pill, inline reply, refreshes after a new send, loads in the init batch. Base md5-matched vs HEAD + PM from live re-fetch (collision discipline held this time).**

**PM-889 (2026-08-08): CC SUPPORT INBOX (Dean ask, completes the PM-887 support loop) — CC `a578887b` partners.html.** New sidebar item "Support inbox" (chat icon, red open-count badge hidden at zero) + full view over `partner_support_messages`: open messages float to top; each card = partner name + sender email + timestamp + subject + full message + status pill + Mark replied / Close / Reopen (admin RLS update, optimistic re-render). Email to team@ remains the REPLY channel (reply-to = the partner — answering is just replying); the inbox is the tracking surface so nothing falls through the team@ pile. Loaded in the loadAll batch; base-vs-HEAD md5-matched and PM claimed from a live commit re-fetch before commit (parallel session active tonight — burned through PM-888).**

**PM-887 (2026-08-08; committed as "PM-882" in 55725a31 — misnumbered, corrected in f830d367): portal HELP TAB — FAQ + message-us (Dean ask) — CC partner-portal.html + partner-support EF v1 + partner_support_messages table.** **PM COLLISION LESSON (§23.24 miss, this session):** number claimed from my own last brain entry instead of re-fetching commits at claim time — a parallel session had burned PM-882–886 (analytics repair aae960b8, unified shell d9479e59, team access 3c6563e9, brand-home 7f381da5, cron lateness 50cee5ed) minutes earlier. Their partners.html edit (7f381da5) preserved my PM-880 gate (verified: assessments_deferred_at_golive present at HEAD ×3); my portal commit clobbered nothing (their commits touched no shared file). partner-support EF header comment still reads PM-882 — cosmetic, corrected at next functional deploy. New Help tab (after Bookings): (1) **FAQ accordion, 10 questions** written to the current truth of the partner model — link/QR/code attribution + activates-at-go-live, member free week + £10 partner rate, 50% of what members actually pay, monthly bank-transfer payouts + bank-details card, bookings at 100% minus payment fees (current model — updates when bookings-through-VYVE ships), content moderation + clinical approval for therapy, the no-identities privacy answer + exactly what the nudge email says [LEWIS COPY PASS throughout]. (2) **Message us** — subject + message form → NEW `partner-support` EF v1 (verify_jwt:true, 401-no-auth verified by invocation; partner self-resolved via caller bearer, partner-nudge pattern; 5/day/partner rate limit; **row written to NEW `partner_support_messages` FIRST** — durable record + future CC inbox; RLS partner-own-SELECT/admin-ALL — then Brevo to team@ with **reply-to the sender**, so a plain email reply answers the partner; honest sent/logged result). Table checks: subject ≤200, message 3–4000, status open/replied/closed.**

**PM-882 → PM-886 (2026-08-08 night): CC STREAMLINE SESSION 1 — ANALYTICS REPAIRED, UNIFIED SHELL PHASE 1, TEAM-ACCESS HOLE CLOSED.** Kickoff of the whole-CC streamline (Dean). **PM-882 ANALYTICS AREA REPAIR (CC `aae960b8`, 18 files):** (1) PM-779 made `VYVE_SUPABASE.client` a getter; auth.js + ALL 8 analytics assets called `.client()` and threw — every Insights page had been fetching EFs/PostgREST WITHOUT a JWT since 12 Jul (admin-read RLS returned empty = eternal loading dashes). All 9 call sites → `getClient()`. (2) app-health.js IIFE-wrapped: the SPA router re-executes page scripts on EVERY navigation and never caches app-health; bare top-level `const SB_URL` = SyntaxError on second entry, script dead ("App Health failed to load"). Inline-handler globals exported; stale-run guards after every await. (3) **wellbeing.js was PARSE-DEAD in prod since ship** — unescaped apostrophe (`don't`) in a single-quoted string; the Wellbeing page had never executed its JS. usage.js + retention.js share the bare-const re-entry bug (LATENT, flagged for the streamline). **Alert triage:** all 15 "Needs attention" rows were info-severity noise (healed charity drift, HealthKit diagnostics, one crawler page_load_timeout, one Brevo-401 GDPR-email one-off) — bulk-resolved via SQL; hygiene rules for the streamline: info never counts toward Needs Attention, self-healed alerts auto-resolve. **NOTE for Lewis: Brevo has authorised-IPs security enabled and Supabase egress IPs rotate — will intermittently 401 transactional sends until disabled.** **PM-883 UNIFIED SHELL PHASE 1 (`d9479e59`, Dean-approved mockup):** desktop >900px gets a persistent left rail (280px, full VYVE_NAV; hamburger collapses to 76px icon rail, `html.rail-collapsed` pre-paint, persisted); topnav domain-tab row RETIRED (router still renders into a hidden container — zero routing risk); mobile ≤900px keeps the drawer + NEW bottom tab bar (Home / #/domain-analytics / #/domain-members / #/domain-partners / More→drawer, safe-area padded, active tab from the slug's VYVE_NAV section). Agreed sequence: phase 2 = partners.html port into the SPA, phase 3 = admin-console.html port, then analytics consolidation. **PM-884 TEAM ACCESS CONTAINMENT (`3c6563e9` + migration is_admin_role_filter_pm884):** FOUND WIDE OPEN — admin_users already held 5 active role='team' rows (Calum/Cole/Heidi/Phil/Ryan) + 2 role='partner', and **is_admin() had NO role filter** — every one resolved as FULL CC OWNER (Settings, Finance, member PII, all 37 is_admin RLS policies). Migration adds `role='admin'`; is_admin_or_team (46 policies) stays the broader gate by design. acl.js PAGE_POLICY: member tier = operating set only (home/tasks/calendar/documents/content/podcast); crm/brief/broadcast/active-users/complaints/bookings/employers + all 8 analytics raised to lead. admin-console.html joins the shared `vyve-cc-supabase-auth` storageKey — ONE sign-in across SPA + partners.html + admin console (old-key sessions orphan once); partner-portal keeps its own key deliberately. **Invite flow settled: teammate = admin_users role='team' row; they land at member tier; per-person promotion to lead via CC Settings.** REMAINING HOLES (access-model session): admin-member-* EFs verify admin_users.active ROLE-BLIND (team passes server-side); members-table is_admin_or_team READ (PM-796, powers Active Users) lets team query member rows via PostgREST regardless of page acl; SPA magic-link vs password sign-in unification. **PM-885 (`7f381da5`):** top-left brand → CC home on all three admin surfaces (rail, admin-console, partners.html); partner-portal untouched. **PM-886 (`50cee5ed`):** cron lateness now judged against the cc_app_health snapshot's refreshed_at, not Date.now() — wall-clock-vs-snapshot maths guaranteed false Late/Overdue on every job with interval < cache age (live check during the third false alarm: all */5 jobs ran 81s prior, zero failures 2h). NEW §23.169/170/171 banked. Dean device checks: rail + collapse, bottom tabs, App Health cron panel post-deploy, single sign-in across surfaces.**

**PM-881 (2026-08-08): wizard landing header rework (Alan feedback via Dean; mocked, Dean approved) — Test-Site partner-onboarding.html.** Alan on mobile: "top of the screen, what do they do? … have it down the bottom after the process bit." He was right twice over — of the three header buttons, 'Process overview' navigated to the screen you were already on and 'Start onboarding' duplicated the in-flow Begin. Now: header = logo + icon-only theme toggle (moon/sun glyph, aria-labelled); '← Overview' back-link renders in the header ONLY inside the wizard (showScreen toggles #hdrBack); primary CTA sits directly after the process flowline ("Start onboarding" + "~10 minutes · save and resume any time" subline); bottom vetting-banner button relabelled Begin → Start onboarding. Mockup-before-code held (4-panel visual, approved before build).**

**PM-880 (2026-08-07, same session): GO LIVE GATE REWRITTEN — the PM-879 flag bit within minutes (Dean: "clicking go live does nothing") — CC partners.html.** Root cause exactly as flagged: the gate demanded safeguarding && gdpr && pct=100 (Dean's partner: false/false/38 — structurally unreachable for the deferred cohort) and rejected with only a toast Dean never saw. **New gate = the PM-857 agreed allow+stamp model:** deferred partners (steps.modules_deferred) need agreement+identity+profile, then an EXPLICIT confirm that names what's deferred, that members will see the partner WITHOUT the assessments, and that **Lewis's sign-off on the posture is still outstanding** — proceeding writes `steps.assessments_deferred_at_golive` (timestamped; write path verified against the admin ALL RLS policy; a failed stamp ABORTS go-live). Review panel shows "WENT LIVE with assessments deferred (date)" in amber permanently. Non-deferred partners keep the hard gate byte-identical. The Lewis decision stays open — the gate now surfaces it at the moment of the click instead of silently blocking, and the stamp is the audit trail either way.**

**PM-879 (2026-08-07, same session): PARTNER SURFACES DEEP DIVE (Dean ask after hitting a live discrepancy) — CC `a21ea11d` (both files).** Dean's own join link answered "invite not active": CORRECT behaviour (`validate_code` gates on status='live' — partners must not recruit pre-go-live) but a genuine display discrepancy — CC Settings and the portal both presented the link/QR as ready with no hint it was dormant. **FIXED:** status-aware amber banner on BOTH surfaces — portal: "Your link activates at go-live… switches on automatically" (link+QR stay visible for poster prep); CC Settings: mirror line naming the partner's current status. **SWEEP FINDINGS (4 surfaces: CC partners.html, partner-portal.html, wizard, join.html):** (1) **FLAGGED, NOT FIXED — the Go Live gate is unpassable for the deferred cohort:** `advancePartner('live')` requires `safeguarding_passed && gdpr_passed && pct_complete=100`, but PM-659/857 deferred the assessments by design, so those flags are false and DB pct can't reach 100 — **the first real go-live click will bounce off this gate**. Deliberately NOT changed unilaterally: it IS the safeguarding/duty-of-care posture question already awaiting the Lewis nod (deferred-at-go-live + visible CC stamp is the standing recommendation). Decision then a ~15-min gate rewrite. (2) **TRUE-TODAY, changes later:** portal Bookings copy "members pay on your own payment link" + bsf-paylink field accurately describe the shipped PM-812 model; they retire with bookings-through-VYVE (already backlogged post-Piece-2). (3) **NOTED:** portal MRR still sums `partner_memberships.subscription_value` (webhook/reconcile-fed) while the referred count is now the members-table RPC — two sources that agree at zero today and converge once the reconciler stamps memberships at first conversion; unify onto one source when the payout run builds. (4) Wizard clean: no price claims, "50% revenue share… through you" and the agreement's "Introduced Customer" definition both match the PM-868 attributed-only model. (5) No other "Not assigned"-class dead reads found on any surface.**

**PM-878 (2026-08-07, same session): CC partner Settings shows the real referral machinery (Dean screenshot) — CC partners.html.** The Settings tab still carried the RETIRED pay-now model: a paste-a-Stripe-URL "Partner payment link" section with the now-FALSE amber warning "partner cannot earn referrals until this is set". Replaced with **Referral link & QR**: derived `vyvehealth.co.uk/join/<slug>` with Copy, client-rendered QR + Download-as-PNG (`vyve-join-<slug>.png`), spoken-code line — mirror of the partner's own portal card (PM-874). `payment_link_url` SOFT-KILLED (column stays, UI + savePaymentLink removed). **Stripe coupon field KEPT deliberately** — Piece 2's minted conversion Checkout Session attaches `partner_partners.stripe_promo_code`; relabelled "applied at conversion" with honest Piece-2 copy. Rev share % + verified untouched.**

**PM-877 (2026-08-07, same session): WATCH-IN-PLACE for CC Content & Moderation (Dean ask) — CC `see commit` + partner-file-url v9.** Media items previously offered Download only; now a ▶ Watch button (moderation queue AND library branches) fetches the same admin-gated signed URL with `disposition:'inline'` into a modal `<video>` (autoplay, Esc/backdrop/Close all pause + release the src). partner-file-url v9: inline signs for **3600s** (was 300 — a full playback's range re-requests would have outlived the token); download unchanged at 300. Non-video media opens the inline URL in a tab. The play icon already existed in the CC icon set.**

**PM-876 (2026-08-07, same session): PAYOUT RAILS GROUNDWORK + phantom attributed-subs purge — CC `aa6bdae5` (partner-portal.html + partners.html) + partner_bank_details migration.** Payments architecture talked through and SETTLED with Dean: **VYVE is merchant of record for everything** — subscriptions already are; bookings MOVE to VYVE-managed payments (currently partner's own payment link) with **100% to the partner minus Stripe fees for now** (commission % = later commercial call); partners never run their own Stripe. **Payout = invoice + bank transfer via the finance director on a set date each month — BACKLOGGED** (self-billing recommendation on the table: VYVE generates the invoice on the partner's behalf, one agreement clause — joins Lewis's owed wording; rollover threshold ~£25 suggested). **BUILT NOW:** (1) `partner_bank_details` table — UK account name / sort code (6-digit CHECK) / account number (8-digit CHECK), RLS own-row via get_my_partner_id + admin SELECT via is_admin, touch trigger; own-row write+read verified under simulated partner JWT. (2) Portal Earnings "Payout details" card — capture form with client validation, masked summary (sort code fully hidden, account last-4), Update flow. (3) CC revenue pane leads with finance-run readiness line: on-file (name + last-4 + updated date) or amber none-yet. **PHANTOM DATA KILLED:** Dean's "every partner shows 1 attributed sub" = 7 stale June-2026 seed rows in partner_payouts (gross £0, pending) — PURGED; partner_memberships checked: all 15 rows are community joins (referred=false, no stripe sub, no value) so nothing real was counted anywhere. runPayouts hardened to **Dean's definition: attributed sub = referred AND active AND subscription_value>0**; £0-gross partners generate no row; empty period toasts instead of writing noise. **BACKLOGGED:** finance-director invoice+payment run (self-billing generator + statement lines + payment-date field); bookings-through-VYVE checkout (EF-minted Checkout Session + webhook + refund path — sequences AFTER Piece 2, reuses its machinery, ~2 sessions).**

**PM-875 (2026-08-07, same session): SEND A NUDGE — the PII-clean partner reach-out (Dean decision: nudge over identified list) — CC `222bdcea` + partner-nudge EF v1.** Portal: "Nudge lapsed members — we'll email them for you" button under the PM-874 funnel strip, shown only when trial_expired > 0; confirm dialog; honest result line (sent / skipped-this-month / failed). **partner-nudge EF v1 (verify_jwt:true — platform 401-no-auth verified by real invocation):** partner self-resolved by calling `get_my_partner_id()` under the CALLER'S OWN bearer (unspoofable — nobody can nudge another partner's members); 1 batch per partner per day (check_rate_limit); 1 nudge per member per calendar month (engagement_emails stream `partner_nudge`, key `nudge_YYYYMM`); eligible bucket = partner_referral_stats expired definition + is_test excluded (SQL-verified: exactly the 8 real men-together-cic lapsed trials). Email = partner-led framing ("{partner} asked us to check in") + continue.html?cid= conversion link on the trial-end-email v1 shell [LEWIS COPY PASS]. **§23.143 discipline held: engagement_emails stream CHECK widened to include 'partner_nudge' BEFORE the first EF write** (migration same wave — this constraint has silently eaten writes three times before). **Member identities never cross to the partner** — the employer-dashboard PII line extends to partners as settled posture. **EXPOSURE NOTE:** the only partner with eligible members (Men Together, 8 lapsed) has NO portal login (no contact_email, never provisioned), and Dean's test partner has 0 eligible — so nothing can fire with placeholder copy today. **Lewis copy pass on the nudge email belongs on the Gate-A/provisioning checklist before Men Together is ever provisioned.****

**PM-874 (2026-08-07, same session): PARTNER REFERRAL CARD AUTOMATIC + QR + FUNNEL (Piece 1 first half) — vyve-command-centre `2790e621`.** Dean's screenshot showed the Earnings card reading "Not assigned yet — contact team@" for his test partner: it read `payment_link_url || stripe_promo_code`, both artefacts of the RETIRED pay-now model, plus stale "£10 off forever pre-applied" coupon copy. Rework: **the link is DERIVED from the slug** (`vyvehealth.co.uk/join/<slug>`) — automatic for every partner past and future from the moment the record exists; "Not assigned" is structurally impossible and Dean's "generate a QR automatically when an area is made" holds by construction: **the QR is rendered client-side from the link** (qrcode-generator via jsdelivr — no CSP on the page, jsdelivr already in use for supabase-js; nothing stored, can never go stale) with Download-as-PNG (640px canvas re-render, print-ready). Code-for-spoken-referrals line shows the slug (validate_code slugifies typed input, so even "Calum Denham Performance" typed loose resolves). **Funnel strip** via NEW RPC `partner_referral_stats()` (SECURITY DEFINER, self-scoped through get_my_partner_id, GRANTed authenticated — safe: **AGGREGATES ONLY, same PII line as the employer dashboard**): On free trial (trial + active/trialing + clock NULL-or-future — shells included) / Paying (account_type='paid' incl. past_due) / Trial ended without joining (amber when >0). Bucket SQL verified against men-together-cic's real 8 attributed members (all expired 22-Jul trials — correct); Dean's test partner correctly zeros; JWT-simulation run clean. **"Referred members" stat REDEFINED per Dean: attributed AND paying** (was partner_memberships active count). **PII DECISION RAISED, PENDING (Dean+Lewis):** Dean asked for identified members ("show which users are which so they can reach out directly") — flagged as member-PII-to-third-party, same line held on the employer dashboard; recommended alternative: counts (shipped) + a "send a nudge" action where VYVE emails the lapsed member on the partner's behalf (PII never leaves VYVE, no privacy-policy change). Identified list would need a Lewis/DPIA call + privacy-policy line first. **Remaining Piece 1:** human promo codes + backfill, share text, joins-by-source split; nudge-on-behalf once decided.**

**PM-873 (2026-08-07, same session): REAL LOGO ON DOOR + JOIN PAGE (Dean device feedback) — vyve-site `ed3886a8` vbb 537 + Test-Site `3bbcd134`.** Both pages shipped with a text-only wordmark; now logo.png + "VYVE Health" in exact login.html parity (door: /logo.png 56px r12; join page: absolute https://online.vyvehealth.co.uk/logo.png 64px r14, cross-host). jsdom 40/40 + 16/16. **Referral field on the door: SETTLED KEEP (Dean accepted).** The clinching structure: link-attributed members create their account on the WEB, so at the app door they hit Welcome back and never see the create state — the referral toggle is only ever seen by people who arrived WITHOUT a link (spoken/printed code), for whom it is the only possible attribution. The two routes cannot meet the same person; the confirmation email contains no code. Partner-side framing (link where tappable, code where not — never both) is a Piece 1 share-kit copy line for Lewis. Revisit only if post-Piece-1 source split shows zero code signups.**

**PM-872 (2026-08-07, same session): DOOR CARD CLIPPED UNDER THE DYNAMIC ISLAND WITH KEYBOARD OPEN (Dean device feedback) — vyve-site `f3c0de33` vbb 536.** With `interactive-widget=resizes-visual`, opening the keyboard shrinks the visual viewport; the body's `align-items:center` flex centering then pushed the card's top edge above the reachable viewport — the "Create your account" title sat under the speaker pill and could not be scrolled back (classic flex-centre overflow clip). Fix: centering moved from body align/justify to `.wrap { margin:auto }` (identical centred look on tall screens, but overflow stays scrollable) + `.wrap` top padding now `calc(env(safe-area-inset-top) + 24px)` additive so the title always clears the island. CSS-only, jsdom 40/40 regression. **Referral field on the door: Dean asked whether to remove; KEPT with reasons accepted** — it is the ONLY place in the product a spoken/written partner code ("use code MAYA") can ever be entered; removing it kills the 'code' half of the attribution model that Piece 1's share kit builds around; it costs nothing collapsed behind the "Have a referral code?" toggle. Revisit with source-split data after Piece 1.**

**PM-871 (2026-08-07, same session): CONFIRM-EMAIL ADDED TO THE APP DOOR'S CREATE STATE (Dean device-walk call) — vyve-site `3d21c260` vbb 535.** Dean walked PM-870 on device (both door paths confirmed working: made-up email → create, real email → Welcome back → in) and called for the confirm-email guard in-app too. He's right and the PM-870 rationale for omitting it was WRONG: a typo'd address in the app is a silent time bomb, not an instant failure — the session persists, so the member uses the app happily and the typo detonates at trial-end (conversion email to a dead address = lost revenue), plus no sign-in-elsewhere or password reset ever. Same failure profile as the join page, so the same guard: confirm field under the locked email in the create state, live mismatch hint pointing at the Change link, Change clears it, create gated on the match, `disabled` outside the create state (PM-870 password-manager hygiene held). Applies in employer mode too. jsdom 40/40. sw cache → v2026-08-07b-pm871-confirmemail. Dean walk of the create path + join page still pending.**

**PM-869 + PM-870 (2026-08-07): ATTRIBUTION PIECES 3 + 4 SHIPPED — /join is account creation, the app front door is three-state.** Spec v2.1 executed in build order; Pieces 1, 2, 5 remain (order 1→2→5), Piece 6 (AASA/assetlinks association) rides the next native binary.

**PM-869 — JOIN PAGE = ACCOUNT CREATION (Test-Site `1f86c662`).** `vyvehealth.co.uk/join/<slug>` now lands on NEW `join.html` (the dead PM-§23.129 file repurposed): validates the slug via app-signup `validate_code` on load and shows “You’ve been invited by {partner}”; **invalid/missing slug or validate-network-failure = HARD STOP** (error card → vyvehealth.co.uk — Dean call this session: never degrade a partner link to an unattributed signup); email + CONFIRM EMAIL (the typo guard that replaced the PIN) + password w/ live requirements in a real `<form>` (autocomplete username/new-password — semantics only until Piece 6); NO price, NO payment, free-week framing only. Success → store buttons + “Sign in with the email and password you just created” + honest we’ve-emailed-you note (shown only when `email_sent`); 409 → friendly already-have-an-account + store buttons. `404.html`: `/join/<slug>` → `join.html?p=<slug>`, tier param DROPPED (identical price on every route — old printed `/join/<slug>/15` links gracefully ignore the tier); `/trial` route + error fallback untouched; `partner-join-redirect` EF stays deployed unused (soft-kill later). **12 partners’ circulating /join links have flipped behaviour: pay-now Stripe → free-week account creation.** jsdom 16/16 (hard stops incl. network-fail, payload shape, email_sent honesty, 409, mismatch/weak-pw blocks).

**Backend (PM-869):** migration `partner_attribution_columns_and_account_state_rpc` — `members.attribution_source` (CHECK NULL|link|code|admin; NULL on historic rows = provenance honestly unknown) + `members.signup_channel` (NOT NULL default 'app'; 'web_join' = the shell flag) + SECURITY DEFINER RPC `public.auth_account_state(p_email)` → (has_user, has_password), REVOKEd from PUBLIC/anon/authenticated, service_role only. **Passwordless detection verified in prod: GoTrue leaves `auth.users.encrypted_password` NULL for admin-created no-password users** — exactly 2 exist (lsvines@hotmail.com, swatts@ — April backfill stragglers); Dean’s b2binvite test account has a real hash because he completed set-password on the 24 Jul device walk. **33 accounts hold random backfill hashes they never chose → the door’s sign-in state carries a prominent Forgot-password resend so none of them dead-end.** `app-signup` **v4 (platform version 6, verify_jwt:false explicit §23.165):** (1) action `account_state {email}` → `{state:'none'|'password'|'passwordless'}` — **deliberate rate-limited enumeration surface (Dean-accepted trade)**: per-IP 20/h + per-email 10/h, response carries the state and nothing else; (2) signup accepts `attribution_source` (allow-listed link|code, honoured ONLY when the referral actually validated — no phantom provenance) + `signup_channel` (allow-listed web_join, else 'app'); (3) web_join signups fire a confirmation email (store buttons + sign-in-with-what-you-just-created + partner mention when attributed) [LEWIS COPY PASS], `email_sent` returned honestly (PM-859 lesson). Live-verified via pg_net: all three account states resolve correctly; full web_join signup → members row carried code+source+channel, trial_ends_at correctly NULL (clock starts at onboarding), email confirmed sent; validate_code regression clean; test residue deleted child-first, zero remaining.

**`re-engagement-scheduler` v14 (platform version 43) — shell-routing fix found during build:** web_join shells stamp privacy_accepted_at at web signup, so v13 would have routed a never-installed shell into **Stream B — “You’ve been in the app”, a false claim to a partner-referred person**. v14: select adds signup_channel; `web_join && !onboarding_complete && no-activity` → Stream A (which IS the spec’s “you never finished” nudge — download the app, sign in with the password you created); buildA gains an honest pre-onboarding copy variant (promises the 2-minute in-app setup instead of “your workout plan is assigned” — no programme exists yet) [LEWIS COPY PASS]; shells have no persona/goal so no AI line fires; dry-run results carry `shell` flag. Dry-run live-verified: version 14, test shell routed **stream A** (v13 → B), zero would-send deltas for the existing 17 members.

**PM-870 — THREE-STATE FRONT DOOR (vyve-site `fd9c7495` vbb 534).** `signup.html` is now email-FIRST: Continue resolves `account_state` and the card morphs in place — **the signup-vs-login fork is GONE for fresh devices; one door, the server routes, the member can never pick wrong.** 'none' → create-password (email locked w/ Change link, referral machinery intact — typed codes now stamp `attribution_source='code'`); 'password' → Welcome-back sign-in (autocomplete current-password; success → index.html, auth.js gates route onboarding vs home; Forgot-password fires the PM-830 branded resend); 'passwordless' → auto-fires the set-password email ONCE + check-your-email state w/ resend. Hidden state inputs are **`disabled`** so password managers never fill the wrong field. Create-409 race morphs to sign-in in-place (no page hop). Employer mode (?c=emp_) skips the door — the invite link IS the routing — full create form immediately, PM-828 behaviour preserved, signin escape-hatch row kept there only (dropped in normal mode: redundant once the door routes). 429 on the door → friendly error, stays on email. jsdom 35/35 first run (state morphs, disabled-input hygiene, payload shapes incl. employer token + no-referral, both navs + device flag, wrong-password inline, forgot→check+resend, session redirect, door 429). sw cache → v2026-08-07a-pm870-threestatedoor.

**Left open on this workstream:** Pieces 1 (codes + share kit), 2 (EF-minted Checkout Session), 5 (CC manual grant — admin source already in the CHECK constraint); Piece 6 association files ride the next binary; Lewis copy passes on the join page, confirmation email, shell Stream-A variants, and door copy; Lewis still owes agreement wording + portal “the link is how you get paid” line (PM-868). Dean device walk pending: join page via fresh `?z=` (Test-Site Fastly cache §23.162), door via server.url dev-loop.**

**PM-868 (2026-08-07): PARTNER ATTRIBUTION DECISIONS SETTLED — self-declared route REMOVED.** Dean's four calls on the PM-867 spec, folded into `playbooks/partner-attribution-and-onboarding-spec.md` (now v2.1): **(1) NO per-partner rates for now** — £10 universal; may change, and Piece 2's minted Checkout Session keeps tiering available so a later change needs no rework. **(2) Rev share is paid on ATTRIBUTED members only** — a member who finds VYVE and buys for themselves is attributable to nobody; the partner's job is pushing their link. **(3) SELF-DECLARED ATTRIBUTION REMOVED — Piece 5's in-app "did someone invite you?" picker is WITHDRAWN.** Dean's reasoning, which is better than the original recommendation: a self-service picker quietly teaches partners that the link doesn't matter, undercutting the exact behaviour the model depends on. The genuine case (Michael joins cold but is Sarah's friend and wants her credited) becomes a **support request** — help-area line → team@vyvehealth.co.uk → admin sets attribution by hand in CC. Not self-service, because it cannot be micromanaged at scale. Manual grants: `attribution_source='admin'` + approving admin recorded (always distinguishable from earned), **pre-conversion ONLY** (once money has moved, attribution is frozen — no retroactive re-cutting of revenue already taken and paid), visible in the partner's joins split. Piece 5 accordingly shrinks from ~1 session member-facing build to ~0.5 session CC-side control. **(4) Partner bio/why/feel limits 60/400/200/160 AGREED** — pending build, may be revisited. The anti-abuse spine is unchanged and still load-bearing: identical price on every route means nobody gains by inventing a referral, which is what makes granting the human exception low-risk. **STILL OWED FROM LEWIS:** agreement wording (rev share on attributed members; attribution via the partner's link or code; anything else a manual exception at VYVE's discretion) + the partner-portal copy line telling partners plainly that the link is how they get paid — Dean to talk both through with him.**

**PM-867 (2026-08-07): PARTNER ATTRIBUTION + IN-APP ONBOARDING SPEC LOCKED (design only — no code).** Dean thought aloud through the funnel end to end; the shape is now settled and lives at `playbooks/partner-attribution-and-onboarding-spec.md` (v2, supersedes the v1 draft). **THE FLOW:** Sarah shares `/join/<slug>` → page collects email + confirm email + password (NO payment, NO questionnaire, NO price) → account created server-side with `signup_campaign_code=<slug>` + `attribution_source='link'` → store buttons + confirmation email → Michael installs, signs in, app sees `onboarding_complete=false` and runs the QUESTIONNAIRE IN-APP (the friction fix Dean wants) → 7-day trial clock from onboarding complete → `trial-end-email` reads the slug, names Sarah, links to checkout with HER coupon pre-applied → £10/mo, Sarah earns £5 (50% of net, matching stripe-webhook v13/reconcile v5). Michael never types or remembers a code. **CONSTRAINTS THAT FORCED IT:** iOS passes NO referrer through the App Store to a fresh install, so any install-first attribution needs probabilistic fingerprinting (Branch) — REJECTED, we'd pay rev share on a guess; attribution therefore happens on the WEB before install. Payment Links can't pre-apply coupons (§23.127) so conversion must become an EF-minted Checkout Session. No price/buy button in-app (Apple). **DECISIONS SETTLED:** 7-day trial for EVERYONE (no partner variation); NO one-time PIN — a confirm-email field catches the typo the PIN was really guarding, and collecting a password on web means the account is fully formed; `/join/<slug>` changes purpose from pay-first checkout to ACCOUNT CREATION. **THREE-STATE FRONT DOOR (Dean's "do they see sign up or sign in?"):** one email field, server resolves — no account → create; account WITH password (the normal partner-link case) → sign in; account WITHOUT password (B2B invitees, provisioned partners) → existing PM-830 branded set-password email. The member never picks, so can never pick wrong. **ANTI-ABUSE SPINE (Dean's "people might claim a referral for a bigger discount"):** the price is IDENTICAL on every route, so naming a partner never saves the member money and there is no incentive to lie; residual collusion risk (~£5/mo per false claim) handled by `attribution_source` tracking + self-declared allowed ONCE and only pre-conversion + source split visible to partner and CC. No partner-confirmation step at launch — friction against an unobserved problem. **AUTOFILL (Dean's ask — honest cost):** a password saved on www.vyvehealth.co.uk is NOT offered inside the Capacitor app without formal association — AASA `webcredentials` + Associated Domains entitlement (iOS), assetlinks.json + package/SHA-256 (Android), both requiring a NATIVE RELEASE (not Capawesome-able). Ship the cheap prerequisite now (`autocomplete="username"/"new-password"/"current-password"` inside real `<form>`s on both join page and app sign-in — without these, managers won't reliably offer to save/fill ANYWHERE); bundle the association files into the next binary. **CORRECTION MADE MID-DESIGN:** Claude initially claimed partner-referred members convert on the wrong price — WRONG; `trial-end-email` v1 is already partner-aware ("Because you joined through {partner}, your rate is £10") and everyone converts at £10. The real gap is narrower: the conversion link is a fixed Payment Link, so partner identity survives only via reconciler fallback (3) rather than being stamped on the Stripe object. **PIECES (order 3→4→1→2→5, 6 rides the next binary):** 1 codes+share kit ~1s · 2 conversion Checkout Session ~0.5s · 3 join page → account creation ~0.5s · 4 three-state door ~0.5s · 5 recognition picker + late attribution ~1s · 6 autofill association. **OWED FROM LEWIS:** per-partner rate tiers y/n; agreement wording that rev share is paid on ATTRIBUTED members and how attribution works (the 50-vs-20 expectation gap bites regardless of tech); self-declared payable at launch.**

**PM-859 → PM-866 (2026-08-07, same night as PM-856/857/858): PARTNER PORTAL MADE REAL — provisioning rewritten through three live bugs, photos published + croppable, every portal write fixed.** Dean and Lewis both walked the funnel end to end; everything below came out of that walk.

**PM-859 — GATE-A PROVISIONING REWRITE (partner-provision v3→v5, platform version 7).** Dean advanced his test partner to onboarding and got no email. Three bugs, all proven in production data before touching code: **(1) SECURITY — wrong account's password reset.** GoTrue `GET /auth/v1/admin/users?email=` is NOT a filter: it ignores the param and returns a page of ALL users. v2 took `users[0]`, concluded "partner already has an account", and PUT a new password on that arbitrary account. Both the 6 Aug (Vanessa) and 7 Aug (Dean) runs recorded `auth_user_id 14cc9b46` = deanonbrown2+b2binvite@gmail.com, whose `auth.users.updated_at` matched each provision to the second; Vanessa herself was never created. **NOBODY had ever actually been provisioned** — the brain's "0 partner logins ever" was literally true even after clicking. FIX: SECURITY DEFINER RPC `public.auth_user_id_by_email` (migration pm859, service_role-only, case/trim-insensitive); the update-password branch DELETED outright so an existing account's password can never be touched by provisioning. **(2) SILENT EMAIL FAILURE.** The credentials email authed to send-email with `LEGACY_SERVICE_ROLE_JWT`, but send-email requires bearer === `SUPABASE_SERVICE_ROLE_KEY` → 401, swallowed by `.catch(()=>null)` with no `.ok` check, while the EF returned success and the CC toasted "credentials emailed". FIX: service-role bearer, `.ok` checked, `email_sent` returned so the CC tells the truth. **(3) PRIVILEGE DOWNGRADE (found provisioning Lewis).** The admin_users upsert PATCHed `role='partner'` unconditionally — Lewis is an ADMIN and also a partner, so provisioning silently demoted him and would have cost him CC access. Restored to admin (he keeps portal access anyway: `get_my_partner_id()` accepts role IN ('partner','admin')). FIX v5: NEVER overwrite an existing role — admin left intact, any other role (team) left intact and reported as `role_conflict` for a human decision; the deprovision path got the mirror guard so a backward drag can't strip a colleague's access, and it no longer bans the auth user (shared identity — banning would lock them out of the member app too).
**IDENTITY MODEL (Dean's call):** ONE Supabase Auth identity for the member app AND the portal. The `admin_users` role grant is what opens the portal, never a password. Existing account → grant role, touch nothing, email says "sign in with the details you already use". No account → create + email generated password. **REVERSE DIRECTION (Dean's ask):** provisioning also ensures a `members` row (`account_type='partner'`, onboarding_complete false, persona null — the shape the B2B invite bypass already creates and the app already routes to onboarding) so the same login works in the app. **WHERE PARTNERS GO:** the portal is `partner-portal.html` with its OWN email+password sign-in (`signInWithPassword`) — NOT the magic-link CC shell at the admin root (that mistake cost a wrong-copy iteration). v4 email always sends, always lists next steps (sign in → welcome video → content/profile → tell us and we go live), both account states carrying the same portal link. `PORTAL_BRAND_URL` const is ready for `partners.vyvehealth.co.uk` — flip one line once Dean adds the Cloudflare custom domain.
**PM-859 data fixes:** Calum's LIVE partner record carried `lewisvines@hotmail.com` (created 22 Jun, seed-era), and `get_my_partner_id()` took the older row — so Lewis's portal login landed in **Calum's space**, which he reported as "it's pulling through the wrong information". Calum moved to `calumdfitness@gmail.com` (Dean's correction; briefly calumdenham@gmail.com). Migration: `uq_partner_applied_email` (applied-only, PM-858) DROPPED and replaced with **`uq_partner_contact_email` across ALL statuses** — the applied-only index structurally could not catch a live row holding the wrong address; `get_my_partner_id()` rewritten case-insensitive + `ORDER BY created_at DESC` so a duplicate can never again resolve arbitrarily. Portal CSS: native date/time inputs carry an intrinsic min-width wider than their `.field-row` grid column and grid items default to `min-width:auto`, so the row could not shrink — date/time pills spilled out of the Notify community panel and the Schedule a session card (`min-width:0` on row + children, clamp on the inputs, overflow guard on card/notify-panel).
**PM-859 photos:** the apply wizard uploads profile/cover photos to the PRIVATE `partner-docs` bucket and submit stores a bare storage PATH; the portal only renders real URLs (`urlish()`), so every partner's own application photos showed as "Click to upload" while the files sat there all along. One-shot EF `pm859-publish-partner-photos` (token-gated, invoked via pg_net) copied bare-path photos into the PUBLIC `partner-thumbnails` bucket and rewrote the columns; Lewis's avatar (133KB jpg) and cover (1.2MB png) verified serving 200. **STILL OPEN AT SOURCE — the wizard keeps writing to the private bucket, so the next partner hits this again.**

**PM-860/861 — PHOTO FRAMING (`e3a0e17d`, `0624ef63`).** The app masks partner avatars to a CIRCLE (`.pc-avatar`/`.comm-av` border-radius:50%) but the portal preview was a rounded square, and uploads went to storage un-framed — a landscape photo became an off-centre circle with the partner in the corner and nothing could fix it. Every image now opens a drag-to-move / slider-to-zoom cropper: avatar 1:1 exported at 512 with a circular frame, cover 3:1 at 1200×400 (between the app's 2.4:1 grid card and 3.3:1 featured slot). Cover-fit minimum scale + clamping mean the frame can never show empty gaps; touch and mouse both wired. **PM-861 fix on top:** the crop stage stretched to the modal width while only its height came from the ratio, so the canvas (width/height:100%) scaled non-uniformly — the square frame rendered as an ELLIPSE and squashed Dean's portrait upload. Stage and canvas CSS box now sized explicitly to the ratio, 1:1 with the drawing buffer.

**PM-862 — EVERY PORTAL WRITE WAS ANONYMOUS (`85a922a4`) — NEW §23.166.** "Error: Post failed" on community posts. The request carried the anon apikey and NO `Authorization` bearer, so PostgREST saw an anonymous caller; `partner_own_posts_insert` requires `partner_id = get_my_partner_id()` which resolves via `auth.email()` — null when anonymous — so RLS rejected it. Same omission in EIGHT more places: delete post, schedule push, preview audience, cancel push, create session, plus the sessions/push-queue/community/earnings READS. **The reads were the quieter half — partner-scoped RLS returns an EMPTY LIST rather than an error, so those panels looked plausibly blank instead of broken.** Single `rlsHeaders()` helper now carries apikey + session bearer on all 11 call sites; post/delete surface real PostgREST error text. Verified by simulating Dean's exact JWT claims in a rolled-back transaction — the insert succeeded, proving the fault was client-side, not RLS.

**PM-863 — SCHEDULED PUSHES EDITABLE + DELETABLE (`8ebfb80c`).** The queue only offered Cancel, so a mistyped push could be stopped but never corrected or removed. Queue rows now carry Edit (repopulates the compose form, PATCHes in place; UTC↔local wall-time round-trip verified) and Delete (new RLS policy `partner_scheduled_pushes_partner_delete` — own rows, unsent only; sent pushes stay immutable as a delivery record).

**PM-864 — QUEUE STUCK ON "Loading…" (`375c6d06`) — NEW §23.167.** Dean's console caught `esc is not defined`. `loadPushQueue()` called `esc()`, which is defined inside the SEPARATE bookings `<script>` block — ReferenceError thrown mid-`map` the instant the queue had a row to render, so innerHTML was never set. Pre-existing since the queue shipped; invisible until a push existed (zero rows took the empty-state path and never touched the line). Switched to `escHtml()` from the same block.

**PM-865 — TARGET AUDIENCE ALWAYS VISIBLE (`7f9ed251`).** Reach was only discoverable by pressing Preview, so partners composed blind. The Notify panel loads the live audience with the tab ("Target audience: N members subscribed to your community"), warns in AMBER at zero ("pushes will reach nobody until they do" — true for every partner until they are live and members subscribe), stamps each pending queue row with who it will reach, and Preview just refreshes the line (and no longer demands a title/message — it is an audience check, not a send).

**PM-866 — COMMUNITY POSTS EDITABLE (`958ff233`).** A typo in a message to your whole community was only fixable by delete-and-repost. Feed rows carry Edit alongside Delete; the compose box doubles as the editor (Update post / Cancel edit — same pattern as pushes, deliberately one editing idiom in this portal) and PATCHes body only. **Edits are STAMPED and SHOWN**: `edited_at` set by a BEFORE UPDATE trigger (server-side so it cannot be forged or skipped) and the feed renders "edited <date>" — a partner silently rewriting a post after members have read it is a trust problem, and showing it costs nothing. Dean accepted the visible marker.

**Verification posture all night:** every RLS question answered by simulating the real JWT claims in a rolled-back transaction rather than guessing; the duplicate-application fix proven with a genuine concurrent double-fire; published photos fetched back over HTTP for status/content-type/bytes. Two "still broken" reports (post failed, then queue empty) turned out to be **browser-cached JS after a soft refresh** and a **3-second screenshot race** — live-file checks via pg_net confirmed the deploy each time before chasing phantom bugs.

**PM-858 (2026-08-07): DUPLICATE APPLICATIONS KILLED + WIZARD MOBILE OVERHAUL.** Dean's live test created TWO partner rows 1.7s apart (17:13:33.0 / 17:13:34.7, both resume_email_sent): the wizard double-fired ensureDraft before the first `start` returned its partner_id, and the EF created unconditionally. THREE-LAYER FIX, each verified: (1) CLIENT — in-flight lock in ensureDraft (Test-Site-Finalv3/partner-onboarding.html `8d326338`, md5-verified): a second call while a start is pending awaits the first promise; jsdom race harness proves double-fire → ONE start call. (2) EF — partner-onboarding **v15 (platform version 22)**: idempotent `start` — mint_invite's dupe guard generalised (existing 'applied' partner with same contact email → returns that id, {deduped:true}; limit(1) not maybeSingle so pre-existing twins can't error the guard) + 23505 recovery re-selects the race winner. (3) DB — migration pm858: partial unique index `uq_partner_applied_email` on lower(contact_email) WHERE status='applied' (declined/live/onboarding untouched — legitimate re-application still works). VERIFIED with a REAL concurrent double-fire via pg_net (net.http_post ×2 simultaneous): both returned the SAME partner_id, one {deduped:true}, exactly one row created; test row + push cleaned. Orphan duplicate from Dean's test (partner bae300f4 + application + progress) DELETED — pipeline shows one Dean Brown test again. **DEPLOY INCIDENT (new hard rule §23.165):** `Supabase:deploy_edge_function` DEFAULTS verify_jwt:true — first v15 deploy (platform v21) silently flipped this public EF to JWT-required, breaking the wizard backend for ~15 min until redeploy (platform v22) with explicit verify_jwt:false. RULE: every deploy of a public EF MUST pass verify_jwt explicitly and the deploy result's verify_jwt field MUST be checked. **MOBILE (same commit):** the left-right white screen = unpainted html element behind the body gradient + iOS focus-zoom (inputs were 15.5px, <16px triggers Safari zoom → scaled viewport pans sideways into white). Fixed: html{background:var(--dark)}, html/body overflow-x:hidden+clip, 16px form controls at ≤640px, plus polish pass (g2 collapse — was missing from the 640 reset, 2×2 stats strip, tighter flownum/flowstep, contract 16px padding, overflow-wrap on prose). jsdom 10/10. Dean device check pending ?z=858. NOTE: Lewis/Alan mid-application during the ~15-min JWT window would have seen save errors — refresh resumes cleanly.**

**PM-857 (2026-08-06): CC APPLICATION REVIEW — HONEST CHIP STRIP + PROGRESS.** Dean's walk of Vanessa's review panel: a COMPLETE application read 38% with grey circles on gates the wizard never collects. vyve-command-centre/partners.html `d52b499d` (md5-verified, Cloudflare <1min): Training modules / Safeguarding / GDPR chips now render AMBER "&#10003; (deferred)" (var(--warning), tooltip "Deferred by design (PM-659) — resolved before go-live") when steps.modules_deferred — they were deferred by policy, not skipped by the partner; Starter video chip REMOVED from STEP_LBL (PM-856 — welcome video is post-approval in the portal); display pct recomputed client-side from the gates the partner can actually complete (agreement/identity/profile; module gates rejoin the denominator automatically for a future non-deferred cohort) so done-applying reads 100%. **prog.pct_complete UNTOUCHED** — the DB trigger value still feeds the go-live gate (partners.html live-flip check: safeguarding && gdpr && pct=100); this is display honesty only. jsdom 14/14 (deferred amber x3, 100%/33%/50% math, non-deferred cohort regression, admin-approval circle kept). **GO-LIVE MODEL AGREED (Dean, this session — build with the portal receiving-end session):** approve → Gate-A provision → partner completes PORTAL minimums (welcome video + space polish) → ADMIN clicks Go Live from CC partner detail once minimums met. Deferred assessments ALLOWED at go-live = the PM-659 HELD migration posture, needs the Lewis nod (non-gating on today's ship) since the wizard landing publicly claims safeguarding review; rec: allow + stamp "assessments deferred" visibly on the partner record so CC always shows true posture, resolved properly by the owed quiz-coherence decision.**

**PM-856 (2026-08-06): STARTER CONTENT OUT OF THE APPLY FLOW — wizard is 4 apply steps; welcome video, content and launch all post-approval in the partner portal.** Executes the PM-855 addendum. Test-Site-Finalv3/partner-onboarding.html `85b500c3` (md5-verified): `ACTIVE=[0,1,2,5]` (About you / Verification / Agreement / Build your space); Welcome-video pane st6 dormant PM-844-style (video slots + welcome-post kept in file for restore); submit machinery RELOCATED — final confirm checkbox + "After you're approved" card moved into st5, `isStepComplete(5)` now gates on `checks.final`, validateStep i===6→i===5, "Submit for review" fires from step 5 via the existing last-active logic, stale saved drafts on step 6 clamp to 5 via the existing ACTIVE-clamp restore. Landing matched to the slimmer shape: lead "4 guided steps… video, content and launch after you're approved, from your partner portal", stats strip 5→4 (**~10 min KEPT** — the removed step was optional, so a faster claim would be the overselling PM-844 banned), flow array drops the welcome-video entry (step 4 = "Build your space & submit"), done banner/lesson now "profile and documents", starter-content summary row removed. Payload shape untouched — starterContent resolves `[]`, welcomePost empty. **EF: NO change** — deployed partner-onboarding v14 (platform version 20) read directly, not just from the brain: submit `Array.isArray`-guards starterContent, an empty array no-ops the insert loop, `starter_videos_uploaded = starter.length>=1` → honest false. jsdom 35/35 (sequence, rail, machinery relocation, gating, clamp, all copy). Dean's 6-step screenshots CONFIRMED stale Fastly cache — live main was already the PM-844 5-step build. Dean device check pending ?z=856. **PORTAL RECEIVING END scoped, NOT built (→ backlog):** upload machinery already exists (partner-portal.html Content tab, partner-content-upload v6, JWT + in_review moderation) — a provisioned partner could upload a welcome video today — but (a) **Gate-A provisioning is the real blocker**: 0 partner portal logins ever provisioned, contact_email missing 7/9 (PM-842 finding), so the portal promise is empty until the ops backfill; (b) no welcome-video designation — portal uploads are generic content items (parity with what the wizard did, but the new copy promises a guided slot); (c) PM-684's mandatory publish_at ≥48h fits scheduled launch content, not a welcome video that should be live at approval. Rec: ~0.5–1 session — post-approval checklist card (record welcome video → polish space → schedule launch, matching the wizard copy) + dedicated welcome-video upload skipping the 48h rule, sequenced with the provisioning/contact_email backfill.**

**PM-855 addendum (2026-08-06, session close): NEW DIRECTION — Starter content moves OUT of the apply wizard into the partner portal (post-approval).** Dean's call after walking the wizard: the apply flow drops the Starter content step entirely; partners upload their welcome/starter videos from inside the partner portal once they're in. Landing page (partner-onboarding.html top) needs the matching copy update — stats strip still says "6 guided steps"; step count, process overview, and journey copy must reflect the slimmer apply flow. Handed to a fresh session as a .md prompt. NOTE from Dean's device walk: he was on the PLAIN URL (no ?z=844) — Test-Site is still Fastly-cached (§23.162), so the 6-step sidebar in his screenshots may be the PRE-PM-844 build; next session must verify live shape with a fresh ?z= before editing.**

**PM-855 (2026-08-06, post-close): SIGNED AGREEMENT COPY LIVE — partner-onboarding v14 (platform v20); Vanessa reset to 'applied' for the proper submit→approve→provision path.** Dean's requirement: on signing, the partner gets an emailed copy of the agreement AND one is saved to their account (previously the signature lived only as JSON in credentials.agreement — no document, nothing sent). At submit, best-effort (never fails submit): agreement text pulled LIVE from the wizard (#contractPane on www.vyvehealth.co.uk/partner-onboarding.html, balanced-div extraction — single source of truth, QUIZ_KEY-lockstep class), merge fields data-mf date/pname/sig filled server-side from the submitted agreement record, wrapped as standalone HTML with an e-signature footer (name, legal name, date, rev share %, server signedAt) + provenance meta → uploaded upsert to partner-docs/<pid>/agreement/signed-agreement.html → ref in credentials.documents.signedAgreement (PM-748 class) → full agreement emailed INLINE to the partner (tag partner-agreement-copy, one-shot _system.agreement_copy_sent set only on successful send; wrapper copy [LEWIS COPY PASS]). **Failure reasons now recorded in _system.agreement_copy_error (SQL-observable, cleared on success)** — the first live test failed SILENTLY (200 ok, no copy, no email, console invisible in request-line logs; PM-836 class), and the error-recording revision diagnosed it in one retest: **partner-docs bucket MIME whitelist rejected text/html** — whitelist extended to images+PDF+text/html (trade-off noted: applicants can technically upload .html as a doc; served off the supabase.co storage domain via signed URLs, not our origin — low risk, accepted). Retest LIVE-PROVEN end-to-end: 27KB stored copy + ref + email at deanonbrown@hotmail.com. Test residue purged to ZERO via partner-draft-purge itself (test draft backdated into purge candidacy per §23.164 — agreement key stripped since purgeable requires agreement NULL — then cron-key invoke; candidates 1, purged 1, storage removed 1): **first live proof of purge v2's storage-deletion path.** Also learned: storage.objects rows can't be SQL-deleted (protect_delete trigger, owner-locked) — the purge EF's Storage-API path is the only sanctioned delete. **PM numbering:** deployed v14 header reads PM-847 (claimed pre-discovery); the parallel CC session consumed PM-847–854 (thumbnails/restore/renumber-sweeps/restore-to-applied — their brain entries not yet landed at this commit). PM-747 precedent applied: production EF NOT redeployed for a comment; local source swept to PM-855 for the next deploy; disambiguate by platform version. **VANESSA:** Dean had flipped her to 'onboarding' at 17:41 believing it would send login details — it did NOT (no auth user, no admin_users row, no partner-provision call in logs: the provision hook only fires via the CC status control) and she was invisible to both stage emails (agreement NULL) and nudges (off 'applied'). Ground truth confirmed: everything done EXCEPT the video — signature typed, agreement step ticked, docs uploaded — but submit never ran, so NO executed agreement exists. Reset to 'applied' (purge clock anchored today via status_changed_at, 30 days' runway; nudge net covers her from 48h). Path: Lewis sends resume link (video now optional) → she submits (agreement copy fires automatically) → Dean approves in CC → provision emails login.**

**PM-844/845/846 (2026-08-06): PARTNER FUNNEL P0 STACK SHIPPED — wizard re-sequenced, nudges + stage emails live-proven, pre-filled invite links end-to-end.** The PM-843 stack, built in order. **PM-844 wizard re-sequencing** (Test-Site-Finalv3/partner-onboarding.html `9a3af440`, md5-verified): application is now 5 steps — Launch step removed from the apply flow (pane dormant, soft-kill style), welcome video OPTIONAL (submit-confirm checkbox moved into step 6; videoless submit reaches doSubmit — EF v12 already wrote gates honestly), landing copy honest ("~10 minutes to apply, 5 guided steps"), on-screen resume-link card in the rail (copy button + spam hint) so the resume path no longer depends on email deliverability, summary shows "None yet — add after approval" for zero videos. 21-assertion jsdom smoke ALL PASS; Dean device check pending (?z=844). **PM-845 nudges + stage emails** (partner-draft-purge v2, cron jobid 54 unchanged 03:30 UTC; migration `partner_draft_nudge_candidates`): two SECURITY DEFINER candidate RPCs — `partner_drafts_nudge_candidates` (applied, agreement NULL, valid email, ≥48h stale by the same GREATEST staleness as `partner_drafts_purgeable`) and `partner_stage_email_candidates` (onboarding/declined, agreement NOT NULL — wizard-submitted only, backfilled live partners excluded by construction), both REVOKEd from anon/authenticated. Cron passes: purge → 48h nudge (friendly, resume link) → 7d nudge (honest deletion-date warning incl. photo ID; ≥7d first-sight sends only the 7d, sets both flags) → stage emails ('onboarding' approved-light, 'declined' gentle with honest delete-on-request wording — declined auto-purge stays an open policy decision, no false automation claims) → rate-window prune. One-shot flags (`nudge_48h_sent`/`nudge_7d_sent`, `stage_email_<status>`) set only on successful send via a credentials-only writer that can NEVER touch status_changed_at/updated_at — nudges cannot reset the purge clock (verified: no triggers on partner_applications). LIVE-PROVEN via pg_net + synthetic draft: 3d-stale → 48h email sent, flag set, last_activity untouched; 8d → 7d email, no 48h re-send; flipped to onboarding → stage email. Three proof emails landed at deanonbrown@hotmail.com; test draft deleted child-first, zero residue; final candidate counts 0/0 — tonight's real cron touches nobody. Tags partner-draft-nudge-48h/-7d, partner-stage-onboarding/-declined. **PM-846 pre-filled invite links** (partner-onboarding v13, platform v15; CC partners.html `4700aab2`, md5-verified, Cloudflare-live <1min): new admin-gated `mint_invite` action (manual JWT → admin_users gate inside the verify_jwt:false EF, partner-file-url v3 pattern, partner role excluded) mints a draft from Lewis's call notes and sends a Lewis-voiced invite email with the personal ?resume= link. CRITICAL shape lesson: wizard resume reads `credentials.formFields` + top-level `credentials.pillar` in WIZARD tokens (physical/mental/social) — mint writes BOTH the contact/practice shape AND the wizard shape (Vanessa's live credentials confirmed), else the warm lead lands on an empty form. `_system` stamped invited:true (the PM-843 invite→submit cohort marker) + invited_by + admin_start_notified:true (no self-inflicted phone buzz) + resume_email_sent:true on successful send (Lewis-voiced invite replaces the generic resume email). Duplicate guard: open applied draft with same email returns its existing link, no twin, no email. CORS: admin.vyvehealth.co.uk added to ALLOWED_ORIGINS (PM-826 class). Skips the 5/hr start IP limit; minted stalls get PM-845 nudges naturally. Live checks: mint no-auth → 401; public resume path intact (Vanessa 200 resumed:true). CC invite modal: "Mint & send invite link" primary (session-JWT fetch, Role field → specialism, bio → description, notes carried; result panel with resume link + copy + honest send status incl. existing-draft and send-failure cases; pipeline + nb-badge refresh in place), "Add manually (no email)" retained as secondary. 13-assertion jsdom harness ALL PASS. **NEW §23.164:** partner_applications.status_changed_at defaults now() at insert AND trg_sync_partner_onboarding_pct bumps progress.updated_at on any write — backdating partner test rows needs the §23.156 disable-trigger-in-transaction pattern PLUS an explicit status_changed_at UPDATE, or staleness math silently reads fresh. (§23.155 re-confirmed: MCP execute_sql returns only the last statement.) **Emails all [LEWIS COPY PASS], non-gating:** 48h/7d nudges, onboarding/declined stage, Lewis-voiced invite. **Dean pending:** wizard device check ?z=844; CC mint walk (plain URL — Cloudflare). **Decisions owed (unchanged, block nothing):** photo-ID policy; quiz-vs-hidden-modules coherence.**

## PM-842–843 — 6 Aug 2026: partner Documents panel + immediate admin notifications; full self-serve funnel deep-dive delivered (2026-08-06)

Session arc: Dean asked for the complete partner-onboarding process mapped for launch readiness, which surfaced two live gaps (fixed same session) and produced two Dean/Lewis-facing docs plus a funnel deep-dive with a P0 build stack awaiting go.

**PM-842 — partner Documents panel (the "approved without seeing the evidence" gap).** Dean had advanced Vanessa Sturman's pipeline stage without ever seeing her uploads: nothing in the CC surfaced private partner-docs files, and for never-submitted drafts the refs aren't even in the application record. partner-file-url **v3**: admin-only `action:'list'` (two-level storage walk by partner_id — drafts included) + `disposition:'inline'` (review in-tab; default stays attachment for existing callers); partner-role callers 403 on list. CC partners.html: Documents card on the partner detail Overview, grouped Photo ID / Qualifications / Profile photo, View + Download per file, 5-min signed links, admin-only footer note. CC `a79ea9fc` md5-verified; EF deployed via MCP, 401 no-auth pg_net-verified. Cloudflare Pages deployed it in under a minute — first dividend of PM-841.

**PM-843 — immediate admin notifications (Dean was blindsided by Vanessa's draft).** Only notify was a team@-only email at SUBMIT — and she never submitted; Dean found her by DB query weeks later. partner-onboarding **v12**: `notifyAdmins()` — email to team@ + deanonbrown@hotmail.com AND native push (scheduled_pushes upsert on the (member_email,dedupe_key) unique, fire_at now) to Dean — fires at (a) application STARTED (first valid contact email, both the start branch and the save fallback; one-shot `_system.admin_start_notified`) and (b) SUBMITTED (one-shot `_system.admin_submit_notified` — the old team@ block was unguarded and re-fired on resubmits). Best-effort throughout; applicant flow can never fail on notify. **Live-proven end-to-end**: pg_net start with test applicant → 200, both flags set, push row queued, admin emails sent; test draft deleted child-first, push row left to fire as Dean's phone-proof. Lewis as recipient = one-line add when wanted.

**Vanessa Sturman ground truth (the launch cohort's one real applicant):** completed steps 1–3 + profile photo on 21–22 Jul (photo ID, qualifications, headshot in partner-docs), stalled at the MANDATORY VIDEO, never submitted, silent since; resume link = partner-onboarding.html?resume=13f61ee9-a991-4a2d-8b34-a096c914cb5a — **Lewis to chase personally**. She is shielded from the 30-day draft purge ONLY because Dean advanced her status off 'applied' (luck, not design — purge deletes drafts' uploaded IDs with no warning email).

**Docs delivered (both .docx via docgen, outputs dir):** (1) *Partner Onboarding operational guide* — both entry paths, all links/emails/gates step-by-step, plus the live-DB gaps table: ZERO partners have portal logins (nobody ever Gate-A provisioned), contact_email missing 7/9, coupons 8/9 missing, all nine "live" partners are 13% backfills with no signed agreement or assessment pass; suggested order of attack. (2) *Self-serve funnel deep-dive* — wizard source + EF + DB + Vanessa-evidence based; warm-lead lens (Dean: ~90% are Lewis-screened before the link — the wizard is FORMALISATION of an existing relationship, not an application; Lewis's screen IS the vetting).

**Deep-dive verdicts (the ones needing Dean/Lewis decisions):**
- **Effort wall**: submit is hard-blocked on a recorded welcome VIDEO + a committed launch date/time — 1–2h of work incl. on-camera, mid-form. Vanessa (a warm lead) proves warmth doesn't soften it. Core rec: split "apply" (~10 min: who/what/ID-per-policy/signature) from "set up" (video, profile polish, launch scheduling → post-approval in the portal). EF already tolerates videoless submits.
- **Photo ID integrity problem**: wizard REQUIRES passport upload under "VYVE verifies your identity" — but NO verification process exists anywhere and the EF auto-stamps `identity_verified:true` at submit. GDPR data-minimisation + honesty issue. ALSO brain-vs-reality: PM-634 recorded ID as removed; the live wizard requires it. Decision: build a real ID-check pipeline action feeding the flag, or defer collection to contract stage.
- **Quiz coherence**: 3-question safeguarding/GDPR quiz gates go-live while the training modules that teach it are hidden — quizzing on unseen material, guessable at 15 attempts/hr. Restore modules (Lewis/Phil content) or reframe as declaration.
- Secondary: single-PUT unresumable uploads (large files on mobile fail); cold profile-writing needs examples; landing-page time honesty; on-screen resume link + spam hint.

**P0 build stack (approved-in-principle, awaiting Dean's go):** (1) wizard re-sequencing — video + launch out of the application, optional video slot stays (~1 session); (2) stalled-draft nudges 48h/7d incl. 30-day-deletion warning + minimal stage emails, riding the nightly cron (~1 session); (3) **pre-filled invite links** (~0.5 session) — post-screen, CC/Claude mints a draft from Lewis's call notes → personal resume link + Lewis-voiced invite ("I've started your application; ~10 min to finish"); warm lead skips to signature+uploads. Success metric: invite→submit conversion, target ≥80%.

Standing context: PostHog already fires partner_step_view/submit_attempt/fail/success — per-step drop-off is queryable once volume arrives.
...
**PM-837 (`983815a0`) — admin Profile editor.** New Profile subtab on the partner detail: name / role_title (tagline) / pillar / bio / why / feel + real avatar & cover uploads. Uploads ride partner-content-upload v6's existing admin path (`sign-thumb`, explicit partner_id) → public partner-thumbnails bucket; timestamped filenames = CDN cache-bust built in. Field parity with the partner portal's PM-788 My Space editor is exact — partners and admins edit the same set, last write wins either way (noted in-panel). All writes use `.select('id')` + row-count check so RLS-blocked writes error instead of silently no-opping (the PM-788 root cause). Go-live trigger confirmed transition-only — profile updates on live rows pass clean. Detail hero + phone Preview render the real avatar/cover, `urlish()`-guarded against wizard-era bare storage paths.

**PM-838 (`41c73340`) — Upcoming live sessions card** on the detail Overview: next 30 `calendar_occurrences` for the partner (cancelled excluded, `starts_at >= now`), when/duration, title, category, Community/Public chip. Same query shape the partner portal diary uses, so admin view matches partner view.

**PM-839 (`cf4f1a31`) — roster avatars.** Shared `av()` helper renders `avatar_url` photos (urlish-guarded, initials fallback) — partners directory, pipeline list, and payouts table all pick it up from the one change.

**PM-840 (`f9dc01e4`) — THE REAL BUG behind "clicking does nothing".** Dean's device walk: partner rows dead, console silent. Root cause: `navByView('partner-detail')` switches views by clicking a matching sidebar `.nav-item` — and the detail view has none, so it silently no-ops. `openPartner` was rendering the ENTIRE detail (Profile tab included) into the hidden `#v-partner-detail` section. Latent since the Phase 5 router shape (PM-762 era); Dean was the first to click into the CC partner detail since. Fix: navByView falls back to switching the `.view` section directly when no nav-item matches. My first verification harness green-lit this bug — it asserted "no exception", and the failure mode IS no exception. Harness upgraded to assert the observable effect (v-partner-detail active after click): PASS.

**Deploy incident (unresolved at write time):** GitHub Actions/Pages had a publicly acknowledged incident all afternoon (githubstatus 16:27 UTC: runs delayed/failing). Our Pages builds: PM-837 errored ×1, PM-838 errored ×2, PM-840 wedged in `building` 40+ min. NOTHING deployed to admin.vyvehealth.co.uk this session — every screenshot Dean took was the pre-session build, which also masked PM-840 as "still broken" until the build history exposed it. Rebuild-kick via POST /pages/builds is 403 on our PAT (no Pages scope). **NEXT SESSION FIRST CHECK: `GET /repos/VYVEHealth/vyve-command-centre/pages/builds/latest` — if latest ≠ `built f9dc01e4`+, push a trivial kick commit; then Dean loads `partners.html?z=840`** (row click opens detail = new build confirmed; Profile tab, schedule card, roster photos all arrive together). Dean was also still on the stale `?v=622` URL — fresh `?z=N` every check, per standing Fastly rule.

**New §23 rules this session:** (159) UI-flow harnesses must assert observable effects (view switched, pane active), not absence of exceptions — null-returning DOM stubs green-light exactly the silent-no-op class that bit here. (160) CC `navByView` historically only knew sidebar views; any future detail-style view (no nav-item) relies on the PM-840 direct-section fallback — don't reintroduce the old shape. (161) When "the deploy didn't land", check `/pages/builds/latest` status BEFORE blaming Fastly/cache — legacy Pages builds can error or wedge platform-side, and three builds silently erroring looked identical to a stale-cache problem from the browser.

## PM-836 — 6 Aug 2026: vyve-agent live and proven; root cause FINAL — silent blindness, not a crash; masters all present (2026-08-06)

**Root cause, final revision (supersedes PM-835's framing):** the agent-fetched journal shows the runner NEVER crashed. One PID from ~24 Jul, 1 week 6 days wall clock, 18h43m CPU; it aired the 4 Aug 17:00 Calum Education session perfectly and then logged nothing again until Dean's restart. At 23:12 on 5 Aug it went **silently blind** — heartbeat writes and calendar fetches stopped dead while the process lived on, and the refresh loop logged not a single error. A plain restart restored everything instantly (heartbeat fresh, upcoming=4). Exact internal mechanism (wedged connection pool vs swallowed auth/network exceptions) is unknowable without instrumenting the runner; what IS certain is the code smell: **the refresh loop swallows failures silently**. Backlog: patch the runner to log and fail-fast after N consecutive refresh failures so systemd's Restart=always converts a wedge into a 10-second blip.

**Agent install saga (two §-grade gotchas):** Dean ran the installer via a `serve-agent-installer` EF (curl|bash — a parallel session's delivery path since the .sh download wasn't on his Mac). v1 failed instructively: the key auto-detect found nothing, fell to `read -rp`, and **under curl|bash, `read` consumes the next line of the script itself from stdin** — it captured `umask 077` as the "key" (9 chars, confirmed by Dean's awk), wrote `SERVICE_KEY=umask 077` into env, and the agent crash-looped on `077: command not found`. Rule: **interactive reads in piped scripts must use `< /dev/tty`, and validate before writing.** v2 (EF v3, base64-embedded after I shipped a placeholder-payload dud at v2 — own goal, caught immediately) reads /dev/tty only, validates any candidate key against the API with a live 200-check, and aborts rather than writing a broken env. Dean pasted the dashboard service_role key; validated OK; both units active. Also observed: `StartLimitIntervalSec` belongs in [Unit] not [Service] — systemd ignores it in our drop-in; harmless (RestartSec=10 never trips the default 5-in-10s burst) but note for any future unit work.

**Agent proven end-to-end:** three queue commands executed and returned (uptime, journal 400 lines, ls_masters). The 5 Aug failure mode — alive-but-blind runner with a stopped heartbeat — now self-heals in ≤5 minutes via the agent's heartbeat-stale restart, and the same class is Claude-diagnosable from any session via the queue.

**Masters verdict — PM-834 thread CLOSED:** exact set-compare of all 91 distinct `notes` filenames in the 350-row schedule against the box listing: **zero missing**, case included (box has `Weekly review 3.mp4` capital-W — the casing fix was right; reset/review 1-3 all present; all 9 "never-box-aired" files present since the 15 Jun / 2 Jun seeds). Every 6 Aug failure was the wedged runner. Schedule is sound through 30 Sep.

**SECURITY:** the service_role key was pasted into a chat transcript during install. PM-714's carried "rotate exposed service_role key" item is now doubly warranted. Rotation is a planned op: new key must land in the runner's creds (location still unidentified — NOT under /srv/vyve, find it during rotation), /srv/vyve/agent/env, and anywhere else the old key is embedded. Do not rotate ad-hoc mid-incident.

**Remaining proof:** 18:30 UTC (19:30 BST) Healthy Knees Session 4 airing — watchdog silence + auto-resolution of the open alerts closes the incident. serve-agent-installer EF v3 retained (serves no secrets; useful for box rebuilds).

## PM-835 — 6 Aug 2026: Live-box remote control (vyve-agent) + true root cause of today's dead air (2026-08-06)

**Root cause (supersedes the PM-834 missing-file hypothesis):** the 17:30 failure hit a box-proven file, which broke the theory. runner_heartbeat: last beat **23:12 UTC 5 Aug**, final detail `{"upcoming": 0}` — the runner died at the exact moment the July calendar ran completely empty, and nothing restarted it (no Restart=always). A `runner_down` alert fired 00:20 BST (email + native push) and went unnoticed overnight; because that alert stayed OPEN, the watchdog's alert_key dedupe suppressed every subsequent runner email — the day's per-session `not_live` alerts were symptoms of a 17-hour outage, not individual failures. Nothing aired on 6 Aug before the fix; today's broadcasts were being created by the server-side session-publish cron, masking the dead box. Lesson banked in the playbook: an open runner_down alert hides the headline — always check `broadcast_watch_alerts WHERE resolved_at IS NULL`, and never claim "runner picks it up on heartbeat" without checking heartbeat age first (PM-833 did exactly that).

**Dean's structural challenge, accepted:** the Hetzner migration's whole point was Claude-managed operations, but Claude's sandbox egress is domain-whitelisted — no SSH, ever. Fix: **Supabase is now the control plane.** Shipped this session: `runner_commands` table (RLS enabled, zero policies — service-role only, invisible to app roles) and `vyve-agent-install.sh` delivered to Dean as a one-paste root script. It installs three layers: (1) **systemd hardening** — Restart=always/RestartSec=10/StartLimitIntervalSec=0 drop-in on the runner unit; (2) **vyve-agent** service (itself Restart=always): 30s loop that restarts the runner on unit-inactive OR Supabase heartbeat ≥5 min stale (covers hangs), 10-min heal loop guard, every heal logged as a `self_heal` command row; (3) **allowlisted command queue** — agent executes `restart_runner | status | journal | ls_masters | disk | uptime` from pending rows and writes output back (12K char cap); no arbitrary shell by design. Claude drives it with a plain INSERT via execute_sql and reads the result ~30s later. Full SQL snippets, status flow, failure modes, and the incident decision tree live in **NEW `playbooks/runner-agent.md`** (load on demand per the playbook rule).

**Open:** (1) Dean-at-Mac: scp + run the installer (restarts the runner — first healthy airing expected same evening), and capture `journalctl -u vyve-live-runner -n 60` around the 23:12 death FIRST — if it's an empty-schedule crash the runner code needs a guard patch on top of the systemd net. (2) After install, Claude runs a remote check-cycle (uptime/status/journal/ls_masters) and finally settles the 9 never-box-aired filenames question from PM-834 — that thread is downgraded from "likely cause" to "unverified secondary risk". (3) The mid-incident SSH-command guidance earlier today stands replaced by the queue for everything on the allowlist.

## PM-834 — 6 Aug 2026: ACHIEVEMENT RE-TOAST FIXED (mark-seen CORS pin) + broadcast-failure diagnosis (2026-08-06)

**Achievements re-popping on login (Dean report; suspected Dexie — it wasn't).** Server truth first: member_achievements showed massive unseen backlogs platform-wide (Dean 42/57, Lewis 43/75, Calum 42/83 — everyone). The loop: dashboard payload carries `achievements.unseen` → achievements.js `replayUnseen()` re-queues it on EVERY page load → toast dismiss POSTs achievements-mark-seen → **that POST never landed from the native fleet**. Edge logs at Dean's 13:30 session: achievements-mark-seen OPTIONS 200 ×3, ZERO POSTs — while achievement-claim from the same device shows full POST 200s (his 12:30 earn landed). The PM-826 failure class exactly: preflight 200 ≠ preflight pass. Root cause: mark-seen v1 shipped a bespoke `getCORSHeaders` pinning Allow-Origin to the two web origins; the store-app fleet serves from the bundled tree at origin `capacitor://localhost` (the PM-741 origin), so the returned Allow-Origin never matched and the browser silently aborted every markSeen. seen_at was never stamped for any native member; unseen grew monotonically; every login replayed the lot.

**Fix (zero client change, zero OTA):** `achievements-mark-seen` v2 deployed via MCP (deployment version 16) — Allow-Origin `'*'`, static headers, no credentials mode (JWT rides the Authorization header), body otherwise byte-identical; auth gate live-verified via pg_net (401 no-auth, `access-control-allow-origin: *` in response headers). Then a one-time backfill stamped every unseen row platform-wide (`UPDATE … SET seen_at = now() WHERE seen_at IS NULL` → remaining 0): the backlog had already been toasted at members dozens of times, so nothing meaningful is lost and nobody gets a 40-toast avalanche when marking starts working. Steady state from here: fresh earns toast once, dismiss stamps, done. Device note: one final replay burst per device is possible from the locally cached dashboard payload; it dies on the first background refresh, and its dismisses now mark successfully (no-ops).

**Standing sweep owed (§23.144 family):** any EF with a bespoke pinned-origin CORS block is broken for the bundled native fleet. achievement-claim, member-dashboard, log-activity are proven-good from device; mark-seen was the one found broken — grep the EF set for `ALLOWED_ORIGINS`/pinned `Access-Control-Allow-Origin` values and fix any siblings before they present as their own mystery bugs.

**Broadcast-failure diagnosis (addendum to PM-833, thread OPEN):** watchdog alerted 13:05 BST — today's 13:00 "Healthy Shoulders Session 4": broadcast nycubHYQlHI created but stayed 'ready'. The 12:00 slot (box-proven file) aired clean, so the failure is session-specific. Hypothesis: `Healthy Shoulders Part 4.mp4` last aired 6 Jul — the day before the Mac→Hetzner migration — and may never have been rsync'd to the box. 9 never-box-aired filenames carry 28 remaining future airings; first at risk **tomorrow 08:30 BST** (`What Is Breathework.mp4` — historical spelling), then 12:00 (`Why Is a Sleep Routine Important.mp4`), Sat 08:30 (`What Is Meditation.mp4`), plus 5 Calum Education mp4s on Tuesdays from 11 Aug (note: the 11/18 Aug rows pre-dated the regen — they were at risk before today). Today's remaining slots are box-proven, safe. **Bonus catch:** July aired review 3 as `Weekly review 3.mp4` (capital W); the regen wrote it lowercase — 3 future rows corrected in DB (Linux box, case-sensitive). Side effect: reset/review 2+3 masters are PROVEN box-resident (July rotated 1→2→3→1 and aired all of them) — the PM-833 "confirm they exist" check is dead, and the regen's resume-at-2 is confirmed correct. **AWAITING DEAN:** `journalctl -u vyve-live-runner --since "12:55" | tail -40` + `ls /srv/vyve/masters/media | grep -iE "breathe|meditation|sleep routine|shoulders part 4|^VYVE"` → rsync missing files from the Mac media dir, or Claude swaps the at-risk rows to box-proven titles in one UPDATE.

## PM-833 — 6 Aug 2026: LIVE CALENDAR REGENERATED THROUGH 30 SEP — schedule had been DARK since 3 Aug (2026-08-06)

Dean opened with "sort out the live stream schedule for the next 2 months". Live-DB check found the calendar effectively DRY: the PM-686/687 July regen ended 2 Aug, only the two pre-materialized Calum Education Tuesdays (11/18 Aug) existed beyond it, and members had seen an empty schedule since 3 Aug. Nothing alerted — broadcast-watchdog detects dead broadcasts on scheduled sessions, not the absence of scheduled sessions.

**Regen (all direct SQL, PM-686 pattern):** window tonight → 30 Sep on the July grid verbatim — 07:00 Movement, 08:30 Mind, 12:00 Mind, 13:00 Movement (Wed = Midweek Reset), 17:30 Movement, 19:30 Mind Mon/Wed/Sat + Movement Tue/Thu/Fri/Sun, Tue 18:00 Education & Experts, Fri 20:30 Weekly Review. Pools (53 Movement / 21 Mind / 10 Education titles) re-slotted with all metadata verbatim from each title's latest airing (host/role/photo, thumbs, descriptions, notes mp4 filename, partner_id attribution, per-title durations), cycled least-recently-aired-first. **PM-714 rotation carried:** Weds notes = midweek reset 2,3,1,… from 12 Aug; Fris notes = weekly review 2,3,1,… from 7 Aug. Education Tuesdays continue from the stalest titles (25 Aug The VYVE Approach → 1 Sep Nutrition Basics → 8 Sep Movement & Exercise → 15 Sep Sleep Stress & Recovery → 22 Sep Building Habits That Last → 29 Sep Fueling Your Body for Performance); the two pre-existing rows untouched. Final verified state: **350 future rows, 0 same-day title dupes, 0 double-booked instants, 0 days under 6 sessions** (37 days at 6, 17 at 7 — Tue/Fri carry the extra slot). Runner needs no restart (heartbeat pickup); first airing 12:00 today.

**Session gotchas → §23 rules:**
- **§23.156** — `partner_session_min_notice` trigger rejects any partner-linked insert starting <48h out, which blocks a legitimate admin calendar regen (most pool rows carry partner_id). Pattern: `DISABLE TRIGGER partner_session_min_notice` inside the insert transaction, re-enable before COMMIT, verify tgenabled='O' in standalone SELECT after.
- **§23.157** — catalogue-pool CTEs MUST bound to a proven airing window and exclude test-title residue. The first insert's pool had no date filter and swept "PM-327 Device Walk — Mindfulness Live" (an old Dean test artefact) into 6 member-facing slots. Caught in post-commit verification; the 6 airings deleted, and the original artefact row **soft-cancelled (cancelled_at stamped)** so no future pool query with the standard cancelled_at IS NULL filter can ever pick it up.
- **§23.158** — when repairing a bulk insert, capture the vacated slots with `DELETE … RETURNING`, never reconstruct them from assumption. The first repair hardcoded six guessed timestamps: only one matched, five backfills landed on already-occupied slots (including a cross-category double-booking on a Movement-only slot) and the five real holes stayed empty. Second pass audited actual per-day deficits and duplicated instants, deleted the strays, filled the true gaps.

**DEAN CHECK (before Fri 7 Aug 20:30, the first rotated airing):** confirm `midweek reset 2/3.mp4` and `weekly review 2/3.mp4` exist in /srv/vyve/masters/media on the Hetzner box — PM-714's rotation note implies they do, but this sandbox can't SSH (egress domain-locked) and the runner airs whatever filename is in notes. One-liner: `ls /srv/vyve/masters/media | grep -iE 'reset|review'`.

**NEW BACKLOG — calendar-runway watchdog (~0.5 session):** tiny cron EF that alerts team@ (platform_alerts pattern) when calendar_occurrences future coverage drops below N days (suggest 7). This outage went unnoticed for 4 days; the next regen cliff is 30 Sep and this class of failure is otherwise invisible until a member complains. Also candidate: fold a monthly "calendar runway" line into the Studio ritual proposed in the PM-803 freshness doc.

## PM-828/PM-829 — 24 Jul 2026: B2B EMPLOYER DOOR SHIPPED (PM-819 S2) — registry, provision-first invites, Members tab, removal loop (2026-07-24)

The full B2B enrolment lifecycle went live in one session. Backend + both client surfaces + cron, all live-verified via pg_net before the front-end was written.

**DB (migrations `pm819s2_employers_registry`, `pm819s2_auth_state_user_id`):** New `employers` registry — name (UNIQUE, the join key everywhere), slug, contact, seat_cap (NULL=uncapped), price_per_seat_pence (default 1000), billing_cycle, status (trial/active/paused/churned), invite_token (`emp_`+64hex, UNIQUE). RLS: is_admin_or_team() full + employer_admins own-row SELECT. `members` gained `b2b_removal_effective_at` + `b2b_removed_at`. SECURITY DEFINER `b2b_auth_state(text[])` → (email, user_id, last_sign_in_at) from auth.users (REVOKEd from anon/authenticated; roster Invited/Active derivation + ban/delete need it — auth schema isn't REST-exposed). engagement_emails stream CHECK widened + 'b2b_removed'. Seeded: **Sage** (slug sage, cap 50, trial — demo-ready) and **VYVE Demo Co** (cap 25, test rail).

**app-signup v3** (deploy counter 4): `validate_employer {token}` → {valid, employer_name, seats_left}; signup accepts `employer_token` — validated (status trial/active only), seat-capped (409 seat_cap_reached), creates member as company/company_slug/**account_type='enterprise'**/subscription_status='b2b_active' + employer_members row. Enterprise means the trial trigger never fires and auth.js passes. B2B signups NEVER carry partner attribution.

**employer-portal v9 → v9.1** (deploy counter 10): GET gains `membersTab` — org meta (status/seatCap/seatsUsed/inviteLink) + identity-only roster with derived state invited|active|leaving|removed (PII line held: WHO is enrolled, never what they do; computed even for empty aggregate roster so a new employer sees their link). POST actions: `provision_members` (≤200 rows; per-row invited/already_enrolled/exists_elsewhere/seat_cap_reached/invalid_email/duplicate_in_upload/auth_exists/failed; auth user with NO password + email_confirm, member row, employer_members, recovery generate_link → set-password.html, branded Brevo invite [LEWIS COPY PASS]; auth-user rollback on member-insert failure), `resend_invite` (never-signed-in only), `remove_member` (never-signed-in → instant un-provision deleting member+roster+auth user; active → b2b_removal_effective_at = first instant of next month UTC, aligned with S3 arrears-on-actuals), `cancel_removal`. Ops `?b2b_sweep=1`: flip to b2b_removed + auth ban 876000h (the HARD enforcement) + £20 continue email → continue.html?cid={member.id} [LEWIS COPY PASS], deduped via engagement_emails (stream b2b_removed, email_key continue_offer). **v9.1 fix:** v9's dedupe insert omitted email_key (NOT NULL, uq member/stream/key) → flip+ban landed but continue email silently skipped; caught by live sweep test, one-line fix, re-verified end-to-end (removed banned:true emailed:true, dedupe row + banned_until confirmed in DB).

**Cron:** `b2b-removal-sweep-daily` (job 60) 01:30 UTC daily, pg_net POST with GDPR_CRON_KEY.

**vyve-site `316f99de` (PM-828, vbb 530):** signup.html `?c=emp_TOKEN` employer mode — validate_employer on load, title/sub swap ("Join {Employer} on VYVE / your workplace covers your membership"), referral hidden, empChip verified banner, employer_token on submit, 409 branched on j.error (seat_cap_reached vs account_exists), 400 invalid_employer_token handled; invalid links fall back honestly to trial mode with warning. auth.js: enterprise passes the gate only when subscription_status !== 'b2b_removed' (wall covers token-lifetime window; ban is the hard stop — wall shows trial-ended copy, slightly wrong for B2B, acceptable, email explains). sw CACHE_NAME v2026-07-24a-pm828-b2bdoor. All 5 files md5-verified at commit SHA.

**Test-Site `14906b3b` (PM-829):** employer-portal.html Members tab (between Impact and Comms, hidden unless payload.membersTab.org) — seats bar, CSV upload (tolerant client parse: email-cell detection, header optional, 200 cap), single add, per-row provision result summary, copyable invite link, roster rows with state chips + Resend/Remove/Cancel-removal (mode-specific confirm copy), 401-refresh retry, light mtRefresh re-render, privacy footer card. Demo mode (?demo=1) renders canned roster for the Sage sales flow; actions alert Demo mode.

**Live verification (all pg_net, before front-end):** validate_employer valid+invalid · provision 2 invited+emailed / invalid row rejected · GET membersTab org+24 rows (22 fixtures active, 2 invited) · remove_member unprovision → zero residue · sweep flip+ban+email+dedupe · sweep-test member fully erased after. **Left deliberately: deanonbrown2+b2binvite@gmail.com as Invited in Demo Co** — Dean's device walk (invite email in inbox → set password → app sign-in → in-app onboarding).

**Hard gates before any real employer:** Lewis copy pass on the invite email + £20 continue email (both flagged [LEWIS COPY PASS] in employer-portal EF source). Sage registry row exists but NO invites out until then.

**Out of scope / S3:** billing snapshot cron + invoices, CC New Employer wizard, trial-end/reactivation arc, SSO/SAML build (available on Pro: 50 SSO MAU incl., then ~$0.015/MAU; ~1 session on top of today's foundation; a line for Lewis's Sage deck). Known-acceptable: seat-cap race on concurrent signups (trial scale), removal wall copy says "trial ended".

**§23 rules banked this session:** (1) Before an EF inserts into a shared table, read the FULL column constraint picture — NOT NULL columns without defaults, plus unique indexes — not just CHECK constraints (§23.143 extension; email_key bit v9). (2) bash_tool's shell is sh, not bash: associative arrays/declare fail mid-script while earlier lines succeed — write multi-step Git Data API sequences in python or with plain variables. (3) GitHub API responses can contain control characters that break naive `python3 -c json.load` piping — on parse failure, verify commit state by listing commits before retrying (the PM-829 PUT had succeeded).

## PM-827 — 23 Jul 2026: FIRST SUCCESSFUL APP ONBOARDING (v120 confirmed) + reveal port gaps fixed (2026-07-23)

EF v120 verified by real invocation: Dean's resubmit went through — persona reveal fired, PM-820→826 funnel now works end-to-end. His device walk exposed three PM-820 port gaps, fixed in vyve-site `39511141` (vbb 529):

1. **Programme card missing from reveal** — the showResults override never rendered `result.programme` / `programme_overview` / `goal_summary` (the web page does). Restored as a compact card on rv1: programme name, split · sessions/week, goal summary (rationale fallback).
2. **Body/Mind/Connect tour bypassed** — `_rvGo(2)` navigated to the plan tab instead of revealing rv2; the tour + Lewis welcome-video slot were unreachable dead code. Flow is now rv1 (coach + programme) → rv2 (video + three-pillar tour) → rv3 → plan tab; navigation only at the end, landing where the plan-building mask waits.
3. **Apple Health never offered on the app path** — consent-gate.html (which fires `healthBridge.connect()`) never triggers for app members because app-signup pre-stamps terms/privacy timestamps and auth.js's gate check passes. Added rv3: the gate's Apple Health card ported into the reveal, native-only (`healthBridge.isNative()`), `vyve_healthkit_dev` + `connect()` on accept, `vyve_healthkit_declined_at` 7-day cooldown on decline. onboarding.html now loads `/healthbridge.js`.

**Consent state on the app path (documented, Lewis decision pending):** Terms+Privacy = browsewrap line on signup.html stamped by app-signup EF (no explicit checkboxes, unlike consent-gate.html); Health/AI = explicit checkbox at step-zero but REQUIRED (gate frames it optional — though the plan genuinely cannot be built without it); Apple Health = now offered at rv3. **Open for Lewis/DPIA: (a) explicit terms/privacy checkboxes on signup vs browsewrap line; (b) required-vs-optional framing of health consent.** Not redesigned tonight — capture is arguably compliant, framing is his call.
...
Fix (vyve-site `bf0600de` vbb 528, client-only, ZERO EF changes): Authorization header removed from both onboarding fetches in onboarding.html and from _pmRetryOnboard in workouts-programme.js — request shape is now byte-identical to the proven web path. The EF is verify_jwt:false and never read the JWT; identity remains the payload email (session-prefilled + readOnly). **Standing note:** if a later session wants real JWT enforcement on the app onboarding path, add 'authorization' to the EF corsHeaders FIRST, redeploy, THEN reinstate the client header. **§23 rule of thumb: before pointing a new Authorization-carrying client at an existing verify_jwt:false EF, read its corsHeaders — allow-list misses fail as silent client-side aborts that look like network errors.**

Recovery: Dean's answers persist in vyve_onb_draft (cleared only on showResults). On 528 the auth gate routes him back into onboarding.html, draft restore lands him on section 10 fully filled — one tap to resubmit.

## PM-824 — 23 Jul 2026: ONBOARDING BOOT WAS DEAD — SUPABASE SDK NEVER LOADED (Dean: Continue inert on Section A) (2026-07-23)

Dean filled Section A completely and Continue did nothing. Root cause is worse than the symptom: welcome.html talks to EFs via raw fetch and has NEVER shipped the Supabase UMD SDK — the PM-820 boot's second line (`window.supabase.createClient`) threw a TypeError and killed the ENTIRE injected IIFE. Everything PM-820 added client-side was dead on device: no session gate (page loaded without auth check), no email prefill into the hidden Section A fields (validateSection(1) fails silently on the empty hidden email/emailConfirm → Continue inert — the visible symptom), no consent PATCH, no password stubs, no showResults override, no draft cache. The transform was verbatim-correct and node-syntax-clean but never EXECUTED end-to-end in the page's real dependency environment — §23 lesson: **a programmatic port must be smoke-run against the page's actual script dependencies, not just syntax-checked; asserting anchors proves the edit landed, not that the code runs.**

Fix (vyve-site `2a95d6f4` vbb 527): Supabase UMD SDK script tag in head (page carries no CSP meta, so no CSP change needed) + explicit boot guard — if the SDK ever fails to load (CDN blocked/offline), bounce to login.html?stay=1 rather than presenting a dead questionnaire; fail visible, never silent.

Note: PM-823 safe-area screenshot at 21:01 predates vbb 526 pickup — header verdict pending Dean's re-check on 527.

## PM-823 — 23 Jul 2026: ONBOARDING SAFE-AREA HEADER (Dean device feedback on the PM-820 walk) (2026-07-23)

Dean's first live walk of the in-app questionnaire: the fixed progress bar (logo / 1-of-10 / theme toggle) rendered UNDER the iOS status bar — welcome.html was authored for browser chrome, and the Capacitor webview is edge-to-edge. vyve-site `44fa99d0` vbb 526: onboarding.html gets viewport-fit=cover + .progress-wrap padded by env(safe-area-inset-top) (height calc-matched) + .container top padding follows. signup.html/login.html gain viewport-fit=cover so their pre-existing env() paddings actually ENGAGE (without viewport-fit=cover the inset reads 0 in WKWebView — worth remembering for any new full-bleed page); login .wrap now safe-area-aware top and bottom. Web unaffected (env() falls back to 0). Rule of thumb going forward: any NEW page destined for the native shell ships with viewport-fit=cover + safe-area padding on fixed chrome from day one — theme.css-linked portal pages already handle this; the three PM-820 pages were self-contained and missed it.
...
**BACKEND (live before the site commit):**
- **Trial clock = DB trigger, ZERO onboarding-EF changes.** `start_trial_clock()` BEFORE INSERT OR UPDATE OF onboarding_complete on members stamps `trial_ends_at = now()+7d` only when onboarding_complete=true AND account_type='trial' AND trial_ends_at IS NULL. The onboarding EF already writes onboarding_complete=true on both its insert paths, so the trigger carries the whole spec; NULL-guard makes idempotent re-onboards safe and keeps apply-trial (web 14-day path) coexisting — its explicit stamp wins because the trigger never overwrites. Assert-verified with a rolled-back scratch row. **Behaviour edge (accepted):** a web-paid member whose Stripe reconciliation fails now walls at day 7 instead of never — reconciler runs */15 with alerting, and a day-7 wall SURFACES broken reconciliation instead of hiding it.
- **`app_videos` config table** (5 keys seeded: onboarding_intro / onboarding_welcome / hub_mind / hub_body / hub_connect — all url NULL = dark; public read, is_admin_or_team write) + public storage bucket `app-videos` (200MB, video mimes). Lewis films → upload → set url → surfaces appear. No OTA.
- **`app-signup` EF v2** (verify_jwt:false — it IS the front door; PM-747 rate-limit rails: signup 5/hr/IP, validate 30/hr/IP). Actions: `validate_code` (live partner slug → partner name) + `signup` (password-complexity mirror of Supabase rules; auth user + minimal member row; auth-user ROLLBACK if member insert fails; 409 account_exists routes to sign-in). Referral code validated server-side and locked at signup on signup_campaign_code — this EF is the sole writer on the app path; onboarding EF's merge-duplicates upsert never touches the column. **v1→v2 catch: consent gate (auth.js vyveCheckConsent) reads `terms_accepted_at`, NOT privacy_accepted_at — v2 stamps both** or every app signup would have bounced through consent-gate.html. Live-verified all 4 paths via pg_net; test account deleted.
- **`trial-end-email` EF v1 + cron `trial-end-email-daily` 01:10 UTC** (10 min after expire_lapsed_trials). `run` action gated on x-vyve-cron-key == new secret **VYVE_CRON_KEY** (minted this session, EF env + vault mirror — the first properly-paired cron→EF key; management API via Composio sandbox because api.supabase.com is blocked in the main sandbox). Selects trial+expired+is_test=false with trial_ends_at in last 48h; dedupe via engagement_emails stream='trial_end'. `resend` action: member JWT, 3/day rate limit, must be expired trial — powers the trial-ended screen button. Email: Brevo direct (no health content — Brevo DPA 3.2(vi) clean), links `continue.html?cid=<members.id>` (VYVE10 prefilled, client_reference_id binds payment); partner-referred copy leads with the partner name; attribution rides members.signup_campaign_code through stripe-reconcile fallback 3 — NO webhook changes. Copy placeholder-honest [LEWIS COPY PASS]. S3 upgrades this into the day-6 arc.

**LIVE INCIDENT + §23.143 STRIKE THREE:** first cron run legitimately emailed the 8 REAL members whose trials expired 22 Jul evening (the '14day' cohort ×7 + Ted Morgan via men-together-cic, who correctly got partner framing) — first conversion email that cohort has ever received. But the engagement_emails stream CHECK (A/B/C1/C2/C3) silently rejected the dedupe writes — third occurrence of §23.143 (read CHECK constraints before writing literal enum values). Without the fix tomorrow's cron would have re-sent to all 8. Migration widened the CHECK (+'trial_end'), backfilled 8 dedupe rows, re-run verified sent:0 skipped:8 (uq_engagement_email makes dedupe airtight). **Open (minor):** email says "free week" to the 14-day cohort — one legacy cohort, offer correct; optional EF copy tweak.

**SITE (vyve-site `c76a34c9`, vbb 523, 12 files, md5-perfect at SHA):**
- **signup.html NEW** — "Start your free week / Create your account — no payment details required." Email + password (live requirement ticks) + collapsed "Have a referral code?" field with blur validation chip: valid → "<partner> referral — discount locked in, applies automatically after your free week" (**NO price anywhere — Apple 3.1.1**); invalid → soft non-blocking. 409 → sign-in link. On 200 → signInWithPassword → onboarding.html. login.html gains "New to VYVE? Start your free week".
- **onboarding.html NEW** — programmatic transform of welcome.html (444KB): questions VERBATIM by construction, all sections + choice engine + full/quick path picker + payload contract intact. App deltas: session-required (no session → login.html); email prefilled+locked from JWT; password q-groups hidden + validators/confirmPasswordSet stubbed (auth exists; payload password:'' is falsy → EF touches nothing); marketing PostHog init neutralised; step-zero health-data consent tick (GDPR Art 9 — writes health_data_consent via members_own_data RLS before question one) + dark Lewis intro video slot; JWT on BOTH submit fetches; **localStorage draft cache** (fields+chips+section, restore-on-load resumes mid-questionnaire — kill the app, come back, carry on); already-onboarded revisits bounce to home. Post-submit: apply-trial branch inert (TRIAL_CAMPAIGN_CODE null in-app), showResults overridden → **persona reveal** (orb + reason + first-week recs + "Your free week starts now — ends <date>") → "View my plan" (stream-routed: workouts→workouts.html, movement/cardio→exercise.html) → welcome-video slot (dark) + Body/Mind/Connect tour cards → "Start my tour" → home. Sets vyve_onb_pending/persona/payload for the workouts mask.
- **auth.js — vyveCheckOnboarding gate** after the access gate: onboarding_complete=false → onboarding.html. Same skip-list/fail-open/session-cache philosophy as consent — the gate can NEVER brick the front door. All 84 existing members already true (EF has always written it) — no backfill needed, verified.
- **workouts masking** — the minutes-long Phase 2 gap is now the reveal, not a bug: #plan-building card ("{persona} is building your 8-week programme… this page updates itself") shown instead of no-plan when vyve_onb_pending; polls loadProgramme() every 15s; plan lands → mask + flags cleared. After 4 min: "Rebuild my programme" → re-submits the cached questionnaire payload JWT-authed to the onboarding EF (idempotent re-onboard). Poll self-kills at 15 min.
- **trial-ended.html HARD-GATE DE-STEER** — the non-compliant in-app Stripe steering is DEAD. No price, no purchase link, no external browser. "Your free week has ended / check your inbox (<their email>)" + **Resend the email** button (trial-end-email resend, 429-aware). Keeps/charity blocks stay (benefit statements, compliant). **Binary submissions are now unblocked on this axis.**
- **hub-videos.js NEW on the nav.js injection rail (§23.44)** — self-scopes to mind/exercise/connect, 1h-cached app_videos config, first-visit card (flag per hub) → "Watch the intro again" row after. Dark until Lewis uploads.
- sw.js CACHE_NAME → pm820-appfirst, precache +signup/+onboarding/+hub-videos; vbb 522→523 in index.html + settings.html + sw.js same commit.

**NOT done this session (deliberate):** welcome.html slimming to pay-and-password — the locked design keeps the web questionnaire functional during transition; slim after app completion rate proves out. Returning-trialist checkout-attach = S3 as planned. OTA: Dean sees 523 immediately (server.url); **member fleet needs a Capawesome OTA publish — Dean's call** (new signups only reachable via login.html link until then, existing members unaffected).

**GATES:** Lewis copy pass — signup screen, onboarding step-zero + reveal/tour, trial-ended screen, trial-end email (all placeholder-honest live NOW, low blast radius: new signups + 8 expired trialists). Videos await Lewis/Calum filming (everything dark until urls set). Phil untouched.

**NEW RULE §23.155:** Supabase MCP execute_sql returns ONLY the LAST statement's result set — a missing earlier SELECT is invisible, not empty. Never infer "zero rows" from a multi-statement call; run verification SELECTs standalone (this masked the engagement_emails candidates during the incident triage).

## PM-819 — 23 Jul 2026: APP-FIRST FUNNEL + B2B EMPLOYER ONBOARDING DESIGNED — 3-session programme locked, NOT started (2026-07-23)

Design-only session off Dean's opening ask (employer purchase onboarding, employer area, member add/remove, invoice billing). Conversation widened into the whole acquisition funnel; converged on one programme. No product commits.

**THE CORE MOVE — onboarding comes INSIDE the app (ALL members, not just B2B).** Dean's observation: the web questionnaire exists only to keep payment outside Apple, but profiling got welded to payment — people complete the 10-section form in Safari, feel "done", never install (visible in usage). Split them: web keeps ONLY money + account creation (PM-696 inline-password machinery); app owns the questionnaire on first login (`onboarding_complete=false` routes to a new vyve-site onboarding page, JWT submit to the same onboarding EF — persona/habits/two-phase workout gen unchanged; workouts.html "being prepared" state already anticipates it). Existing members backfilled `onboarding_complete=true`. Web questionnaire stays functional during transition, soft-kill after app completion rate proves out. Resumable by design (local answer cache, retry-safe submit) — first-login gate must not brick the front door.

**FREEMIUM REJECTED / 7-DAY TRIAL LOCKED (Dean, pending Lewis packaging pass).** Freemium = permanent second product tier (free-vs-paid ruling on every feature forever, enforcement across ~40 pages, non-payers polluting leaderboards/charity/community aggregates) — wrong for a 2-person pre-revenue team. Trial wiring largely exists (35 trial/29 expired split, trial-ended page, subscription_status vocabulary, past_due grace PM-598). Funnel becomes: App Store → in-app signup → in-app onboarding → 7 free days → pay on the web. **Trial clock starts at ONBOARDING COMPLETE, not account creation** (measures "did VYVE work for me for a week"). Trial arc rides re-engagement rails: day-1 programme ready → nudges → mid-week check-in prompt → day-6 summary into the conversion ask. 29 expired trials = proof time alone doesn't convert; activation does — hold at 7 days.

**APPLE COMPLIANCE — the sneaky path dies WITH Session 1, not before.** Live app currently flips trial-enders to a Stripe prompt in-app (non-compliant steering). Dean's cover holds today: reviewers walk his non-trial demo account and there is no in-app signup, so no reviewer can ever reach the trial state — discovery risk ≈ 0 on the current binary. But in-app signup hands reviewers a fresh 7-day-trial account = the exact flow they scrutinise hardest, and the next binary submission is already owed. **HARD RULE: de-steer ships inside Session 1, before any binary submission.** Compliant shape = Netflix pattern: app sells nothing, trial-end screen says only "check your email"; email carries Stripe link + discount coupon (partner-coupon machinery); returning-trialist checkout attaches to the EXISTING member (small webhook/reconcile branch, email = join key); reconcile flips active, member resumes exactly where they left off (Dexie + server rows intact = the conversion lever). Interim submissions keep handing Apple the demo account.

**B2B DOOR (the original ask) — smaller once onboarding is in-app.** `employers` registry table (name/slug/contact/seat_cap nullable/price_per_seat_pence default 1000/billing cycle/status/invite token) — employer_name stays the join key everywhere (7 shipped surfaces are name-keyed; no migration). Entry: `welcome.html?c=EMPLOYER_TOKEN` → set-password only, NO payment, NO trial clock → b2b_active, seat counted, EF enforces seat_cap. Employer portal gains a **Members tab**: roster (name/email/joined/status), email invites via Brevo + copyable link, pending list, Remove. **PII principle stated for the DPA: employer sees WHO is enrolled (contract/billing data), NEVER what anyone does — activity/wellbeing stay aggregate-only as today.** Removal = period-end (access to end of billing period → b2b_removed + auth off → email offering £20 B2C continue [LEWIS COPY]); data untouched, normal retention/erasure applies.

**BILLING — invoiced, no Stripe, monthly ARREARS ON ACTUALS (recommended default).** Cron on the 1st snapshots active B2B seats per employer → auto-raises structured invoice (cc_invoices extended: employer_name/period/seat_count/unit_price) → CC Invoicing send + Mark-paid as today. No proration maths, bill always true. Schema supports advance/committed (seat_cap doubles as committed seats) if Sage procurement forces annual PO + upfront — per-contract commercial call, not a build fork. CC **New Employer wizard** on pages/employers.html: org + seats + price + admin email → registry row + employer-provision EF admin login + invite token + admin welcome email [LEWIS COPY]; links to the CRM-won cc_clients row.

**BUY-IN VIDEO LAYER (Dean's activation diagnosis: features with no WHY).** How-to pages are the cautionary tale (zero visits ever, soft-killed PM-804) — context must be IN the path. Two layers: (1) questionnaire step-zero Lewis intro ~60s (five-pillar framing: nutrition/sleep/physical/mental/social → "answers build your tailored plan"), skippable after a few seconds, skip rate = signal; (2) first-visit videos on the three hubs — mind.html / exercise.html / connect.html — one-time card, Dexie per-hub flag, "watch again" row; Dean dictated ~80% of the three hub scripts in-session (Mind: meditation/visualisation/sleep/financial wellbeing as mental-health tools; Body: physical and mental health are one and the same — movement/steps/tailored workout+running plans; Connect: connection IS mental health — live sessions/social). Hosting: Supabase storage bucket (R2 later if egress bites), never bundled — Lewis reshoots without OTA. Build ships DARK (card renders only when config video URL exists) — never waits on filming. Content = Lewis/Calum lane.

**LOCKED SEQUENCE (~3.5 sessions):** **S1** in-app signup + in-app onboarding + trial clock (from onboarding complete) + video slots (dark) + **Apple de-steer + trial-end email** + web page slimmed to pay-and-password. **S2** employers registry + invite-token door + portal Members tab + removal loop. **S3** CC New Employer wizard + billing snapshot cron + invoice generation + full trial-end/reactivation arc (upgrades S1's placeholder email into the day-6 arc). S1 pays for itself in B2C conversion regardless of Sage timing.

**DEAN CONFIRMED 23 Jul (executive decision; [LEWIS REVIEW] before first employer contract — added to his decision doc):** (a) arrears-on-actuals billing default; (b) roster-identity-visible / health-never principle; (c) period-end removal; (d) invite link + email invites with seat_cap as abuse guard. **LEWIS DECISION DOC OWED (Claude drafts):** trial mechanics (7 days, clock-from-onboarding), discount code + conversion email framing, 4 video scripts (intro + 3 hubs, drafted from Dean's dictation), trial-end flow change heads-up, copy pile (slimmed web page, invite email, employer-admin welcome, removal/continue email, invoice email, trial-arc emails).
...
**PM-814 — B2 MEMBER FLOW.** Design shift forced by the money model: payment lands in the PARTNER'S OWN Stripe → no webhook possible → payment confirm is PARTNER-MANUAL. Member books (pending_payment, pays immediately on the service's payment link, slot held), partner's diary gains **Confirm paid** → confirmed + member confirmation email (meeting link or location) + 1h-before reminder push (`scheduled_pushes`, dedupe_key `booking_reminder:{id}`). Hold window 30min→24h (`pm814_booking_hold_24h`). `create-booking` EF v1 (verify_jwt, JWT-derived email; service live+clinical check; slot validated inside Europe/London weekly windows minus blocked dates, ≥1h ahead; price/link/delivery snapshots; DB slot guard = race authority → 409 slot_taken with friendly retry). `booking-paid-confirm` EF v1 (partner JWT via `get_my_partner_id` RPC-as-caller). vyve-site `28946cdf` vbb 520: partner-profile booking card + bottom-sheet slot picker (14-day chips, duration-stepped times, external pay link per no-IAP); partner-space Joined 'Your bookings' (status chips, join link, member cancel via own-RLS PATCH). CC `39697580`: diary Confirm-paid button. Known trade-off logged in-commit: slot computation is client-side vs availability only (RLS hides other members' bookings) — collisions surface as 409 at booking; fine at trial scale. Recursion bug (loadMyBookings call injected inside itself) caught pre-commit.

**LIVE-WALK FIXES (PM-814.1–.5):**
- **.1 'Not linked' + HTTP 400 on service save:** `get_my_partner_id()` demanded `admin_users.role='partner'` — Dean/Lewis (role='admin') were locked out of partner records deliberately linked to their emails. Migration `pm814_partner_id_admin_link`: role gate now `IN ('partner','admin')`; contact_email match unchanged. Dean→Emma Clarke, Lewis→Calum Denham now resolve.
- **.2 Multi-day availability** (CC `0d28c3d2`): single Day dropdown → 7 toggle pills + Weekdays/Every-day shortcuts; one Add = one row per selected day (single array POST).
- **.3** `bx_unique_day` UNIQUE (service_id,date,kind) — dupe guard for range blocking.
- **.4/.5 committed via ONE-SHOT RUNNER EF** (bash container died mid-session — even `true` erroring; §23.154 born): vyve-site `c0399314` vbb 521 — **Book becomes the 4th profile tab** (Community|Sessions|Library|Book, hidden without bookable services; onclick-property wiring duplicate-safe) + **opaque slot sheet** (was `--surface` = translucent in app dark theme → `--bg` + border + shadow; §23.153). CC `37474f18` — **blocked-date From/To ranges** (holiday-style, per-day rows, 60-day cap, per-row inserts skip dupes) + **'Apply changes to' service pills + All services** (windows/blocks fan out across selected services, cross-product with day pills; MutationObserver rebuilds pills off the service dropdown; row hidden under 2 services). Runner: `pm814-commit-runner` EF (Git Data API in Deno, PAT via new service-role-only `vyve_github_pat()` vault accessor, all-anchors-or-abort, full-content verify at commit SHA). PAT never left VYVE infra. Runner is a dead one-shot — delete when convenient.

**DEVICE VERIFICATION (Dean):** member loop CONFIRMED end-to-end — service create (Emma Clarke) → Book tab on profile → slot → pay link → diary Confirm paid → confirmation email. Employer loop: Emma Clarke workshop request (31 Jul) sitting in CC→Partners→Bookings awaiting Dean's confirm (his email is the contact → employer confirmation will fire); earlier Calum plumbing request CONFIRMED from CC → Lewis received the partner email (his inbox is Calum's contact_email). Diary handles employer rows already (`member_email || employer_name` fallback — B1 got it right, no patch needed). Partner-portal RLS confirmed: partner sees all bookings on their partner_id, both doors.

**OPEN:** B4 polish (.ics on confirmations, per-service cancel policies w/ enforcement — member cancel is currently ungated client PATCH, no-show/booked-vs-attended stats, runner-EF deletion). Gates unchanged: Lewis copy pass (~12 email variants across complaints+bookings), Phil therapy category, Lewis/Alan launch-partner list (empty booking shelf = freshness failure mode). Page-docs follow-up owed per §11B README: help.md, partner-profile.md, partner-space.md.

---

## CHECKPOINT — 14 Jul 2026 late (PM-812/813): Booking engine B1 + B3 SHIPPED (2026-07-14)

Mid-session checkpoint (session continues into B2) so parallel sessions see the booking build.

**PM-812 — B1 ENGINE + PARTNER SIDE.** Migration `pm812_booking_engine`: `booking_services` (title/desc/category/delivery_mode online|in_person|both/duration/price_pence/meeting_link/venue-address-city/payment_link_url/active; trigger `booking_service_clinical_flag` — therapy ⇒ requires_clinical_signoff + clinical_approved RESET on any flip into therapy, so nobody renames past Phil), `booking_availability` (weekly windows), `booking_exceptions` (blocked/extra dates), `bookings` (member_email OR employer_name; statuses requested/pending_payment/confirmed/completed/cancelled/no_show; **partial unique slot guard on live statuses — double-booking physically impossible**), `booking_events` audit. `partner_partners.booking_share_pct` DEFAULT 100 (dormant — partners keep 100%, Dean). RLS: members read active+clinically-cleared services only; partner own via get_my_partner_id(); admin all; bookings member-own + partner-diary + admin. `expire_booking_holds()` + cron `expire-booking-holds` */5 (30-min pending_payment release). **Engine assert-verified live**: therapy gate ✓ slot guard ✓ hold expiry+event ✓ (deliberate-rollback test block). Partner surface (vyve-command-centre `c3a855ce`): partner-portal.html **Bookings tab** — services CRUD (delivery-mode-aware fields, therapy approval notice, pause/resume), weekly availability + blocked dates per service, diary with completed/no-show on past confirmed. Self-contained lazy-init script, token fields per §23.152.

**PM-813 — B3 EMPLOYER FLOW (the Sage line).** Migration `pm813_booking_employer_fields` (+employer_contact_email/format/headcount/fee_pence). **employer-portal EF v7** — GET gains `bookableExperts` (live community-visible partners) + `bookings` (employer's requests); POST action `book_expert_request` (validates live partner, inserts status='requested' with INDICATIVE preferred time, booking_event, team@ notify + requester ack [LEWIS COPY PASS]); **pg_net-verified live against VYVE Demo Co: 5 experts, v1–v6 blocks intact** (note: Sage employer_name has zero roster rows — payload short-circuits `empty:true` BEFORE booking blocks; only real roster today is 'VYVE Demo Co'). **confirm-booking EF v1** (admin_users-gated): confirm sets final starts_at/ends_at + fee_pence + emails employer AND partner contact_email (partner notify CORRECT for bookings — their business — unlike complaints); decline emails employer with note. **CC** (`c81a9e59`): Partners → Bookings — request queue + review panel (final date/time/duration/fee/note → confirm|decline), full ledger (member 1:1s + corporate), KPIs incl. top-partner-30d; never-cached. **Employer portal** (Test-Site `1d623c82`): Book Experts tab UN-HIDDEN in live mode (was demo-only) — live partner cards, request modal (format/date/time/headcount/notes) → EF with 401-refresh retry, "Your booking requests" panel with refs/status chips; ?demo=1 sales flow untouched. **Plumbing test request live in the CC queue**: VYVE Demo Co → Calum Denham, workshop, 24 Jul, ~40 — Dean to confirm/decline from CC (confirms partner email path; no employer ack — ops-path request).

**OPEN: B2 member flow (profile booking section, slot picker, pay + confirm, My Bookings, reminder push) + B4 polish (.ics, cancel policies, no-show stats). Gates unchanged: Lewis copy ×(complaints+bookings emails), Phil therapy category, launch-partner list.**

---

## SESSION CLOSE — 14 Jul 2026 eve (PM-803 → PM-811): Alan's remaining three — freshness map run, complaints loop shipped + device-verified, booking system designed (2026-07-14)

Dean opened continuing from the PM-802 close: items 8/2/7 remaining. All three dispositioned this session, plus the hygiene sweep and two device-feedback fixes.

**PM-803 (migration `pm803_habit_theme_rotation`) — HABIT THEME ROTATION AUTOMATED** (freshness map fix #1). `habit_themes.rotation_order` 1–5 (Move More → Sleep Well → Eat Well → Mind Matters → Connect), `rotate_habit_theme()` SECURITY DEFINER advances cyclically + stamps `month_label` Europe/London, pg_cron `rotate-habit-theme-monthly` 00:10 UTC on the 1st. Run once live: April-stale "Move More" → **July 2026 = Sleep Well**. Badge can never go stale again.

**PM-804 (vyve-site `c9c75a04`, vbb 516→517) — HOW-TO PAGES SOFT-KILLED** (Dean: "can be killed, we're not using them"). Finding: pages already had ZERO inbound links (nothing in nav/More/hubs) and `how_to_resources` has had 0 rows since birth. Footprint removed: sw.js precache −2 + CACHE_NAME bump; sync.js `how_to_resources` pull + `CATALOGUE_FRESH_TABLES` entry removed (revival comment left in place). Files + db.js Dexie store retained per soft-kill rule.

**PM-805 (data-only) — PARTNER HYGIENE SWEEP.** 3 declined "Dean Brown" test partners deleted with all dependents (ratings/posts/content/memberships/programs/pushes/onboarding/payouts) → 10 partner rows remain, 0 declined. Hidden placeholders (Antonia/April/David/Ivan) kept hidden per Dean default; "New partner"[contract] left for Dean to name or kill.

**FRESHNESS MAP (Alan item 8) — doc delivered to Dean** (`vyve-freshness-map-2026-07-14.md`, not repo-committed, matching the coverage-grid pattern). Core finding: every member-data-driven and cron-driven surface is healthy; **every stale screen is a human-fed catalogue table with no feeding ritual**. Verified flags: theme April-stale (fixed PM-803); 4/8 streams dead end-to-end — Workouts/Therapy/Events/Podcast have 0 replay videos AND 0 occurrences next 7 days (**Dean: leave as-is, content coming soon**); how-to empty since birth (killed PM-804); podcast page current per Dean ("still publishing… up to date"); breathwork `mind_videos` kind has 0 rows → page runs on FALLBACK_ only (content ask → Lewis/Calum); `checkin_questions` rails 0 rows — NOT done, Lewis copy task; partner libraries quantified (5 content items ever, 2 approved Emma Clarke test clips, 0 programs, newest post 7 Jul) — backs the PM-802 coverage gates. Recommended monthly Studio ritual for Lewis in the doc.

**PM-806 (migration + 2 EFs + vyve-site `5a7fec1d`, vbb 517→518) — COMPLAINTS LOOP LIVE (Alan item 2).** Dean design decisions: entry point = Help section in settings (not per-session/replay/profile); **partners NEVER in the loop — "a partner wouldn't deal with this. We will."** (rewrites Alan's original auto-partner-notify spec). Shipped: `member_complaints` (RLS member insert/read own, admin full, partner nothing); `submit-complaint` EF v1 — verify_jwt, email derived server-side from token, safeguarding regex scan → status urgent + flag, Brevo ack (safeguarding variant signposts Samaritans 116 123 / 999 per HAVEN rule), team@ notify with CC link, emails fire-and-forget (401-verified via pg_net); `resolve-complaint` EF v1 — admin_users-gated, resolution apology/free_session/refund/other + member-visible note + internal-only toggle, fires resolution email; help.html (quick links + Report a problem form, category picker, community selector for community/session categories) + Settings About card "Help & Support" row + sw precache. ALL member-facing email copy [LEWIS COPY PASS] placeholder tier — gate before member promotion.

**PM-807 (vyve-command-centre `33a977d2`) — CC COMPLAINTS QUEUE.** pages/complaints.html under Members (shield icon): active-users gate pattern, KPI strip (open / urgent / resolved-30d / repeat-partners-30d), Open+Urgent/Resolved/All filters, urgent rows tinted, detail panel with safeguarding banner + resolve form → resolve-complaint EF. Router: complaints added to never-cache list (live data).

**PM-808 (vyve-site `ade7226818`, vbb 518→519) — HELP.HTML APP-CHROME RESTYLE** (Dean device: "looks nothing like our app"). Root cause: PM-806 shipped without the `/theme.css` link — page had NO tokens, so no light/dark, no fonts, no surfaces. Fixed: theme.css linked, canonical hero (hero-label / DM Serif hero-title + em accent / hero-sub), dual-token cards + inputs with light-mode overrides, --teal submit, nav.js subPageLabels 'help' → "Help & Support". Form logic untouched (Dean's live report had already flowed). **New hard rule §23.151.**

**PM-809 (vyve-command-centre `db579a48`) — CC DARK-MODE INPUTS FIXED** (Dean: "CANT SEE THE WRITING IN DARK MODE"). Root cause: shell.css styles inputs with `color: var(--text)` but never background → UA-default white field + near-white dark text = invisible typing in the resolve form. Scoped token fields added to complaints.html (surface-2 / text / border / shadow-focus / checkbox accent). **New hard rule §23.152.**

**Dean device-verified the full loop**: report from unstyled help.html → CC queue → apology resolution → member email received. Correctly noted "swearing" complaint filed Open not Urgent — profanity is a quality complaint, not a crisis signal; the regex is right to exclude it.

**PM-810 (VYVEBrain `0be626bd`) — REPLY-INGEST BACKLOGGED.** Dean replied to the resolution email expecting it in CC; it lands at team@ by design (sender). Reply-ingest (subdomain reply address, MX → Brevo inbound parsing, webhook EF → thread + reopen) backlogged and sequenced WITH the email tenant migration (finding U) so the DNS work isn't done twice. Team inbox = accepted v1 reply path.

**PM-811 — BOOKING SYSTEM DESIGNED (Alan item 7, REDEFINED by Dean).** Not save/remind: Dean wants real bookings — (a) members book+pay 1:1 sessions with experts (group-therapy attendee → private therapist session), (b) employers book VYVE experts through their portal. Design doc delivered to Dean (`vyve-booking-system-design-2026-07-14.md`). Locked in session: **partners only bookable v1** ("we may eventually broker other partners, for now that way"); **partners keep 100%** → payment on partner's own Stripe link, VYVE never holds booking money (`booking_share_pct` col dormant at 100); **video = expert-supplied link v1** (Dean floated building a Teams-alike — steered to never-build; Daily/Whereby embed is the v2 if in-app rooms are wanted); therapy category ships dark behind `requires_clinical_signoff` until Phil. Employer v1 request-based (portal request → CC queue → confirm+invoice) — doc default pending Dean's explicit confirm. Schema: booking_services / booking_availability / booking_exceptions / bookings (pending_payment 30-min hold → confirmed → completed/cancelled/no_show; employer rows requested → confirmed) / booking_events. Refund wrinkle flagged: partner holds money, so booking-dispute refunds are VYVE-instructed, partner-executed — partner-agreement line needed. ~4.5 sessions; recommended order B1 engine → B3 employer (Sage demo line) → B2 member → B4 polish. Open: Dean employer-v1 confirm, Lewis ×6 email copies + agreement line, Phil therapy gate, Lewis/Alan launch-partner list (empty booking shelf = freshness-map failure mode again).

**Alan's ten final state: 1/3/6/8/10 + 2 SHIPPED · 4/9 docs · 7 designed · 5 = Lewis.** Device checks open: help.html restyle (post-cache), featured partner (needs Lewis's flag), employer Comms tab, carousel taste.

---

## SESSION CLOSE — 14 Jul 2026 (PM-797 → PM-802): Alan's ten actions — six closed in one session + the Sage integration position (2026-07-14)

Dean opened with Alan's ten-action list from his Lewis catch-up (12 Jul). Captured, triaged, then six items shipped/closed same session; the back half of the session became the Sage "app inside their app" strategy conversation, which produced the item-9 deliverable.

**PM-797 (VYVEBrain `3360a50b`) — Alan's ten actions captured + triaged** into backlog with owners and sizing.

**PM-798 (migrations ×2 + vyve-site `820a48cc`, vbb 513→514) — PARTNER RATINGS LIVE (Alan 1).** `partner_ratings` table: member insert/update own via RLS (membership required to rate — WITH CHECK EXISTS on partner_memberships), VYVE (is_admin_or_team) + the host (get_my_partner_id) read everything from rating #1. Threshold-gated publish: SECURITY DEFINER trigger recomputes on every insert/update/delete and writes `partner_partners.rating`/`rating_count` ONLY at ≥ `rating_publish_threshold` (per-partner column, default 20) — sub-threshold partners read 0/0 by construction, so member-facing gating lives in the data, never client-side. Second migration made source_id NOT NULL DEFAULT '' with plain-column UNIQUE (PostgREST on_conflict can't use expression indexes — new gotcha §23.107). partner-profile.html: "Rate this community" star card for joined members (renders immediately after joining, POST upsert on_conflict, honest microcopy "Shared with VYVE and the community host"), published ★ stat in hero only when rating_count>0. Trigger live-verified full cycle on a throwaway partner (below threshold 0/0 raw 2 → crossing publishes 2.7/3 → re-rate re-averages → cascade cleanup clean). DEAN DEVICE-CONFIRMED ("Stars working well"). NOT DONE (deliberate): CC partners.html raw-ratings surface (M3 monolith — small add next visit), session/content source_types (schema ready), partner-portal host view.

**PM-799 (migration + vyve-site `c7a08ee5`, vbb 514→515) — FEATURED PARTNER SLOT (Alan 6).** `partner_partners.featured` boolean + partial unique index (DB physically enforces max ONE featured platform-wide). partner-space.html Discover: "Featured" section-hdr + full-width gold-chip card pinned above the grid, featured partner excluded from grid below; zero visual change until a flag flips (nobody featured yet — whose face is Lewis's call; flip = one UPDATE). LATENT BUG FIXED: `.map(partnerCard)` would have leaked the array index into the new featured param and badged every card past the first — all card renders now wrap the call (§23.108).

**PM-800 (migration + employer-portal EF v6 + Test-Site-Finalv3 `df12a4e8`) — EMPLOYER COMMS & ENGAGEMENT TAB (Alan 3).** `employer_comms` table (is_admin_or_team RLS, VYVE-side managed; portal reads via EF service role): posted_at/channel(8 kinds)/title/summary/audience/asset_url. EF v6 GET payload gains `comms` block (newest-first, limit 50) — live-verified via pg_net + cron key (200, seeded row present, payload intact). Portal: Comms tab in BOTH modes — demo shows 5 sample posts (the sell-in story), live renders real record w/ honest empty state; footer routes requests via Ask VYVE, positioning the tab as the shared record Alan described. One seeded row kept on VYVE Demo Co (created_by='pm-800-verify') so Dean's live check shows a real entry. Logging comms = INSERT into employer_comms; CC form when volume justifies.

**PM-801 (vyve-site `6c9201a2`, vbb 515→516) — CONNECT EXPLORE CAROUSEL (Alan 10).** The three stacked full-width 16:8 gradient billboards (Calendar/Podcast/Community) → one swipeable carousel: 82%/340px-max 16:9 cards, scroll-snap, next-card peek; gradients + light-theme overrides untouched; PM-724 community-tile toggle unaffected (sets display:'' → reverts to stylesheet, flex-safe). Header "What's on"→"Explore", calendar header-link dropped (each card carries its own CTA). Taste call — fallback if it doesn't land on device: two-up compact grid (~10-line CSS swap). Imagery deferred: no real assets for podcast/calendar tiles yet.

**Coverage grid doc (Alan 4 — delivered to Dean as .md).** Live-data truth: 5 member-visible live partners but only 3 carry session cadence (Alex 37 / Nicola 40 / Calum 12 upcoming; Emma + Men Together live in name only); ZERO live content items platform-wide (every Library tab empty); Mind = 2× yoga (no stress/mental-fitness subject); no Connect partner running sessions; 3/5 live partners lack contact_email. Proposed gates (Lewis/Alan to set): Sage-present 3–5 partners / 1-per-pillar / ≥6 sessions-wk; trial 6–8 / 2-per-pillar / nutrition+stress+active-community; scaled corporate 9–12 / subject redundancy / library depth. Dean-side fallout: hygiene sweep needed (4 hidden "VYVE Partner" placeholder rows + 3 declined Dean test rows + 1 unnamed contract-stage row); decision owed on sessions-only as official trial model.

**PM-802 = this close. Sage integration position (Alan 9 — spec doc delivered to Dean as .md, build-options scoped per Dean; commercial/contract stripped to Lewis/Alan).** The strategy that emerged across the conversation: (a) THREE DOORS — Sage HR app (product org) vs SEB shelf (partnerships + Personal Group) vs Sage-as-client (wellbeing lead); which door = question 1. (b) KEY INTEL: Sage Employee Benefits is NOT Sage's — provided/administered by Personal Assurance Services (Personal Group Holdings plc), white-labelled; Sage's proven category behaviour is partner-supplies-Sage-badges-Sage-sells, so Sage-badged VYVE = their existing model. SEB's wellbeing portal is a passive 24/7 advice library (mental health/nutrition/exercise/sleep) — no sessions, no habits, no community, no engagement data; pitch: "SEB advises; VYVE gets them doing it and shows you the numbers". (c) ARCHITECTURE (push + HealthKit non-negotiable per Dean): LAUNCHER model for employees — Sage tile = universal link into our NATIVE app (pre-provisioned, SSO'd, no signup; one-time install is the only cost) — NOT a member webview (webview = fallback tier only: no push inside their app, HealthKit physically impossible in any webview, manual logging only). EMBED for employers — employer dashboard is pure-web aggregate-only, embeds clean; native summary card via data-out API + "open full dashboard" webview. (d) THREE APIs: provisioning (guest list — endpoints specced, maps to employer_members flow), SSO (SAML/OIDC into Supabase Auth, JIT), data-out (report card — employer-portal EF payload packaged as versioned keyed endpoint). Supporting: webhooks-to-Sage receipts, GDPR erasure endpoint, SCIM 2.0 dialect option, status endpoint, optional timetable feed. (e) TWO BUILD GROUND RULES kept in doc as engineering constraints: embedded surfaces point at our LIVE ORIGIN (never bundled snapshots — protects ship-same-day), purchase flows NEVER inside the iOS app (Apple 30%/IAP). Build estimate: ~2–3 sessions once Sage fields a technical counterpart; DO NOT start before door question answered.

**Remaining from Alan's list:** 2 complaints loop (needs Dean's 10-min design chat: complaint entry points, resolution menu incl. non-refund options, auto-escalation criteria), 7 booked sessions (talk first — no booking concept exists; save/remind vs capacity booking), 8 freshness map (ready to run, no input needed), 5 = Lewis deck content (today's two docs feed it).

**Device checks outstanding:** featured slot (needs a partner flagged — one UPDATE), Comms tab demo+live, Explore carousel taste-check. Stars confirmed working.

---

## SESSION CLOSE — 13 Jul 2026 (PM-792 → PM-796): CC access unified + App Health repaired + the Paige finding (2026-07-13)

Dean opened on "different logins for different parts of the site", scoped to the CC: Dean/Lewis/Alan full access on their normal VYVE app credentials. That one complaint unwound into three distinct broken layers, all fixed and device-confirmed ("Both work").

**PM-792 (CC `cf140c3f`) — ACL server-derived owner role.** Root cause of the "different access everywhere" feel: lib/acl.js seeded its owners as FICTIONAL emails (`lewis@vyvehealth.co.uk`/`dean@vyvehealth.co.uk` — neither exists), so every real login auto-added as `member` in a PER-DEVICE localStorage list — Finance/Invoicing/Investor/Strategy/Org/Team/Settings hidden and financial KPIs redacted for everyone including Lewis, differently per browser. Fix: anyone with `admin_users.role='admin'` is now `owner` on every device (localStorage can never demote a server admin); seeds corrected to real logins; **Alan Bird added to admin_users as admin** (alanbird1@gmail.com — he had an auth account, just no allowlist row).

**PM-793 (CC `b5f6c24d` + cc-app-health EF v9 + cron 38 alter) — App Health repaired.** Dean: "App health page isn't doing anything." Two real EF bugs: (1) the `ai_interactions` query selected a `model` column that DOESN'T EXIST — 400 on every single run since ship, AI pipeline stats silently zero (real figure: 35 calls/7d); (2) a transient cron-RPC blip let Dean's manual Refresh overwrite 49 cached cron jobs with `[]` (the hourly run healed it 5 min later — the "No cron data" screenshot exactly). v9: dead column dropped, cron_json NEVER overwritten by an empty snapshot, per-source error logging + `errors[]` in the response, and a SECURITY fix — the EF was fully unauthenticated (anyone could trigger cache refreshes + PostHog spend); now admin-JWT or X-CC-Cron gated, cron 38 altered to send the secret. Live-verified: 200 `errors:[]` with secret / 401 without / cron_json 49 / AI stats real. Front-end: stale cron badges cleared on empty, visible error bar per §23.147, refresh surfaces EF warnings.

**PM-794 (same commit + cc-usage EF v15) — Usage push-notifications column.** Dean's ask: see who has notifications enabled. v15 adds `push_enabled`/`push_platforms` per member (non-revoked `push_subscriptions_native` tokens; the any-token-ever `installed` signal unchanged). New **Push** column in the Usage members table (🔔 platform / off; '—' on pre-v15 cache). Live: 30/83 members push-on; Dean = ios.

**PM-795 (same commit + migration `cc_acl_people_pm795`) — per-user settings server-backed.** Dean: "we do want per user settings." `cc_acl_people` table (email PK, role member/lead/owner CHECK, external flag; SELECT is_admin_or_team / write is_admin; touch trigger), seeded with Dean ×2, Lewis, Alan as owners. acl.js: hydrates from the server after login (cross-device), writes through on setRole/removePerson/first-login auto-add; localStorage is now only a fast-boot cache. PM-792's rule stands as the hard floor. Settings → People UI unchanged. Cache-bust follow-up `e593164f` (script version params — the payload commit changed files but not their versioned URLs; lib/acl.js had NO version param at all).

**PM-796 (CC `e1cd3394` + migration `cc_admin_read_policies_pm796`) — Active Users unbricked + dark-theme pills.** Dean hit "Access restricted" signed in as team@ (which HAS been role=admin since April). ROOT CAUSE, the big one: **admin_users had RLS ENABLED with ZERO policies — default-deny.** The shell gate works because auth.js uses SECURITY DEFINER RPCs; but supabase-client.js `getCurrentAdmin()` does a RAW client-side select on admin_users → returned nothing for EVERYONE — every page-level gate (active-users + the PM-779-era pages sharing it) denied all comers, admins included. Fix: self-read + is_admin_or_team read policies on admin_users. SECOND layer: the page's data queries were written against columns that NEVER EXISTED (`member_activity_log.member_id/created_at`, `weekly_scores.week_start`) — both 400'd since the page shipped; data layer rewritten to the real email-keyed schema, plus read-only `is_admin_or_team()` SELECT policies on members/member_activity_log/weekly_scores (same audience as the cc_usage cache). Verified under team@'s exact JWT-claims posture: gate 1 row, 83 members / 367 activity rows / 18 scores. THIRD: Dean's "highlight pills at the top are light" — `.topnav-tab.active` used raw `--vyve-teal-pale`, which has NO dark override (`--accent-pale` does) → light mint pill + light text, unreadable. Swapped to `--accent-pale`; same class fixed on `.qs-result.active` + `.view-tab.active`. The teal-text chips using the raw token were left (legible). shell.css link gained ?v=PM-796.

**"Why paid users shows 0?" — not a bug, tonight's news.** Paige Coult (the ONLY paying member, £20/mo since 13 Apr) had her renewal FAIL at 18:59 UTC tonight: `invoice.payment_failed` → past_due → webhook wrote it; the paid tile counts paid+active. Stripe pull via Composio: decline = `card_declined` / **`revocation_of_authorization`** on her Visa ···2042 (card valid to 12/2031, three prior clean renewals) — the recurring-payment authorisation was revoked AT HER BANK. Read: deliberate cancellation via banking app, not a dunning case; smart retries will keep bouncing. auth.js grants past_due grace (PM-598) so she has NOT lost access. Dean's own diagnosis: "our payments are done through stripe and it's pretty hard to cancel" — no self-serve manage/cancel path exists. **Dean decision: Stripe Customer Portal BACKLOGGED, not urgent** ("not too fussed about it being easy to cancel"). **Lewis action: personal no-pressure outreach to Paige; if she wants out, cancel cleanly in Stripe so retries/dunning emails stop.**

**DEAN DEVICE CHECK: DONE** — nav pill readable + Active Users renders ("Both work"). Remaining verification carried in backlog: cc_kv smoke (Competitors/Clients edit), Update 512 PostHog health-props device check.

## SESSION CLOSE — 13 Jul 2026 (PM-789 → PM-791): Employer portal round 2 — snapshots/deltas, Timetable, Impact, working Ask, real Challenges skeleton, live board pack (2026-07-13)

Dean: "there's a lot missing" from the live employer portal → "all". Scope call agreed mid-session: build the REAL skeleton for everything (no demo data leaks into live — compliance-honest line held pre-Sage); member-side challenge surfacing (B2) deferred since zero live employer users exist; **Book Experts stays HIDDEN in live (Dean did not overrule)** — a booking UI backed by nothing breaks the honest line; surface only when a real roster + booking mechanism exist.

**1. employer-portal EF v4 + migration `employer_metrics_weekly_snapshots`.** New `employer_metrics_weekly` table (per-team + `__company` rows, UNIQUE(employer,team,week_start), service-role RLS) + cron **`employer-metrics-weekly-snapshot`** (Monday 05:40 UTC → `?snapshot=1` ops action, x-vyve-cron-key). EF v4: core compute extracted into `computeEmployer()` (shared GET/snapshot); GET emits `teams[].delta` (engagement-rate points vs latest PRIOR-ISO-week snapshot) + `index.delta` — **null until a prior-week snapshot exists, no invented history**; `timetable` block (next-7-days public active non-cancelled calendar_occurrences); POST = Ask relay ({message, topic?} → send-email EF internal service-key call → team@, EP-XXXXXX ref, HTML-escaped, no member data — Brevo cl.3.2(vi) safe). CORS gains POST. LIVE-VERIFIED via pg_net: GET 200 (timetable 44 rows, deltas null-correct, index 59), snapshot 200 (`VYVE Demo Co ok:4` — company 22/4/18%/idx 59, Engineering 9/1/11%, Operations 9/2/22%, Other 4/1/25%, week 2026-07-13), Ask 200 ref EP-CADRFO (**marked test email landed in team@**). First auto-snapshot next Monday; deltas render from 20 Jul.

**2. Front-end PM-789 (Test-Site-Finalv3 `752639b1`, md5-verified).** Live mode gains **Timetable** tab (grouped-by-day session list) + **Your Impact** tab (months donated, progress-to-30 bar, all-time per-type bars, CIC giving paragraph); `mapLive` wires real team deltas (▲/▼ render already existed); Index hero gains "±N points on last week" when delta present; **Ask VYVE live = real form** (topic + textarea → EF POST, 401 refresh retry, ask-ok box with ref; `resetAsk()` was referenced-but-undefined in the original — now defined). `?demo=1` byte-identical in behaviour.

**3. EMPLOYER CHALLENGES B1 — EF v5 + migration `employer_challenges_foundation` + PM-790 (`0f2b2fb6`).** Tables: `employer_challenges` (FK challenge_library.slug, starts/ends/status, service-role write, **member SELECT policy via employer_members roster join — B2-ready**) + `employer_challenge_optins` (**member-RLS self insert/select/delete — the consent artefact**; opted-in members only ever appear as team aggregates). EF v5: `challenges` payload block — 8 curated templates (pillar-balanced easy/medium + VYVE 75 flagship, description_md stripped) + launched challenges with opt-in team aggregates (activities counted in the challenge window, <MIN_COHORT departments fold to Other); POST action router `launch_challenge` (Monday validation client-side, dupe-guard 409 already_running, duration from library, **notify email to team@ on launch** — comms promise) / `cancel_challenge` / ask (default). Front-end: Challenges tab UNHIDDEN in live, template grid from payload, launch modal (Monday picker → real POST), launched cards with honest empty state ("No members have opted in yet"). LIVE-VERIFIED: templates 8/launched 0 → launch first-steps 20→26 Jul 200 (row + email confirmed) → renders as scheduled. **Test row LEFT IN PLACE on VYVE Demo Co for Dean's device check** (real data, invisible to members until B2).

**4. PM-791 (`8dbd4fe9`) — BOARD PACK COMPLIANCE FIX + Frankfurt.** Found live-mode board pack printing DEMO numbers (hardcoded 72/100, SAMPLE_DATA tables, fictional insights prose). New `buildPrintReportLive()` renders from LIVE_PAYLOAD: real index/grade/sub-scores/weekly delta/suppression note, real headline + team tables (null Δ prints —), real EF insights, generic CIPD/Deloitte ROI framing. Demo print path unchanged. Also 2× "EU (Ireland)" → "EU (Frankfurt, Germany)" (board-pack privacy section + canned Ask privacy answer — factual hosting claims, PM-767 correction reaches this page). Server-side PDF generation DEPRIORITISED — print-to-PDF is now honest; revisit only if an employer asks.

**DEAN DEVICE CHECK (Pages CDN lag):** live sign-in → Timetable + Your Impact + Challenges tabs render; Challenges shows "First Steps · Scheduled · 20–26 Jul" + 8 real templates; Ask VYVE form sends (email lands at team@ with EP ref); board pack Download prints REAL numbers; `?demo=1` unchanged.

**NEXT (employer portal):** (a) **B2 — member-side challenge surfacing** (vyve-site challenges.html: "Your workplace challenge" card via the new member-readable RLS, join = enrolment + opt-in write + consent line; ~1 session; Lewis copy flag on consent line); (b) Lewis copy pass now ALSO covers Timetable/Impact/Challenges/Ask copy; (c) Lewis/Alan benchmark figures (unchanged gate); (d) per-employer custom sessions in Timetable (needs calendar_occurrences audience scoping beyond visibility=public); (e) Book Experts — parked by design.
...
**DEAN VERIFICATION PENDING (next session opens here):** (1) CC hard refresh → Calendar renders month grid with live sessions; (2) CRM/Finance still work post client-instance swap (if a sign-in gate appears, one magic link — sessions unified under 'vyve-cc-supabase-auth'); (3) Competitors/Clients edit → Claude confirms the row in cc_kv (proves the PM-777 loop); (4) device check is now against **Update 512** (not 511) — settings build marker, then test wellbeing check-in arrives in PostHog WITHOUT score/feeling/energy props.

**Decisions still owed by Dean:** PostHog historical health-property deletion request (recommend: support request citing the DPA), Demo Co synthetic roster rebuild, sub-processor register row 2 entity tweak (captures confirm UK contracting entity is **Anthropic Ireland, Limited**, register says Anthropic PBC — same accuracy class as the Frankfurt find; Dean not yet answered go).

**NEW BUILD SPEC — CC Calendar completion (Dean's vision, agreed 12 Jul):** (a) session management on calendar_occurrences — edit title/description/host photo/cover image (storage upload, first-party pattern), pull/cancel a session with a MANDATORY pre-check that the Hetzner runner respects cancelled_at before shipping a cancel button; edits propagate to member portal by design; mockup-first per standing rule; (b) meeting invites — "Send invites" on cc_calendar_events via send-email EF with ICS attachment (works with any pasted link); real Google Meet link GENERATION blocked on Google OAuth app Testing mode (rides the existing YouTube-token backlog line). Estimated one Claude session each.

## PM-779 — ONE Supabase wrapper: lib/supabase.js stub clobber fixed (2026-07-12)

**Root cause of the blank Calendar page (and more):** the CC shell loaded TWO wrappers — `lib/supabase-client.js` (real: isReady/onReady/getSession/getCurrentAdmin, client as GETTER, storageKey vyve-cc-supabase-auth) then `lib/supabase.js` (localStorage-era stub: client() as FUNCTION, isStub/url, VYVE_CONFIG-dependent) — and the stub loaded LAST, clobbering `window.VYVE_SUPABASE`. Every new-API page (calendar, tasks, documents, sessions, active-users, settings) died against the stub shape; PM-777's cc-sync sat inert waiting on an onReady that didn't exist. PM-778's error card surfaced it live from Dean's phone: "S.isReady is not a function".

**Fix (CC `9bda443a`, 11 files, all md5-verified):** stub script load SOFT-KILLED from index.html (file kept, Dean policy); back-compat surface (getClient()/isStub()/url()) added to the real wrapper; the 9 Phase 3–6 pages written against the stub (`VYVE_SUPABASE.client()` call style — broadcast/content/crm/employers/finance/investor/invoicing/podcast/home) mechanically patched to the client getter. Single client instance, single auth session key. Members may need one magic-link re-login if their live session sat under the stub's default storage key.

## PM-778 — Calendar page: boot failures surface on-page (2026-07-12)

Calendar rendered hero-only blank; page build is COMPLETE (month/week/agenda, cc_calendar_events + calendar_occurrences feed, 247 occurrences in window, table verified live) but boot died silent. CC `4d49cba3`: boot().catch renders a visible error card; the designed-in silent-exit path (`if (!window.VYVE_SUPABASE) return`) also surfaces. Card fired on Dean's phone within minutes and named the PM-779 root cause. Pattern is now a §23 rule: page boots never die silent.

## PM-777 — cc_kv strangler: ALL localStorage-era CC pages server-backed (2026-07-12)

Dean's audit question ("are these placeholders?") → source audit → the 16 legacy pages (brief/inbox/dashboard/strategy/intel/knowledge/competitors/clients/compliance/delivery/marketing/org/team/action-plans/social-blueprint/trash) read/write device-local localStorage via VYVE_DATA/VYVE_STORE. Dean call: "anything that uses local storage needs fixed so it works properly like a proper command centre."

**Shipped (migration `cc_kv_sync_layer` + CC `f8759820`/`04ff7fe7`):** `cc_kv` table (key/value jsonb/updated_at/updated_by, is_admin_or_team() RLS, touch trigger) + `lib/cc-sync.js`: (1) hydrate after authed session — server rows into legacy localStorage keys, then router re-render; (2) write-through — patched localStorage.setItem, vyve_*/vyve.store.* allowlist (auth keys excluded), debounced whole-collection upsert; (3) focus re-pull (15s throttle) for cross-device; (4) fill-gaps migration seed — local collections with NO server row auto-push, never overwrites (whichever machine holds the data populates it); (5) read-only MIRRORS of graduated tables (cc_tasks/cc_leads/cc_finance/cc_posts/cc_episodes/calendar_occurrences) into the legacy VYVE_DATA keys with field mapping, so Brief/Dashboard/Inbox KPIs read live truth — mirror keys excluded from write-through. Known limits (accepted, 2-person team): whole-collection last-write-wins; audit trail client-generated. First-class promotions (Strategy, Intel first) backlogged.

## PM-776 — CC desktop nav + home tiles fixed (2026-07-12)

Dean: "nav is overlapping page and pills on home are really bad." Two real bugs, one commit (`b1783492`): (1) PM-753 made the topnav PRIMARY and the sidebar a drawer, but only the mobile half of the breakpoint was written — desktop shipped drawer stuck open (base translateX(0)) over un-offset content, hamburger hidden >900px, .topnav-tabs still display:none from the PM-644 era. Now: tabs visible desktop (flex, scrollable), hamburger ALL widths, drawer closed by default with overlay+shadow. (2) Home tiles were `<a class="hm-tile">` WRAPPING `<a class="hm-tl">` chips — nested anchors are invalid HTML; the parser split each tile and ejected .hm-tlinks as its OWN stretched grid item (Dean's "sliver pills"; verified via html5lib: 12 grid children instead of 6). Tiles now div > [hm-tile-main anchor, hm-tlinks]; parser-verified 6 children.

## PM-775 — Alert auto-ageing cron (2026-07-12)

Migration `alert_autoresolve_daily`: `autoresolve_stale_diagnostics()` (SECURITY DEFINER, REVOKE PUBLIC/anon/authenticated, GRANT service_role) resolves diagnostic-class alerts >14 days (js_error/promise_rejection/client_diagnostics/page_load_timeout/pwa_not_installed/unknown/schema_snapshot_*/ota_verified + auth_401_%/api_500_%/network_error_%/skeleton_timeout_%). Crisis/GDPR/business types deliberately never matched. Cron jobid 55 daily 03:45 UTC. Live-run 0 (correct post-sweep). **Rule: new diagnostic alert types must be added to this function or they accumulate forever.**

## PM-772→774 — Anthropic captures + alert flood triage (2026-07-12)

**PM-772 (VYVEBrain `aaa97b7c`):** Dean's browser print-to-PDFs of the two canonical Anthropic legal pages (Commercial ToS + DPA, dated 12/07/2026) filed to reports/compliance/ + cc-documents via compliance-doc-loader (200, SHA-256 exact both: 909c7d74…/90d0cdb8…). PM-771's optional upgrade CLOSED — Anthropic position note now backed by verbatim evidence. Compliance category = 8 documents. **Entity find while reading the captures:** ToS defines the EEA/Switzerland/UK contracting entity as Anthropic Ireland, Limited — register row 2 says Anthropic PBC (patch pending Dean).

**Alert triage:** CC showed 2797 open / 71 critical / 214 high — NOT an incident: 3 months of unresolved telemetry (client_diagnostics 1839, promise_rejection 254, js_error 166, stale May network/auth/api rows). Recent "highs" were Googlebot/Bytespider failing SW+Dexie on signed-out loads (member_email null). Bulk-resolved in three passes → 22 open, 0 critical, 0 high. **PM-773 (vyve-site `46f172fd`, vbb 511→512 + sw lock-step):** auth.js vyveMonitor session-gates js_error/promise_rejection (localStorage vyve_auth OR vyveCurrentUser — covers members pre-auth-init, excludes crawlers); API-failure alerts stay unconditional (broken auth path must alert). **PM-774 (CC `c6458119`):** Needs Attention unwraps the `{"raw":{…}}` JSON envelope to the human message.
...
**THE RULE THAT MATTERS — Brevo DPA cl. 3.2(vi) + Schedule 3.B:** the Customer warrants NOT to include any special-category personal data in Customer Data, and the transfer description states "No sensitive data is transferred." **Standing VYVE rule: no email sent through Brevo (send-email EF or any future path) may ever contain wellbeing scores, check-in content, or any health information.** Session names, reminders, certificates, credentials: fine. Current templates audited clean by inspection (credentials/certificates/re-engagement nudges carry no health content) — but every future email template gets checked against this. Also useful for Sage diligence: Brevo's own sub-processor hosting is EU (GCP Belgium, Scaleway/OVH France, Hetzner Germany).

**Capawesome notables banked:** 48-hour breach notification (tighter than GDPR's 72h to authority); customer data never used for AI/ML training; SOC 2 Type II; US transfers via DPF + SCCs Module 3; Apple/Google are independent controllers (API pass-through to our own dev accounts), which matches how the privacy policy now describes push (PM-766).

**DPA pile state: Supabase DONE (PM-769) · Brevo DONE · Capawesome DONE · remaining: PostHog (the only one needing an actual signature — generate at app.posthog.com/legal signed in as our EU org, countersigned automatically) + Anthropic (save https://www.anthropic.com/legal/data-processing-addendum and /legal/commercial-terms as PDFs, send them over for filing).**
...
**The stronger cut instead of a value sweep:** the page's local dark/light token blocks are DELETED, not re-valued — assets/tokens.css v2 (already loaded first) is now the single source of truth. All 25 `var()` names the page uses were verified defined in tokens.css before the cut, so there is no fallback gap, and this page can never value-drift from the system again. The pages that got name-preserving value sweeps (PM-753/760/762) own their tokens locally; this one didn't need to.

**Playfair drift correction (PM-760 claim was wrong):** partner-portal.html still carried the Playfair+Inter Google Fonts link — so "Playfair ZERO across the entire CC repo" was false at PM-760. Worse, the link was a *dead download with a real cost*: `--font-head/--font-body` already resolved to 'DM Sans' via tokens.css, but DM Sans was never loaded on this page, so partners have been seeing system-font fallback while downloading two fonts the page never used. Link swapped to DM Sans + DM Mono — partners now actually get the brand type. Playfair is now genuinely zero in the CC repo (repo-wide grep clean).

**Theme boot consolidated:** the page had TWO boots — a head boot reading `vyve-cc-theme` (no 'system' handling) then a second boot unconditionally overriding from `vyve-partner-theme || 'dark'`. Now ONE authoritative head boot: `vyve-partner-theme` first (external partners keep their own key — they never see the shell, so PM-760's key unification deliberately does NOT apply here), `vyve-cc-theme` honoured as fallback for admins previewing, default dark preserving the page's historic default. Redundant second boot removed; toggle still writes `vyve-partner-theme`. No flash-of-wrong-theme window: the boot runs in head before first paint.

Also: theme-color meta → #081616; last stray brand hex tokenised; all script blocks node-syntax-checked.

**DEAN DEVICE CHECK:** www-side partner-portal.html as a partner login — DM Sans renders (most visible change), both themes via the portal's own toggle, login gate + content/publish/notify flows behave identically. LOW-KEY ROLLOUT: visual-only change for external users; nothing to announce to partners.

**CC OVERHAUL CAMPAIGN (PM-752→763): all seven phases BUILD-COMPLETE.** Design system v2 everywhere (one system, dual-theme, DM Sans, Playfair truly zero), 5-domain IA, Run-the-Business fully live-data on cc_* (dead-RLS class fixed), soft-kill retirement policy codified, admin-console re-skinned, Employers domain live (provisioning UI + benchmarks editor), Partners re-skinned with both mocks removed, partner-portal on the token source. **Remaining before the campaign is CLOSED-closed: Dean's cumulative device walk (PMs 752–763) and the standing Lewis gates.** Strangler extraction of admin-console views remains refactor-value backlog, not campaign scope.
...
**cc-home EF v1** (verify_jwt:false, internal auth — standard pattern): admin/team JWT → /auth/v1/user → admin_users active lookup, plus ops path via x-vyve-cron-key (gdpr_cron_key vault accessor) or Bearer SERVICE_KEY. Single GET returns: members total (is_test-excluded) + active-7d (distinct member_activity_log 7d), revenue passthrough from cc_revenue_cache.headline_json, today's sessions (calendar_occurrences, Europe/London day bounds via Intl longOffset — PM-748 pattern; state aired/live/upcoming derived from starts_at/ends_at), open platform_alerts (latest 5 + severity counts), live partner count, open cc_tasks, employer_admins count. All reads parallel. **LIVE-VERIFIED via pg_net: 200 with real payload (72 members, 6 active-7d, MRR £20, 6 sessions today) + 401 on bad key.** Live-truth note: members now 72 (35 trial / 29 expired / 7 comp / 1 paid) — memory-cached counts are stale as always.

**pages/home.html** (vyve-command-centre `d70be650` + greeting fix `2fe9091e`, both md5-verified): shell partial on tokens v2 — time-aware greeting (name from Supabase session email), 4 KPI tiles (MRR gold, active-7d, sessions today with live-now highlight, open alerts with critical/high split), Today's sessions card (London times, aired/live/upcoming pills), Needs-attention card (unresolved platform_alerts), 6 domain tiles (RtB/Members/Partners/Employers-gold/Analytics/System) with LIVE badges (open tasks, member count, live partners, employer accounts, alert count turning the Analytics badge red) and quick-links. Honest error split per PM-751 (network vs 401 vs render errors distinct; render errors console.error'd). render() verified against the real EF payload via a DOM-stub harness pre-commit.

**Router + nav:** DEFAULT 'brief'→'home', crumb/fallback/error-page refs swept; Home nav item added top; Morning Brief delinked (brief.html kept in repo, soft-kill pattern — restore is one nav line).

**DEAN DEVICE CHECK:** admin.vyvehealth.co.uk/?z=755 — should land on Home with live numbers; verify greeting name, KPIs match reality, today's sessions list, tile badges; flip themes.

**NEXT (Phase 3 continues):** CRM/pipeline rebuild on cc_leads/cc_clients (first write-side page — RLS/EF decisions), then Finance+Invoicing. Lib-layer retirement still parked until the rebuilds land.
...
**Audit method:** full repo tarball pull + per-page data-source trace (grep signals: supabase/EF refs, seed-data, localStorage, VYVE_MAKE, lib layer) + live Supabase cross-check of every `cc_*` table.

**Findings:** the CC is three products in one repo across **four design systems** — (1) shell SPA (light cream tokens.css) hash-routing ~40 `pages/*` partials, (2) partners.html 108KB monolith (GitHub-dark + off-brand mint #5ec4b0), (3) partner-portal.html 76KB (VYVE dark-teal DUAL light/dark — external-facing), (4) admin-console.html 131KB (VYVE dark-teal dual-theme, **still on Playfair** — PM-739 retirement never swept the CC). Data classification: 13 surfaces genuinely live (8 Analytics on fresh cron-fed cc_* caches, broadcast, active-users, calendar, + the 3 monoliths); **~20 pages are the Lewis localStorage-SPA era** — data frozen at the 13 May 2026 Make.com snapshot (seed-data.js 166KB) or trapped in one browser's localStorage, write path tries retired VYVE_MAKE first; 5 pages dead/unreachable. **All 17 business `cc_*` tables exist and are EMPTY — including cc_tasks/cc_calendar_events/cc_documents: the PM-640–643 team app has zero rows, zero adoption.** cc-adapter.js written, never enabled. Rot: index.html loads TWO supabase-js versions (2.39.3 + 2.45.0); lib/auth.js fail-OPEN fallback ("any signed-in user" if is_admin missing) must die.

**Decisions locked (Dean):** (1) ONE design system, VYVE brand colours, **dual light/dark on every surface**, built from the partner-portal/admin-console dark-teal lineage — partners.html mint retired, Playfair swept; (2) **five-domain IA**: Run the Business / Members / Partners / Employers / Analytics (Employers new — PM-750 provisioning UI lands there); (3) Lewis SPA layer → curated rebuild on the empty cc_* tables (Brief as live rollup, CRM, Finance+Invoicing, Investors+Grants, Content, Podcast, Tasks, Documents, Calendar), rest KILLED — **kill list pending Lewis confirm, nothing deleted before that**; lib localStorage machinery (seed-data/store/entities/make/data/drafts/etc.) retires at migration end.

**Phase plan (each phase own session+PM):** 1 design system + shell mockup (Dean gate, then Lewis look) → 2 shell ship + Analytics re-skin (auth fail-closed, single supabase-js) → 3 Run the Business rebuild + kill-list execution → 4 Members (admin-console strangler, Playfair sweep) → 5 Partners (re-skin + kill the two known mocks: attendances-by-week, engagement scorer) → 6 Employers (provisioning UI off employer-provision EF, benchmark-figures editor) → 7 partner-portal LAST (external users, works today). Full disposition tables + risks: `playbooks/cc-overhaul.md`.

**NEXT:** Phase 1 mockup — shell + component kit in both themes.

## PM-751 — Employer portal live-mode device fixes: the "network error" that wasn't (2026-07-12, evening close)

Dean device-tested PM-750 and hit "Could not reach VYVE — check your connection" with the sign-in button stuck on "Signing in…". Diagnosis chain worth recording: the error string identified WHICH catch fired (loadLive's, not epSignIn's) → sign-in had succeeded; Composio curl repro proved preflight + CORS were clean; **edge-function logs proved Dean's actual GET returned 200 in 1.3s** — so the failure was client-side AFTER a successful response, masked by an over-wide catch. jsdom repro against the committed page + the real payload pinned it in one shot.

**Root cause:** `renderDeepDive` runs inside `renderAll` during live boot. `ddTeam` initialises to the demo team 'Finance'; live-mutated TEAMS (Engineering/Operations/Other) doesn't contain it → `TEAMS.find(...)` undefined → TypeError reading `.d` → swallowed by loadLive's catch and **mislabelled as a network error**. Second, independent bug: `epSignIn` never resets the button on the success path, so when `showLogin` re-opened the overlay it was stuck disabled.

**Fixes (Test-Site-Finalv3 `6d7e6508`, md5-verified at commit SHA):** (1) `renderDeepDive` demo-only guard (`if (LIVE) return;`) — live deep dive is benchmarks via applyLiveDeepDive, drilldown hidden by design; (2) `showLogin` resets the sign-in button state; (3) **loadLive catch split — network errors and display errors now get distinct honest messages, display errors console.error'd** so this class of bug can't hide behind "check your connection" again. Full jsdom boot-path verification clean: stored session → live dashboard, no window errors, index animates 63, KPIs 22/18%/610/83, 3 team rows, 5 insight cards, hero "Building", suppressed wellbeing renders as gaps + "Awaiting check-ins", fictional tabs hidden, 2 live benchmark rows.

**DEAN DEVICE CHECK: DONE — logged in, live data confirmed on device.** Noted honestly: the live view looks sparse because the fixture IS sparse (22 test members, 6 check-ins → suppression everywhere, near-empty sessions card). That is the privacy/honesty model working, not a defect — but it means **the UNSUPPRESSED rendering path (real wellbeing chart, full index composition) has never been visually verified.** Optional backlog item added: seed the VYVE Demo Co fixture with 12 weeks of check-ins across 10+ test members to exercise it before a real client does.

**Technique note for future sessions:** jsdom (npm install jsdom, local not -g) + the committed HTML + a real EF payload reproduces live-mode client bugs in one run — stub fetch/localStorage in beforeParse, call the entry function unwrapped, read the stack. Faster and more conclusive than device round-trips.

## PM-750 — EMPLOYER PORTAL END-TO-END LIVE — provisioning EF + insights (EF v2/v3) + employer-portal.html live mode; Alan/Lewis doc gap analysis drove the scope (2026-07-12, afternoon session)

Fable-last-day session. Dean's ask: extract what the Lewis/Alan Sage-prep documents commit us to building, then build it. All four unread docs extracted (employer deck, employer prospectus, Strategy V1–V5, partner impact report). **The Strategy paper's V2 stage ("convert, retain, make enterprise turnkey") explicitly commits "employer dashboards productionised for the first live client" — that item is now DONE end-to-end.**

**Doc-diff findings (full list):** V2 items — billing loop ✅, guided tour ✅ v1, employer dashboards ✅ THIS SESSION, "see your impact" giving feature ❌ unbuilt (backlogged), clinical sign-off HAVEN ❌ Phil gate — **the prospectus now states "sensitive content held behind clinical sign-off" in writing; only true if Phil signs or HAVEN is pulled pre-Sage-diligence.** Deck promises delivered this session: dashboard "with recommended actions" (insights block), team-level view (activity-only). Still unbuilt from the deck: employer team challenges, per-employer bespoke timetable. Partner impact report = a fifth gap (per-partner metrics engine, none computed) — partner-side, deferred. Data-rights promises check out (gdpr-export/erase-request/cancel/status EFs all live).

**1. `employer-provision` EF v1** — Gate-A provisioning for employer logins, partner-provision v2 pattern with two deliberate fixes: **(a) NEVER resets an existing auth user's password** (partner version resets unconditionally — would break a contact who's also a member); existing users are told to use their existing password / Forgot Password. **(b) deprovision deletes the employer_admins row and only bans auth for non-members** (checks members + admin_users first; partner version bans blind). Actions provision/deprovision; admin-JWT OR ops (cron-key/service) gate; audits with the REAL admin_audit_log shape (PM-748 gotcha); credentials email via send-email pointing at employer-portal.html. Ops-only `return_session:true` performs a server-side password grant and returns the new user's session — a verification/smoke-test hook that grants nothing the ops caller lacks. **FULL AUTH MATRIX LIVE-VERIFIED:** provision 200 (synthetic contact, VYVE Demo Co) → **that JWT → employer-portal 200 scoped to VYVE Demo Co (JWT happy path CLOSED)** → deprovision 200 (auth_disabled:true, correct for non-member) → same JWT → **403 not_employer**. Test artefacts fully cleaned.

**2. `employer-portal` EF v2 + v3 (platform v2/v3).** v2: **`insights[]`** — deterministic rule-based recommended actions (kind win/watch/action; engagement strength, momentum ±, quiet-member share, wellbeing suppression + check-in-adoption nudge, low-wellbeing watch, pillar-underuse actions, laggard-team focus; sorted, capped 6; **no AI call — no spend, no variability in front of an employer**). Live-verified: fixture produced momentum win (+48%), quiet-members watch (73%), suppression watch + 2 actions. v3: payload extended for 1:1 page mapping — session_views/replay_views pulls carry category+minutes_watched → `sessions` block (84d, byCategory + total views/mins, legacy SKIP_CATS honoured); `activity.byTypeAllTime` (reuses the charity count queries + a wellbeing_checkins count), `activity.thisMonth` (calendar month), `activity.allTimeTotal`. Live-verified (byType populated, thisMonth 83, allTime 610, 3 categories).

**3. `employer-portal.html` LIVE MODE** (Test-Site-Finalv3 **`e1cbfe9b`**, md5-verified at commit SHA; node --check clean on the script block). Boot forks: **`?demo=1` preserves the PM-740 sales demo byte-identical in behaviour** (rename, projection, all 7 tabs); otherwise a styled sign-in overlay (Supabase Auth password grant via the modern **publishable key** `sb_publishable_LvM…` — first use of it client-side; legacy anon deliberately avoided per PM-731) with silent refresh-token retry on 401, clear 403 copy ("no employer dashboard access"), and sign-out in the nav. Live mode: employer name + "Live data" pill from the payload (rename disabled — scope is server-derived); **index hero rebuilt from real components** (grade banding Excellent/Good/Building/Early days, sub-scores as %-of-weight, suppression sentence when wellbeing hidden, **the invented "typical UK employer: 54" benchmark line removed**); insights cards render the EF's win/watch/action list (fictional story strip hidden); KPIs/activity bars/trend/sessions table/status buckets/charity all from mapLive() (legacy-shape adapter — renderOverview untouched); teams table from payload (Δ column renders — until a history mechanism exists; status derived from engagement bands); wellbeing series null-safe (suppressed weeks render as gaps, tag falls back to "Awaiting check-ins"); Deep Dive shows **real benchmarks only** (your company vs live VYVE community; curated externals render only from employer_benchmark_figures rows with source attribution — fictional UK/EU/Global bars gone from live) and the fictional per-team drilldown is hidden; **fictional tabs (Book Experts / Challenges / Events) + projection mode hidden in live** — Reports (prints live DOM) and Ask VYVE (canned + team@) remain. Honesty relabels: engagement sub "Active in the last 7 days", session KPIs "(12 wks)", benchmark labels live-swapped.

**DEAN DEVICE CHECK (only open verification):** www.vyvehealth.co.uk/employer-portal.html → sign in with deanonbrown@hotmail.com member credentials (employer_admins row for VYVE Demo Co is live) → live dashboard. Pages CDN lag applies. `?demo=1` for the sales demo.

**NEXT (employer portal):** CC admin UI for employer provisioning (EF exists — needs a partners.html-style surface + Sage row when real emails land); Lewis copy pass on login screen + live hero/insight copy (shipped un-Lewis'd — compliance-honest but his voice pass will land); Lewis/Alan source curated benchmark figures (engagement_rate / activities_per_member metrics, per scope, with source); teams Δ needs a snapshot mechanism (employer_metrics_weekly table) before deltas can be real; then PM-740 spec remainder: employer challenges, board pack v2 (server-side), Ask VYVE form→send-email, per-employer timetable. **Sonnet-safe next builds:** "see your impact" giving feature (V2 doc commitment), partner impact metrics engine.

**Namespace:** PM-750 claimed (this morning's session closed at PM-749; no parallel sessions).

## PM-749 — GDPR member-bucket erasure sweep (challenge-photos blocker CLOSED + 2 undocumented public buckets) · Employer Portal backend v1 LIVE (employer-portal EF + Wellbeing Index + min-cohort model) · Alan partner/Sage doc gap analysis (2026-07-12)

Session opened on Alan's document drop (Sage partnership proposal, partner welcome pack, launch playbook, impact report, employer deck/prospectus). Read against the brain: the docs are a commercial promise set and the gap between them and the platform is the build list. Three items executed.

**1. gdpr-erase-execute v6 (platform v18) — member-keyed storage sweep, PM-718 HARD BLOCKER CLOSED.** The bucket audit found the blocker was one of THREE: member-email-keyed buckets outside the erasure pipeline were `challenge-photos` (private — the known PM-718 item, progress photos at `<email>/<challenge>/<date>.jpg`), **`certificates` (PUBLIC bucket, member-named HTML certs at `<email>/…`) and `member-avatars` (PUBLIC bucket, the member's own photo)** — the latter two undocumented as erasure gaps until tonight. Fixed as a class: new `purgeMemberBuckets()` sweeps all three by email prefix (recursive two-level list, same shape as the v5 partner sweep) as step 2e in processRequest, best-effort + HIGH `platform_alerts` per bucket (`gdpr_erase_member_bucket_failed`) — same posture as 2c/2d because the DB purge is non-re-runnable. `member_buckets` added to the execution summary. Header rule: **any future bucket keyed on member email MUST be added to `MEMBER_BUCKETS`.** Verified: content check at v18 (placeholder-trap), live pg_net tick 200 `picked:0` (correct, no due erasures); email-prefix list mechanics already production-proven by the v4 gdpr-exports sweep.

**2. PF-NEXT-7 already resolved — brain drift.** `partner_partners.revenue_share_pct` column DEFAULT is 50 and all 12 rows are 50. The 30→50 flip happened somewhere without the backlog ticking; live DB wins, item closed. Relevant because Alan's welcome pack commits 50% in writing.

**3. EMPLOYER PORTAL BACKEND v1 LIVE — the PM-740 demo now has a real data spine.** Migration `employer_portal_foundation_pm749`: `employer_admins` (email PK — deliberately NOT `member_email`, business-contact data outside the GDPR member catalog by design; RLS on, service-role only) + `is_employer()` SECURITY DEFINER (grants: authenticated + service_role, matching `is_partner()`) + `employer_benchmark_figures` (curated UK/EU/Global published figures, authenticated-read, **ships EMPTY by design — nothing invented renders to an employer; Lewis/Alan source real figures**) + index on `employer_members(employer_name)`.

New **`employer-portal` EF v1** (verify_jwt:false, both paths inside): (a) employer JWT → `/auth/v1/user` → `employer_admins` lookup → **scope server-derived, never client-supplied** (the legacy employer-dashboard EF's company query-param weakness is designed out; legacy EF untouched, still serves vyve-dashboard-live.html); (b) ops path x-vyve-cron-key (GDPR_CRON_KEY) or Bearer SERVICE_KEY with `?employer=`. **Privacy model hard-coded (PM-738 discipline): wellbeing company-level ONLY, MIN_COHORT=8** — wellbeing avg + per-week series suppress below 8 distinct checked-in members and the index renormalises; teams expose activity/engagement only, departments <8 fold into "Other". **Wellbeing Index 0–100:** engagement 30 (active-7d%), wellbeing 30 (avg score_wellbeing 28d, scaled 1–10, suppressible), balance 20 (Shannon entropy of Body/Mind/Connect 30d shares), momentum 20 (last-30d/prior-30d, 0.5→0pts 1.5→20pts). Payload: index+components+weights+suppression note, member status buckets, teams table, 12-week wellbeing series, pillar/trend activity, all-time charity (count-only queries), benchmarks (vyveCommunity computed live over non-test members + curated). Pillar map: habits engagement-only; body=workouts/cardio/movement; mind=mind_activities; connect=session/replay/connect_checkins. Roster in-filters chunk at 80 (fine to ~thousands of seats; >5k wants a SQL aggregate pass).

Dev fixture seeded: **'VYVE Demo Co'** — 22 test/comp/trial members into `employer_members` (Engineering 9 / Operations 9 / Finance 4 — Finance exercises the Other fold), `employer_admins` row for deanonbrown@hotmail.com. **Live-verified three ways:** no auth → 401; ops without param → 400 `employer_param_required`; full call → 200 with index 63, `wellbeingSuppressed:true` + renormalisation arithmetic confirmed (43.9/70→63), Finance folded into Other, per-week series suppression firing, curated `[]`. JWT happy path + 403 `not_employer` await Dean's device at front-end wiring (getAuthUser pattern itself production-proven across many EFs).

**Alan doc gap analysis (new backlog items):** welcome pack prints **partners@vyvehealth.co.uk** (mailbox existence unconfirmed — Lewis); playbook promises **1-2-1 booking with no fee** (no booking feature exists); welcome pack promises **monthly statements in Partner Studio** (earnings read-only today); playbook metric set (weekly-active %, 30-day retention, avg rating per partner) not computed in the partner portal. Prioritisation = Lewis/Alan call.

**Namespace:** PM-749 claimed clean (max 748 across all repos, no parallel sessions since 8 Jul — only auto crons on VYVEBrain 12 Jul). Note: memory's "vyve-capacitor not a git repo" is stale — PM-707 committed ship-state to remote.

**NEXT (employer portal):** wire employer-portal.html (Test-Site-Finalv3) to the EF with Supabase Auth login — Lewis copy pass rides along; employer Gate-A provisioning flow (create auth user + employer_admins row — partner-provision pattern); Lewis/Alan source curated benchmark figures; then board pack, Ask VYVE v1 (form→send-email), employer challenge launch per the PM-740 spec list. Sage post-meeting feedback (9 Jul) still wanted as a scope input — none of tonight's build depends on it.

## PM-748 — Partner funnel Session 4 (completion plumbing) CLOSED — launch materialization, welcomePost seeding, playbook reconciled, vestige sweep; audit-log shape bug found + fixed (2026-07-09)

Final session of the four-session partner-funnel audit. All four sessions now closed.

**Launch materialization at go-live (the architectural piece).** Application launch details must materialize at APPROVAL, not submit — an unapproved partner's session can't sit on the public calendar. New **`partner-launch-materialize` EF** (platform v3): admin-JWT gated (admin_users role=admin) with the standard SERVICE_KEY / `x-vyve-cron-key` ops path; requires `status='live'` (409 `not_live` otherwise; service callers may `skip_status_check` for ops). Reads `credentials.launch` → one `calendar_occurrences` row (`type=live_session`, `visibility=public`, partner_id + host fields from the partner row, `starts_at` parsed as **Europe/London wall clock** via Intl longOffset — live-verified: 19:00 BST → 18:00Z; `ends_at` +durationMin, default 60). Category mapped from pillar (body→Workouts, mind→Mindfulness & Mindset, connect→Education & Experts) and the mapping is surfaced in the response/toast for admin recategorisation. **Past-launch-date guard:** an application that sat in review past its proposed date is SKIPPED with `launch_date_past` — never silently publish a dead event. `credentials.welcomePost` → one **pinned** `partner_community_posts` row (author_kind partner). Idempotent via `_system.launch_materialized` (second call → `already:true`, live-verified). Console (`vyve-command-centre/partners.html` `d70786fa`, md5-verified): `advancePartner('live')` calls the EF after the flip — exactly the Gate-A pattern partner-provision uses at 'onboarding' — toast surfaces calendar/post outcomes incl. the past-date warning. Full live verification with a synthetic draft (created through the real public-EF path), all artifacts deleted after.

**AUDIT-LOG SHAPE BUG (found during verification, now §23-grade).** `admin_audit_log`'s columns are `admin_email / admin_role / member_email / action / table_name / old_value / new_value / reason` — **NOT** `target_type / target_id / performed_by / details`. `partner-provision` v1 used the latter shape, its sbFetch never checked the insert response, so **every provision/deprovision audit write failed silently from ship until tonight**. Fixed: partner-provision v2 (platform v3) + the materialize EF both write the real shape and check the insert error. Materialize's audit row live-verified landing. Gate-A provisioning between ship and tonight has NO audit trail — accepted, nothing reconstructable.

**Vestige sweep.** `partner-onboarding` **v10**: `dbs` key dropped from submit's documents write (the wizard never collected a DBS upload; always null). And the real find — **the wizard's submit payload omitted `qualifications: fileRef('qualDoc')`**: buildRecord (local summary) included it but the `efCall('submit')` documents object didn't, so uploaded certificates landed in partner-docs storage without ever being referenced in the application record. Fixed, Test-Site-Finalv3 `494e6476` (md5-verified). No existing submitted application is affected (none carried a qualifications upload).

**Playbook reconciled** (`playbooks/partner-onboarding.md`, this commit): documents the two entry paths (referral/playbook vs self-serve wizard + pipeline stages/gates), the S2 go-live INSERT gate with the exact audited `SET LOCAL vyve.partner_golive_override='actor|reason'` pattern + honest-backfill shape for any playbook partner created straight at live, the PM-748 materialization step at Gate B, the S3 abuse/GDPR posture, and the audit-log shape gotcha.

**PM-747 DUPLICATION (correction of record, PM-742 precedent):** a parallel session stamped PM-747 on vyve-site tonight ("Mental Fitness boot fix, Dean device walk") while this stream's S3 also shipped as PM-747 (Test-Site-Finalv3 `2f5275ec` + VYVEBrain `b3c4b4ea`). Neither renumbered — disambiguate by repo + commit SHA. Session 4 recomputed to PM-748 (`d70786fa` console, `494e6476` wizard, this brain commit).

**Partner-funnel audit: ALL FOUR SESSIONS CLOSED.** Remaining partner items live in the backlog: Lewis CTA gate (S1), Gate-A emails for Men Together/Alex/Nicola pending Dean, declined-draft purge policy, Lewis privacy-notice pass (non-gating), post-trial modules un-deferral.

## PM-747 — Partner funnel Session 3 (abuse hardening) CLOSED — rate limits, purge cron, GDPR wiring, privacy notice; email-verification gate built then shelved (2026-07-09)

Session 3 of the approved 4-session partner-funnel audit. Backend fully live and live-verified; wizard half deliberately shrank mid-session on Dean's call.

**Rate limiting (closes the v6 known gap).** Migration `partner_funnel_s3_rate_limits_and_draft_erase`: `ef_rate_limits` table (RLS on, no policies — service-role only) + atomic fixed-window `check_rate_limit(p_key,p_max,p_window_seconds)`. `partner-onboarding` EF now enforces: all actions 120/hr/IP (first hop of `x-forwarded-for`), new-draft `start` 5/hr/IP, `assessment` 15/hr/draft, `upload-url` 30/hr/draft, dormant verify paths 5+2/hr/draft. Over-limit → 429 `{code:"rate_limited"}` with human-readable messages. **Fail-OPEN on RPC error** — abuse tooling must never take the funnel down. Live-verified from a real origin: 6th new draft in an hour → 429.

**Email verification: built (v8), shelved (v9) — Dean's call, and the right one.** v8 shipped a full mailbox-proof gate: 6-digit code riding the resume email in server-owned `credentials._system.verify_code`, `verify-email` action ({code} verifies, {resend:true} re-sends), `upload-url` → 403 `email_unverified` until verified, and `_system` STRIPPED from resume responses (returning it would leak the code and defeat the gate — only an `email_verified` boolean is surfaced, and that stripping stays live in v9). All live-verified (403 → wrong code rejected → SQL-planted code → verified → upload 200). Dean challenged it as friction; with rate limits + private buckets + the 30-day purge live, residual abuse value is ~nil against a code-entry wall at the ID-upload step with email deliverability as a mid-funnel hard dependency. **v9: `REQUIRE_EMAIL_VERIFICATION=false`** — codes not generated, resume email carries no code line, upload-url doesn't require verification; the whole gate stays DORMANT in the EF and re-arms by flipping one const. Replacement ceiling: the 30/hr/draft upload cap. v9 live-verified (fresh draft uploads immediately; `rl:upload` window row confirmed).

**Abandoned-draft purge.** Migration `partner_draft_purge_helpers`: `partner_drafts_purgeable(p_days,p_limit)` (status `applied` AND `credentials->'agreement' IS NULL` — submit always writes it — AND untouched `p_days` across progress.updated_at / app.status_changed_at / applied_at) + `prune_ef_rate_limits()` (>2h windows). New **`partner-draft-purge` EF v1**, cron **jobid 54** `vyve-partner-draft-purge-daily` @ 03:30 UTC (same vault-key pattern as crons 21/22). 30-day retention (Claude's number — Dean to override if wanted), batch 50, **storage-first**: `partner-docs/<id>/` + `partner-content/<id>/` swept (two-level list) BEFORE row deletion; storage failure skips that draft's row delete so nothing orphans. Gate Bearer SERVICE_KEY or `x-vyve-cron-key` via `gdpr_cron_key()`. Live-verified: pg_net invocation 200 (candidates 0, correct — nothing is 30 days old), outside call with no key 401.

**GDPR erasure wiring.** The member pipeline is email-centric (`gdpr_member_scoped_tables()`) but partner children key on `partner_id` — a catalog entry can't reach them. New `partner_draft_erase(p_email)` RPC: deletes **applied/declined** drafts matching the subject's email (contact_email OR credentials contact/formFields email), child-first, returns partner_ids for storage sweep. **Live/onboarding partners deliberately excluded** — contractual data; erasure there is a legal decision, worth a line in the DPIA/register. **`gdpr-erase-execute` v5** (platform version 17): additive-only patch — `purgePartnerDrafts()` called as step 2d after export-artefact purge, storage prefixes swept per returned id, `partner_drafts` in the execution summary. **Best-effort + HIGH alert, NOT hard-fail** (`gdpr_erase_partner_drafts_failed` / `gdpr_erase_partner_storage_failed`): the member purge is non-re-runnable (`gdpr_erasure_purge` raises on a missing members row), so a hard-fail here would send retries into guaranteed terminal failure. RPC verified in a rolled-back test incl. proving the live calum row (same email shape) untouched; v17 content verified via get_edge_function post-deploy (placeholder-deploy trap).

**Wizard PM-747** (Test-Site-Finalv3 `2f5275ec32e598bc34291d3180248a4855f9e4cc`, md5-verified at the commit SHA): privacy notice on the overview screen — what's collected, purpose, **30-day draft retention**, erasure mailto, privacy.html link (Dean approved mockup frame A; Lewis copy pass logged non-gating — a missing notice is worse compliance posture than un-Lewis'd copy). `efCall` now attaches the EF's machine `code` to thrown errors (EF 429 messages are already human-readable and surface as-is). The verification-code UI (mockup frames B/C) was never built — correctly, per the v9 decision.

**Namespace note:** parallel sessions landed PM-744 (Men Together logo), PM-745 (composed cover), PM-746 (Mind/Money audit) while this session ran — recomputed to PM-747 at commit time. The `PM-744` refs hardcoded in the `partner-draft-purge` v1 and `gdpr-erase-execute` v5 source headers are STALE (should read PM-747); not worth redeploying two EFs for comments — this entry is the correction of record.

**Surfaced for backlog:** Dean's 3 `declined` test drafts (`dean-brown-*`) sit outside the auto-purge (it only sweeps `applied`) and outside request-based erasure unless asked — auto-purge of declined drafts after N days is a policy call for Dean/Lewis.

**Session 4 (completion plumbing) remains:** launch details → calendar_occurrences, welcomePost seeding, playbook reconcile (incl. S2 override path), photoId/dbs vestige sweep.
...
**1. Go-live gate** — migration `partner_golive_gate_insert_and_audited_override`:
- `trg_assert_partner_golive` now `BEFORE INSERT OR UPDATE` (per-`TG_OP` branch; INSERT has no OLD).
- **Audited override:** transaction-scoped GUC `vyve.partner_golive_override` holding `actor|reason`. The TRIGGER writes the `admin_audit_log` row itself (action `partner_golive_override`, partner id+slug in new_value) — an override can never be silent even if the calling EF forgets to log. Raw SQL inserts at `live` stay blocked; the referral-partner playbook flow must go through an audited EF action that sets the GUC (PF-NEXT-15 surface).
- Verified in a rolled-back transaction: direct INSERT at live → blocked; override INSERT → succeeds + audit row; UPDATE transition → still blocked; zero residue confirmed.

**2. Pre-gate backfill** — the 9 live partners (men-together-cic, alex-movement, nicola-yoga, calum-denham-performance, emma-clarke-yoga, antonia, april, david, ivan) got minimal HONEST progress rows: all steps false except `admin_approved:true`, marker `admin_onboarded_pre_gate:true`, safeguarding/gdpr false, pct 13 (trigger-computed). Data stops lying; the INSERT gate is the forward mechanism (PM-732 rule). No status changes. HELD deferred-aware migration STAYS HELD — it now only matters to the `modules_deferred` cohort (currently just Dean's declined test row).

**3. Server-side assessment scoring** — both forge paths closed:
- Discovery: `trg_sync_partner_onboarding_pct` derives the `safeguarding_passed`/`gdpr_passed` COLUMNS from `steps` — so the forgeable surface was BOTH `save`'s `payload.assessment` AND client-merged `payload.steps` keys.
- **EF v7** (platform version 7, sha256 `ec5a8c26…`, content-verified): new `assessment` action — client sends raw answers, EF grades against server-held `QUIZ_KEY=[1,1,0]`, always writes `assessment_score`, ONLY on 100% merges steps safeguarding/gdpr true + stamps `assessment_passed_at`. `save` strips `PROTECTED_STEPS` (safeguarding_passed, gdpr_passed, admin_approved, modules_deferred) from client steps, drops `payload.assessment` entirely, ignores client `pct_complete`. `submit` reads the graded progress row instead of trusting `training.assessmentPassed` (videos_watched stays client-attested). Bonus: `credentials._system` is server-owned — client patches can no longer reset the one-shot email flags. **QUIZ_KEY must stay in lockstep with quizData order in the wizard.**
- Live-verified from the real origin (Composio curl): forged save (all protected keys + legacy assessment payload) left every gate false; wrong answers → 33/not-passed; correct answers → 100/passed + gates set. Both test drafts fully purged after.
- **Wizard PM-743** (`Test-Site-Finalv3` `e5205090`, md5-verified at commit SHA): quiz answer key REMOVED from the HTML; `gradeQuiz` async-posts raw answers to `assessment`; requires online + partnerId (toast otherwise); save payload stops sending assessment/protected keys (honest client); cross-device `?resume=` adopts a server-graded pass from `r.progress`. Deliberate UX change on the dormant quiz pane: wrong answers marked red but the correct answer is no longer revealed (key isn't client-side).

**4. NOT done — contact_email backfill + Gate-A provision:** Dean doesn't have the addresses yet for Men Together CIC / Alex / Nicola. Agreed scope: skip the 4 referral placeholders. Carried in backlog; provisioning sends credential emails so the send list gets confirmed with Dean first.

**Numbering:** parallel sessions claimed PM-739–742 tonight (font sweep, employer portal, replay fix, numbering correction) — this session's ship recomputed to PM-743 at commit time.
...
**Tabs:** Overview · Deep Dive · Book Experts · Challenges · Events · Reports · Ask VYVE.

**Ship sequence (all md5-verified at commit SHA):**
- `608c82a3` (msg PM-734) v1: portal shell — Overview (preview KPIs/charts + Current/Projection toggle), Book Experts (9 demo profiles, booking modal → Events), Events (calendar + weekly schedule), Ask VYVE form. Click-to-rename company name (localStorage) so Lewis can brand it "Sage" in the room.
- `33385d24` (msg says PM-735 — **true PM-736**, collided with parallel PostHog PM-735) v2, Lewis round 1 ("micro view / no insights / tell a story"): workforce story strip, 4 insight cards (win/watch/action/trend w/ recommended actions), team micro view table, wellbeing trend, pillar participation, CIPD ROI model scaling with projection headcount, experts 9→17 (menopause, MHFA, MSK clinic, men's health, leadership, breathwork, S&C block, mindfulness course).
- `4305dace` (PM-737) v3 ("wow Sage / friendlier Ask / more tabs"): **VYVE Wellbeing Index** hero (animated 0–100 ring, score 72, sub-scores, "typical UK employer w/ EAP: 54" benchmark — the roadmap employer barometer demoed early), **Challenges tab** (live 10K Steps w/ team leaderboard + 6 launchable templates → Monday-start modal → Events), **Reports tab** (board pack via print-to-PDF `#print-report` build, monthly snapshot, GDPR summary), **Ask VYVE rebuilt as chat** (chips, typing indicator, canned answers incl. privacy-as-selling-point, free text → ref number + account-team handoff).
- `f06c91a1` (PM-738) privacy alignment, **Dean call**: demo had overstepped the aggregate-only model. Team wellbeing column → activities/member; Finance insight engagement-only; wellbeing trend + Index captioned "anonymised company-level, individual check-ins never visible"; challenge leaderboard reframed opt-in/team-aggregates-only; board-pack privacy para hardened ("never shared — in either direction"). Sales line for Lewis: "you see how they're engaging; how they're feeling stays between the member and VYVE — that's why your people will actually use it."
- `d4b8aad5` (msg PM-739 — third 739 in the namespace alongside partner-wizard cb340d44 and brain-Playfair 1d6f7a66; numbering resynced at the PM-740 session close) v4, Lewis round 3 (23:17): **all emojis removed** (status dots, challenge/report card icons, proj-badge warning, confirmations — typographic ✓ only), **Deep Dive tab**: UK/EU/Global + VYVE-community benchmark bars (engagement rate + activities/member, labelled illustrative, VYVE Average/Best teased as client base grows — Lewis's ask verbatim) + per-department drill-down (pill selector, stats scaling w/ projection, activity-mix bars, 6-month engagement trend, per-dept recommended action; defaults to Finance so the dip→drill→book-workshop arc tells itself).

**Demo-honesty guardrails baked in:** expert profiles fictional per Lewis's "create some demo ones"; team/insight specifics illustrative (told Dean to brief Lewis not to quote as Sage's real data); benchmarks labelled illustrative; ROI model labelled conservative/absence-only w/ Deloitte referenced for full return — **Lewis to sanity-check ROI framing vs his Stat Bank before quoting in the room**. Board-pack download uses the browser print dialog — Lewis should rehearse "Save as PDF" once.

**Process notes:** two PM collisions in one session (735, 739) — both from fetching heads and committing in the same tool batch while 3+ parallel sessions were landing; §23.24 hardened (see §23.126). Lewis's final message cut off ("will catch you tomorrow please do…") — possible uncovered ask, Dean to check the thread.
