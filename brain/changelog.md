## PM-844/848/849 — 6 Aug 2026: PARTNER FUNNEL P0 STACK SHIPPED — wizard re-sequenced, stalled-draft nudges + stage emails live, pre-filled invite links minting (2026-08-06)

Session arc: single session executing the full PM-843 P0 stack in order, each build live-verified before the next.

**PM RENUMBER (collision):** this session originally claimed PM-844/845/846 at session start; a parallel CC session claimed and pushed PM-845/846/847 the same evening (CC commits eac64340 "Application answers card", 59fd1516 "pipeline review answers+documents", f09f048f "document thumbnails"). Resolution: PM-844 (wizard, Test-Site `9a3af440`) is uncontested and stands; this session's nudges renumbered PM-845→**PM-848** and invites PM-846→**PM-849**. Both EF headers redeployed with the new numbers (partner-draft-purge platform v4, partner-onboarding platform v16 — comment-only changes, both re-verified live: resume 200, cron 200 zero candidates). CC comment sweep committed as PM-850 `b69d640e` (md5-verified); the immutable commit messages `4700aab2` ("PM-846 Mint...") and the parallel session's PM-845/846-labelled messages keep their historical labels — THIS entry is the authoritative mapping. The parallel session's PM-847 commit landed on top of `4700aab2` and preserved the mint modal intact (verified per-marker). §23.24's five-commit scan missed the collision because both sessions claimed within the same hour — the commit-time re-scan caught it, working as designed.

**PM-844 — wizard re-sequencing** (Test-Site-Finalv3 `9a3af440`, md5-verified; Dean device check ?z=844 pending): the application is now 5 steps — ACTIVE=[0,1,2,5,6], Launch step removed from the application (dormant pane retained, soft-kill style) and the welcome video made OPTIONAL: the submit-confirm checkbox moved st7→st6 (`isStepComplete(6)=state.checks.final`), validateStep(6) drops the cVideo1 requirement, contentSlots slot 1 req:false. Landing honesty: "~10 minutes to apply, 5 guided steps", stats grid updated, flow[] trimmed to the 5 apply steps + an "After approval — go live" flowline block. On-screen resume-link card in the rail (#resumeLinkCard: personal ?resume= link, copy button, spam hint; rendered on ensureDraft success + applyRestoredDom). buildSummary drops the First-session row and shows "None yet — add after approval" for zero videos. EF untouched (v12 already tolerated videoless submits and writes gates honestly). 21-assertion jsdom smoke ALL PASS including a videoless submit reaching doSubmit.

**PM-848 — nudges + stage emails** (migration `partner_draft_nudge_candidates`; partner-draft-purge **v2**, platform v3, cron jobid 54 / 03:30 UTC unchanged): two SECURITY DEFINER candidate RPCs, both REVOKEd from anon/authenticated — `partner_drafts_nudge_candidates(p_limit)` (status applied, agreement IS NULL, valid contact email, ≥48h stale by the same GREATEST(progress.updated_at, status_changed_at, applied_at) staleness the purge uses) and `partner_stage_email_candidates(p_limit)` (status onboarding/declined AND agreement IS NOT NULL = wizard-submitted, which excludes the 9 backfilled live partners by construction; one-shot via `_system.stage_email_<status>`). Cron passes in order: purge (unchanged) → 48h nudge (friendly, resume link, soft 30-day mention) → 7d nudge (honest deletion-date warning: last_activity + 30 days, explicitly names the uploaded photo ID; a draft first seen ≥7d stale gets ONLY the 7d email, both flags set) → stage emails ('onboarding' = light "you're approved", 'declined' = gentle decline with honest delete-on-request wording at team@ — declined auto-purge remains an open policy decision, no false automation claims) → rate-window prune. One-shot flags (`nudge_48h_sent`/`nudge_7d_sent`/`stage_email_*`) are set only on successful send via setSystemFlags(), which updates credentials ONLY — verified zero triggers on partner_applications, so nudges can never reset the purge clock. Live-proven via pg_net + the cron key: synthetic draft backdated to 3d stale → 48h email sent, flag set, last_activity untouched; at 8d → 7d email (no 48h re-send); flipped to submitted+onboarding → stage email. Three proof emails landed at deanonbrown@hotmail.com; test draft deleted child-first with zero residue; final candidate counts 0/0 — the night's real run touches nobody. Tags: partner-draft-nudge-48h/-7d, partner-stage-onboarding/-declined. All copy [LEWIS COPY PASS], non-gating.

**PM-849 — pre-filled invite links** (partner-onboarding **v13**, platform v15, verify_jwt:false; CC partners.html `4700aab2` md5-verified, Cloudflare live <1 min): new admin-gated `mint_invite` action — manual JWT → auth.getUser via ANON client → admin_users ilike email, active, role≠'partner' (partner-file-url v3 pattern; the EF stays public for the wizard actions). Mints a draft exactly like `start` from Lewis's call notes (contact{name,email,business?,phone?,location?,website?} + practice{pillar,specialism,description?,yearsExperience?} + optional internal notes → partner_applications.notes, 2000 cap) and sends the Lewis-voiced invite email ("I've started it for you", personal ?resume= link, tag partner-invite, [LEWIS COPY PASS]). CRITICAL SHAPE LESSON: the wizard's resume path reads `credentials.formFields` + TOP-LEVEL `credentials.pillar` in WIZARD tokens (physical/mental/social) — mint writes BOTH shapes (contact/practice for the record AND formFields + pillar via body→physical / mind→mental / connect→social) or the warm lead lands on an EMPTY form and the whole point is lost (Vanessa's live credentials confirmed the shape). `_system: {invited:true, invited_by:<admin email>, admin_start_notified:true}` — no self-inflicted v12 start alerts, and the invited cohort is the PM-843 invite→submit conversion metric; resume_email_sent:true stamped after a successful send so the generic resume email stays suppressed. Duplicate guard: an existing 'applied' draft with the same contact_email returns its existing link (existing:true), no email, no twin. Skips the 5/hr start IP limit (admin-authenticated); the 120/hr global limit applies; minted drafts that stall get the PM-848 nudges naturally. CORS: `admin.vyvehealth.co.uk` ADDED to ALLOWED_ORIGINS (PM-826 lesson — the CC is now a caller). CC invite modal upgraded: "Mint & send invite link" is the primary action (session-JWT fetch; Role/title field → practice.specialism, bio → description, notes carried; result panel with the resume link, copy button, and honest send status — existing-draft and send-failure cases both surfaced; pipeline local state + nb-pipeline badge refresh in place), with the old direct-insert path retained as "Add manually (no email)". Live checks: mint_invite with no auth → 401 (gate live); Vanessa's resume → 200 resumed:true (public path intact). 13-assertion jsdom harness ALL PASS (§23.159 observable effects).

**NEW §23.164:** backdating partner test rows requires the §23.156 disable-trigger-in-transaction pattern on trg_sync_partner_onboarding_pct PLUS an explicit status_changed_at UPDATE — partner_applications.status_changed_at defaults now() at insert, and the trigger bumps progress.updated_at on ANY progress write, so naive backdating leaves the row un-stale.

**PENDING Dean:** wizard device check ?z=844; CC mint walk (plain URL — Cloudflare). **PENDING Lewis (non-gating):** copy pass on the 5 new emails (48h nudge, 7d nudge, onboarding stage, declined stage, invite); decisions (a) photo-ID policy and (b) quiz coherence still owed (see backlog).
