...
## 19. Current status — 23 May 2026 PM-251 (Live page state machine: 5-state engine + 8 shells + sessions.html hub-awareness shipped to vyve-site `765c5b69`. PM-251 schema migration applied — `calendar_occurrences` gains `youtube_broadcast_id` + 5 override columns; `service_catalogue` gains 3 default_host_* columns. Pages render Quiet state immediately from `replay_playlists`; live states activate when PM-215 cron starts populating `youtube_broadcast_id`. §23.60 codified — bash_tool `cd` non-persistence between calls.)
