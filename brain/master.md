...
<!--CURRENT_FRONT_START-->
**PM-761 IN FLIGHT (2026-07-12): Phase 6 Employers — recon done, build handed to fresh session. Plan: employer-provision v2 +list action (employer_admins is service-role-only RLS, client can't read; contact_email guard must not block list), benchmark write-policy migration (is_admin_or_team; table stays empty — Lewis/Alan source figures), pages/employers.html + nav slug, pg_net verify, then commit + brain close. Only employer live: VYVE Demo Co (dev fixture). See changelog entry for v1 EF quirks to preserve. After: Phase 5 partners re-skin, Phase 7 partner-portal tokens.**
