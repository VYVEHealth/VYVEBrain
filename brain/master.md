...
**PM-1006/1007/1008 (2026-09-04): CC design system Waves 0-2 LIVE — W0 `c6a9fb49` (tokens.css v3 + components.css v2 + shell slimmed to chrome + density boot), W1 `ca188b5e` (8 Insights pages), W2 `0102d872` (partner-management — re-landed after a revert; the two faults found on Dean's check were pre-existing specificity bugs, not the migration, see §23.204). Spec = `tasks/cc-design-system-locked.md`. Waves 3-4 (admin-console → small-page sweep) pending, one per session, Dean device check between each, dark theme first. `--fs-xxs`/`--fs-hero` are compat aliases due to retire in Wave 4; partner-management carries a 12-name mint-era alias block to retire when its JS can be touched (§23.203).**
