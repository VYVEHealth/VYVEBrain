...
## 19. Current status — 23 May 2026 PM-212.1 (Podcast hub follow-up shipped — nav.js wired so podcast.html now has the standard sticky topbar + bottom-nav; thumbnail fallback for missing Drive thumbs is now the VYVE logo over teal gradient instead of bare gradient; Express Interest CTA is now an in-app bottom-sheet modal that opens mailto:team@vyvehealth.co.uk pre-filled instead of an external marketing-site link; new playbook `playbooks/podcast-episode-add.md` captures Lewis's no-deploy episode-add workflow)
