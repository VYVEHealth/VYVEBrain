...
**PM-874 (2026-08-07): partner referral card AUTOMATIC (CC `2790e621`)** — link derived from slug (every partner has it by construction, 'Not assigned' impossible), client-rendered QR + PNG download, funnel strip via `partner_referral_stats()` RPC (aggregates only — employer-dashboard PII line held), 'Referred members' = attributed AND paying (Dean redefinition). Stale pay-now coupon copy replaced [LEWIS COPY PASS]. **PENDING Dean+Lewis: identified referred-member list vs nudge-on-behalf** (PII-to-partner call; recommendation is nudge). Remaining Piece 1: human promo codes + share text + joins-by-source.
