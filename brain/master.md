...
| Streaming | Riverside (7 studios, permanent links) + YouTube (**9 channels, all under one Google account** — Group Therapy / Workouts / Mindfulness / Podcasts / etc., each paired to a dedicated Riverside studio with its own stream key) + Castr (scheduled pre-recorded). Architecture for scaling to 12-15 live sessions/day locked PM-191: reusable-stream pattern (YouTube `liveStream.isReusable=true`, one persistent stream per channel, many `liveBroadcast` resources bound via API) with Command Centre orchestrating batch broadcast creation. See backlog "Session content management surface" v2 layer. Test next week before phase 2 build. |
...
## 19. Current status — 23 May 2026 PM-229 (Replays hub shipped — replays.html new + Connect "Latest replay" featured card; 6 past test broadcasts seeded across Yoga/Workouts/Mindfulness; YouTube reusable-stream pattern proven end-to-end this morning via PM-215 pipeline)

**PM-229 (23 May 2026, vyve-site `7d7e11e1`).** Five-file atomic commit landing the member-facing replay surface. Architectural choices worked through in the design conversation (mockup-first, two design rounds Dean approved): one global `replays.html` hub with chip filter (not per-category browse pages — chip filter IS the view-all mechanism); in-place YouTube iframe player on tile tap (sidesteps PM-215 automation dependency for v1); Variant B for the Connect entry point (16:9 featured video card above "What's on", not a third destination tile alongside Calendar/Podcast — because the latest replay IS content, not a destination); `youtube_broadcast_id` as a new nullable column on `calendar_occurrences`, not overloading `replay_url` (which still holds the page-path placeholders from PM-210a backfill). Supabase pre-shipped: column + partial index `idx_calendar_occurrences_replays ON (ends_at DESC, category) WHERE active AND youtube_broadcast_id IS NOT NULL` (first attempt failed `functions in index predicate must be marked IMMUTABLE` because `now()` isn't IMMUTABLE — dropped that clause; `ends_at < now()` applied at query time via index-order scan). Six past rows seeded with real test broadcast IDs from the PM-215 pipeline-proving broadcasts this morning + the March-era originals, discovered via Vault OAuth + pg_net hitting `youtube/v3/liveBroadcasts?mine=true&maxResults=50`. The `bash_tool` sandbox can't reach `*.googleapis.com` so the Vault-OAuth-via-Supabase-MCP-`net.http_post` path becomes the canonical recipe for YouTube API discovery work. API quirk: `broadcastStatus=completed` and `mine=true` are mutually exclusive parameters — use `mine=true` and filter client-side. **`replays.html` NEW (24,524 bytes)**: Pattern 2 catalogue per §23.48, Dexie-first paint per §23.46, no skeleton/no localStorage snapshot; chips render from data on paint so only categories with replays show; hero defaults to poster + play overlay; tap mounts iframe with `autoplay=1&modestbranding=1&rel=0&playsinline=1` (playsinline mandatory for iOS WKWebView); subsequent tile tap = poster reset → mount iframe on next tap (each user gesture is what YouTube needs for mobile autoplay); visibilitychange refresh; §23.10 honest offline (tile metadata local, iframe is the network-bound surface and YouTube's own error UI surfaces). `connect.html`: `renderLatestReplay()` added, wired into `paintAll()` after `renderCalendarTile()`; section wrapper carries `hidden` by default, JS un-hides only on real row (§23.46). `sync.js`: `CATALOGUE_INVALIDATION_KEY` bump `pm211-podcast-episodes` → `pm228-replays-youtube-id` per §23.50, forces one-time `calendar_occurrences` re-pull on every device so new column reaches Dexie. `nav.js`: `'replays': 'Replays'` in `subPageLabels`. sw cache `pm228-settings-restructure-a` → `pm229-replays-hub-a` + `/replays.html` in `urlsToCache`. Index.html vbb-marker bump dropped from scope — Dean was running three parallel sessions (this + settings + mind-update); HEAD moved twice during build (`8f98bc49` → `f9620c66` transient corrupted state → `d1657514` corrective). §23.14 rule 3 invoked: PM number renamed PM-228 → PM-229 (parallel settings session took PM-228); connect.html had also been touched by the parallel mind session (taglines query gained `hub=eq.connect` filter), detected via md5sum, rebased clean. Two tooling lessons codified: (a) heredoc EOF marker scope — `<<'PYEOF'` single-quoted disables `$VAR` expansion, broke first commit attempt with `nil is not a string`, fix is route values through `os.environ` or build message file with bash-interpolating outer `python3 -c`; (b) dash vs bash for associative arrays — `bash_tool` shell is `/bin/sh` (dash), explicit `/bin/bash << 'BASH' ... BASH` required for `declare -A`. New protocol note for §23.14: every multi-step Git Data API ship now needs HEAD re-fetch immediately before the ref-PATCH step, not just before tree creation — my first PATCH attempt failed `422 Update is not a fast forward` because HEAD moved between commit-create and ref-update. No new §23 hard rule earned (lessons are mechanics-shaped not architecture-shaped).
