...
**Shared JS:** auth.js (17KB, v2.4), nav.js (18KB), theme.js (4.4KB), theme.css (4.5KB), sw.js (4KB, cache `vyve-cache-v2026-04-17u`), vapid.js (2.6KB), tracking.js (8.5KB), offline-manager.js, supabase.min.js.
...
### Onboarding Form
`www.vyvehealth.co.uk/welcome` = `welcome.html` in `Test-Site-Finalv3` repo. 150s timeout, 45s slow timer. Calls `onboarding` EF v74.
...
## 4. Database (Supabase — 68 tables, 25 FKs, 119 triggers, 31 public functions)
...
| `enforce_cap_*` (BEFORE INS) | workouts, cardio, daily_habits, kahunas_checkins, session_views | **DB-level cap enforcement.** Over-cap rows are routed to `activity_dedupe` via the cap functions. |
...
**Trigger support (14):** `vyve_lc_email`, `set_activity_time_fields`, `set_checkin_iso_week`, `cap_cardio`, `cap_daily_habits`, `cap_kahunas_checkins`, `cap_session_views`, `cap_workouts`, `increment_cardio_counter`, `increment_habit_counter`, `increment_checkin_counter`, `increment_session_counter`, `increment_workout_counter`, `vyve_sync_activity_log`.
...
- `workouts`, `cardio`, `session_views`: 2/day per member
...
|---|---|---|---|
| `onboarding` | v74 | Persona + habits + inline workout plan + welcome email | CORS (public) |
...
**EF:** `onboarding` v74
**Staged reference:** `staging/onboarding_v67.ts` — **stale by 7 versions, delete or refresh**
...
*Last full reconciliation: 18 April 2026*
