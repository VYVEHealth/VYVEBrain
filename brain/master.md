...
**Sequence after PM-36:** Seven Layer 1c migrations down (1c-1 through 1c-7), seven to go (1c-8 nutrition.html weight log → 1c-14 share-workout). Halfway exactly through Layer 1c by surface count; we're past the diversity peak — the remaining 7 are mostly smaller surfaces. Then option-(b) cleanup commit closes Layer 1. Next session = PM-37, likely 1c-8 (nutrition.html weight log → `bus.publish('weight:logged', ...)` — taxonomy says REFACTOR + scope-fix on `members + wb_last`).
