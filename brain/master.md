...
**iOS 2.0 (build 5) + Android 1.0.9 (code 55) SUBMITTED 1 Sep 2026 (PM-954) — supersede live 1.9(4)/1.0.8(54). OTA verified end-to-end (§23.106 CLOSED PM-701, Dean re-confirmed in production PM-954); Capawesome subscription active.**
...
| Native app delivery | `VYVEHealth/vyve-capacitor` — canonical Mac checkout is the fresh clone **`~/vyve-capacitor-git`** (PM-954; old `~/Projects/vyve-capacitor` RETIRED — keystore.properties + `app/keystore/vyve-release-key.jks` copied into the clone, gitignored/Mac-local). Bundles the `vyve-site` web shell into both store binaries. **iOS 2.0 (build 5) + Android 1.0.9 (code 55) SUBMITTED 1 Sep 2026 (PM-954), in review — carry vbb 564 with the 7 Lewis videos bundled in www/videos/ (§23.175). Supersede the LIVE pair iOS 1.9 (build 4) / Android 1.0.8 (code 54, live since ~4 Jul).** Signing: Dean's own Apple Distribution cert (minted 01/09/2026 — Lewis's two older certs are keyless on Dean's Mac); Android release keystore alias `vyve-key`. Fresh-clone Android build needs JAVA_HOME→Android Studio JBR + local.properties + keystore files (§23.178). Deployment model unchanged: members frozen on the bundled SHA, updated via Capawesome OTA (app `f9961f66`, prod channel `89e12796`, bundles RSA-signed, **OTA verified end-to-end — §23.106 CLOSED PM-701, Dean-reconfirmed PM-954**); Dean alone runs the server.url dev-loop shell (sees commits in 2-15min). |
