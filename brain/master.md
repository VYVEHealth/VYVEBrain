...
## 19. Current status — 25 May 2026 PM-330 → PM-337 (Live-tracker overnight chain ended with the actual bug. Started at PM-327's handoff: video playing in pre-baked iframe but `onReady` silent, zero rows in `session_live_views`, ratio of patches-to-progress was bad. **PM-330** restored the pre-baked iframe shape that PM-327 had replaced with a `<div>` placeholder (YT.Player silently refused to mint the iframe in WKWebView — diagnostic strip from PM-328 instrumentation showed `#sl-live-iframe tag: DIV` and `iframe src: -` and `YT postMessage count: 0`); fix included explicit `origin=` restored on BOTH the iframe src URL AND in YT.Player playerVars (PM-326 had dropped origin from the URL on the wrong assumption that origin enforcement was the bridge-killer — actually omitting it was, per YT IFrame API docs). Result: iframe attached + Big Buck Bunny video played, but ready still false. **PM-332** added two more fixes drawn from byte-level diff against replays.html (working since PM-294): (a) `YT.ready(cb)` gating in `loadYTApi` so we don't construct YT.Player while PM-323's static iframe_api script has made `window.YT.Player` truthy but the postMessage dispatcher isn't yet bootstrapped — replays uses runtime injection so its `onYouTubeIframeAPIReady` callback only fires after full bootstrap, the static-tag path needs YT.ready instead; (b) `referrerpolicy="strict-origin-when-cross-origin"` on the live iframe (replays has it on line 631) — WKWebView's default referrer policy may strip/downgrade headers YouTube needs for postMessage handshake. Result: `YTready_fired` fires (PM-332's gating working as designed) but `ready: false` + postMessage count 0 still — the YouTube embed iframe in this WKWebView genuinely won't open a postMessage channel to the parent, and that's a layer below what we can patch from the page. **Decision pivot.** Dean asked "but tracking.js should send info to Supabase" — and yes, it does: session_views had a 1.05min row from the earlier walk. He'd watched for 35 minutes. **PM-337** found the actual bug: tracking.js v9's `visibilitychange → hidden` handler was wired to `onPageExit` which permanently `clearInterval`'d the heartbeat with NO resume on `visible`. In iOS Capacitor, visibilitychange-hidden fires on every transient interruption (notification swipe, app switcher peek, control centre, lock-screen flicker) — Dean's notification-centre swipe to screenshot the debug strip killed the tracker for the rest of the session. Fix: split visibility-hide from genuine unload. `onVisibilityHide()` PATCHes current total, collapses minutesThisVisit into `baseMinutes` so the running total is preserved both server-side and locally, clears the heartbeat, nulls visitStartTime — but does NOT permanently kill. `onVisibilityShow()` re-stamps visitStartTime and restarts the heartbeat in the correct mode (pre-threshold polling OR post-threshold patching, mirroring onVisitStart's branching). `pagehide` and `beforeunload` continue routing to `onPageExit` unchanged because those ARE genuine unload signals. Device verification: Build 224, Dean watched ~45min with intentional backgrounding cycles, final row 02:20 UTC showed `minutes_watched: 45.99` — same row upserted across the night, hide/show cycles aggregating cleanly. Same fix applies to replay tracking (tracking.js is the unified writer). **Per-broadcast attribution via session_live_views deferred to v2 post-trial** — the YT IFrame postMessage bridge is genuinely broken in this WKWebView at a layer below the page, and tracking.js's category-per-day grain gives Lewis everything he needs for trial: who's engaging, how much, what categories. Per-broadcast precision is only useful if Lewis is doing per-stream drop-off analysis, which isn't a trial requirement. vyve-site PM-330 `a9a98ac3` + PM-332 `739660cb` + PM-337 `e61857ec`. sw cache chain pm329 → pm330-live-iframe-restore-explicit-origin-a → pm331/332/333/334/335/336 (parallel session interleaved) → pm337-tracking-resume-on-visible-a. vbb 215 → 217 → 219 → ... → 224 on both marker files (parallel ships taking some integers). **§23.71 vindicated** — codified at PM-327 ("when two patches in a row don't move the diagnostic needle, stop patching") and three more patches (PM-330/332/337) shipped after it, of which the first two (PM-330/332) were exactly the "real-bug-each-fixed-doesnt-move-primary-failure-mode" pattern the rule warned about. PM-337 was different — different bug class found by stepping back from the YT IFrame frame and asking what attribution was already working. **§23.72 new** — visibilitychange-hidden in WKWebView fires on transient interruptions, handlers must pair with visibilitychange-visible resume or short backgrounding kills the surface permanently. §23.66 fired ~6 times across the chain (parallel session shipped PM-328/329/331/333/334/335/336 interleaved with the live-tracker work — Achievements v2 schema + Home pills + movement icons; every commit was rebase-then-ship). §23.41 first-100 verify clean on every commit. No regressions to other surfaces — all changes scoped to session-live.js + player-tracker.js + tracking.js + markers.)

## 19. Current status — 25 May 2026 PM-348 (Reorder Exercises drag now shows live destination slot. Dean's feedback: drag feels off, can't tell where the row will land. Considered up/down buttons as alternative — landed on fixing the drag instead since drag is the iOS-native pattern members expect. Three changes in `workouts-exercise-menu.js` `pointerdown`/`rOnMove`/`rOnUp`: (1) source row switches to `position:fixed` with `scale(1.02)` + shadow + 0.96 opacity = floating tile that tracks pointer cleanly above the stable list; (2) dashed teal placeholder inserted where source was, repositioned every `pointermove` via midpoint detection (pointer-y < otherRow midpoint = insert before) using `insertBefore`/`appendChild` → other rows reflow naturally; (3) `pointerup` trusts `rTargetIdx` (placeholder's current slot), splices source into `reorderExercises[rTargetIdx]` directly, no more closest-midpoint re-search. Index conversion verified for top/middle/bottom/end cases — otherRows excludes source so splicing source out makes otherRows match reorderExercises order, rTargetIdx = direct insertion index. Added `pointercancel` listener routing to cleanup for iOS interrupts (call/background mid-drag). Limitation noted: no auto-scroll of `.reorder-list` when dragging near top/bottom — fine for typical 4-6 exercise PPL sessions, backlog candidate for ≥8-exercise sessions. vyve-site `3c142262`, sw cache `pm347-ach-toast-deeplink-a` → `pm348-reorder-live-slot-a`, vbb 234 → 235 on both marker files. §23.66/§23.70 fired once — initial PM-347 claim collided with parallel session's ach-toast-deeplink, clean sed-rebase to PM-348 + re-fetch of live marker files to rebase from 233 → 235. §23.41 first-100 verify all 4 files clean. No new §23 hard rule.)
...
**Postscript (PM-337, 25 May 2026 02:25 UTC).** Three more patches shipped after §23.71 was codified — PM-330, PM-332, PM-337. The rule was vindicated and partially mis-applied. PM-330 and PM-332 were exactly the "real-bug-each-fixed-doesnt-move-primary-failure-mode" pattern the rule warned about (iframe attached after PM-330; YT.ready fired after PM-332; primary failure `ready: false` and `session_live_views` empty unchanged). The right call at the end of PM-330's failed walk would have been to stop and attach devtools, not ship PM-332. **However**, PM-337 was the actual root-cause fix — different bug class found by stepping back from the YT IFrame frame and asking "what attribution is already working?" The answer (tracking.js had been writing to session_views all along, but a visibility-hide bug was truncating it) was discovered by Dean's lateral pivot, not by attaching devtools. The §23.71 escalation path should add: "When two patches don't move the needle, also consider whether the metric you're trying to fix is the right metric — there may be a parallel attribution path already working that you've been ignoring."


### §23.72 — visibilitychange-hidden in WKWebView fires on transient interruptions; pair with visibilitychange-visible resume (PM-337, 25 May 2026)

**Status: HARD RULE.** Earned at PM-337 after diagnosing a 35-minute live-session walk that logged 1.05 minutes to `session_views` and stopped. tracking.js v9's `visibilitychange → hidden` handler `clearInterval`'d the heartbeat permanently with no corresponding resume on `visible`. Dean's notification-centre swipe (to screenshot the debug strip) killed attribution for the rest of the session. Same bug class would silently destroy attribution on every notification, app switcher peek, control centre swipe, or lock-screen flicker that happens during any session of any length.

**The principle.** In a browser tab, `visibilitychange → hidden` typically means the tab is being backgrounded or closed and the user will return through page lifecycle (DOMContentLoaded fires again, fresh module state). In iOS Capacitor / WKWebView, `visibilitychange → hidden` fires on **every transient interruption** — and the module is NOT torn down. The same JS context survives. If your hide-handler killed timers, cleared listeners, or otherwise tore down state, none of it comes back unless you explicitly wire a visible-handler to restore.

**Concrete failure modes this rule prevents:**
- Heartbeat timers that survive the page but die on the first notification.
- Realtime subscriptions that close on hide and never reopen.
- WebSocket connections, Supabase Realtime channels, EventSource streams that get torn down on hide.
- Idle-detection timers that reset on hide and never restart.
- Analytics flush-on-hide patterns that fire correctly once but never re-arm.

**Rule.** Any `visibilitychange → hidden` handler that does cleanup MUST be paired with a `visibilitychange → visible` handler that restores the equivalent state. The hide handler should *checkpoint* state (PATCH server, persist to local storage, collapse running totals into a base counter), not *destroy* it. The visible handler should re-stamp time anchors, restart timers, re-establish subscriptions.

**Canonical shape (tracking.js v10 PM-337 pattern):**
```js
function onVisibilityHide() {
  if (!initialized) return;
  if (heartbeatTimer) { clearInterval(heartbeatTimer); heartbeatTimer = null; }
  baseMinutes    = totalMinutes();   // collapse the current segment into base
  visitStartTime = null;             // signals "not currently in a foreground segment"
  if (inserted) patchSession(baseMinutes, true);  // checkpoint to server
}
function onVisibilityShow() {
  if (!initialized) return;
  if (heartbeatTimer) return;        // defensive against double-fire
  visitStartTime = Date.now();
  heartbeatTimer = setInterval(
    inserted ? () => patchSession(totalMinutes(), false) : onHeartbeat,
    HEARTBEAT_MS
  );
}
window.addEventListener('pagehide',     onPageExit);   // genuine unload — kill cleanly
window.addEventListener('beforeunload', onPageExit);   // genuine unload — kill cleanly
window.addEventListener('visibilitychange', () => {
  if (document.visibilityState === 'hidden')      onVisibilityHide();
  else if (document.visibilityState === 'visible') onVisibilityShow();
});
```

**Distinguish genuine unload from transient hide.** `pagehide` and `beforeunload` ARE genuine "page is going away" signals — kill cleanly on those. Only `visibilitychange → hidden` is the ambiguous one in WKWebView.

**Test path.** When walking any surface that does work on a timer or maintains stateful background activity, INTENTIONALLY background the app mid-task — swipe up to home, peek at app switcher, swipe down for control centre, lock the screen briefly — then resume. The thing you're tracking should continue accumulating. If it freezes at the hide moment, you have a §23.72 violation.

**Audit candidates.** Any file in the repo with `visibilitychange` should be checked against this rule:
```bash
grep -rn "visibilitychange" --include="*.js" --include="*.html"
```
Known candidates beyond tracking.js (PM-337 fixed): replay-tracker.js if it has its own (no — replays uses tracking.js too), session-live.js's tracker module (no own visibility handling — relies on tracking.js), VYVE Local DB sync layer (sync.js — to be audited).

**Pre-existing concern partially resolved by PM-337.** Backlog line ~3020 ("tracking.js v9's dwell accumulator resets on page unload — sub-60s visits don't sum") describes a DIFFERENT bug class — cross-page-load accumulation, where `baseMinutes` itself resets on full page reload because it's in-memory only. PM-337 fixes the within-page-load case. Cross-page-load still open, acceptable for trial per the same rationale.
