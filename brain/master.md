...
**Phase 1 (PM-640) SHIPPED:** `is_team()` + `is_admin_or_team()` RPCs live. `admin_users` role constraint extended. Per-cmd RLS on `cc_tasks` + `cc_calendar_events`. `cc_task_attachments` table live. 5 team members in roster. **Remaining missing surface:** `cc-task-docs` private Storage bucket + signed-URL EF (Phase 2); live-session scheduler write into `calendar_occurrences` (Phase 4).
