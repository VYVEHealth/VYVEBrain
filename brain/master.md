...
- **Home carousel wired PM-453** (vyve-site `44de2cf996b76a1c71446e1c4ba3fc0a9eb5bc8c`): the home "live sessions this week" carousel now reads the REAL upcoming sessions from `calendar_occurrences` (titles/hosts/thumbnails, next 7 days, currently-live first), with the hardcoded `sessions-data.js` 8-card catalogue kept only as a cold-boot fallback that auto-upgrades once Dexie hydrates. Was previously the static catalogue only (PM-333). `paintWhatsNext` remains a no-op.
...
| Native app delivery | `VYVEHealth/vyve-capacitor` (Dean's Mac at `~/Projects/vyve-capacitor`) is the Capacitor project that bundles the `vyve-site` web shell into both store binaries. **iOS:** App Store live; 1.4 Build 3 in review (PM-413, 26 May 2026 — iPad orientation + storyboard splash fixes). **Android:** Play Store live; 1.0.5 versionCode 50 in review same session. Both stores ship the same web shell — single codebase, two binaries. Deployment model since PM-115/116: Dean's iPhone runs server.url dev-loop pointing at `online.vyvehealth.co.uk` (every vyve-site commit live for Dean immediately, subject to WKWebView cache 2-15min); all other members run bundled-mode iOS 1.3 / Android 1.0.3 — they see only the SHA frozen in their IPA/AAB until OTA via Capawesome (app `f9961f66`, prod channel `89e12796`). |
