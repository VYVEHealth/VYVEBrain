...
- **§23.31 (PM-858): `Supabase:deploy_edge_function` DEFAULTS `verify_jwt:true`.** Deploying a public EF (partner-onboarding, onboarding, send-email, etc.) without passing `verify_jwt:false` EXPLICITLY silently flips it to JWT-required and breaks every unauthenticated caller — this took the partner wizard backend down for ~15 min. Every EF deploy must pass verify_jwt explicitly AND verify the field in the deploy result. (`EF deployment: always use Supabase:deploy_edge_function` rule still stands — this extends it.)
