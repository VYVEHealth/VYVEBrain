...
- **PM-158 (retro, 16 May 2026)** — seven Mind-section pages shipped to vyve-site: `mind.html` (hub), `mind-library.html`, `breathwork.html`, `journal.html`, `affirmations.html`, `visualisation.html`, `mind-insights.html`. Built from ChatGPT mockups; landed as `ca08f2966` (raw upload). nav.js gained the Mind tab (`e1c29a722`) — bottom nav is now **Home / Body / Mind / Sessions / More** (5-tab). The Mind pages are **scaffold** — placeholder content, no Dexie/Supabase wiring.
- **PM-159 (`59a3fff6`)** — fixed the Mind-page nav bug Dean caught on device. The seven pages were uploaded raw with the ChatGPT mockup's hardcoded `<nav class="bnav">` (Home/Body/Mind/**Connect**/**Profile**, brain-pair glyph, dead `href="#"`) + `.bnav*` CSS, and **zero `<script>` tags** — no theme.js, auth.js or nav.js. They rendered only the fake mockup nav and were **ungated**. Fix per page: stripped the bnav block + CSS; injected theme.js + auth.js into `<head>` and sw.js-registration + nav.js before `</body>`. Chrome now byte-identical to every other portal page. **Scope:** minimal chain only (theme/auth/nav) — full local-first stack lands per page when each Mind page gets real `mind_sessions` wiring.
- **sw.js** — the 7 Mind pages were missing from `urlsToCache` (PM-158 gap; same shape as PM-67a/PM-118). Added all 7. Cache key → `vyve-cache-v2026-05-17-pm159-mind-nav-fix-a`.

`node --check` clean on all inline JS + sw.js; post-commit verified all 8 files against pinned commit SHA. **Device verification pending Dean.** No EF or schema change.

**Mind section architecture (decided, recorded; not all built):** nav direction is **BODY / Mind / Connect** — Nutrition folded into BODY (done PM-154), Mind being built out, Connect is the future social tab (not yet a live nav slot). Mind activity persists to a dedicated **`mind_sessions`** Supabase table with an `activity_type` discriminator — **never** `daily_habits`; schema deferred until breathwork's real fields exist. `mind-insights.html` is **Phil-gated** (clinical sign-off) — reachable but staged, correctly absent from nav. **Breathwork is the next real feature build.** Goal/certificate re-pillaring onto the new pillars is deferred post-launch.
...
**16 May 2026 PM — PM-150: session view storage + anti-farm.** Page-by-page paint audit (index.html) surfaced the home "This week's goals" sessions row stuck at 1/2 vs "Your Progress" 2. Root cause was not paint: the `cap_session_views`/`cap_replay_views` BEFORE INSERT triggers enforced a flat 2/day cap (cross-table sum, category-blind) by *rerouting* over-cap inserts into `activity_dedupe` — PostgREST still returned 201, so the loss was completely silent. The app model is one row per category per day across 8 categories; from the 3rd category on, every session view vanished. `test1@test.com` had 10 stranded rows; 544 total across 5 members. Also found: the magic `2` hardcoded across four triggers with three different counting rules, and `cert_sessions_count` written by two triggers (latent divergence). **Decision (Dean): keep 2/day as a certificate COUNTING rule only, not a storage rule; anti-farm moves to a tracking.js dwell threshold; certificates explicitly NOT uncapped.** Migration (applied, verified via pg_trigger): dropped both `cap_*` triggers+functions; dropped redundant `counter_sessions`+`increment_session_counter()` (single writer now); rewrote `charity_count_session_views()` keeping per-day <2 guard; `update_cert_sessions_count` LEFT UNTOUCHED (LEAST(daily,2) intact); backfilled `cert_sessions_count` all members. tracking.js v9 (vyve-site `9a95ab5c`): 60s cumulative dwell threshold — `onVisitStart` no longer inserts on load, the heartbeat doubles as threshold poller, `crossThreshold()` fires insert+publish once at `totalMinutes()>=1`; `baseMinutes>=1` fast-path; failed insert retries; PM-43/54/61 bus discipline moved verbatim. Build banner Update 14→15, sw key `pm150-session-threshold-a`. test1's 10 stranded rows replayed (collapsed per category, minutes SUMmed); 534 historical rows for other members left in `activity_dedupe` by design. **Device-verified:** Yoga 20s → no row; Workouts short bounces → no row; Podcast 70s → row `minutes_watched=1.24`. Both ship units confirmed. New rule §23.34. Open: dwell accumulator resets on page unload (correct for anti-farm; cross-visit accumulation is a post-launch backlog item); `tracking.js` still has no outbox (direct-fetch, §23.10 hardening candidate).

## 19. Current status — 16 May 2026 PM-149 (site-wide login outage resolved: `platform-alert` storm drained the nano connection pool; no-op v9 + project restart)

**16 May 2026 PM — PM-149: login outage.** A site-wide login break (`/auth/v1/token` 522, "Auth failed to initialise") was traced to the PM-145 `platform-alert` storm at full severity. `platform-alert` v8 (no rate limit, broken dedup) produced 40+ concurrent 90-150s invocations that exhausted the **nano-tier 60-connection Postgres pool** (`max_connections`=60 confirmed); GoTrue could not get a DB connection (`dial error host=localhost ... supabase_auth_admin`), and `/auth/v1/token` died at the Cloudflare edge as 522. Not CPU saturation — connection-slot starvation (box had spare CPU). Fix: `platform-alert` deployed v8→**v9 no-op** (instant 200, no DB/Brevo/push — severs the storm; DB pool drained 60/60→13/60 and Postgres recovered) + a **Dean-executed project restart** (recycled the nano instance's edge/proxy layer — the API gateway stayed 522-wedged after the DB recovered, fixable only by a restart). API log confirms recovery: 522s end 18:44, restart 503s ~18:52, clean 200s from 18:53. The four PM-148/148b vyve-site commits were confirmed irrelevant and NOT reverted. **Open:** `platform-alert` is the no-op v9 — platform error-monitoring is OFF until a real v10 (working dedup, rate limit, drop the dead `push_subscriptions` write — the PM-145 work, now P0); the nano 60-connection ceiling is the structural fragility (backlog P3-1, Dean's call). New rule §23.33.

## 19. Current status — 16 May 2026 PM-148b (completeWorkout optimistic-first shipped; PM-148 broke workouts.html via a duplicate-declaration, hotfixed in 207aa1b0)

**16 May 2026 PM — PM-148: completeWorkout optimistic-first.** vyve-site commits `b1470698` (PM-148 — `completeWorkout` rewritten, sw.js cache key → `vyve-cache-v2026-05-16-pm148-workout-optimistic-a`), `b5ee7854` (build banner Update 12 → 13). Root cause: `completeWorkout` did `await VYVEData.writeQueued(...)` — and `writeQueued` (vyve-offline.js) awaits the network POST internally; it is NOT a fire-and-forget queue, it only falls back to the localStorage outbox on 5xx/network-failure. A slow-but-successful response blocked the "Complete Workout" button on "Saving..." for 75s+, and because the `optimisticPatch` + `workout:logged` bus publish sat downstream of that awaited POST, the home-page engagement score never updated until the round-trip completed (the stale 74-vs-87). Fix: `completeWorkout` rewritten optimistic-first mirroring cardio.html `logCardio` — Dexie write + home patch + bus publish + completion screen all before the network; workouts POST + plan_cache PATCH fire un-awaited in background closures; 4xx → `workout:failed` + drop optimistic row, 5xx/network → keep. `ordinalSuffix()` (referenced but never defined — latent ReferenceError) defined this commit. Engagement page was already correct (reads Dexie directly, PM-119/146/147); only the home page was stale. Device confirmation pending. Open: per-set save path (`tickSet`/exercise_logs, workouts-session.js) has the identical `await writeQueued` shape, latent; PM-145 / `platform-alert` back-burnered by Dean (see backlog); check-in + sessions missing from "This Week's Goals" is a separate write-path issue. New rule §23.32. **PM-148b (`207aa1b0`):** PM-148 also added a `function ordinalSuffix()` to workouts-session.js that collided with the existing `ordinalSuffix` in workouts-config.js (shared global scope) — a `SyntaxError` that broke workouts.html (Exercise page stuck on skeleton). Hotfixed by deleting the duplicate; sw key `vyve-cache-v2026-05-16-pm148b-hotfix-a`. §23.32 extended to cover cross-file shared-scope declaration collisions.

**PM-145 findings (investigated, not shipped — Dean back-burnered).** member-dashboard v69 504s are NOT primarily the achievements payload. Chain: workout/exercise write errors → `auth.js` client error catcher POSTs `platform-alert` per error → `platform-alert` v8 dedups on raw `type` (varies per error, never dedupes), has no rate limit, and pushes to the dead `push_subscriptions` table with the pre-v12 `'raw'` VAPID bug → alert storm drains the shared EF compute pool → member-dashboard (fattest function) 504s. Two achievements evaluators also query dead columns (`shared_workouts.member_email` → real col `shared_by`; `monthly_checkins.logged_at` → no such column). §11A is stale (describes v55/23-serial; live is v69, v60 parallelised). Full detail in backlog PM-145 block.

## 19. Current status — 16 May 2026 PM-147 (engagement + cardio local-first fully painting from Dexie; load-order root cause fixed; 2/day Dexie cap shipped)

**16 May 2026 PM — PM-142–147: engagement + cardio local-first.** vyve-site commits `f5675542` (PM-142, botched-PM-135 recovery: cardio running-plan functions restored, engagement `data.score` cache fix), `81144bb8` (PM-143, read-Dexie-first / background-hydrate on engagement + index), `0d4933b8` (PM-144 probe), `50e7332c` (PM-146 ROOT CAUSE: engagement's inline `<script>` called `loadPage()` during parse, before the `defer`'d db.js/sync.js/home-state-local.js had run — `loadPage()` now gated on `DOMContentLoaded`), `05fabadb` (PM-147: `computeHomeStateFromDexie` workout/cardio all-time totals now `cappedDailyCount` 2/day, matching the Supabase trigger; PM-144 probe stripped). Build banner Update 12, sw.js `vyve-cache-v2026-05-16-pm147-cap-a`. Engagement confirmed on device painting progress cards from Dexie instantly. Open: PM-145 (`member-dashboard` v69 504-ing — 150s timeouts, achievements payload suspect), PM-137 (`server.url` WKWebView cache lag — dev-loop bottleneck), workout-logging multi-fire (phantom inserts, PM-147 caps the count only). New rules §23.30, §23.31.

**16 May 2026 PM — Updates 1–6 (PM-128–133): local-first paint & write-path fixes, vyve-site portal.** Six device-visible commits to vyve-site main, labelled "Update 1–6" in the home build banner. Update 1 (`3c3730b1`): cardio/movement localStorage paint-cache TTL 10/30min→24h. Update 2 (`00519049`): build-banner iOS safe-area-inset fix. Update 3 (`06a48f3d`): cardio logCardio() rewritten optimistic-first (Dexie write + repaint + button flip before the un-awaited network POST). Update 4 (`f292af3e`): cardio logging — button flip moved before repaint, repaint non-blocking, last awaited criticalHydrate (fetchHistory) un-awaited; logging now instant. Update 5 (`9067e814`): on-screen diagnostic probe added to movement.html. Update 6 (`db4156835d713c8d677dc4957f8e6de613ed81bc`): movement local-first ROOT FIX — db.js/sync.js/firstPaintHydrate.js were missing from movement's script chain, so VYVELocalDB was undefined and all Dexie code was dead; 3 script tags added. Confirmed on device: cardio instant paint + instant logging, build banner. Movement Update 6 shipped + byte-equal verified — device confirmation pending. Movement probe instrumentation still in movement.html pending a cleanup commit. New hard rules §23.27 (never await criticalHydrate on a paint path), §23.28 (a page needs db.js+sync.js+firstPaintHydrate.js to be local-first), §23.29 (the server.url dev-loop build has an unidentified caching layer — trust the build banner, not a single reopen).
