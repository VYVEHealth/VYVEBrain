...
## CURRENT FRONT (updated 2026-07-14, PM-814)
...
| `booking-paid-confirm` | LIVE v1 (PM-814) | Partner JWT via `get_my_partner_id` RPC executed AS caller. pending_payment→confirmed + paid_at; member confirmation email (link/location) [LEWIS COPY PASS]; 1h-before reminder into `scheduled_pushes` (dedupe_key `booking_reminder:{id}`). |
| `confirm-booking` | LIVE v1 (PM-813) | admin_users-gated CC action on employer requests: confirm sets final starts_at/ends_at + fee_pence, emails employer AND partner contact_email (partner notify is CORRECT for bookings, unlike complaints); decline emails employer with note. [LEWIS COPY PASS]. |
