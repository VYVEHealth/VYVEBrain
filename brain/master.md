...
**Sequence after PM-37:** Eight Layer 1c migrations down (1c-1 through 1c-8), six to go (1c-9 settings.html persona switch → 1c-14 share-workout). Past halfway by surface count; diversity peak well behind us. Then option-(b) cleanup commit closes Layer 1. Next session = PM-38, likely 1c-9 (settings.html persona switch → `bus.publish('persona:switched', ...)` — taxonomy ADD; event doesn't exist today).
