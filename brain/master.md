...
| `partner-content-upload` | LIVE | **PM-683/684 v5.** Partner-portal video upload + scheduling (DEMO; `verify_jwt:false`, `x-portal-key` gate per §23.133). Actions: `sign` / `sign-thumb` / `commit` (publish_at required & ≥48h, optional thumbnail_url) / `list` / `update` (alias `reschedule`; title/desc/publish_at, in_review ONLY, 48h). Writes `partner_content_items` at `in_review`; reads via service role. (§23.139) |
