...
## 19. Current status — 25 May 2026 PM-355 (Fullscreen video overlay z-index 2000 → 20000 so it sits above swap picker. Dean: "it doesn't load in front of the swap exercise library… when you click back, the video is playing." Final manifestation of the PM-346 / PM-352 / PM-355 cascade — wiring worked (PM-352 verified handler fires), but the overlay rendered behind the picker which is at z-index 10000. Other modals on the same page: session-view 500, ex-menu-backdrop 800, reorder-modal 900, exercise-search-view 10000. Picked 20000 (not 10001) for headroom against future modals. Why latent: `openVideoFullscreen` pre-dates the picker modal; its z-index was set against the world that existed when in-session card thumbnails were the only caller. PM-346 added a new caller from a dramatically-higher-z-index context. Close button bumped 2001 → 20001 to track. Three-strike retro on PM-346: wiring (PM-346), close-button safe-area (PM-351), wire mechanism (PM-352), z-index (PM-355) — four ships for one feature; pattern is "pre-existing helper has assumptions baked against pre-existing caller contexts." Banking §23 candidate: when wiring a pre-existing helper into a new context, audit its assumptions (z-index, scroll, focus, escape handlers) against the new caller. Not yet promoting — adjacent to PM-352's "copy working mechanism" insight, want a second occurrence to confirm. vyve-site `7894a3e9`, sw cache `pm354-leaderboard-avatars-a` → `pm355-video-overlay-zindex-a`, vbb 240 → 241 both marker files. §23.66/§23.70 fired once — claim PM-353 → PM-354 → PM-355 cascade as parallel session shipped twice (Fuel retired, leaderboard avatars); sed-rebase clean. §23.41 first-100 verify all 4 files clean. No new §23 hard rule.)
