...
## 19. Current status — 22 May 2026 PM-208 (Connect cluster + Feed redesign: PM-199 reaction parity + PM-200 posted-state Direction B + PM-201–205 spacing iteration + PM-206 hub card alignment + PM-207 feed All-members/today-only/cache-paint + PM-208 silent background refresh — nine commits, one new Edge Function, one new §23 rule)

**PM-208 (22 May 2026, vyve-site `558c80cf`).** Silent background refresh on connect-feed.html. PM-207 added paint-from-cache + parallel background fetch but always called `render()` when the fetch returned — on a slow network the 3-5s fetch made the page feel like it was still loading after the cache had already painted, because the second render visibly replaced the first. PM-208 adds a content-equivalence diff-check before the post-fetch render. `computeFeedSig(rows, rxMap)` generates a deterministic signature from row ID order + per-row reaction total + my_reaction key. `__lastRenderedSig` is stamped at the end of every `render()`. `loadFeed()` computes a fresh sig from fetched data and compares: if identical, in-memory state and `_kv` cache update but `render()` does NOT run. User sees zero visible change for the entire silent refresh. Cleared in `setTab()` because sig is scope-specific. Trade-off: a new check-in posted by another member during a visit shows up at the end of the 3-5s silent refetch, not in realtime — that's the gap Option C (global Dexie sync + Realtime, Sunday/Monday deep dive) will close. sw cache `pm207-feed-redesign-a` → `pm208-silent-refresh-a`. vbb-marker 78 → 79. No new §23 rule (§23.48 Pattern 4 done right; PM-207 half-shipped the pattern).

**PM-207 (22 May 2026, vyve-site `b37f8f90`).** Feed redesign. Three changes in one commit. (1) Tab semantics: "Workplace" demoted from default to secondary. New default `All members` (data-tab="all") shows every check-in today regardless of employer. `Workplace` tab now hidden entirely if the viewer's `members.company` is null; revealed by `loadMemberInfo` when company is set. Tab order: All members | Workplace? | Elite (locked) | Following (soon). (2) Today-only feed: dropped 7-day window + day separators. Daily midnight reset by cache-date mismatch + visibilitychange refetch. Day-separator markup, CSS, and rollover-date logic all removed. (3) Cache-paint performance: new `_kv` key `connect_feed_rows_v1` stores `{tab, rows, reactions, today, computed_at_ms}`. `loadFeed()` Step 1 paints from cache instantly (~100ms), Step 2 fires rows + reactions REST in parallel (`fetchReactionsForToday` filters by `created_at>=today` so it doesn't need row IDs first — replaces the sequential `fetchReactionsFor(checkinIds)` which was deleted). Visual chrome polish: tabs got an outlined-pill treatment, banner font slightly up. Legacy `fetchReactionsFor` removed. sw cache `pm206-hub-card-alignment-a` → `pm207-feed-redesign-a`. vbb-marker 77 → 78. No new §23 rule (PM-208 caps Pattern 4 properly).

**PM-206 (22 May 2026, vyve-site `9e9bfc8f`).** Connect hub Recent Check-ins aligned to feed card shape + reactions interactive. `connect.html` `renderRecentCheckins` rewritten to use the canonical `cin-*`/`rx-*` markup lifted verbatim from `connect-feed.html`: 6-emoji reaction strip, "YOU" gold badge on own cards, heart total in header row, full feed-shape card. Wrap-link around each card removed so reaction taps don't navigate; "See all ›" in the section header carries the feed-navigation affordance. Hub now writes reactions: delegated `.rx-btn` click handler on `#recent-list` triggers `toggleReaction` — optimistic Dexie write via `checkin_reactions.upsert`/`deleteByPair` + bus publish (`connect:reaction:logged`/`cleared`) + REST POST/DELETE + outbox fallback. Identical pattern to feed. `paintAll` now builds the richer `reactionsByCheckin` shape (was: flat count map). Rows cached in `__lastRecentRows` so `toggleReaction` repaints without a fresh Dexie read. Zero added load cost — hub was already Dexie-only synchronous read; PM-206 adds bigger HTML strings + one delegated handler + the reaction write path. Hub docstring updated: no longer 100% read-only. sw cache `pm205-wrap-padding-28-a` → `pm206-hub-card-alignment-a`. vbb-marker 76 → 77. No new §23 rule.
