...
| `alert-digest` | LIVE | Morning / afternoon / evening platform-alert digest. |
...
| Movement track | LIVE — `movement.html` + `movement-history.html`, writes to `movement_activities`. PM-307 promotion to first-class table closed dual-write to legacy `cardio.logged_via='movement'` + `workouts.plan_name='Movement'` shapes. |
