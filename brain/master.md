...
| **Achievement push fan-out** | **LIVE end-to-end (28 April 2026 PM, Session 2 item 1).** `achievement-earned-push` v1 + `log-activity` v23 (inline path) + `achievements-sweep` v2 (sweep path). One push per earned tier, dedupe-off (multiple tiers in same day = feature), in-app suppressed (caller writes `member_notifications` separately). Smoke-tested live on Dean (synthetic earn → APNs HTTP 200) and on Vicki (real `member_days` tier 2 crossed during sweep invoke → push delivered). **Note (28 April PM continuation):** native delivery worked from day one; **web delivery was silently broken until `vyve-site` `sw.js` push + notificationclick handlers shipped at `124ecb53`** — sub registrations and VAPID encryption were fine, but the SW had no `push` listener so the OS never rendered the banner. All web pushes since the initial rollout were lost. Native users (Capacitor wrapper) unaffected throughout. |
...
**SW push handler requirement (codified 28 April 2026 PM continuation).** Web push delivery requires the service worker to register `self.addEventListener('push', e => e.waitUntil(self.registration.showNotification(title, opts)))`. Without this listener, the browser receives the encrypted payload, decrypts it successfully, and discards it silently — no banner rendered, no error logged anywhere visible. Confirmed dead from initial web push rollout through 28 April PM. Patched in `vyve-site@124ecb53`. ALL future SW changes must preserve the `push` and `notificationclick` listeners; touch them only with intent.

**SW notificationclick must read `data.url` (codified 28 April 2026 PM).** Click-through routing for any push trigger that ships a deep-link URL (`achievement-earned-push` v1, `habit-reminder` v14, `streak-reminder` v14, all future Session 2 nudges) requires `self.addEventListener('notificationclick', e => { const url = e.notification.data?.url || '/'; ... })` followed by `clients.matchAll` + `client.focus()` (preferred) or `clients.openWindow(url)` (fallback). Without this, taps fall back to browser default (usually focus existing tab on origin root, sometimes nothing on iOS PWA). Confirmed working in `vyve-site@124ecb53` with hash-routing fallback via `postMessage` to the focused client. All trigger EFs setting `data.url` rely on this handler being present.
