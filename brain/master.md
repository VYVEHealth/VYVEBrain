...
**Phase 3 (PM-642) SHIPPED:** `pages/calendar.html` — `calendar_occurrences` replaces `cc_sessions` as session source (`starts_at`/`ends_at`, `session_title||name`). 3-source union read complete (meetings + live sessions). Role gating: New Event + Delete admin-only. Legend updated. **Remaining:** scheduler write surface → `calendar_occurrences` (Phase 4).
