...
| Analytics | PostHog (`phc_8gekeZglc1HBDu3d9kMuqOuRWn6HIChhnaiQi6uvonl`, EU instance `eu.i.posthog.com`). Identity wired since PM-408 (26 May 2026) — `posthog.identify(email)` fires from `auth.js:vyveCapturePageView` with `{email, name, given_name, family_name, is_dean, host_kind}` initial props; `vyveAnalyticsRefreshIdentity(email)` re-enriches via `posthog.setPersonProperties` from `vyve_members_cache_<email>` localStorage row (persona, goal_focus, gender, age, activity_level, first_name, last_name) at 2s/5s/10s after identify (idempotent retries cover the eager-prefetch idle-callback window). Section-level + activity events mirrored via `analytics.js` bridge — subscribes to 29 VYVEBus events at `envelope.origin === 'local'` (no cross-tab/cross-device double-counting), captures to PostHog with allowlisted payloads. Session recording configured 100% in `posthog.init` (auth.js) but pre-empted on index.html by an inline duplicate `posthog.init` block at lines 1043-1046 that lacks the recording config — banked for cleanup (timing-sensitive, needs incognito verification). `window.vyveEFFetch(fnName, url, opts)` wraps fetch and captures `ef_error` on non-2xx/network. `window.vyveAnalyticsCapture(eventName, props)` exposed for one-off page-local captures. |
...
| `persona_welcome_copy` | Hydration overlay welcome lines (persona × goal). 13 rows since PM-372 (25 May 2026). NULL goal = persona generic fallback. HAVEN never goal-echoes (CHECK invariant + code short-circuit). Hydrated by sync.js catalogue plan, 5-min stale window. Edit via Supabase Studio — devices pick up within 5min OR immediately after `CATALOGUE_INVALIDATION_KEY` bump. `hydration.js` `FALLBACK_COPY_TABLE` carries cold-paint defaults; keep in lockstep when finalising. |
| `how_to_resources` | How-to library catalogue. `kind IN ('pdf','video')` discriminator drives which sub-page surfaces each row (`how-to-pdfs.html` / `how-to-videos.html`). Added PM-377 (25 May 2026). Empty by design — Lewis adds rows in Supabase Studio. CHECK enforces payload validity (PDF rows need `url`; video rows need `url` OR `youtube_video_id`). Render paths show empty-state when no active rows. |
| `podcast_platforms` | Podcast platform links catalogue. One row per platform (Spotify / Apple Podcasts / Amazon Music initially; designed for future Pocket Casts / Overcast / YouTube Music adds). Added PM-378 (25 May 2026). Drives the `.hero-listen` chip row on `podcast.html`. Lewis edits URLs, `display_name`, or `display_order` in Supabase Studio — devices pick up within 5min via `CATALOGUE_FRESH_TABLES`. UNIQUE on `slug`, CHECK on `url`/`display_name`/`slug` non-empty. `icon_url` nullable per §23.49 for future iconography. `podcast.html` `FALLBACK_PLATFORMS` carries cold-paint defaults — keep in lockstep when finalising any URL change. |
...
| `movement_activities` | **PM-307 (25 May 2026) — Body section's Movement track.** Mirror of `mind_activities` shape: `id` uuid PK, `member_email`, `activity_date`, `day_of_week`, `time_of_day`, `logged_at`, `kind` (CHECK ∈ walk/stretch/yoga/mobility/pilates/other), `duration_minutes`, `duration_seconds`, `distance_km` (null when not a walk), `notes`, `client_id`, `focus_slug`, `source` default 'manual'. Member-scoped RLS single ALL policy. Triggers: `set_activity_time_fields` BEFORE INSERT, `vyve_lc_email` BEFORE INSERT/UPDATE, `home_state` dirty trio. No charity/counter triggers v1 — Movement doesn't yet drive those aggregations; layer in when needed. Backfilled 24 rows from `cardio.logged_via='movement'` (12) + `workouts.plan_name='Movement'` (12), source rows deleted with charity/counter triggers DISABLED to preserve aggregates. Writes from `movement.html` only; publishes `movement:logged` / `movement:failed` bus events. Replaces the two-table routing of PM-159 + PM-169. Supabase migration name `pm_304_create_movement_activities` — historical artefact from numbering churn during the parallel session; the canonical reference is brain PM-307. |
...
| 374 | `761a027c` | Food log entry-point locked Coming Soon (soft lockdown — `nutrition.html` CTA + `log-food.html` cover) |
...
**PM-377 (25 May 2026, vyve-site `c00a24ca`, supabase migration `pm377_create_how_to_resources`).** How-to library catalogue shipped. `public.how_to_resources` table (kind discriminator drives `how-to-pdfs.html` / `how-to-videos.html`); both pages rewritten from 2.7KB Coming-Soon placeholders to ~8KB / ~12KB catalogue-read surfaces with empty-state fallback. SCHEMA_V19 + sync.js catalogue plan entry + invalidation-key bump `pm375-member-prompts` → `pm377-how-to-resources`. PDF rows tap-open in new tab; video rows tap-open in YouTube embed modal with `env(safe-area-inset-*)` close button per §23.72, event-delegated tap handler (PM-367 pattern), ESC + scrim-click + button all close. CHECK constraint on the table catches incomplete payloads (`pdf` rows require `url`; `video` rows require `url` OR `youtube_video_id`) — validated by the migration itself bouncing my bad-seed test data before commit. 7-file atomic via Git Data API. sw cache `pm376-...-a` → `pm377-how-to-resources-a`. vbb 260 → 261 both marker files. node --check clean on db.js + sync.js + both new pages. First-100 + MD5 verify clean on all 7 files. Sixth catalogue migration on the §23.49+§23.50 pattern — fully muscle-memorised. **No new §23 hard rule earned.**

**PM-378 (25 May 2026, vyve-site `abcf3d3e`, supabase migration `pm378_create_podcast_platforms`).** Podcast platform links catalogue shipped. `public.podcast_platforms` table replaces the three hardcoded Spotify / Apple Podcasts / Amazon Music `<a>` tags in `podcast.html` `.hero-listen`; chip row now renders from Dexie ordered by `display_order` with `FALLBACK_PLATFORMS` const carrying the same three rows for cold paint + safety. SCHEMA_V20 + sync.js catalogue plan entry + invalidation-key bump `pm377-how-to-resources` → `pm378-podcast-platforms`. UNIQUE on `slug` + CHECK non-empty on `url`/`display_name`/`slug` (Studio-safe — won't accept half-filled rows). `icon_url` nullable per §23.49 (future iconography ready, v1 chips text-only). Render path: `renderPlatforms(rows)` sorts by `(display_order ASC, slug)` and replaces `#hero-listen` innerHTML in one assignment — no layout shift; `loadAndPaintPlatforms()` reads Dexie with safety-fallback to FALLBACK_PLATFORMS if VYVELocalDB isn't ready, Dexie returns empty, or read rejects; `boot()` wires platforms repaint alongside episodes repaint, listens for `podcast_platforms` table-pulled. Cold paint synchronous: `renderPlatforms(null)` fires at script-execution before Dexie initialises. Hero copy (eyebrow/title/subtitle/CTA/coming-soon-note) NOT migrated — out of audit-queue scope; sibling `podcast_meta` table reserved for future ask. 6-file atomic via Git Data API. sw cache `pm377-...-a` → `pm378-podcast-platforms-a`. vbb 261 → 262 both marker files. db.js + sync.js + podcast.html inline JS all node --check clean. First-100 + MD5 verify clean on all 6 files. **§23.58 caught a parallel-session brain drift + §23.70 retro logged a cross-repo PM-NUMBER COLLISION** — parallel `VYVEHealth/VYVEBrain` commit `451f849` titled "PM-378 — Architecture map (staging)" landed in `brain/staging/architecture-map-pm378.md` ~13min before my vyve-site `POST /git/commits`. vyve-site `abcf3d3e` keeps PM-378 (permanent in commit msg + 6 source files + migration name + sw cache + invalidation key); parallel brain staging artefact proposed for rename to `architecture-map-pm378b.md` at next housekeeping. §23.70 currently scans one repo; cross-repo PM-namespace cases out of grep window — possible extension candidate banked, not promoted to a new rule on first occurrence. Seventh catalogue migration on the §23.49+§23.50 pattern — fully muscle-memorised. **No new §23 hard rule earned.**

## 19. Current status — 26 May 2026 PM-411 (Park entry, no vyve-site changes. Two items locked for Dean Thursday-pickup ahead of Pro 20x weekly limit reset. (1) Bundle-prep prompt READY at `/mnt/user-data/outputs/bundle-prep-prompt.md` — single conversational prompt loads brain → reads vyve-capacitor-mac-sync playbook → surfaces 7 named bugs in vyve-capacitor remote `4f5f55ae` PM-250 (iOS LaunchScreen.storyboard root bg white, Android ic_launcher_background.xml white, **capacitor.config.json is DEV-LOOP MODE not bundled — contradicts brain memory #4 and §23.42, requires Dean architectural decision before any other Capacitor work**, Android versionCode 1/versionName 1.0 needs bump beyond Play Console live state, Android styles.xml Light.DarkActionBar base theme contributes to bright first-load, package.json no @capawesome/capacitor-live-update committed though Mac local has it installed per Xcode Package Dependencies panel, Android Health Connect AndroidManifest missing permissions + queries block + Gradle SDK + healthbridge.js Android branch — plugin @capgo/capacitor-health@8.4.7 is present). Walks the 9-step playbook workflow (Mac diagnose → stash → ff-pull → cleanup → bug fixes → asset regen → rsync www/ → cap sync → keystore safety + 1Password upload → Xcode Archive + ./gradlew bundleRelease → App Store Connect + Play Console submission). Project rules embedded: memory #7 native-not-browser, Lewis no emojis. (2) Body-hub overhaul campaign documented from deanonbrown2@gmail.com onboarding walk this session. Three bugs surfaced: **Bug A (architectural, 4-6h, post-trial)** Movement plan structurally homeless — exercise.html L350 hardcoded `href="workouts.html"` regardless of programme category; SQL audit confirmed every workout_plan_cache row has `category: null` so no branching surface exists; fix needs programme_library category backfill + onboarding EF v37 to write category + exercise.html branching + movement.html programme-card section (currently only has session-logging pills + Mark as Done button, no programme display block; movement.html L440-486 already filters category === 'movement' so it's READY to consume categorised plans, nothing writes them). **Bug B (surgical, 30-45min, Thursday)** activateProgramme in workouts-library.js correctly clears localStorage + nulls programmeData + cacheRow + calls loadProgramme, but loadProgramme's PF-7 Dexie local-first path at workouts-programme.js L78-89 calls VYVESync.criticalHydrate UN-AWAITED then reads Dexie immediately — Dexie still has OLD plan because sync hasn't propagated. renderProgramme paints OLD plan. Fix: await criticalHydrate, OR skip Dexie when called from cache-bust context, OR invalidate Dexie row before reading. Single-file edit. **Bug C (surgical, 30-60min, needs device console)** Browse Library tab broken at runtime. Static check clean (switchTab + loadLibrary + DOM elements + RLS + 30 programmes + CATEGORY_LABELS + loadPausedPlans all confirmed present). Failure is runtime JS error swallowed by outer try/catch. Candidates: getJWT undefined for new accounts, VYVEData API drift, async error in renderLibrary first-paint. **Schema-architecture note banked NOT codifying solo**: workout_plan_cache has contradictory UNIQUE indexes — `workout_plan_cache_member_email_key` (UNIQUE on member_email, no WHERE) blocks paused-plan multi-row design that `workout_plan_cache_one_active_per_member` (UNIQUE on member_email WHERE is_active=true) assumes works. workout-library EF v13 paused-plan logic at L60-84 likely never functions correctly — upserts at L98-110 onConflict:'member_email' silently overwrite due to broader UNIQUE constraint. Empirically confirmed on test account: 1 row only, no paused Movement preserved. Promotes to §23 on second contradictory-UNIQUE occurrence. **Brain HEAD post-commit PM-411. Cross-repo max=411. State preserved for Dean's Thursday return. §23.58 collision check caught parallel `d58e6a2b` PM-409.b debug surface gating brain ship landing after my PM-410 brain close earlier — no content collision, recomputed claim to PM-411 uncontested. PAT-direct via Vault throughout, Composio still down ~6 days. Atomic 3-file commit via Git Data API with §23.41 sha256-MATCH verification.)

## 19. Current status — 26 May 2026 PM-413 (iOS 1.4 + Android 1.0.5 BOTH SUBMITTED TO APP REVIEW. Marathon bundle session execution following the PM-411 bundle-prep prompt. vyve-capacitor PM-412 storyboard fix (iOS splash background → VYVE Dark `#0D2B2B`) already landed in remote earlier in the evening at `7a54c876`. Tonight's work was Mac-local recovery + bundle + submit, all of which lives on Dean's Mac uncommitted to the vyve-capacitor remote (audit-and-curate pass pending — see PM-413 changelog Pending #2). **iOS 1.4 build 3** in App Store Connect: sidebar `1.4 Waiting for Review`, build 3 attached, encryption set Standard (Dean's call against my research suggesting None for HTTPS-only apps), auto-release on approval, 24-48hr expected. 1.3 sidebar entry `Ready for Distribution` will auto-supersede on 1.4 approval. **Android 1.0.5** versionCode 50 in Play Console: AAB accepted, 4 non-blocking warnings (22 phones / 5 tablets / 2 TVs dropped at 0.2% device coverage; ~7MB bundle size increase from new content; no R8/ProGuard deobfuscation file; no native debug symbols), production release saved. As of brain commit time the "Send 1 change for review" button is staged in front of Dean — confirm next session that Play Console shows `In review` not `Draft`. **New §23.76 hard rule earned** (iPad orientation 4-array, iOS Code 90474). **Two banked-not-codifying-solo candidates**: (1) Android Studio Generate Signed Bundle dialog caches Windows-style destination paths across machines via IDE config sync — first build on a new machine must visually verify keystore + destination + variant before pressing Next; promotes on second occurrence. (2) `www/` refresh from current vyve-site SHA must be an explicit checkbox step in Capacitor session checklist, not an inferred one — Dean caught Claude almost telling him to Archive without it; promotes on second occurrence. **App Review notes correction**: Dean's pre-existing iOS App Review notes contained "PWA-based wellness app" framing inherited from a pre-1.2 submission. Per §23.20 the product is no longer a PWA. Submitting with PWA framing risks Apple Guideline 4.2 (Minimum Functionality / thin web wrapper) scrutiny. Dean rewrote the note pre-submission. Brain doctrine recap: future submissions describe VYVE as "native iOS app built with Capacitor wrapper". Worth scanning future submissions for residual PWA framing. **Mac local at session end** has uncommitted changes to `capacitor.config.json` (still hybrid dev-loop + LiveUpdate, contradicts §23.42), `android/app/build.gradle` (versionCode 50), `ios/App/App/Info.plist` (iPad 4-orientation restoration), `package.json` + `package-lock.json` (LiveUpdate dep committed via SPM in Xcode but not in package.json), `ic_launcher_background.xml` (`#FFFFFF` → `#0D2B2B`), regenerated mipmap PNGs. Remote `7a54c876` is missing all of this — selective audit-and-curate commit pending future session. **Brain doctrine correction**: assumption "vyve-capacitor remote is source of truth" was wrong this session — Mac local was the actual source of truth for what shipped previously (Xcode Package Dependencies + working-tree state). Going forward: remote should match Mac-local ship-state after every bundle session via curated atomic commit. **Working-style notes**: Claude over-architected Bug 3 into a full dual-channel Capawesome doctrine at session open; Dean pushed back three times before reset. Bundle session ≠ deployment pipeline redesign. Pre-existing memory held — Claude drifted, recovered. Session got tense mid-flow; Claude held brief response per memory #1, moved forward. **Tooling**: PAT-direct via Vault per memory #8 (Composio still down since 21 May incident, ~6 days). §23.41 fresh-HEAD checked on all 3 repos before brain commit. §23.58 collision-recovery clean (brain HEAD unchanged at `00fbf558` PM-411). §23.66 + §23.74 cross-repo scan: vyve-site max=410, vyve-capacitor max=412, brain max=411 — claim PM-413 uncontested. §23.69 zero parallel-feature collisions on opening brief. Brain HEAD at session end: post-this-commit PM-413. Cross-repo max PM = 413.)

## 19. Current status — 26 May 2026 PM-410 (Replay catalogue wipe via YouTube archive + refresh-replay-videos v2 reconciliation. All 33 test-content replay videos across 8 YouTube category playlists archived into a new private playlist `PLyaCafiXVssjqWqYpqntk3kdjb0BSCOIL` ("VYVE Archive — Dev & Testing (pre-launch)", `privacyStatus: private`, chronological by publishedAt ASC), source playlists emptied via `playlistItems.delete`, ghost "Deleted video" row in yoga-stretch separately cleaned. Three throwaway EFs deployed for the work: `replay-inventory-tmp` (read-only enumeration), `replay-archive-tmp` (creates archive playlist, copies items, deletes from sources with safety guard that skips delete if insert failed), `replay-ghost-cleanup-tmp` (one-off dangling-item delete). All 3 ACTIVE but dormant in Supabase — Dean to delete via dashboard when convenient (MCP doesn't expose delete-EF tool). **refresh-replay-videos v2 shipped** (function `b776b6a7-...`, was v1 PM-241 ship 23 May): added reconciliation step `DELETE FROM replay_videos WHERE youtube_video_id NOT IN (<all seen IDs across all 8 playlists>)` after the per-playlist upsert loop. Reconciliation runs ONLY if all 8 playlists fetched cleanly — failure on any playlist preserves existing data (loss of completeness, not loss of data). Edge case: zero-videos-seen triggers full wipe via `.neq("youtube_video_id", "")`. Also fixed pre-existing bug where empty-playlist branch only zeroed `video_count` but left stale `latest_video_id/title/thumb_url/published_at/duration_sec` — now all 5 cleared. v1 was upsert-only; this closes the permanent architecture gap where YouTube-side deletions never propagated to Supabase. Manual invocation post-archive: 200 OK, 8/8 playlists clean, 0 upserts, reconciliation ran, all 30 `replay_videos` rows deleted via the NOT IN clause, FK ON DELETE CASCADE wiped all 4 `replay_video_views` rows automatically. Legacy `replay_views` (no FK to replay_videos, still UNIONed into `refresh_member_home_state` per PM-294) wiped separately via direct DELETE (10 → 0). `session_live_views` was already empty. **Member-visible result**: Replays hub shows empty state across all 8 categories on next page load. `connect.html renderLatestReplay` (reads `replay_playlists.latest_video_*`, now all null) shows no latest-replay card. PWA Dexie catalogue sync propagates the empty state to all members on next sync. Members who watched test replays during dev had their attribution rows cascade-wiped — clean engagement-score recalculation on next dirty-mark. **§23 candidate banked NOT codifying solo**: upsert-only sync without reconciliation = stale-row class bug whenever upstream supports deletion. Promotes on second recurrence — audit signal is any new sync EF against a deletable upstream missing the reconciliation pattern from refresh-replay-videos v2. **Tooling**: pg_net for EF invocations, Supabase MCP for SQL + EF deploys, PAT-direct via Vault for the brain commit (Composio still down per memory #8). OAuth scope confirmed `https://www.googleapis.com/auth/youtube` (master §24, no rotation needed this run). Quota usage ~3450 units across all 3 throwaway EFs (well under 10K daily). **§23.58 collision recovery at brain commit**: initial work-name PM-407, brain HEAD moved to `fa7a6fa4` PM-409 (parallel sessions shipped PM-406 my own offline fix + PM-407 Facebook scrub + PM-408 analytics + PM-409 Facebook scrub continuation between EF deploy and brain commit). Claim recomputed to PM-410, ~30s wall-clock cost. Brain HEAD at session end: post-this-commit PM-410. Cross-repo max PM = 410.)

## 19. Current status — 26 May 2026 PM-408 (Analytics taxonomy ship pre-bundle. PostHog identity wiring (master §5 stale claim) actually live since PM-18 — but only with `{email, name}` and no event taxonomy beyond `portal_page_viewed` / `live_session_accessed` / `replay_page_accessed`. PM-408 lands the real instrumentation before iOS 1.3 / Android 1.0.3 OTA bundle ships, so bundled members start producing cohort-attributable analytics from day one. **New file `analytics.js`** (267 lines, precached via sw.js): central PostHog bridge subscribing to 29 VYVEBus events at `envelope.origin === 'local'` filter — no double-counting from cross-tab `remote` or cross-device `realtime` echoes of the same physical user action. Polls `window.posthog` + `window.VYVEBus` for up to 5s, no-ops if unavailable. Allowlisted payload field set (~30 keys: kind, source, client_id, durations, scores, reactions, etc) keeps capture payloads small. One PostHog event per envelope, event name = bus name with `:` → `_` for PostHog UI cleanliness. `window.vyveAnalyticsCaptureEFError` exposed for the EF wrapper. `window.vyveAnalyticsCapture(eventName, props)` exposed for one-off page-local captures (future use: tour completion, settings toggles, modal dismissed). **Mirrored events (29)**: habit:logged/failed, workout:logged/failed/shared, cardio:logged/failed, movement:logged/failed, mind:logged/unlogged/failed, session:viewed + viewed:failed, replay:viewed, wellbeing:logged/failed, monthly_checkin:submitted/failed, food:logged/deleted/failed, weight:logged/failed, persona:switched, connect:checkin:logged/failed, connect:reaction:logged/cleared. **Deliberately NOT mirrored**: body:logged (aggregator over workout/cardio/movement — would double-count), set:logged (per-set high-volume, summary already at workout:logged), habits:set-changed/failed (config not activity), avatar:changed / specific_goal:changed / programme:imported / focus:write_failed (low signal or already-internal). Curated from `grep VYVEBus.publish(` whole-tree audit at HEAD `20a23f7d` (37 distinct event names live; 86 publish call sites). **auth.js v2.4 → v2.5**: `vyveCapturePageView()` identify enriched from `{email, name}` to `{email, name, given_name, family_name, is_dean, host_kind}`. `is_dean` is case-insensitive match on deanonbrown@hotmail.com — for filtering Dean's dev traffic out of dashboards. `host_kind` splits `capacitor://` (bundled native) from `https://` (web fallback). New `vyveAnalyticsRefreshIdentity(email)` reads `vyve_members_cache_<email>` localStorage row populated by `_vyvePfMembers` and pushes persona/goal_focus/gender/age/activity_level/first_name/last_name via `posthog.setPersonProperties`. Scheduled at 2s/5s/10s after first identify because the eager-prefetch idle-callback path may not be ready immediately; `setPersonProperties` is write-on-change so repeated calls are no-ops. New `window.vyveEFFetch(fnName, url, opts)` wraps fetch and fires `ef_error` capture on non-2xx OR network failure with `{fn, http_status, duration_ms, error_code}`. Pass-through error semantics — caller handles failure as before. **EF wrapper adoption is opt-in per call site; not retrofitted across the portal in this ship** (would touch ~20 fetch sites — separate sweep). **sw.js**: cache `pm406-offline-scope-fill-a` → `pm408-analytics-taxonomy-a`. `/analytics.js` added to urlsToCache for offline cold-boot support. **vbb-marker 285 → 286 in BOTH index.html and settings.html per memory #10**. **45 HTML pages got the analytics.js `<script>` tag**: inserted right after the existing `<script src='/bus.js' defer></script>` preserving local style (rooted `/analytics.js` vs relative `analytics.js`, same indentation). perf-test.html skipped (dev surface, no bus.js). **Quota math**: 15-member cohort × ~20 actions/day × 30 days = ~180k events/month. PostHog free tier 1M events/month — comfortable headroom for trial + several months post-launch. Session recording stays at 100% (configured in auth.js posthog.init); flip to sampled in PostHog dashboard ahead of OTA bundle if needed. **Banked NOT shipped (deliberate, follow-up)**: index.html lines 1043-1046 contain a duplicate `posthog.init` inline block that pre-empts auth.js's deferred init (`__SV=1` flag set first by inline), so the session_recording config in auth.js is NEVER applied on index.html. Removing changes `window.posthog` availability timing window between body parse and auth.js defer — safer as its own ship with incognito verification. Logged on backlog. **Tooling**: PAT-direct via Vault (Composio still down since 21 May incident, ~6 days). §23.41 fresh-HEAD checked twice — pre-tree-create (HEAD `20a23f7d`) + pre-ref-update (same). §23.52(a) all 48 blob bodies via Python urllib raw-bytes (no argv overflow path even for the 1102-line auth.js post-patch). §23.69 collision scan: zero analytics/posthog landings in last 12 vyve-site commits. §23.70 PM-claim recomputed at commit time (vyve-site max=406, brain max=407, claim 408 uncontested). §23.74 cross-repo scan clean. All inline JS blocks in changed surfaces (45 HTMLs) + standalone files (auth.js, analytics.js, sw.js) `node --check` clean. Post-commit MD5 verification on all 48 files at commit SHA matched local. 48 files atomic at vyve-site `e75b2d7a`. **No new §23 hard rule earned** — standard atomic-multi-file ship using already-codified primitives. **Bundle readiness**: pre-bundle hygiene continues. Dean's intent was to ship before Capacitor bundle freeze so bundled members produce analytics on first launch.)

## 19. Current status — 26 May 2026 PM-405 (Bottom-nav opacity split on Connect [PM-403 vyve-site `ff5cd4f4`] + offline-scope audit pre-bundle. Two pieces this session: (1) PM-216 doctrine's body-scoped `--nav-bg-mob` override on connect.html L53 was making the topbar frosted-glass AS INTENDED but also dropping the bottom nav from theme.css default 0.97 → 0.42 because nav.js L148 `.vyve-bottom-nav` and L131 `.mobile-page-header` were both reading the same `--nav-bg-mob` token. Dean caught on pre-bundle device walk: Connect bottom nav reads see-through while Home/Body/Mind read opaque. **Fix shape**: split the token rather than override on Connect. Added `--bottom-nav-bg-mob: rgba(8,24,24,0.97)` to theme.css `:root` + `[data-theme="light"]` blocks (defaults identical to `--nav-bg-mob` so zero visual delta for non-Connect pages); swapped `.vyve-bottom-nav` background var-name in nav.js L148. Topbar `.mobile-page-header` L131 still reads `--nav-bg-mob`. Connect body-scoped override at L53 left untouched. 5 files atomic at vyve-site `ff5cd4f4`: theme.css + nav.js + sw.js (cache `pm401-recent-checkins-same-layout-skip-a` → `pm403-bottom-nav-bg-split-a`) + index.html (vbb 283 → 284) + settings.html (settings-vbb 283 → 284). `node --check` clean on all 15 inline JS blocks + standalone files. §23.41 sha256-MATCH on all 5 files at commit SHA via Contents API raw accept. §23.66 + §23.74 fired clean (vyve-site max=401 / brain max=402, claim PM-403 uncontested at vyve-site commit time). PAT-direct via Vault per memory #8 (Composio still down). (2) **Pre-bundle PF-14c offline audit** against `ff5cd4f4` / vbb 284. Two real findings: **22-file precache gap** (8 live-broadcast shells, 8 replay-category shells, 4 new mind/more surfaces meditation/sleep/how-to-pdfs/how-to-videos, 2 session CSS files session-live.css + session-rp.css) — uncached HTML falls back to `/index.html` per SW handler L370 (graceful, not a hang, but premium-feel regression: tap a session card offline → land on home not on the surface). Root cause: PM-251 + later ships added new HTML surfaces without sw.js precache additions in same atomic commit. 3 days drift. **wellbeing-checkin.html injects Chart.js from cdnjs at runtime** (L905 + L1419) — PF-14c §2a violation. SW correctly skips cross-origin (L338), so offline = blank chart canvas with no error UX. Not critical-path (both inside chart-render helpers, no `await` on script.onload, page doesn't hang) but invisible chart breaks premium feel. **Fix shape — PM-406 single atomic vyve-site commit, pre-bundle**: sw.js precache 22 new paths + cache key bump `pm403-...` → `pm406-offline-scope-fill-a`; vendor `chart.umd.js` locally (~210KB) + add to precache; swap both Chart.js URLs in wellbeing-checkin.html; vbb 284 → 285. **Two §23 candidates banked NOT codifying solo**: (a) sw.js precache must be audited on every new HTML surface ship — audit signal: any vyve-site commit adding HTML file in repo root should have sw.js diff in same atomic commit. Promotes on second drift episode. (b) Cross-origin runtime injection on member-facing surfaces is a PF-14c violation regardless of critical-path status — audit signal `grep -rn "script.src.*http" *.html` should return zero on member-facing surfaces. Promotes on second occurrence. Full per-surface audit doc at `/audit/pm404_offline_audit.md` in this session (filename PM-404 is pre-collision working name, canonical PM is 405). **Capacitor block parked** — Dean said vyve-site not bundle-ready yet ("couple of hours"); Capacitor work (portrait lock verify, asset regen, Health Connect wire-tonight per Dean) resumes once vyve-site freezes SHA. Investigation found: vyve-capacitor remote `4f5f55ae` PM-250 has portrait lock + bundled-mode config correct, but Dean's local Mac at `3432aab` is 1 commit BEHIND with 12 modified working-tree files + 90+ untracked www/ + `--exclude=.git` + `--exclude=.github` junk files from earlier session typo. Source PNGs clean (icon.png 1024×1024 RGB no-alpha, splash.png 2732×2732 RGB no-alpha). Health Connect plugin (`@capgo/capacitor-health@8.4.7`) and Browser (`@capacitor/browser@8.0.3`) already in package.json. AppDelegate.swift exists for APNs bridge. Recovery plan locked + codified as new playbook `playbooks/vyve-capacitor-mac-sync.md` this commit: stash + ff-only pull + cherry-pick stash review + Health Connect wire + bundle. **§23.58 collision recovery this session**: initial PM claim 404 composed against brain `47b2af50`; final §23.58 check before commit caught parallel session shipping `0fb09c9b` PM-404 (In-App Tour PF-23 elevation) ~minutes before my commit. Replayed patches with PM-404→PM-405 sed sweep, no master §19 drift, ~120s recovery cost. §23.58 doing its job. **Brain HEAD at session end**: `0fb09c9b` PM-404 In-App Tour. PM-405 mine. PM-406 reserved for offline scope fix ship.)

## 19. Current status — 26 May 2026 PM-402 (Broadcast push infrastructure shipped end-to-end. Lewis-facing manual broadcast UI live at `admin.vyvehealth.co.uk/#/broadcast` (Command Centre `pages/broadcast.html`, 21.5KB, sidebar entry in Delivery section between Sessions and Tasks) + scheduled-push cron rails (no UI v1 per Dean's call). **SQL surface**: `is_admin()` SECURITY DEFINER RPC (was pending dependency in `lib/auth.js`, now enforceable — three admins active: dean/lewis/team), `admin_broadcast_log` table with admin-RLS-gated SELECT, `broadcast_schedules` table (renamed from `scheduled_pushes` to avoid collision with the pre-existing per-member one-shot reminder table — brain didn't surface that collision, learned at first migration attempt), `resolve_broadcast_audience(jsonb)` SECURITY DEFINER resolver supporting 6 audience shapes (`all`, `inactive` with N-day cutoff including never-active members, `company` case-insensitive, `company_slug`, `email`, `emails[]`) — single source of truth for UI count-preview + manual EF + cron runner. **Two new EFs**: `admin-broadcast-push` v2 (verify_jwt:true, decodes caller JWT for email, calls `is_admin` RPC under that JWT as defence-in-depth Layer 2 — front-end gate is Layer 1; resolves audience via service-role RPC; fans out via `send-push` in 100-email batches; audit-logs to `admin_broadcast_log` under `EdgeRuntime.waitUntil`; supports `mode='preview'|'send'` so the UI can show real audience count before firing) and `scheduled-push-runner` v2 (verify_jwt:false; pulls active rows from `broadcast_schedules` inside `[active_from, active_until]`; `isDue` matches recurrence vs current minute in row's `tz` with 5-min slot tolerance and 6-min duplicate-fire guard; daily + weekly recurrences supported v1, jsonb shape future-proof; same audience resolver + send-push batching + audit log path as manual). **New pg_cron job 28** `vyve-broadcast-scheduler` every 5min via `net.http_post` to scheduled-push-runner. **Auth-shape gotcha banked NOT codified — first occurrence**: post-key-rotation `SUPABASE_SERVICE_ROLE_KEY` env in EFs is the new `sb_secret_*` publishable shape; `send-push` v13's `Bearer` equality check needs JWT-format `LEGACY_SERVICE_ROLE_JWT` instead. First runner deploy got `401 UNAUTHORIZED_INVALID_JWT_FORMAT`; both new EFs redeployed at v2 with the correct env. The pattern is documented inline in `achievement-earned-push` v2 source. Promotes to §23 on second occurrence — for now the discipline note: any EF→EF call where the callee has `verify_jwt:true` + `Bearer === SERVICE_KEY` equality check needs `LEGACY_SERVICE_ROLE_JWT`, not `SUPABASE_SERVICE_ROLE_KEY`. **Smoke test end-to-end green**: seeded one `broadcast_schedules` row scoped to Dean's email with London-time slot at 17:38, fired runner via pg_net, result `schedules_seen:1, fired_count:1, recipient_count:1, in_app_written:1, native_banners_sent:1, web_banners_sent:4`. All four channels exercised: `member_notifications` row written with full route `/index.html`, `admin_broadcast_log` row id=2 (source='scheduled', schedule_id=1, error=null), APNs banner to Dean's iPhone Capacitor receiver, VAPID web push to 4 legacy web subs (brain noted as dormant since 15 April — they fanned out cleanly, useful confirmation web path still works). Smoketest row deactivated post-verification. **Command Centre ship**: `vyve-command-centre@291a21cf` — 2 files atomic via Git Data API per §23.52(a) Python urllib (`pages/broadcast.html` + `assets/sidebar-config.js`). §23.41 first-100-bytes verified at commit SHA on both files — MATCH/MATCH. `node --check` clean on inline JS (10736 chars) + sidebar config. UI shape: title (80 char) + body (240 char) + optional deeplink route + push_type tag + 4-mode audience picker (all/inactive/company/email) + iOS-style live notification preview card + "Preview count" button (returns resolved count + 5-email sample) + "Send broadcast" button with JS confirm + "Recent broadcasts" list of last 20 audit rows. Gate states: notready/signin/denied/ready — denied state shows when `is_admin` RPC returns false. **Out of scope v1 (parked)**: scheduler creation UI in Command Centre (rails ship dark — adding a schedule = inserting a row in Supabase Studio with title/body/audience/recurrence; UI lands when Lewis has content to schedule); "pool of quotes" infrastructure for morning quotation (Lewis decides content shape, v1 schedule rows take static title+body); Android FCM banners (standing backlog #6, Android members get in-app rows + correct tap routing, no system banner); same-day dedupe on broadcasts (default false — intentional, admin firing a broadcast knows what they're doing, toggle exists in EF input contract but not surfaced in UI v1); custom audience JSON editor (power user surface, defer until use case arrives). **Operational implications for Lewis**: UI at `admin.vyvehealth.co.uk` → Delivery → Broadcast. Workflow: compose → audience → Preview count → Send → fan-out. Recent broadcasts list shows last 20 with recipient counts + source (manual vs scheduled) + caller email. Lewis can target a single member by email — useful for personal nudges. **Operational implications for Dean**: adding a recurring schedule = INSERT into `broadcast_schedules` via Supabase Studio. Required cols: `slug` UNIQUE, `title`, `body`, `audience` jsonb, `recurrence` jsonb, `is_active true`. Recurrence shapes: `{type:'daily',hour:7,minute:0,tz:'Europe/London'}` or `{type:'weekly',hour:7,minute:0,tz:'Europe/London',days:['MO','WE','FR']}`. Audit log shows whether the schedule actually fired and how many recipients got it. **No new §23 hard rules earned tonight** — auth-shape gotcha banked as first-occurrence note above.)

## 19. Current status — 25 May 2026 PM-401.b (Connect-page flicker arc closed in THREE atomic vyve-site ships after Dean reported residual surfaces post-PM-398: PM-399 `718678f4` Your Journey snapshot first-paint, PM-400 `85fd00d2` live carousel background-image + reaction-tap site surgical patch, PM-401 `e02304a3` renderRecentCheckins same-layout guard skipping innerHTML rewrite. Originally framed as the "Chat 2 Body snapshot" item from PM-398.b's NEXT FOCUS but Dean confirmed Body / Mind don't flicker — `exercise.html` already has its own PM-96 `paintCacheEarly` from `vyve_exercise_cache_v2` localStorage (snapshot-first pattern predates PM-394 architecture), `mind.html` doesn't show the shape. Real residual surfaces were Your Journey LOADING hang + Connect reaction-tap avatar flicker + live carousel image flicker. **PM-399 — Your Journey snapshot first-paint**: Dean tabbed into "My Progress" from Home post-PM-398 walk and saw score "—", band label "Loading", pillar list with "—" + "0/30" defaults, all pillar counts showing 0 for 30+ seconds before any data appeared. Page felt broken because subsequent taps were queued behind the in-flight render. Root cause: `refreshScore()` does heavy 10-table Dexie read via `computeHomeStateFromDexie(email)` then computes full engagement score + multipliers + per-pillar points + tile counts + streak fields. Warm Dexie 50-200ms; on iOS WKWebView memory-pressure stalls 1-30 seconds. HTML defaults paint during that window. Same surgical pattern as PM-396 Home snapshot reader: synchronous reader inline at top of `<body>` scans for `vyve_journey_snapshot_<email>` localStorage prefix, stashes onto `window.__vyveJourneySnapshot`. Synchronous paint IIFE inside main script immediately after `renderScore()` is defined — if `__vyveJourneySnapshot.v2` exists, calls `renderScore(snap.v2)` synchronously painting full ring fill + band label/title + multiplier values + bars + details + all 6 pillar rows (Today's Focus, Daily Habits, Body, Mind, Connect, Check-ins) + Activity Breakdown tile counts + streaks from cached numbers. `refreshScore()` success writes freshly-computed v2 object back to localStorage (~500 bytes typical). The 30-second hang scenario now visually impossible: even if `computeHomeStateFromDexie` hangs forever, snapshot already painted real numbers in first frame. **PM-400 — live carousel + reaction-tap site**: two fixes one commit. (a) `paintLiveCarousel()` in index.html + `renderLiveThisWeek()` in connect.html switched scroll-card thumbs from `<img loading="lazy">` to `background-image:url()` on the `.scroll-card-thumb` wrapper div — CSS backgrounds paint synchronously from image cache without lazy-load decode round-trip that produces visible blink-then-image flicker on first paint even when cached. Fallback gradient + stripe pattern still shows when `s.thumb` is missing. (b) `toggleReaction()` in connect.html L1680 was calling `renderRecentCheckins(__lastRecentRows)` synchronously on every reaction tap — full innerHTML rewrite destroying + recreating every `<img>` avatar. Replaced with surgical IIFE that finds the tapped card by `data-checkin=<id>`, iterates `.rx-btn` children, toggles `on` class to match `bucket.my_reaction`, updates `.rx-count` text spans, updates aggregate `.cin-heart-n` + `on` class. Avatars + names + bodies untouched. **PM-401 — the renderRecentCheckins same-layout guard (the actual fix)**: PM-400's tap-site patch ran clean but Dean confirmed avatar flicker persisted on device. Root-cause re-traced: bus chain `connect:reaction:logged` → subscribers at L2210/2214 call `syncReactionStateToCache` (PM-395) + `repaintDebounced()` → 150ms timer → `paintAll()` → Path A (cache hit per PM-393) → `renderRecentCheckins(rows)` → `list.innerHTML = data.map(...).join('')` → all 3 `.cin-card` elements destroyed + recreated → all 3 avatar `<img>` elements destroyed + recreated → browser must re-decode each img from cache → visible avatar flicker even with hot cache (loading=lazy + element replacement = mandatory decode step). PM-400's site-level patch caught the synchronous path but the 150ms debounced bus repaint wiped it. Fix: guard at top of `renderRecentCheckins`. Before innerHTML rewrite, check if existing DOM cards have same IDs in same order as incoming data. If YES (common case — reaction taps don't change row layout, just reaction counts), do surgical update of every card's `.rx-btn` set + `.cin-heart-n` text + `on` classes — avatars/names/bodies/timestamps untouched, no img destruction. If NO (new check-in posted, old one cycled out, cache→EF upgrade with different rows), fall through to existing full innerHTML rebuild. The surgical path covers every reaction tap. Full rebuild only runs on actual layout changes (which the user expects to be a visible repaint anyway). PM-400's site-level patch left in place as defence-in-depth: runs synchronously on the tap (instant feedback, no 150ms wait), PM-401's guard prevents the inevitable bus-driven re-render from undoing it. **Device walk evidence**: Dean confirmed PM-399 "looks alright now" after walking Your Journey twice (first visit honest cold-load, second visit instant from snapshot). Confirmed PM-400 live carousel + Home tap surfaces clean ("nothing that you click on the home screen makes it flicker") but flagged Connect reaction-tap avatar STILL flickering at Update 282 — surfaced root cause to PM-401. Dean confirmed PM-401 fix at Update 283: "fixed". **§23.65 in-page-vs-cross-page distinction reinforced**: PM-391 banked the candidate that envelope-trusted subscribers are for cross-page only, not in-page where publisher already mutated. PM-401 surfaces the SAME class of bug from a different angle — even bare `repaintDebounced()` subscribers (no envelope semantics) can wreck an in-page surgical state mutation by rebuilding the DOM. The fix isn't subscriber-side, it's render-function-side: a renderer that's idempotent over the no-layout-change case naturally absorbs both same-page debounced repaints AND cross-page bus invalidations without flickering. Second occurrence of "audit-all-paint-sites-when-adding-fast-path" (PM-393, PM-395) — promotion criterion met if Dean wants. Suggested codification: any state-change render function that's called from multiple bus events should be idempotent over the no-change case (skip DOM rewrite when current DOM already matches incoming state). **§23 candidate banked NOT codified solo**: "innerHTML rewrite destroys child `<img>` elements forcing re-decode even with hot HTTP cache" — three occurrences in two ships now (PM-400 live carousel + PM-400 tap-site rewrite + PM-401 debounced repaint). Codification rule: when state changes don't affect layout/identity, prefer surgical DOM patches over innerHTML rewrites. Promotes to §23 hard rule on next independent surface ship. **Markers across 3 ships**: vbb 280 → 281 (PM-399) → 282 (PM-400) → 283 (PM-401). sw cache `pm398-habits-instant-paint-a` → `pm399-journey-snapshot-first-paint-a` → `pm400-connect-flicker-images-bg-a` → `pm401-recent-checkins-same-layout-skip-a`. 12 files total atomic across 3 ships (engagement-v2.html + connect.html + index.html × 3 + settings.html × 3 + sw.js × 3). PAT-direct via Vault throughout (Composio still down). `node --check` clean on all inline JS blocks + sw.js on each ship. §23.41 byte-perfect verify clean on every committed file at every commit SHA. §23.74 cross-repo PM scan clean before every commit. No parallel-session ships during the chat. **Premium-feel tab-flicker campaign now effectively done** — Home (PM-396/397/398), Body (already PM-96), Mind (no flicker per device walk), Connect tab-in (PM-391-395), Connect reaction tap (PM-401), Connect+Home live carousels (PM-400), Your Journey (PM-399). View Transitions API wire from PM-394 plan still parked, lower priority now that perceived flicker is gone. Deferred Chat 1 sweep items (keystore 1Password, vyve-capacitor git, debug strip gating) STILL OPEN per PM-398.b backlog block.)

 Chat 1's original brief was cheap-stuff sweep (keystore 1Password + vyve-capacitor git-init + debug strip gating) but Dean pivoted mid-chat away from sweep work toward direct flicker fix — backlog L116 Chat 1 framing accepted that pivot. Chats 2-4 (Body / Mind / Connect snapshot + View Transitions) still owe the hub-by-hub coverage per the original plan. **Bug shape (3 independent flicker classes on the SAME page)**: (1) value mutation — rings ticking up from 0/30 → real values after `loadPillarCounts` 10-table Dexie read 100-300ms after boot, hero name flashing "there." → "Lewis." after `await loadMember()`; (2) layout collapse-then-expand — `#focus-carousel` + `#habit-list` zero-height in HTML before `paintFocus()` + `renderHabitList()` paint, "Live sessions this week" row jumps up to fill the gap then jumps back down when content lands; (3) content-late — even with layout reserved, "Today's habits" label shows but the slot is blank for ~250ms before the empty-state CTA or habit rows appear because both `#habit-list` AND `#habit-empty` were default-hidden waiting for JS to decide which to show. **PM-396 ship — value mutation**: synchronous snapshot reader inline at top of `<body>` (immediately after the build-banner script) scans localStorage for `vyve_home_snapshot_<email>` prefix (one key per device per logged-in member, first match wins, no separate "current email" key needed), parses JSON, stashes onto `window.__vyveHomeSnapshot`. Runs before any frame paint. `PROGRESS_TRACKS` module-init seeds `.value` + `.certs` from `snapshot.pillarCounts` BEFORE `renderPills()` called from `boot()` — first paint draws real ring values. `paintNameFromSnapshot()` IIFE at script-parse reads `snapshot.memberName` and writes synchronously to `#ih-name`. `loadPillarCounts()` tail writes pillarCounts back to snapshot — single write site covers all 18 bus events that fire `_rerenderHome` (no per-publisher hooks needed). `boot()` `loadMember` success writes `memberName`, merge-with-existing preserves pillarCounts. Stale-by-1-boot acceptable because async paths overwrite on every render. First-launch no-snapshot path falls through to honest empty paint — no regression. **PM-397 ship — layout shift**: 2-line CSS only, no JS. `.focus-carousel min-height: 190px` reserves focus-card aspect-ratio 16/9 slot at ~338px (86% viewport on 393px iPhone). `.habit-list min-height: 204px` reserves ~3 habit rows (60px × 3 + 8px gap × 2 + 4px breathing). Members with 0 habits unaffected because `renderHabitList()` sets `.habit-list display:none` and shows `#habit-empty` sibling — min-height doesn't apply to display:none, so empty-state CTA doesn't get pushed down. **PM-398 ship — content late**: two changes. (1) Default visibility inverted on the markup — `#habit-empty` no longer `style="display:none"`, visible by default with the CTA; `#habit-list` now `style="display:none"` by default. `renderHabitList()` flips them when habits actually exist. Members with 0 habits (new accounts, test accounts) see the empty-state CTA in first paint with zero JS dependency. (2) Tiny inline `<script>` immediately after the markup — self-contained, doesn't depend on the main script's `renderHabitList()` being defined yet. Reads `window.__vyveHomeSnapshot.habits` + `.habitsDate`, sorts uncompleted-first, caps visible at 5 per PM-291 overflow rule, hand-rolls a minimal row HTML structure matching `renderHabitList()`'s output (no Lucide glyph in the icon tile — main script's `renderHabitList()` overwrites with the Lucide on next pass, icon-only swap is much subtler than empty→rows). Date-gate: on a new day clears `doneToday` to false on all rows (assignments stay valid, ticks reset, natural alignment with `daily_habits` 1/day cap). `loadHabits()` tail extended to write `__habits` + `habitsDate: todayStr()` + `writtenAt` to home snapshot. **Backlog L120 vs reality**: the backlog block lists Home snapshot scope as "activity ring + components, weekly goals 5-row tick state, daily check-in pill strip + streak, progress tracks 5-pillar, collective impact banner, hydration overlay state, mood check-in panel state, member prompts dismiss/cooldown state." But the PM-256 redesign at index.html L1254 removed activity ring + weekly goals + check-in strip + collective impact banner; the actual Home is simpler than the plan assumed. Current Home elements: hero greeting + name + rotating tagline (synchronous), mood panel (PM-257 localStorage gate, already instant), Apple Health re-prompt banner (localStorage cadence, already instant), Today's focus carousel (3 cards from `VYVEFocusCatalogue` static module, painted from `paintFocus()` but the static catalogue means PM-397 reservation closes the only visible flicker), Today's habits list (PM-398 closes), Live sessions this week (from `VYVE_SESSIONS` static module, synchronous), My Progress 5 rings (PM-396 closes). The §23.7.7 trap (every visible element on first paint must be covered) is therefore satisfied with just PM-396 pillar counts + memberName + PM-398 habits. Static elements were already covered. Backlog L120 should be rewritten to reflect the simpler shipped surface — done in backlog patch this commit. **Device-walk evidence**: Dean walked all three ships on iPhone Capacitor wrap. First load post-PM-398 took 5-10 seconds and showed broken-looking page (links dead, taps unresponsive) — root cause was SW recache one-shot from cache key bump `pm397-...` → `pm398-...`, NOT a bug in the code (every member hits the same recache window once when sw.js cache key changes; first-load slow is the SW fetching+caching the new bundle from network before activating). Steady-state after recache: clean. Force-quit + reopen → tab-in to Home = no flicker, real ring values + name + focus cards + habit rows or empty CTA in first frame, no Live sessions jumping. **Trade-offs explicitly accepted**: (a) members with 5+ habits see only 3 in the snapshot-reserved slot then 5 paint on `loadHabits()` resolve — micro-shift on the overflow rows acceptable vs over-reserving for 0-habit accounts; (b) snapshot habit rows paint without Lucide icons until main script's `renderHabitList()` overwrites — icon-only swap is the subtlest possible upgrade vs bloating localStorage with rendered SVG HTML. **Markers**: vbb 277 → 280 (three +1 bumps), settings-vbb 277 → 280, sw cache `pm395-reaction-tap-cache-mutate-a` → `pm396-home-snapshot-first-paint-a` → `pm397-home-layout-reservation-a` → `pm398-habits-instant-paint-a`. 9 files total atomic across 3 ships (index.html × 3, settings.html × 3, sw.js × 3 — each ship a clean atomic Git Data API commit). PAT-direct via Vault throughout (Composio still down). `node --check` clean on all inline JS blocks + sw.js on each ship. §23.41 byte-perfect verify clean on every committed file at every commit SHA. §23.74 cross-repo PM scan run before every commit: PM-396 claim uncontested (vyve-site max=395, brain max=395.b), PM-397 claim uncontested (vyve-site max=396 my own, brain max=395.b), PM-398 claim uncontested. No parallel-session ships during the chat — clean run. **§23 candidate banked NOT codified solo**: SW cache key bump causes a one-shot slow first-load for every member when they next open the app; consider adding a `?_cacheBumpWarning=1` flag or a Settings indicator so a slow first-load post-deploy doesn't read as broken. Single occurrence so far (Dean's confusion at PM-398 first-load), promotes if another member surfaces the same confusion. **Chats 2-4 still owe** the same surgical pattern across Body (`exercise.html`) / Mind (`mind.html`) / Connect (`connect.html`): same three flicker shapes likely on each surface. View Transitions wire still parked for Chat 4 per the original plan. Original Chat 1 sweep items (keystore 1Password, vyve-capacitor git, debug strip gating) ALL STILL OPEN — backlog L118 updated to reflect that the sweep was deferred not done.)

 **Fix**: new `syncReactionStateToCache(checkinId)` helper reads `_kv` cache, finds the matching cached.checkins row by id, copies `reactionsByCheckin[checkinId].reaction_counts` + `my_reaction` back into `cached.checkins[idx]`, writes cache back with computed_at_ms UNCHANGED (surgical patch, 90s TTL anchored to original fetch). Both reaction subscribers now sync cache + repaintDebounced, no bypass. paintAll reads the now-correct cache via Path A, renders all 3 check-ins in a single paint. **Cross-repo PM-394 collision noted**: parallel session shipped brain-only PM-394 (`b4354876`) at premium-feel architectural sequencing for tab-flicker — strategic 4-chat plan (Option 1 snapshot-first + Option 4 View Transitions API across 5 hubs); PM-395 is tactical surgical fix for connect.html reaction-tap specifically. Complementary, will likely be subsumed when chat 4 (Connect + View Transitions) lands. §23.74 caught the collision at claim time, swept PM-394→PM-395 in 4 staged files before commit. **Edge case handled**: reacted card not in cache (own-only Dexie row not in EF top-3) → syncReactionStateToCache returns early, own-only paint stays visible, no community rows to flash. **connect:checkin:logged still uses `__cacheBypassOnce`** — new check-ins aren't in cache to mutate, genuine refetch is correct shape. **Connect-hub flicker arc summary (PM-390→PM-395)**: 5 ships iterating to clean tab-in/tap experience, two §23 candidates banked (in-page-vs-cross-page subscriber distinction from PM-391, audit-all-paint-sites-when-adding-fast-path from PM-393+PM-395 = second occurrence, promote on third). sw cache pm393-cache-first-paint-ordering-a → pm395-reaction-tap-cache-mutate-a, vbb 276 → 277 both marker files. 4 files atomic. 3 inline JS blocks + sw.js standalone node --check clean. §23.41 byte-perfect verify clean on all 4 files at commit SHA.)

## 19. Current status — 25 May 2026 PM-394 (Premium-feel tab-flicker architectural decision locked: Option 1 [snapshot-first paint per hub: `vyve_<hub>_snapshot_<email>` localStorage key written on every state-change, read synchronously at script-parse before first frame paints, skeleton removed from HTML entirely, honest empty state on no-snapshot] + Option 4 [View Transitions API on nav.js bottom-nav tap path] over Option 3 [persistent-shell SPA with mounted Home/Body/Mind/Connect views]. Decision driver: trial cohort 15-20 testers + Dean's Pro 20x weekly limit at ~75% leaves ~25% headroom; Option 3 is correct long-term architecture but burns whole headroom on a campaign with all-or-nothing semantics (half-migrated nav routes both shell + real-navigation, no clean partial-ship). Option 1+4 covers ~80% of perceived premium-feel for ~30% of the token cost with per-commit blast radius = exactly one hub. Option 3 explicitly parked post-trial. Chat sequence committed: Chat 1 cheap-stuff (keystore 1Password + vyve-capacitor git-init + debug strip gating across 8 *-live.html shells + mind tracker behind `localStorage.vyve_debug_tracker === '1'`), Chat 2 Home snapshot migration, Chat 3 Body + Mind in one chat, Chat 4 Connect + View Transitions wire + brain close. §23.7.7 trap watched at each chat: snapshot must cover EVERY first-paint DOM element or any non-covered element sits on placeholder until live data arrives, shipping a worse flicker than the skeleton. Strict design-phase enumeration required per hub. Open question for Chat 1 framing: View Transitions wire ships Chat 1 (helps device walks Chat 2/3) or Chat 4 (heavy snapshot work lands clean first). Banked §23 candidate NOT codified solo: chat-budget-aware architectural sequencing rule [prefer surgical per-commit-blast-radius patterns over all-or-nothing refactors when token-rate-limited]; single occurrence, promotes on second recurrence. Brain-only decision lock, no vyve-site changes this chat. §23.74 cross-repo PM scan at compose: vyve-site max=393 [`184e7bd3` Connect tab-in flicker real fix], brain max=393.b [`7deed506` PM-393.b brain close]. §23.70 fired loud — PM-393 was taken mid-compose by parallel session, re-fetched all 3 brain files at new HEAD and replayed patches with PM-393→PM-394 sed sweep. §23.58 fresh-HEAD re-fetch fired at both initial compose AND post-collision rebase, no content drift on either pass.)

## 19. Current status — 25 May 2026 PM-393.b (Real fix to Connect tab-in flicker via inverted cache-first paint ordering, vyve-site `184e7bd3`. PM-392's fix only skipped the EF call on cache hit — the own-only Dexie paint at the top of paintAll's Promise.all .then handler UNCONDITIONALLY fired first regardless of cache state, producing a visible 1→3 flicker on every Connect tab-in even with cache populated. Dean confirmed on device at Update 275. **Fix**: invert order entirely. Read `_kv` cache FIRST, before any Dexie reads or renderRecentCheckins call. Path A (cache hit, fresh, no bypass): load Dexie reactions in parallel to overlay own-row reaction state on top of cache's server-side counts, but call renderRecentCheckins exactly once with cache's community check-ins. No own-only paint. Path B (cache miss/stale/bypass): own-only Dexie fallback paint + EF fetch upgrade + cache populate. Path B retained for cold-boot UX and after-own-action freshness. **Known-remaining**: reaction-tap own-only flicker because bypass forces Path B; possible follow-on is mutating cached row directly on tap instead of bypassing cache. Banked if Dean still sees tap flicker. sw cache pm392-connect-feed-prefetch-cache-a → pm393-cache-first-paint-ordering-a, vbb 275 → 276 both marker files. 4 files atomic. 3 inline JS blocks node --check clean. §23.41 byte-perfect verify clean. §23.74 cross-repo PM scan: vyve-site max=392, brain max=392.b, claim PM-393 uncontested. **§23 candidate banked (NOT codifying solo)**: when refactoring a render path to add a fast path that skips work, audit EVERY render call site in the path, not just the one closest to the skipped work. Two ships to land the fix when one careful audit would have done it. Promotes on second occurrence.)

## 19. Current status — 25 May 2026 PM-391 + PM-392.b (Connect tab-in flicker eliminated in two ships: PM-391 reverts PM-390's reaction-subscriber double-mutation (`e98e67fe`) and PM-392 adds boot-time prefetch + cache-first paint for connect-feed-preview (`624552ce`). **PM-391**: PM-390's envelope-trusted `connect:reaction:logged` + `connect:reaction:cleared` subscribers re-applied a +1 to reactionsByCheckin that toggleReaction() at L1661 had already applied synchronously, producing a visible +2 flicker on every reaction tap until repaintDebounced rebuilt from Dexie scratch and snapped back to +1. Root cause: §23.65 misapplication — envelope-trusted pattern is for CROSS-PAGE subscribers only, not for in-page same-surface subscribers where the publisher already did the optimistic mutation. Reverted both reaction subscribers to bare `repaintDebounced()`. Inline comment documents the in-page-vs-cross-page distinction. Other 4 PM-390 connect.html subscribers untouched. **PM-392**: Dean confirmed the two-paint flicker on Connect tab-in (own-only Dexie paint flashes for 200-500ms then gets overwritten by EF response) had been visible all along, pre-existed PM-390. paintAll() architecture: own-rows-from-Dexie first paint + community-feed-from-EF second paint. Fix reads existing `VYVELocalDB._kv` cache slot `connect_feed_preview_v1` (FEED_PREVIEW_KV_KEY, infrastructure since PM-201 + prewarmFeedPreview) BEFORE the EF call. On cache hit (<90s fresh): paints from cache directly, no EF call, no flicker. On miss/stale: falls through to existing EF fetch and writes cache for next tab-in. Cache populated proactively by new index.html boot prefetch at L2403 behind 1200ms defer (skipped if cache fresh, idempotent across home re-renders, failure-silent). Module-scoped `__cacheBypassOnce` flag declared alongside reactionsByCheckin at L1025; set true by connect:checkin:logged + connect:reaction:logged + connect:reaction:cleared subscribers (member's own action = cache stale to other members); consumed once per paintAll then cleared. Cross-member updates may lag up to 90s — acceptable at cohort scale (~15 members). sw cache pm390-section65-sweep-events-rp-unify-a → pm391-revert-reaction-double-mutation-a → pm392-connect-feed-prefetch-cache-a, vbb 273 → 274 → 275 both index + settings markers per memory #10. 4 files atomic per ship via Git Data API. PAT-direct via Vault (Composio still down). 16 inline JS blocks + sw.js standalone node --check clean on both ships. §23.41 byte-perfect post-commit verify clean on all 8 file-writes across both ships. §23.74 cross-repo PM scans: PM-391 claim (vyve-site max=390, brain max=390.b) uncontested; PM-392 claim (vyve-site max=391, brain max=390.b) uncontested. 3 §23 candidates banked NOT codified solo: (1) §23.65 in-page-vs-cross-page distinction — audit signal should check if same file contains matching VYVEBus.publish for the event being subscribed to; (2) orphan cache audit — every _kv.set should have at least one matching _kv.get consumer; (3) boot prefetch as a pattern — home-page prefetch warming downstream EF-bound caches. Out of scope: connect-feed.html + connect-checkin.html could read the same cache slot for symmetric benefit, banked follow-on.)

## 19. Current status — 25 May 2026 PM-390.b (§23.65 envelope-trusted subscriber sweep on connect.html + mind.html closing the Audit PM-379 §23.65 queue items, engagement-v2.html L1682 dead `vyve-localdb-table-pulled` subscribe converted to `window.addEventListener` (Audit PM-379 Flag 14), events-rp.html migrated from standalone 17.6KB self-rendered page to canonical session-rp shell (2,473 bytes) joining the other 7 replay-category shells on shared `session-rp.js` + `session-rp.css`. Closes Audit PM-379 P0-5. **connect.html — 4 of 6 subscribers upgraded**: connect:checkin:logged sets module-scoped `__optimisticCheckinClientId` synchronously on envelope.client_id, connect:checkin:failed clears it on matching client_id, connect:reaction:logged applies +1 to `reactionsByCheckin[checkin_id]` map + sets `my_reaction` synchronously, connect:reaction:cleared decrements `my_reaction` count + clears the bucket. `connect:challenge:progress` + `connect:hydrated` + `mind:logged` + `body:logged` left as pure Dexie re-read (aggregate signals, not per-row counter emitters, race-immune by design). **mind.html — all 3 subscribers upgraded** with module-scoped `__mindDeltas` map keyed by client_id holding `{kind, activity_date, sign}` overlays + 3 helpers (applyMindDelta / revertMindDelta / mergeMindDeltasInto). `readMindActivities` terminal `.then` overlays `mergeMindDeltasInto(base)` so renderProgress sees envelope-confirmed activity before Dexie/REST converge. Idempotent — once the real row lands with same client_id, existingIds dedupes to a no-op. **events-rp.html replacement** baked in 4 side fixes: layout drift closed (one less rp shell out of band), stale playlist ID fixed (`PLIQ-snig8bSm7qQ2a1GFhN9U58wmkSEjm` not in catalogue → canonical `PLyaCafiXVssiPt5whqWDiK0EVTMYxbCyh` from replay_playlists slug='events'), theme.js now loaded (dark/light tokens reach the page), embedded YouTube API key removed from this surface (still lives in session-rp.js shared across all 8, not a credential rotation). **engagement-v2.html L1679 recompute fan-in (14 events into one Dexie re-read) NOT upgraded** — already wrapped in 50ms debounce serving as §23.65 pass-2; sibling to index.html `_rerenderHome` fan-in retrofit parked low-urgency in PM-381.b backlog. 7 files atomic at vyve-site `f2f07b99`. sw cache `pm389-breakdown-tiles-dexie-direct-a` → `pm390-section65-sweep-events-rp-unify-a`, vbb 272 → 273 both marker files. PAT-direct via Vault per memory #20 (Composio still down). 22 inline JS blocks + sw.js standalone node --check clean. §23.41 byte-perfect post-commit verify all 7 files via Contents API with raw accept matched local staged sha256. §23.66 per-file content rebase clean (no drift on any file). §23.74 cross-repo PM scan: vyve-site max=389, brain max=389.b, claim PM-390 uncontested. §23.69 collision scan at session start clean (PM-386/387/388/389 parallel debug arc didn't touch these subscribers). **Audit PM-379 queue effectively closed** apart from P1-4 rp-side (gated on per-replay attribution doctrine, low urgency at trial scale) and two fan-in retrofits banked low-urgency (engagement-v2 recompute, index.html _rerenderHome) — both wait-for-glitch per PM-381.b disposition. 3 §23 candidates banked NOT codified solo: (1) session-rp.js reads playlist ID from inline VYVE_SESSION not Dexie catalogue — could move to replay_playlists Dexie lookup as follow-on; (2) periodic rp-shell drift audit (byte-count outliers across `*-rp.html`); (3) brief-vetting against live source — second occurrence after PM-372.b banked the original, codifying as `/playbooks/content-surface-audit.md` on third recurrence.)

## 19. Current status — 25 May 2026 PM-388 (Mood check-in faces swapped from PM-258's hand-rolled line-drawn faces to canonical Lucide glyphs: frown=Not great, annoyed=Meh, smile=Good, smile-plus=Great, laugh=Amazing. viewBox 22x22 → 24x24 to host Lucide canonical geometry; stroke-width retained at 1.5 (not Lucide canonical 2) to match the surrounding glyph weight tuned for 30px container per PM-258. stroke=currentColor + fill=none inheritance preserved so existing .mood-icon CSS palette (dim-white default, bright teal on .selected with glow shadow) carries through unchanged. Tap interaction, .selected hold, 350ms-before-thanks-swap arc, localStorage gate, §23.39 optimistic-first write to daily_mood_checkins — all unchanged from PM-258. Single visual swap, zero behavioural delta. Closes the last bespoke-SVG island on home; every other glyph (habit icons PM-286, bottom nav PM-376, progress pills, sessions carousel) was already Lucide-canonical. §23.70 fired loud at commit time — first attempt (orphan commit 011be99f) parented at PM-386 (f5d84012), ref PATCH 422 because parallel session shipped PM-387 (bf5109bf, JS twins of PM-386 + Activity Breakdown tile parity) in the recheck-to-PATCH window. Recovery: re-fetched all 3 staged files at new HEAD bf5109bf (parallel session had bumped vbb 269 → 270 + cache key), re-applied patches, bumped vbb 270 → 271 + cache key pm387-js-twins-pm386-body-connect-tile-a → pm388-mood-faces-lucide-a, re-claimed PM-388. ~90s wall-clock cost. §23.74 cross-repo PM scan at commit time clean: vyve-site max=387, brain max=386.b, claim 388. §23.41 post-commit byte-perfect verify via Contents API + sha256 + first-100-byte on all 3 files at commit SHA — matched. 9 inline JS blocks in index.html + 4 in settings.html + sw.js standalone all node --check clean. Composio still 401 per memory #8 — direct PAT via Supabase Vault throughout. 3 files atomic at vyve-site `4882baec`. No new §23 rule earned.)

## 19. Current status — 25 May 2026 PM-389.b (Activity Breakdown tile counts on engagement-v2.html rewritten as Dexie-direct — PM-387 left the tile reads on a 3-stage compose: v1 home_state columns set first via renderScore [`v2.workouts_total + v2.cardio_total` for Body, `v2.sessions_total` for Connect], then loadMindCount async-patched Mind directly + Body additively + Connect by overwrite. Two layered problems: (A) v1 columns lag the optimistic Dexie write — on test3 post-cardio-log, `cardio_total` was still 0, movement.length was 1, Body tile rendered `0+1=1` not the expected 2; same shape on Connect where `sessions_total` was 0 [v1 column counts session_views which aren't being written anymore — PM-294's replay_video_views + PM-304's session_live_views are where the writes go]. (B) chained-await with outer try/catch (_) swallowed silently — if any single Dexie allFor() threw, the rest of the tile reads never ran. New loadMindCount mirrors renderProgressTab's pattern [which drives the home page pills correctly]: single parallel Promise.all across 8 source tables via safeAllFor helper, each tile set independently from row counts. Body = workouts.length + cardio.length + movement.length. Mind = mind_activities.length. Connect = connect_checkins.length + session_views.length + replay_video_views.length + session_live_views.length. Habits + Check-ins still set from v1 columns at the renderScore call site [server-side per-week unique, no clean Dexie-direct equivalent, v1 lag acceptable on weekly cadence]. **§23.70 collision rebase fired** — initial commit hit a fresh CACHE_NAME from parallel session's PM-388 ship [mood check-in faces swapped to canonical Lucide glyphs, vyve-site `4882baec` + brain `4c46df0c`]; §23.70 recompute caught it, re-staged sw.js/index.html/settings.html from post-PM-388 HEAD (engagement-v2 untouched by parallel ship, no patch conflict), PM claim recomputed 388→389. **§23.77 candidate third reinforcement** — PM-386.b banked "new activity table requires touching SQL function + v_active_days + dirty-mark triggers", PM-387.b extended to "...AND JS mirror + breakdown tile renderers", PM-389 is the third surface of same root cause [tile reads still v1-column-additive even after PM-387 made them movement/connect-aware]. Promotion criterion met if Dean wants it codified — suggested wording: "When a new activity table goes live, every read surface that displays counts from it must be audited — score functions [SQL + JS], tile renderers, home pills, v1-column-additive reads. Default to Dexie-direct parallel reads over v1-column dependencies for surfaces displaying post-write state." 4 files atomic at vyve-site `09c8f7d7`: engagement-v2.html [loadMindCount rewrite + breakdown header comment], sw.js [cache `pm388-mood-faces-lucide-a` → `pm389-breakdown-tiles-dexie-direct-a`], index.html [vbb 271 → 272], settings.html [settings-vbb 271 → 272]. All inline JS node --check clean. §23.41 byte-perfect verify clean on all 4 files at commit SHA via Contents API. PAT-direct via Vault.)
## 19. Current status — 25 May 2026 PM-387.b (JS twins of PM-386 SQL fix + Activity Breakdown tile parity for Body/Connect — Dean's device walk on test3 after PM-386 surfaced two surviving symptoms on engagement-v2.html that PM-386 missed: Body shows 0pts despite the walk being in movement_activities, and Connect tile reads 0 despite a connect_checkin existing. Server data was correct in both cases [PM-386 SQL fix doing its job]; bug was on the CLIENT side — PM-386 patched the SQL function but missed two of its twin JS surfaces. **Bug A — `home-state-local.js` `computeEngagementComponentsV2` is the JavaScript mirror of `compute_engagement_components_v2`** and had the same blind-spots: Body UNION was workouts+cardio only [movement absent], Focus UNION omitted movement, active_days_7 set omitted movement, `loadV2Tables` didn't even pull movement_activities from Dexie. engagement-v2 reads via this JS function for first paint / offline, so the SQL fix landing didn't help engagement-v2. Same surgical shape applied: loadV2Tables now pulls movement_activities as 5th parallel safeLoad, both call-sites thread movement through, computeEngagementComponentsV2 reads movementAll, Focus block adds v2FilterFocus(movementAll, true), Body block adds v2FilterFocus(movementAll, false), active_days_7 adds addDates(movementAll, 'activity_date'). **Bug B — Activity Breakdown tiles in engagement-v2.html reading wrong sources.** Line 1418 had Body = workouts_total + cardio_total [no movement]. Line 1419 had Connect = sessions_total [v1 column counting session_views + replay_views only — connect_checkins ENTIRELY MISSED, session_live_views + replay_video_views also absent]. Extended loadMindCount into Mind+Body+Connect async loader: Body tile does parseInt(currentText) + movement.length, Connect tile builds count from scratch as union of connect_checkins + session_views + replay_video_views + session_live_views per §11C canonical Connect pillar source set. **§23.77 candidate reinforced** — PM-386.b banked the rule "any new activity table requires touching compute_engagement_components_v2 + v_active_days + dirty-mark triggers"; PM-387 adds "...AND the JS mirror computeEngagementComponentsV2 AND the activity-breakdown tile renderers across every page that displays them". SQL fix alone is insufficient. Second observation but technically a follow-up patch of same root cause — not promoting to hard rule yet, ban-on-next-independent-recurrence still applies. 5 files atomic at vyve-site `bf5109bf`: home-state-local.js [7 surgical edits], engagement-v2.html [Mind/Body/Connect tile async loader], sw.js [cache `pm386-id-handshake-and-reentrancy-a` → `pm387-js-twins-pm386-body-connect-tile-a`], index.html [vbb 269 → 270], settings.html [settings-vbb 269 → 270]. PAT-direct via Vault [Composio still down]. All inline JS node --check clean. §23.41 byte-perfect verify clean on all 5 files at commit SHA. §23.69 + §23.70 + §23.74 all fired clean at session start + commit time. Test3 walk evidence: 4-pillar walk pre-PM-387 showed Body 0pts on score panel + 0 on tile + Connect 0 on tile; post-PM-387 device refresh should show Body 2pts + 1 on tile + Connect 1 on tile.)
## 19. Current status — 25 May 2026 PM-386.b (Mind/Body double-counting class fixed at root — three independent bugs surfaced via a clean test2 account walk. **Bug 1 server scorer blind to movement_activities**: `compute_engagement_components_v2` Body UNION omitted movement entirely [workouts+cardio only], Focus UNION omitted movement, v_active_days UNION omitted movement. Migration `pm386_engagement_v2_movement_in_body_focus_and_active_days_plus_mind_connect_dirty_triggers` rewrites the SQL function: Body UNION adds `movement_activities WHERE focus_slug IS NULL`, Focus UNION adds `movement_activities WHERE focus_slug IS NOT NULL` [closes focus-tagged walk = 0pts gap], v_active_days adds movement [walk-only days now count toward consistency multiplier]. **Bug 2 missing dirty-mark triggers**: `zzz_mark_home_state_dirty_{ins,upd,del}` triggers existed on the canonical 8 source tables and on movement_activities [PM-307] but were missing on mind_activities [PM-173] and connect_checkins [PM-289]. Same migration adds 6 triggers [statement-level, REFERENCING NEW TABLE/OLD TABLE, calling existing public.tg_mark_home_state_dirty_* functions]. **Bug 3 Dexie primary-key duality on optimistic write path**: writer optimistic Dexie row keyed `id: clientId` but POST payload sent only `client_id` not `id`, so server gen_random_uuid() fired and the row landed with `id != client_id`; sync hydrate brought server row back keyed on server id; Dexie ended up with TWO rows per logical activity. Census 33/33 movement, 38/42 mind, 13/13 cardio rows in production had id != client_id. **Bug 4 genuine server double-rows from iOS double-tap**: test1 had multiple instances of two rows seconds apart with different client_ids on movement + cardio + affirmations + journal/breathwork — function ran from scratch twice, btn.disabled paint lagged next event by enough frames. **Client ship vyve-site PM-386 `f5d84012`** [11 files atomic via Git Data API, PAT-direct via Vault per memory #20, Composio still down]: `id: clientId` added to POST payload on 8 writer files [journal.html / meditation.html / visualisation.html / sleep.html / breathwork.html / affirmations.html / movement.html (2 sites: markDone + logMovement) / cardio.html]; module-level reentrancy flag on movement.html markDone [`_markDoneInFlight`, cleared after Session Complete UI flip], movement.html logMovement [`_logMovementInFlight`, cleared in 2200ms button-reset setTimeout], cardio.html logCardio [`_logCardioInFlight`, cleared in its button-reset setTimeout]. sw cache `pm385-tracking-retire-live-a` → `pm386-id-handshake-and-reentrancy-a`, vbb 268 → 269 both marker files. Affirmations gets the id fix only — its per-day-cap design issue is parked separately. Connect-checkin already clean [writer at L550 already passed id: clientId]. Verification: test2 refresh post-migration showed engagement_body_points 0.00 → 2.00, engagement_pillars_touched_7 3 → 4, engagement_score_v2 53 → 54. All 11 files byte-perfect at commit SHA via Contents API. node --check clean on every modified inline JS block. §23.69 + §23.41 + §23.66 + §23.70 + §23.74 + §23.58 all fired clean. Three §23 candidates banked [NOT codifying solo]: §23.75 [server should accept client-supplied id when client_id present — dual-column shape is anti-pattern without explicit POST of id=client_id], §23.76 [btn.disabled alone insufficient reentrancy in iOS WKWebView — every writer needs module-level in-flight flag checked synchronously at function entry], §23.77 [adding a new activity table requires touching compute_engagement_components_v2 + v_active_days + dirty-mark triggers — three independent occurrences PM-307 movement, PM-173 mind, PM-289 connect have all hit this; strongest candidate, promote to hard rule on next recurrence]. Production cleanup deferred: existing Dexie duplicate rows clear on app-data-wipe + fresh hydrate [server clean, in-app side only]; 2 stale connect_checkins backfill rows banked for next housekeeping. Backlog item L5630 PM-309 follow-up "movement_activities table integration into streak/engagement" partially closed [server-side scorer + triggers done; client-side computeHomeStateFromDexie extension confirmed already done on index.html L2120].)
## 19. Current status — 25 May 2026 PM-385.b (tracking.js retired from 8 *-live.html shells — Audit PM-379 P1-4 closed. PM-251 brain entry claimed tracking.js was retired; live state pre-this-ship showed it loaded on all 8 *-live.html shells [yoga/mindfulness/workouts/therapy/events/podcast/education/checkin] AND all 8 *-rp.html shells AND sessions.html — 17 loads total, contra brain claim. Every live session ran two trackers in parallel: legacy tracking.js writing session_views with 30s heartbeats + PM-304 player-tracker.js writing session_live_views with 30s heartbeats. Aggregator de-dup [PM-294/PM-304 + wellbeing-checkin recap] hid the overlap from members; pure battery/data/write-volume waste on every live page. Per-shell change: identical 45-byte strip on each shell, line 42 `<script src="/tracking.js" defer></script>` removed; player-tracker.js remains as sole tracker. Pre-strip audit: all 8 shells confirmed loaded both tracking.js (line 42 identical script tag across all 8) + player-tracker.js (precondition for safe retirement). Post-strip: 0 tracking.js refs, player-tracker.js still loaded on all 8. Byte deltas exactly 45b each — identical edit pattern = clean atomic. **Rp shells unchanged per audit guidance**: per-replay attribution doctrine not finalised — PM-294 replay_video_views is engagement-grade, but legacy replay_views still referenced in refresh_member_home_state UNION semantics. Rp retirement banked as follow-on PM gated on doctrine clarification. §23.69 collision scan clean. §23.66 content rebase pre-commit: PM-383 ship had landed during this session, bumping markers 265→266 + cache key; rebased onto live state, bumped 267→268 for Commit 2. §23.74 cross-repo PM scan at commit time: vyve-site max-PM=384 [post-Commit 1], VYVEBrain max-PM=382, cross-repo max=384, claim PM-385. §23.41 byte-perfect verify on all 11 staged files at commit SHA. Memory #10 dual-marker invariant held [vbb 267→268 both markers], memory #14 cache key lockstep [pm384-checkin-questions-a → pm385-tracking-retire-live-a]. 11 files atomic at vyve-site `abd2fadd`. §23.67 extension candidate banked [NOT codifying solo]: §23.67 audit applies to module ADD; P1-4 is the reverse direction [module REMOVE — audit retiring module is gone from every consuming surface]. Single occurrence so far, promotes when second retirement of this shape recurs. Next: P0-5 [events-rp.html missing session-rp.js + theme.js + session-rp.css — single-file add, bundle with next replay-related ship]. Audit queue effectively closed apart from P0-5 + §23.65 sweeps [mind.html, connect.html] which remain backlog.)

## 19. Current status — 25 May 2026 PM-384.b (checkin_questions catalogue shipped — Tier 2.3 from PM-379 architecture audit, last unblocked content-surface audit queue item from PM-372. Versioned slider questions for weekly + monthly check-ins. Lewis [or eventually the Command Centre] authors rows in Supabase Studio; bump `version` to roll wording without breaking historical answers [check-in submissions reference question_id from the version they were written against]. **Schema** [`public.checkin_questions`, Supabase migration `pm383_create_checkin_questions` — migration name retained for migration immutability, canonical PM is 384 per cross-repo §23.74 claim at commit time]: id BIGINT IDENTITY PK + slug TEXT + survey_type TEXT CHECK IN ('weekly','monthly') + version INTEGER DEFAULT 1 + display_order INTEGER DEFAULT 100 + question_text TEXT + question_subtitle/min_label/max_label TEXT + min_value/max_value INTEGER [CHECK min < max] + default_value INTEGER [CHECK NULL OR in range] + active BOOL DEFAULT true + active_from/active_until TIMESTAMPTZ + audit cols. UNIQUE (survey_type, slug, version) lets versions roll without collisions. CHECK constraints on slug + question_text non-empty after trim. RLS authenticated SELECT WHERE active=true; no INSERT/UPDATE/DELETE policies, table is service-role-curated. Partial read index on (survey_type, display_order, version DESC) WHERE active=true. **Sync wiring** [eighth application of §23.49+§23.50 pattern after service_catalogue PM-190, replay_playlists PM-235, replay_videos PM-245, mind_videos PM-367, persona_welcome_copy PM-372, how_to_resources PM-377, podcast_platforms PM-378]: db.js SCHEMA_V21 + db.version(21) + makeCatalogueTable consumer; sync.js catalogue plan entry hitting `/checkin_questions?active=eq.true&select=*&order=survey_type.asc,display_order.asc,version.desc`; CATALOGUE_FRESH_TABLES extended [5-min stale window]; CATALOGUE_INVALIDATION_KEY bumped pm383-movement-dual-write-retire → pm384-checkin-questions per §23.50. **Surfacing out of scope**: wellbeing-checkin.html + monthly-checkin.html continue to render existing hardcoded slider questions until Lewis populates rows. Rails in place; hydration is a follow-on touch on those two pages [add VYVELocalDB.checkin_questions.allFor() read with hardcoded slider as FALLBACK_QUESTIONS for cold paint, mirroring PM-378 podcast_platforms shape]. **P1-3 closed negative**: audit asked for §23.65 envelope-trusted subscriber sweep on workouts-session.js publish surface. workouts-session.js publishes set:logged/workout:logged/body:logged/workout:failed/workout:shared; own internal subscriber [workout:failed L1023] is envelope-trusted by design [calls VYVEHomeState.revertPatch synchronously from envelope, no Dexie re-read]. Cross-page subscribers for set:logged audited [6 sites via GitHub code search]: workouts.html L571 calls only VYVEAchievements.evaluate() debounced 1.5s with explicit inline comment that it does NOT re-read exerciseHistory mid-session [SAFE by design]; index.html L2664 `_rerenderHome` Dexie re-read [§23.65 surface, but belongs to home-state coherence sweep across every _rerenderHome subscriber, not workouts-session-scoped, out of P1-3]; engagement-v2.html L1626/L2641 both lists end in setTimeout(repaint, 150) — workaround pattern absorbing fire-and-forget commit window [SAFE]; achievements-evaluator.js L1301 metric eval registration not render subscriber [not §23.65]; auth.js L155-167 Realtime bridge bypasses race [not §23.65]. **Conclusion: zero envelope-trusted retrofits needed on workouts-session.js publish surface specifically. P1-3 closed clean.** §23.65 follow-on backlog: mind.html [6 fire-and-forget publishers, subscribers re-read Dexie] + connect.html [headline §23.65 risk per audit Flag 15] remain — each a separate session. §23.69 collision scan clean at session start. §23.66 fired loud: parallel PM-383 ship [movement.html dual-write retire] landed during build, bumped markers 265→266 + cache key; three Commit-1 staged files [sw.js + index.html + settings.html] needed rebase. db.js + sync.js untouched by PM-383, applied cleanly. Single sed-sweep PM-383→PM-384 in db.js + sync.js comments per §23.70 PM-claim recompute. §23.74 cross-repo PM scan at commit time: vyve-site max-PM=383, brain max-PM=382, claim PM-384. §23.41 post-commit byte-perfect verify clean on all 5 staged files at commit SHA `aa43aa49`. Tooling lesson banked [NOT codified, single occurrence]: post-commit verification must compare bytes-to-bytes; my initial first-100 check used open() in text mode against bytes from API, showing false MISMATCH due to newline normalisation. 5 files atomic at vyve-site `aa43aa49`. Memory #10 dual-marker invariant held [vbb 266→267], memory #14 cache key lockstep. Operational implication for Lewis: add a slider via single INSERT in Studio with slug+survey_type+question_text+labels+default_value; roll wording via INSERT new row same slug bumped version + UPDATE old row active=false. Devices pick up new active version within 5 min.)

## 19. Current status — 25 May 2026 PM-382.b (`body:logged` aggregator publish from the three Body publishers — Audit PM-379 P0-2 closed. Three Connect subscribers [`connect.html:2071`, `connect-feed.html:944`, `connect-challenge.html:586`] have listened on `body:logged` for months with zero publishers; Connect tile counters stayed stale after a Body activity log until a page nav or sync.js hydrate triggered repaint. Added `VYVEBus.publish('body:logged', { kind, ... })` immediately after each existing per-table publish: `workouts-session.js` L676 kind:'workout' alongside workout:logged, `cardio.html` L921 kind:'cardio' alongside cardio:logged, `movement.html` L910 kind:'movement' alongside movement:logged. Critical placement: NOT next to movement.html's L637 legacy `workout:logged` publish — that path is going away in P0-3 next ship, adding body:logged to both sites would double-publish per movement tap. Inline comment documents the choice. Discriminator `{kind}` is informational only — all 3 subscribers today are unconditional [repaintDebounced / computeEliteProgress / rePaint regardless of envelope]; future subscribers can filter per-strand without a new event name. Precedent: workouts-builder.js L118 publishes workout:logged with `source:'builder'` for the same pattern. §23.65 race surface acknowledged but not patched — connect.html handlers re-read Dexie; backlog item kept open from PM-381.b L17, surface separately. Tooling: §23.69 clean at session start, §23.41 + §23.66 + §23.70 clean at commit time, §23.74 cross-repo scan clean [vyve-site max-PM 381, VYVEBrain max-PM 381 — first clean application of §23.74 since codification at PM-381.b]. 6 files atomic at vyve-site `124f78f9`, sw cache `pm381-bus-regex-3-segment-a` → `pm382-body-logged-aggregator-a`, vbb 264 → 265 both marker files. All 19 inline JS blocks across 4 touched HTML files node --check clean. First-100-char verify on all 6 files at commit SHA via Contents API clean. Curl arg-list trap surfaced [second occurrence — PM-374 was the read-side sibling]: `POST /git/blobs` for blobs >~100KB needs `--data-binary @file` not `-d "$json"`. Promotable to §23 hard rule at next ship if it recurs. Stale `PM-XXX` placeholders in workouts-session.js L333 + L350 from a prior haptic ship surfaced during sweep — reverted to PM-XXX to avoid mis-attribution to PM-382; forensic + correct sweep banked as backlog follow-up. Next from audit queue: P0-3 [movement.html dual-write retirement to `workouts` table], separate session per Dean's no-batching framing.)

## 19. Current status — 25 May 2026 PM-379.b (Monthly check-in recap rebuilt around canonical 4 pillars [Habits/Body/Mind/Connect] — sibling to PM-362.b weekly recap shipped same morning. Pre-PM-379 the step-0 recap was still on the legacy 6-tile flat shape [Habits/Workouts/Cardio/Sessions/Replays/Weekly Check-ins] with Mind / Movement / Connect tables completely absent from BOTH the client `loadRecap` and the server EF activity rollup — the AI report was blind to half the platform's activity even though members were doing it. Page now mirrors weekly recap shape exactly: 4-pillar grid with Lucide icons [check #4DAAAA / person-standing #E09B3D / brain #9B7AE0 / users #E06060] + per-pillar drilldown [_bodyDrill / _mindDrill / _connectDrill cloned from weekly] + sessions/check-ins band below. EF v19 [Supabase deployment v25] rolls activity into the same 4-pillar shape, restructures the Anthropic prompt's `=== ACTIVITY (4 PILLARS) ===` block, adds a pillar-aware step-3 to the STRUCTURE clause, bumps word budget 500 → 550, and adds a naming rules clause forcing the AI to refer to pillars as Habits/Body/Mind/Connect. Client-side `loadRecap` rewritten as 11-table Dexie-first read [was 6] — adds movement_activities, mind_activities, connect_checkins, session_live_views, replay_video_views. Sessions deduped by [activity_date, category] across the 4 view tables per PM-294/PM-304 overlap. `inMonth(r, dateCol)` bounds both sides per PM-371 ghost-row guard. wbCheckins source switched from "last 4 by iso_year/iso_week" to "in-month filter" — matches what the EF rolls into the prompt. Calendar heatmap density now 4-pillar [was 5 raw types]; legend "1/2/3/4 pillars". Habits days NOT clamped on monthly [calendar months 28-31 days are naturally bounded; no Math.min belt needed unlike PM-371 weekly]. Bus subscriber's mini-refresh repointed to renderRecapPillars. EF response.activity payload gains pillar totals while preserving legacy fields [totalHabits/totalWorkouts/totalCardio/totalSessions] for back-compat. 4 files atomic at vyve-site `4b445767`, sw cache `pm378-podcast-platforms-a` → `pm379-monthly-checkin-pillars-a`, vbb 262 → 263 both marker files. 3 inline scripts node --check clean. §23.41 first-100-char verify clean on all 4 files at commit SHA via Contents API. §23.69 fired clean — PM-364 [haptics sweep] touched monthly-checkin.html 14 commits ago, no parallel session active on this surface. §23.41+§23.70 fired clean at vyve-site commit time — HEAD `abcf3d3e`, max-PM 378, claimed 379. **Cross-repo PM-379 collision logged** — parallel brain commits `ed70eedf` [PM-380] + `8777c083` [PM-379] landed staging-only architecture audit docs while my vyve-site PM-379 claimed the recap rebuild; this brain entry takes `.b` suffix per PM-362.b precedent. Third occurrence of cross-repo PM collision in 24 hours — §23.70 cross-repo extension promoted from "banked" to "candidate", earns hard rule on next recurrence. Out of scope this ship [unchanged from PM-362.b L561]: monthly check-in DB triggers [charity_count_monthly_checkins sibling + counter trigger + zzz_mark_home_state_dirty parity + retroactive UPDATE for 3 existing rows] — still banked next on Dean's stack, separate DB-only session. Plus slider copy mirror to onboarding wording [Lewis-blocked] + 8-slider weekly-mini-monthly question [Lewis scope call].)

## 19. Current status — 25 May 2026 PM-378 (Podcast platform links catalogue shipped — `podcast.html` `.hero-listen` chip row now reads from `public.podcast_platforms` via Dexie, ordered by `display_order`, with `FALLBACK_PLATFORMS` const for cold paint and safety fallback. Lewis can edit/reorder/add platforms via Supabase Studio — no vyve-site commit per content change. Seventh catalogue migration on the §23.49+§23.50 pattern (service_catalogue PM-190, replay_playlists PM-235, replay_videos PM-245, mind_videos PM-367, persona_welcome_copy PM-372, how_to_resources PM-377, podcast_platforms PM-378). 6 files atomic at vyve-site `abcf3d3e`. **Cross-repo PM-378 collision logged**: parallel brain commit `451f849` claimed PM-378 for a brain-only staging architecture map; vyve-site `abcf3d3e` retains PM-378 as canonical (already permanent in commit message + 6 source files + migration + sw cache key + invalidation key), staging artefact proposed for rename at next housekeeping pass. §23.70 cross-repo extension banked as candidate, not promoted on first occurrence.)

## 19. Current status — 25 May 2026 PM-376 (Bottom nav Body icon swapped from dumbbell to person-standing — single SVG path swap in nav.js `navItems[2].icon`, closing the Body-pillar visual identity gap. PM-344 had set the 5 bottom-tab icons to Lucide-canonical paths with Body on `dumbbell`. Body pillar has since expanded conceptually to cover all physical activity (workouts + cardio + movement + walks), and dumbbell only reads as the weight-training strand. PM-362.b wellbeing-checkin recap card already moved Body to person-standing — the inline comment at wellbeing-checkin.html L939 says "matches the bottom nav identity for the Body tab" but was written prospectively before nav actually matched. PM-376 closes that gap. New SVG path is canonical Lucide person-standing: head circle (`circle cx="12" cy="5" r="1"`) + arms-out torso line (`m6 8 6 2 6-2`) + body trunk (`M12 10v4`) + V-shaped legs (`m9 20 3-6 3 6`). Stroke-width 1.7 retained from PM-344 calibration (Lucide source is 2 but nav reads softer at 1.7 at 24px tab size). PM-344 comment block updated inline to document the PM-376 swap. Desktop nav (`desktopLinks`) is text-only — no icon row — so single change covers all Body-icon surfaces in nav.js. §23.70 fired clean — parent `9aac3c11` (PM-375 Member Prompts), parallel session shipped PM-374 (food log Coming Soon, `761a027c`) + PM-375 between PM-373 commit and this commit-time; neither touched nav.js, both touched index.html + settings.html + sw.js for marker/cache bumps. Re-fetched those three live, replayed bumps 259 → 260 on post-PM-375 content. Base-tree-merge safe. PM claim recomputed 374 → 376. 4 files atomic at vyve-site `278f2af2`, sw cache `pm375-member-prompts-a` → `pm376-nav-body-person-standing-a`, vbb 259 → 260 both marker files. nav.js node --check clean. First-100 char verify on all 4 files via Contents API clean. Live grep confirmed person-standing path landed (1 occurrence) + dumbbell path gone (0 occurrences). Identity audit follow-up banked: `exercise.html` hub-page hero icon may still use a different glyph — single-session sweep worth doing next time someone touches exercise.html, audit hub-page Body identity for symmetry with nav + recap. Not pressing.)

## 19. Current status — 25 May 2026 PM-375 (Member Prompts system shipped — Lewis-driven in-app questionnaires sourced from Supabase, surfaced via bottom-sheet modal on home boot. New first-class content surface following §23.49 catalogue / §23.50 invalidation contract. Four Supabase tables: `member_prompts` (popup container — slug + title + eyebrow + CTA labels + trigger config jsonb + segment jsonb + dismiss mode + priority + active window), `member_prompt_questions` (FK to prompts.id, types: `multi` / `single` / `slider_1_10` / `text_short` / `text_long` / `yes_no`, options jsonb for multi/single, placeholder for text), `member_prompt_dismissals` (member-scoped, per-member gate state, unique `(member_email, prompt_id)`), `member_prompt_responses` (member-scoped, answers, jsonb `answer_value` shape depends on question_type, indexed `(prompt_id, iso_year, iso_week)`). RLS authenticated read on catalogues (active=true), member-scoped on dismissals + responses. Client: `prompts.js` 665 LOC renderer + `prompts.css` 335 LOC styles, both new files, dark + light theme parity via app tokens. `db.js` SCHEMA_V18 with all four stores (catalogue tables via `makeCatalogueTable`, member-scoped via `makeTable`, compound indexes on dismissals + responses). `sync.js` three plan entries (2 catalogue + 1 member-scoped dismissals — responses write-only), `CATALOGUE_INVALIDATION_KEY` bumped `pm372-persona-welcome-copy` → `pm375-member-prompts` per §23.50, fresh-tables extended. `index.html` boot wire: `VYVEPrompts.maybeShow('index')` after `moodShowPanel(email)` with 600ms internal defer so first paint is uncontested. Two canary prompts seeded: **weekly-preference** (Monday focus picker, multi-select 8 options + free text, dismiss_mode `weekly`, priority 200, fires Mon ±3d on home) + **app-feedback** (Sunday pulse, 1-10 sentiment slider required + two optional textareas, dismiss_mode `cooldown_days=28`, priority 150, fires Sun on home for members ≥3d active via `target_segment.min_days_active`). Submit path is canonical §23.39 optimistic-first — UI tears down via `teardown()` synchronously, then Dexie upsert of dismissal row + REST POSTs to `member_prompt_dismissals` (upsert on conflict) + `member_prompt_responses` fire in background with `keepalive:true`. Skip path same shape, dismissal row gets `last_dismissed_at` instead of `completed_at`. Lewis reads responses from Supabase Studio for the trial — Command Centre editor + reader on backlog post-trial. **§23.70 fired twice in one build**: PM-372/373 collision rebased SCHEMA_V17 → V18 + replayed marker bumps 255→257; PM-374 collision rebased three more files (index/settings/sw) + replayed 257→258→259 + cache-key swap to `pm375-member-prompts-a`. PM claim recomputed 372→374→375 across the chain. Composio still 401 — direct PAT via Supabase Vault throughout per §23.45. §23.41 first-100 verify all 7 files clean. §23.52(a) `--data-binary @file` honoured (Python urllib with body written to /tmp, never argv). GitHub Blob API quirk surfaced and banked: `POST /git/blobs` does NOT return `size` in the response (it does on `GET`), so post-write size-check has to be skipped or routed through a follow-up read — single occurrence so far, NOT promoted to §23 hard rule. 7 files atomic at vyve-site `9aac3c11`, sw cache `pm374-food-log-coming-soon-a` → `pm375-member-prompts-a`, vbb 258 → 259 both marker files. 9 inline JS blocks in index.html + 4 in settings.html node --check clean. node --check on prompts.js, db.js, sync.js, sw.js all clean. Out of scope this ship (deferred): Command Centre editor UI (schema is editor-ready, awaits the Command Centre roadmap slot); `first_login` trigger type wire (code path exists in `passesTriggerGate` but no v1 prompts use it — surface_enter + min_days_active covers canary cases); server-side aggregation EF (Studio SQL fine at trial scale, EF gets a backlog slot when read surface lands in Command Centre); other surfaces (`habits.html`, `mind.html` etc) opt in to `VYVEPrompts.maybeShow(surfaceName)` from their own boot paths — incremental adoption as Lewis writes more prompts.)

## 19. Current status — 25 May 2026 PM-374 (Food log locked Coming Soon as soft lockdown — `log-food.html` no longer reachable from `nutrition.html` and direct-link / cached-SW navigation hits a cover, not the live UI. PM-353 had already retired the `fuel` focus + `nutrition_logs` from Focus pillar source-table reads; PM-374 closes the entry-point loop. Surface 1 (`nutrition.html`): `<a id="food-coming-soon" href="/log-food.html" class="food-log-link fade-up-3">` replaced with `<div id="food-coming-soon" class="coming-soon fade-up-3">` using the house `.coming-soon` CSS already at L217-219 — same idiom as connect-feed.html Following v1 + connect-challenge.html `.no-challenge`. id preserved verbatim so the L913 mode-toggle skeleton-control routine (Off-mode display toggle on `['cal-main-card','cal-stats-row','explainer-box','recalc-wrap','food-coming-soon']`) keeps working. Surface 2 (`log-food.html`): `<div id="food-coming-soon-cover">` + self-contained `<style>` block inserted between `<body>` and `<div id="app">`. Cover is full-viewport, z-index 10000, includes top-bar back-link + centred teal-tinted icon tile + Playfair "Food log — coming soon" headline + Inter body copy + `.fcs-cta` button matching the existing `.empty-cta` pattern (linear-gradient teal-to-dark) → routes to `/nutrition.html`. Four boot points gated on `document.getElementById('food-coming-soon-cover')`: (1) `paintLogFoodCacheEarly()` IIFE early-return — no Dexie hydration; (2) `loadPage()` async early-return — no Supabase fetches, no bus events publishable; (3) `DOMContentLoaded` revert-subscriber listener early-return — no inert wiring; (4) skeleton-timeout watchdog IIFE early-return — prevents false-positive `skeleton_timeout_log-food` severity:high alerts to platform-alert (which would otherwise fire because `getComputedStyle` returns `display:block` for `diary-skeleton` even though its `#app` ancestor is hidden). Single sed sweep on the gate-string is the entire reversal. §23.70 fired at commit-time: parallel sessions shipped PM-372 (hydration COPY_TABLE catalogue) + PM-373 (habits tile palette parity) between session-start (PM-371 was HEAD) and commit-time, both touching index.html + settings.html + sw.js — three of my five staged files. PM-claim recomputed 372 → 374, re-fetched those three files at the new HEAD, replayed marker bumps 255 → 258 on post-PM-373 content, swept source comments PM-372 → PM-374 (1 ref in nutrition.html, 5 in log-food.html). nutrition.html + log-food.html not in either parallel commit's file set, substantive edits carried forward without rebase. §23.58 fired clean on the brain commit: re-fetched all three brain files at the post-PM-373 HEAD before composing. Discovered changelog.md (2.48MB) exceeds the GitHub Contents API inline-content limit (~1MB) — `content` field returns empty with a `download_url` and a naive caller writes 0 bytes silently. Switched to the Git Blob API which returns base64 regardless of size. §23.79 candidate banked (NOT codified — first occurrence): brain reads of files >1MB MUST route through `/git/blobs/{sha}` not `/contents/{path}`. 5 files atomic at vyve-site `761a027c`, sw cache `pm373-habits-tile-palette-parity-a` → `pm374-food-log-coming-soon-a`, vbb 257 → 258 both marker files. 7 inline JS blocks node --check clean (3 nutrition + 4 log-food). sw.js standalone parse clean. First-100 + byte-size verify all 5 files clean against committed SHA via Contents JSON (all 5 under 1MB so blob route not needed for verify). Out of scope (deferred): bus event scrubbing — `food:logged`/`food:deleted` subscribers in engagement-v2.js + achievements-evaluator.js + home-state-local.js remain registered but unreachable; cosmetic cleanup deferred to whenever PM-374 becomes permanent rather than coming-soon.)

## 19. Current status — 25 May 2026 PM-373 (habits.html icon tile palette brought to exact parity with index.html PM-287 — closes the colour-mismatch Dean flagged after PM-370 ship. PM-370 inlined Lucide glyphs into habits.html but kept the OLD POT_CONFIG palette for tile colours (sleep #6366B8 / movement #4DAAAA / nutrition #2D9E4A / mindfulness #5BA8D9 / social #E879A3) wired via inline `style="background:${pot.color}1A;color:${pot.color}"`. Index.html PM-287 uses an entirely different palette matched to progress-ring colours — mindfulness #B186C9 purple / movement #8FC470 green / nutrition #D89760 deep amber / sleep teal-lt / social #F0A075 orange — plus a teal done-state override regardless of pot. Fix: decoupled tile palette from POT_CONFIG on habits.html, switched to CSS attribute-selector pattern mirroring index.html L803-832 exactly. `.habit-pot-tile` base CSS defaults to teal `rgba(77,170,170,0.15)` + `var(--teal-lt)` fallback (graceful degrade for missing/unknown habit_pot). Five `.habit-card[data-pot="..."] .habit-pot-tile` selectors carry the per-pot palette — byte-identical rgba values + hex codes copied from index.html. `.habit-card.done .habit-pot-tile` done-state override matching index L825-828, `[data-theme="light"] .habit-card.done .habit-pot-tile` light-theme done matching L829-832 (PM-288 deep-teal-on-light). `habitCardHTML()` rewritten — `data-pot="${h.habit_pot || ''}"` added to `.habit-card` wrapper, inline tile style stripped, tile is now just `<div class="habit-pot-tile">${iconForHabit(...)}</div>`. `.habit-card::before` 3px left rail still reads `--pot-color` from POT_CONFIG (rail is a habits.html-only design element with no index equivalent, kept out of scope). POT_CONFIG is now ONLY the rail's source of truth; tile colours are CSS-only. §23.70 fired clean — parent `30c2d23b` (PM-372 hydration COPY_TABLE), parallel session shipped PM-371 (`3244e596` weekly check-in recap window clamp) + PM-372 between PM-370 land and our commit-time. Neither touched our 4 files' substantive content, only marker bumps overlapped. Re-fetched live HEAD versions immediately before applying patches, replayed marker bumps 256 → 257 on post-PM-372 content, swept source-comment PM ref from `PM-371` (initial draft) to `PM-373` (final live HEAD). 4 files atomic at vyve-site `90d92495`, sw cache `pm372-persona-welcome-copy-a` → `pm373-habits-tile-palette-parity-a`, vbb 256 → 257 both marker files. 3 inline JS blocks in habits.html node --check clean. First-100 char verify all 4 files via Contents API clean against committed SHA. §23 candidate banked (NOT codified — second occurrence): duplicated palette/lookup data across index.html + habits.html — first was PM-370 iconForHabit duplication, now PM-373 palette duplication. Promotion criterion is third independent occurrence on a third surface; both should absorb at once when backlog L885's `habit_library.icon` column work lands. Out of scope: aligning the `.habit-card::before` rail palette to the index palette — would either drop the rail entirely on habits.html (lose a habits.html visual identity element) or single-source POT_CONFIG to the index palette and inherit (cleaner — bank for next pass when Dean's looked at it on device).)

## 19. Current status — 25 May 2026 PM-371 (Weekly check-in recap window-math hardening — fixes 8-days-in-7-day-window display bug on `wellbeing-checkin.html` recap card. Three surgical edits to the `shape()` helper in `fetchMemberData` (lines 1080-1183): (1) added `weekEnd = new Date().toISOString().slice(0, 10)` next to `weekStart` so the rolling-7 window has both bounds; (2) `inWindow` filter tightened from `r[dateCol] >= weekStart` to `r[dateCol] >= weekStart && r[dateCol] <= weekEnd` — closes the future-date ghost class where a Dexie row with a tomorrow-or-later `activity_date` (clock skew, timezone roll, malformed sync envelope) was lexically passing the lower-bound-only filter; (3) `habitDays = Math.min(7, new Set(habits.map(h => h.activity_date)).size)` — mathematical truth as belt-and-braces, a rolling-7 window contains at most 7 distinct dates by definition. Surfaced by Dean at the device: Supabase truth showed 2 distinct dates (13 ticks across 25 May + 19 May) but the recap card displayed 8 days. Diagnosis confirmed a Dexie ghost row in IndexedDB that no longer matches canonical truth. No behavioural change to Body/Mind/Connect totals — those sum raw activity counts per pillar not distinct days, so the 7-cap doesn't apply there; the same `inWindow` upper-bound protects all four pillars from the same ghost-row class. §23.70 fired: parallel session shipped PM-370 (habits.html difficulty-tag strip + settings habit picker, `5703c9da`) ~90s before my pre-commit recheck. Re-fetched sw.js + index.html + settings.html from post-PM-370 HEAD, replayed marker bumps 254 → 255 on fresh content, swept PM-370 → PM-371 in `wellbeing-checkin.html` source comments. wellbeing-checkin.html was not in PM-370's file set so the substantive code edits carried forward without rebase. 4 files atomic at vyve-site `3244e596`, sw cache `pm370-habits-icons-difficulty-strip-a` → `pm371-recap-window-clamp-a`, vbb 254 → 255 both marker files. 6 inline JS blocks in wellbeing-checkin.html node --check clean. First-100 verify on all 4 files via Contents API clean against committed SHA. Banked §23 candidate (NOT codified — single occurrence): defensive bounds on PF-10 Dexie-first reads — any client-side filter on a date range should bound BOTH sides at the read site, because Dexie is a write-through local store with no integrity check at read time and lower-bound-only filters silently admit stale or future-dated ghost rows. Promoting to §23 hard rule on the next independent occurrence per §23 promotion criteria. Architectural follow-up banked to backlog: PF-30+ companion task for a Dexie reconciliation pass on `visibilitychange` — full member-scoped 30d refetch from Supabase, replace Dexie rows for the window. Out of scope this session: the AI prompt rebuild (PM-363/364 banked in the previous brain commit), and the weekly check-in slider copy mirror to onboarding wording (§22 open decision, Lewis-blocked).)

## 19. Current status — 25 May 2026 PM-370 (Settings habit-picker difficulty tags stripped + habits.html icons brought to parity with home — two unrelated cleanups shipped atomically. Surface 1 (settings.html): `<div class="habit-difficulty">` pill removed from both "My habits" custom-row render (L1551) and Library row render (L1576); `<select id="hc-difficulty">` Easy/Medium/Hard dropdown removed from custom-habit creation form, `<select id="hc-pot">` retained as sole row content; `toggleCreateForm()` close-reset and `saveCustomHabit()` value-read updated, `difficulty: 'medium'` hardcoded in POST body so the `habit_library.difficulty` column stays non-null (forward-compat over migration); CSS at L304-326 + DB column retained dead-but-harmless. Surface 2 (habits.html): pre-PM-370 the page rendered an 8px coloured `.habit-pot-dot` next to each habit title with no per-habit glyph — home page (index.html) PM-286 has rendered the proper Lucide icon per habit (heart-pulse for cardio, footprints for walking, brain for meditation, etc.) in a pot-tinted 36px tile per PM-287 since 24 May. Brought habits.html into parity: Lucide `<symbol id="hi-...">` defs block (36 symbols, ~9KB) inlined immediately after `<body>` open as verbatim copy of index.html L1031+; `iconForHabit(title, pot)` + `_habitIconSvg(name)` functions inlined directly above `habitCardHTML()` as verbatim copy of index.html L1583-1660 — both files now carry duplicate copies of the 34-title HABIT_MAP + 19-regex keyword fallback + 5-pot fallback + `circle-dashed` generic, flagged as §23 candidate (extract to `/habit-icons.js` OR land `habit_library.icon` column per backlog L885 — whichever comes first); `habitCardHTML()` bare `.habit-pot-dot` replaced with `.habit-pot-tile` carrying the Lucide glyph — `<div class="habit-pot-tile" style="background:${pot.color}1A;color:${pot.color}">${iconForHabit(h.habit_title, h.habit_pot)}</div>` (10% alpha tint via `1A` hex suffix + `currentColor` glyph paths matching PM-287 pattern); CSS added for `.habit-pot-tile` (36×36 rounded tile) + `.habit-ic-svg` (stroke styling). Result: every habit_library title now shows identical glyph + identical pot tint across habits.html and the home page — cardio = heart-pulse, walking = footprints, meditation = brain, etc. 4 files atomic at vyve-site `5703c9da`, sw cache `pm369-settings-toggle-haptics-a` → `pm370-habits-icons-difficulty-strip-a`, vbb 253 → 254 both marker files (per-PM uniqueness restored per PM-369 prediction). §23.41 fresh-HEAD-recheck fired clean (parent `026dee9e7f...`, PM-369 still HEAD, claim PM-370 uncontested, no §23.70 re-bump needed). 4 inline JS blocks in settings.html + 3 in habits.html node --check clean. First-100 verify on all 4 files via Contents API (not raw CDN) clean against committed SHA. Banked §23 candidate: duplicated iconForHabit lookup tables across two files — not codified yet (single occurrence + backlog item L885 already tracks the proper structural fix). Out of scope: applying the same icon treatment to the settings.html habit-picker modal rows (Dean's brief was specifically habits.html + tag removal, mockup-first if/when added).)

## 19. Current status — 25 May 2026 PM-369 (Settings toggle haptics — 11 new wires on settings.html bringing it to 13 total. Backlog item #7 from PM-278 closed. `setDisplayNamePref` name-pref buttons → selection() at synchronous markActiveNamePrefBtn flip, error() on supaFetch revert path; added `if (pref === prev) return` no-op guard so already-active buttons don't haptic. `savePersona` → success() at the bus-publish moment (synchronous, BEFORE the Dexie await per PM-368 anti-pattern lesson) — persona switch is a configuration commit, not a low-stakes toggle. `toggleHabit` habit-picker modal → selection() both directions (add+remove) + warning() on cap-reached (size >= 10) — FIRST USE of warning() in the codebase since bridge shipped PM-277, closes the "warning unused" caveat from PM-365. `saveHabits` modal-close + toast → success() (per §23.39/PM-151 the network is fire-and-forget background, haptic locks to the perceived "saved" moment). `saveCustomHabit` → success() on optimistic-render, error() on catch. `deleteCustomHabit` → medium() on optimistic delete-render (matches PM-366 deleteCustomWorkout), error() on catch. Skipped: `toggleCreateForm` (no write, would over-buzz) + auth/sign-out (security-critical, feedback could confuse "saved" signal). 3 files atomic at vyve-site `026dee9e`, sw cache `pm368-home-habit-haptic-realign-a` → `pm369-settings-toggle-haptics-a`, vbb 252 → 253 — note: marker invariant recovered here, PM-367 and PM-368 both bumped 251→252 leaving marker 252 dual-claimed in the brain narrative; PM-369 bumping to 253 restores per-PM uniqueness going forward. 4 inline JS blocks node --check clean. First-100 verify all 3 files clean. Remote VYVEHaptics call-site count in settings.html = 13 (expected). §23.70 PM-claim fired clean: parent f33a231a → claim 369. **Haptic palette now fully populated across 7 PMs and 8 surfaces** (selection/light/medium/success/heavy/error/warning) — banked as §23.74 candidate for next codification pass with the rule shape "use the closest semantic match; reach for heavier band only when the moment is qualitatively heavier." Prior §23.73-candidate from PM-356 (helper-context-extension) still on the bench, gets §23.75 when its third occurrence lands.)

## 19. Current status — 25 May 2026 PM-368 (Home habit haptic re-aligned to the synchronous tap-flip site + untick branch added. Dean reported "haptics aren't on the habits on index yet"; investigation showed PM-360's wire at line 2270 sat AFTER an awaited `VYVELocalDB.daily_habits.upsert()` that can stall 100-400ms on iOS WKWebView when the indexeddb transaction queues — haptic decoupled from the tap, felt like nothing fired. §23.39-adjacent: the optimistic-first contract says "flip UI synchronously, then un-awaited background work" — the haptic IS part of the UI flip and belongs in the same animation frame as `renderHabitList()`, not gated behind the data write. Fix: haptic block moved to fire immediately after `renderHabitList()` at line 2253, before any await. Both branches now haptic'd: tick (`next === true`) → `VYVEHaptics.success()` matching habits.html PM-359 manual-flow (Dean's "feels really good" reference); untick (`next === false`) → `VYVEHaptics.light()` overriding PM-360's deliberate "untick stays silent" — dead-feeling untick tap is a worse UX than the celebrate-correction concern, and the workouts-set-untick precedent (PM-364) has settled at light(). Old PM-360 haptic call at line 2270 removed. 3 files atomic at vyve-site `f33a231a`, sw cache `pm366-custom-workout-delete-haptic-a` → `pm368-home-habit-haptic-realign-a`, vbb 251 → 252 both marker files. §23.70 fired at commit time — parallel session shipped PM-367 (mind videos Supabase catalogue, `5fb88b63`) between my restart and HEAD recheck; PM claim re-computed 367→368. Verified PM-367's mind/meditation/sleep/visualisation touch did NOT drop my PM-364 haptics.js inserts (base-tree-merge held). 9 inline JS blocks node --check clean. First-100 verify all 3 files clean. Marker collision noted: both PM-367 and PM-368 bumped 251 → 252; live marker is now 252 but covers two ship narratives — cache key (pm368-...) is unambiguous, only the vbb-marker UI string is dual-claimed. Next vyve-site commit should bump to 253 to recover per-PM uniqueness. Lesson banked, not codified: haptic dispatch belongs at the synchronous UI-flip site, not gated behind awaited writes — single occurrence so far (PM-360 → PM-368), would sharpen §23.39 explicitly listing haptics as part of UI-flip phase if a second instance occurs.)

## 19. Current status — 25 May 2026 PM-367 (Mind videos Supabase catalogue — mind.html / meditation.html / sleep.html / visualisation.html now read from public.mind_videos table instead of hardcoded HTML/JS literals. Dean can swap videos, edit titles, change hero designation, reorder, or add new entries directly in Supabase Table Editor; devices pick up within the 5-min CATALOGUE_FRESH_TABLES window. Schema: youtube_video_id PK / kind discriminator / title / sub / duration_minutes / is_hero (partial unique per kind) / display_order / active / timestamps. RLS public-read for authenticated. Seeded with 12 rows from the existing hardcoded set. sync.js catalogue plan entry + CATALOGUE_INVALIDATION_KEY bumped 'pm251-live-state-machine' → 'pm367-mind-videos' per §23.50. db.js SCHEMA_V16 + makeCatalogueTable('mind_videos') store registration. mind.html FOCUS_POOL preserved as FALLBACK_POOL for cold-load paint, replaced from Dexie via vyve-localdb-table-pulled listener + 1000ms probe. Three sub-pages: static .vz-hero + .streams gutted, inline render script paints from Dexie filtered by PAGE_KIND, fallback array embedded for cold-load. Click handlers rewritten from static querySelectorAll-on-DOMContentLoaded to event delegation on document.body matching e.target.closest('[data-video]') — dynamically-injected cards now bind cleanly. vyve-site `5fb88b63`. 9-file atomic via Git Data API (Composio still down). sw cache `pm366-custom-workout-delete-haptic-a` → `pm367-mind-videos-supabase-a`. vbb-marker 251 → 252. All 13 inline JS blocks node --check clean. §23.41 first-100 verify all 9 files clean. §23.52(b) files[].status: 9× modified. §23.70 fired correctly — PM claim rebumped from 363 → 367 mid-build after parallel haptics ships PM-363/364/365/366 took the lower numbers. No new §23 rule earned — fourth application of §23.49/§23.50 catalogue contract.)

## 19. Current status — 25 May 2026 PM-366 (Custom workout delete haptics — `workouts-builder.js` `deleteCustomWorkout()` wired with `VYVEHaptics.medium()` on the optimistic `renderCustomWorkouts()` paint after server 2xx (the row-vanish IS the perceived "deleted" moment; NOT on the background `loadCustomWorkouts()` reconcile — 200-500ms on slow networks would feel uncoupled from the original tap) plus `VYVEHaptics.error()` on both `!res.ok` and the `catch` block. medium() chosen because deletion is a destructive commit — heavier than the light() set-untick, lighter than the success() reserved for additive logs. Backlog item "swipe-to-delete confirm on the custom workouts list" was the target but the swipe gesture itself isn't built; wired the existing button + `confirm()` flow as the available win, the threshold-crossing haptic will be a one-line add at the gesture-confirm callback when swipe lands. 4 files atomic at vyve-site `61876943`, sw cache `pm365-nutrition-haptics-a` → `pm366-custom-workout-delete-haptic-a`, vbb 250 → 251 both marker files. First-100 verify clean. node --check clean.)

**PM-372 (25 May 2026, vyve-site `30c2d23b`, supabase migration `pm372_create_persona_welcome_copy`).** Hydration welcome COPY_TABLE migrated off hardcoded JS const onto `public.persona_welcome_copy` (13 rows seeded verbatim from the PF-13 draft). PM-367 pattern verbatim: SCHEMA_V17 + sync.js catalogue plan entry + Dexie consumer + `CATALOGUE_INVALIDATION_KEY` bump `pm367-mind-videos` → `pm372-persona-welcome-copy`. hydration.js `COPY_TABLE` renamed to `FALLBACK_COPY_TABLE`; new `resolveLiveTable(800)` does timeout-guarded Dexie read; `pickLine` gained optional `tableSource` arg; `maybeShow()` paints fallback synchronously then swaps `textContent` in-place if live row wins within the 1500ms minimum display. HAVEN short-circuit + SAFE_ECHO_GOALS whitelist + `{name}` interpolation all stay code-side (safety logic / mechanics, not content). PostHog `welcome_hydration_shown` gained `copy_source: 'dexie' | 'fallback'`. 6-file atomic via Git Data API (Composio still down). sw cache `pm371-recap-window-clamp-a` → `pm372-persona-welcome-copy-a`. vbb 255 → 256 both marker files. 3 inline-touched JS files node --check clean. First-100 + MD5 verify clean on all 6 files. §23.70 PM-claim recomputed PM-370 → PM-372 mid-session as parallel sessions landed PM-370 + PM-371. **No new §23 hard rule earned** — §23.49 + §23.50 + §23.41 + §23.58 + §23.70 all applied as documented; this is the fifth catalogue migration on the pattern. **Session context.** Opened with Dean tabling a Claude-authored brief proposing four tiers of content-surface migrations starting with `sessions-data.js` → `service_catalogue`. Brief was factually right on file presence and most details but strategically wrong on Tier 1.1 — PM-190.d + PM-333 both deliberately retreated FROM service_catalogue to sessions-data.js for flash-of-empty UX reasons; reversing without the admin editor in place re-opens a paid-for fix. Reordered to Tier 2.1 hydration COPY_TABLE first. Tier 4 (`how-to-pdfs.html` / `how-to-videos.html` as catalogue reads from day one) is the next-natural ship.)

## 19. Current status — 25 May 2026 PM-365 (Nutrition + food-log haptics — `nutrition.html` water stepper / weight log / TDEE save + `log-food.html` food add/delete. `adjustWater(+/-)` → `selection()` per tap, upgraded to `success()` on the threshold-crossing tap only (willHitTarget guard ensures success doesn't repeat once at target). `logWeight()` success → `success()` at the optimistic-paint moment per PM-60 local-first contract; catch → `error()`. `saveTargets()` success → `success()`; catch → `error()`. `log-food.html` food add (renderDiary after diaryLogs push) → `success()` regardless of online/offline; food delete (renderDiary after filter) → `light()` mirrors workouts-session.js untick pattern. food:failed and weight:failed dead-letter reverts intentionally silent — no counter-haptic, preserves PM-62 silent-recovery contract. Strict scope: write actions and outcomes only — mode toggles, range tabs, unit buttons, and goal/activity pills left bare (over-buzz risk). Architectural correction surfaced: brain referenced a home-page hydration sibling at index.html ~2407 but that line is a Dexie-hydrate `vyve-localdb-table-pulled` comment, not water UI; `hydration.js` is the PF-13 welcome overlay (PM-92), NOT water tracking — naming clash flagged for future sessions hunting phantom renderers. Emerging haptic palette across PM-359/360/363/364/365/366 worth noting: selection (steppers) / light (un-ticks, optimistic list deletes) / medium (destructive commits) / success (additive logs) / heavy (achievement reveal) / error (failure) / warning (unused, reserved for pre-confirm). 5 files atomic at vyve-site `d328c9c7`, sw cache `pm364-haptics-sweep-a` → `pm365-nutrition-haptics-a`, vbb 249 → 250 both marker files. 7 inline scripts node --check clean. First-100 verify clean.)

## 19. Current status — 25 May 2026 PM-364 (Haptics bridge swept platform-wide + workouts-session set-tick wired. PM-363 had wired `VYVEHaptics.heavy()` into the `achievements.js` `showNext()` chokepoint earlier in the session — a single global wire intended to fire from every activity surface on tier reveal. PM-364 prep audit found `haptics.js` was loaded on 3 HTML pages total (index, habits, settings — the only surfaces PM-359/360 had wired). The global wire was silently no-op'ing on 38 other surfaces because the defensive `if (window.VYVEHaptics)` guard returned false — no error, no symptom, completely invisible until audited. Fix: single 42-file atomic commit inserting `<script src="/haptics.js" defer></script>` immediately after the existing `/theme.js` anchor on each of 38 candidate pages (theme.js present on 38/38 candidates with engagement.html excluded as a pure redirect stub; indent preserved per page; idempotent — index/habits/settings already had the tag and were skipped). All pages use script-src 'self' or no CSP — no CSP issues. Same commit also wired `workouts-session.js` `tickSet()`: tick branch → `VYVEHaptics.success()` at the .ticked class flip line 345 (NOT in saveExerciseLog or the set:logged bus publish — those fire after the network/Dexie round-trip and feel uncoupled from the original tap); untick branch → `VYVEHaptics.light()` (gentle correction); state-restoration path in restoreDom intentionally bypasses (programmatic restore, not user gesture). Lesson banked → §23.73 codified: when wiring a cross-cutting capability behind a bridge, audit bridge-load coverage on every target surface BEFORE claiming any per-surface wire complete. 42 files atomic at vyve-site `ddba1577`, sw cache `pm363-haptic-tier-reveal-a` → `pm364-haptics-sweep-a`, vbb 248 → 249 both marker files. First-100 verify on 6 representative files clean. node --check on workouts-session.js clean.)

## 19. Current status — 25 May 2026 PM-363 (Achievement tier-reveal haptic — single wire at `achievements.js` `showNext()` chokepoint. Every visible tier-earned toast — whether from a fresh inline-evaluator hit, a replay of unseen-cache entries on page boot, or a manual `VYVEAchievements.queueEarned([...])` call — flows through `queueEarned()` → `showNext()`. Haptic fired inside the inner requestAnimationFrame that starts the opacity+transform transition — NOT on appendChild (one frame too early, DOM offscreen), NOT after the 320ms transition completes (uncoupled from perceived arrival). `VYVEHaptics.heavy()` per the bridge's own documented intent ("strong feedback, e.g. achievement earned") — covers 32 metrics × 327 tiers from one line of code. 4 files atomic at vyve-site `db9ae1e8`, sw cache `pm362-weekly-checkin-recap-pillars-a` → `pm363-haptic-tier-reveal-a`, vbb 247 → 248 both marker files. §23.70 fired at commit-time: parallel session shipped TWO PM-362 commits between our max-PM session-start check (361) and commit time, our claim re-computed to PM-363. Without §23.70 we'd have collided. Latent bug discovered downstream — wire global, bridge local — fixed in PM-364 via platform-wide sweep; §23.73 codified to prevent recurrence. First-100 verify clean. node --check clean.)


## 19. Current status — 25 May 2026 PM-362.b (Weekly check-in recap rebuilt around canonical 4 pillars + rolling 7-day window + Lucide pillar icons. Decoupled the recap window from the submission slot: recap shows last 7 days regardless of cadence (fixes Monday cliff Dean saw on device); submission still ISO-week-locked via the wellbeing_checkins UNIQUE(member_email,iso_week,iso_year) constraint. fetchMemberData rewritten — 10-table Dexie-first read across daily_habits + workouts + cardio + movement_activities + mind_activities + connect_checkins + session_views + session_live_views + replay_views + replay_video_views, sessions de-duped by (activity_date, category) across the 4 view tables per PM-294/PM-304 overlap shape, cardio + movement + mind grouped by kind for drilldown. renderActiveFlow reshaped: 4-pillar grid (Habits days+ticks · Body activities+drilldown · Mind activities+per-kind · Connect activities+drilldown) replacing the legacy 4-row emoji shape. Lucide icons: check (habits #4DAAAA) · person-standing (body #E09B3D — matches the body-nav identity Dean's separately swapping via parallel session) · brain (mind #9B7AE0) · users (connect #E06060). Habits cap Math.min(...,7) dropped per spec (this surface is intentionally uncapped — see §23 candidate banked). EF compat: new shape maps back to legacy field names in EF POST body, zero EF change. Copy: "Here's where you're at, {firstName}" / "Your last 7 days, in your own activity." vyve-site 6037f3cc, sw cache pm361-journal-premium-prompt-a → pm362-weekly-checkin-recap-pillars-a, vbb 247 → 248 both marker files. PM-label collided with parallel session's PM-362 (Connect check-in optimistic-flip, 22885751); brain-narrative integrity preserved via .b suffix per PM-294 precedent. §23.41 fired four times across the session (PM-358 → 360 → 362, plus a fourth mid-commit gap caught only by parent_sha verification). 6 inline scripts node --check clean. First-100 re-fetch verify all 4 files clean. §23 candidate banked: "weekly check-in recap is a deliberate uncapped surface, diverges from engagement-v2 Σ min(2, dayCount) by design — don't 'fix' it" — second occurrence of deliberate canonical-divergence on this surface family, one more promotes to hard rule. Out of scope this ship: slider copy mirror to onboarding (Lewis-blocked), 8-slider monthly-shape question (Lewis scope call), monthly check-in DB triggers (next on Dean's stack — separate session).)

## 19. Current status — 25 May 2026 PM-362 (Connect check-in optimistic-flip — Dean reported tapping Post check-in did nothing on his iPhone, button stuck disabled, page sat dead. Root cause: `postCheckin()` in `connect-checkin.html` had `history.back()` gated on `fetch().then(res.json()).then(...)` — a holdover from PM-289's defensive fix for a different bug. If `jwtHeaders()` (Supabase `getSession()`) was pending or the fetch stalled, navigation never fired. Page looked dead even though Dexie + bus had already succeeded and the hub was repainting fine. Fix: move `history.back()` to run immediately after the bus publish, before the awaited network work. POST stays un-awaited in background with `keepalive: true` (kept from PM-289). 4xx → Dexie delete + bus publish `connect:checkin:failed`; 5xx + network throw → outbox enqueue. Page is gone before any of that resolves — canonical §23.39 shape, matches `focus-shell.js` `complete()` and `journal.html` `saveEntry()`. 4 files atomic at vyve-site `22885751`. sw cache `pm361-journal-premium-prompt-a` → `pm362-connect-checkin-optimistic-flip-a`. vbb 247 → 248 both marker files. §23.41 first-100 verify all 4 files clean. node --check clean. §23.70 HEAD re-check immediately before commit (was 8a164f17 PM-361, PM-362 free). Worth noting for next §23.39 doctrine touch: the audit signal "write without bus publish above" doesn't catch the regression shape — PM-289 had the bus publish but bolted awaited work after it. Sharpen to "navigation/UI-flip must happen immediately after bus publish, before any awaited fetch" next time the rule gets revised.)

## 19. Current status — 25 May 2026 PM-358 (Workout builder header safe-area + Lucide dumbbell empty-state. Two follow-ups from Dean's PM-357 verification screenshots: (1) Cancel button on the New Workout builder overlapped iOS status bar/Dynamic Island — header was `padding:0 20px; height:60px` with no safe-area-inset. §23.72 violation (now third occurrence in two days after sv-close + PM-351). Fixed: `padding:env(safe-area-inset-top,0) 20px 0; min-height:calc(60px + env(safe-area-inset-top,0))`, same pattern as `.session-view-header`. (2) Empty-state dumbbell glyph was old hand-rolled `<rect>`-stack SVG, not Lucide canonical. Replaced with 5-path Lucide dumbbell inlined directly (workouts.html has no SVG sprite — exercise.html hosts the symbol set as of PM-343). Size bumped 24×24 → 28×28 to better fill 56×56 `.empty-state-icon` container. Stroke conventions match Lucide canonical so existing CSS color cascades automatically. vyve-site `245a8afc`, sw cache `pm357-custom-workout-delete-purges-dexie-a` → `pm358-builder-header-safearea-lucide-dumbbell-a`, vbb 243 → 244 both marker files. §23.41 first-100 verify all 5 files clean. Tag balance 101/101 unchanged. JS syntax clean. §23.72 fired correctly — third instance since codified at PM-351, doing its job. Worth a one-shot grep sweep next session for remaining `position:fixed` + bare-px positioning in modal/overlay surfaces.)

 Dean: "it won't let me delete my custom workout." Confirm dialog appeared, DELETE hit Supabase fine — but the workout reappeared because `loadCustomWorkouts()` (workouts-programme.js:430-475) is local-first via `VYVELocalDB.custom_workouts.allFor(memberEmail)` and short-circuits when Dexie has rows. Dexie still had the deleted row (never purged on server-only DELETE). Canonical Dexie-first read + server-only write anti-pattern: every write path needs to purge the local mirror or the next read serves stale. Fix in `deleteCustomWorkout` (workouts-builder.js:138-178): DELETE Supabase → check `res.ok` → purge Dexie via `VYVELocalDB.custom_workouts.delete(id)` → flush VYVEData cache (defensive, no-op today) → optimistic in-memory filter + re-render → background `loadCustomWorkouts()` reconcile. Catch block surfaces errors via alert (previous code swallowed silently). Latent risk verified clean post-ship: sync.js:337 uses `replaceForMember(e, rows)` for `custom_workouts`, which `bulkPut`s incoming rows then deletes locals whose PK isn't in the incoming set (db.js:500-548). Any background hydrate after delete fetches server state (row absent), calls `replaceForMember` with shorter list, diff step removes the row again if present — converges to correct state under any race. No re-insert hazard. §23 candidate banked: server-only DELETE/UPDATE must purge corresponding Dexie row(s) before next read — second occurrence (first was PM-153 §23.7.9 optimistic INSERT FAT rows); promote to §23.7.10 on next instance. vyve-site `7795deb5`, sw cache `pm356-video-hide-modal-a` → `pm357-custom-workout-delete-purges-dexie-a`, vbb 242 → 243 both marker files. §23.41 first-100 verify all 4 files clean. Clean ship.)

 — z-index didn't beat iOS WKWebView compositor. Dean on build 241 (verified via Settings vbb-marker): "still sitting behind the exercise library." Z-index 20000 vs picker 10000 should win in CSS engine, doesn't under iOS WKWebView. Compositor promotes opaque fullscreen modals to GPU tiles and resolves stacking at the compositor layer not the DOM. Workaround: snapshot `display` of `#exercise-search-view` + `#history-view` (both z-index 10000), set `display:none` while video open, restore in closeBtn onclick. DOM and scroll position preserved — picker returns intact. Five-strike retro on PM-346 feature: PM-346 wiring → PM-351 close × safe-area → PM-352 binding mechanism → PM-355 z-index → PM-356 compositor workaround. Each layer revealed underneath. Pattern: pre-existing helper `openVideoFullscreen` written for in-session card callers, bolting on a second caller from inside a modal exposed five latent assumptions in sequence. §23.73 candidate banked: when extending pre-existing helper to new caller context, audit baked-in assumptions (z-index, stacking, compositor, opacity, focus, scroll, keyboard). Will codify on third occurrence. vyve-site `6ed2a4fd`, sw cache `pm355-video-overlay-zindex-a` → `pm356-video-hide-modal-a`, vbb 241 → 242 both marker files. §23.41 first-100 verify all 4 files clean.)

## 19. Current status — 25 May 2026 PM-355 (Fullscreen video overlay z-index 2000 → 20000 — attempted fix for video opening behind swap modal. Picker at z-index 10000, video was at 2000, overlay rendered behind. Bumped to 20000 with 10000 headroom for future modals. Close × bumped to 20001. PM-356 follow-up needed because z-index alone didn't beat iOS compositor. vyve-site `7894a3e9`.)

## 19. Current status — 25 May 2026 PM-352 (Picker thumb tap-to-preview rewritten to robust pattern after PM-346 wasn't reliable in production. Inline onclick with lazy-loaded `<img>` child unreliable in iOS WKWebView; rewrote to addEventListener post-render + pointer-events:none on inner img/svg + routed through `getVideoUrl(name)` for catalogue name normalisation (en-dash/hyphen). Copies the working in-session-card binding mechanism verbatim. vyve-site `3ae13c71`.)

## 19. Current status — 25 May 2026 PM-351 (Fullscreen exercise video close × clears iOS safe-area inset. Button at `top:16px` sat under iPhone notch — untappable. Fix: `top: calc(env(safe-area-inset-top, 0px) + 12px)` + `right: calc(env(safe-area-inset-right, 0px) + 16px)`. Hit target 40x40 → 48x48 (Apple HIG min is 44pt), × glyph 1.5rem → 1.75rem, bg opacity 0.6 → 0.7. New §23.72 hard rule: any fixed-position modal control in top/bottom 48px of viewport must use `env(safe-area-inset-*)`. vyve-site `9bef8bcd`.)

## 19. Current status — 25 May 2026 PM-348 (Reorder Exercises drag shows live destination slot during drag. Old shape: static dashed placeholder, target only computed on pointerup. New shape: floating `position:fixed` lifted tile follows pointer + travelling dashed placeholder repositioned on every move via midpoint detection + index-based skip (not name-based) for source. Added pointercancel listener for iOS interrupts. Index conversion verified for top/middle/bottom/end. Limitation: no auto-scroll near list edges for ≥8 exercise sessions, backlog candidate. vyve-site `3c142262`.)

## 19. Current status — 25 May 2026 PM-346 (Exercise picker thumbnail tap-to-preview opens fullscreen demo. Picker rows had inert thumbnails despite `workout_plans.video_url` populated for 168/297 rows. In-session card thumbnails already opened demos via `openVideoFullscreen`; picker rows missed the wire. Initial implementation used inline onclick + `ex.video_url` direct read — superseded by PM-352 robust rewrite. Data gap: 129/297 workout_plans rows have NULL video_url. vyve-site `2b1a3db8`.)

## 19. Current status — 25 May 2026 PM-345 (Workout swap preserves in-flight session state. Mid-session swap was destroying every typed-but-unticked input on every card via renderSessionBody() full re-render, orphaning sessionLog by old name, and not reconciling completedSetsCount. Three independent bugs fixed in one patch: snapshot domState by exercise_name BEFORE swap → replay post-render; carry sets/reps/rest_seconds from original slot (not reset to 3/8-12/90); delete sessionLog[old]; decrement completedSetsCount by droppedTicks. Call saveSessionState() after replay for localStorage durability. vyve-site `d296abf4`.)

 Two arrays reordered in nav.js: desktopLinks (top nav) and navItems (mobile bottom tab). Final order across both surfaces: Home → Mind → Body → Connect → More. PM-333 had reordered PROGRESS_TRACKS on home pills with the spec "apply everywhere progressively as surfaces get touched" — bottom nav stayed at the old order until tonight. SVG bodies for Body barbell + Mind brain preserved byte-identical, nav.js file size unchanged 39209→39209 (content-identical, just reordered). Companion bumps per §23.41: sw cache pm348-reorder-live-slot-a → pm349-nav-mind-before-body-a, vbb 235 → 236 on both marker files. vyve-site d4cdab8c, atomic 4-file Git Data API commit (Composio still 401-ing, Vault PAT throughout per memory #8). §23.41 first-100 verify clean. §23.69 session-start scan surfaced the parallel-session PM-341/342/343 brain-debt — those four entries (incl this one) shipped in the same brain commit. No new §23 rule.)

## 19. Current status — 25 May 2026 PM-343 (Cardio + Body section per-kind Lucide icons — three-surface overhaul bringing cardio + Body hub onto the same Lucide-canonical visual language already on Movement post-PM-336/PM-338. exercise.html (Body hub 4 tiles): ico-walk → footprints, ico-dumbbell → canonical dumbbell, ico-run → heart-pulse, ico-leaf → apple (resolves leaf=yoga conflict). cardio.html: per-kind LUCIDE_CARDIO map — running=heart-pulse, cycling=bike, swimming=droplet, rowing+other=activity — with per-kind tints (red/teal/blue/amber/purple). cardio-history.html: same map + tints + bug-fix surfaced — collectLogs was feeding hardcoded kind:'cardio' which wiped the dispatch key, now correctly feeds r.cardio_type. Symbol IDs preserved across all surfaces so downstream `<use href="#ico-X"/>` references unchanged. vyve-site 0129598d, sw cache pm342-ach-all-pillars-a → pm343-cardio-body-icons-a, vbb 229 → 230. §23.69 + §23.41 + §23.70 fired during commit — parallel PM-342 touched cardio.html (script tag) + markers; forward-applied + re-bumped from new baseline. Composio fallback throughout. No new §23 rule.)

## 19. Current status — 25 May 2026 PM-342 (Dexie-first achievements: Body + Mind + Connect + Check-ins pillars all wired in one ship. Collapses PM-335's four planned pillar sessions (PM-336/337/338/339) into one atomic 19-file commit. **Body (19 metrics)** driven by workout/set/cardio/movement:logged + workout:shared — workouts_logged, workout_minutes_total, streak_workouts, workout_before_7am, workout_days_in_a_row, programme_week_complete, first_workout, programme_complete_8wk, cardio_logged, cardio_minutes_total, streak_cardio, first_cardio, distance_total (cardio+movement union), movement_sessions_logged, streak_movement, reps_lifted_total + sets_completed_total + volume_lifted_total (sanity-capped reps<=100 weight<=500), first_custom_workout, workouts_shared_v2. **Mind (13 metrics, ALL phil_approved=false)** driven by mind:logged — handlers write earns silently, UI gates via phil_approved=false; when Phil signs, single UPDATE flips all 13 visible with backdated state. **Connect (15 metrics)** driven by session/replay:viewed + connect:reaction:logged + workout:shared — sessions_watched (session_views + session_live_views union), replays_watched (replay_views + replay_video_views union), streak_sessions, session_minutes_<category>, reactions_given, distinct_session_categories, first_session/replay_watched, workouts_shared_v2. v1 no-ops: reactions_received, checkins_with_reactions, chat_messages_posted. **Check-ins (9 metrics)** driven by wellbeing/monthly_checkin — checkins_completed, monthly_checkins_completed, first_checkin, streak_checkin_weeks, checkin_text_responses, weekly_score_climb + monthly_avg_improved (phil-gated). UI rewrite in engagement-v2.html: HABITS_DISPLAY/ICONS preserved + added BODY/MIND/CONNECT/CHECKINS maps + per-pillar computePillarValues + renderHabitsPillar collapsed to generic renderPillar(slug). Bus subscriptions expanded to 13 events, debounced 150ms repaint. Script tag fan-out to 14 publishing pages so threshold-cross toasts fire instantly. Server-side _shared/achievements.ts coexists safely (member_achievements unique constraint makes double-claim a no-op); retirement deferred. Mind silent-write justification: Dean-only test territory pre-trial, UI reveal is a one-line SQL UPDATE post Phil sign. vyve-site cbbeb62f, sw cache pm341-settings-reset-ach-a → pm342-ach-all-pillars-a, vbb 228 → 229. No new §23 rule earned, but the four-pillars-in-one-ship pattern is worth banking — saved four separate rebase cycles.)

## 19. Current status — 25 May 2026 PM-341 (Settings "Reset achievements" debug button + window.VYVEAchEval.devResetEarns helper — addresses sync.js additive-pull only behaviour for member_achievements which never reconciles server-side deletes. Capacitor app has no Web Inspector so the only path to clear stale Dexie achievement rows from device was an in-app button. Sequence: delete every member_achievements Dexie row for current member, empty in-memory earnedSet, VYVESync.hydrateTable('member_achievements') re-pull, rebuild earnedSet from fresh Dexie, UI repaints next bus tick. Settings copy stays generic with "debug" subtitle. 4-file atomic. vyve-site 3bbb916e, sw cache pm340-… → pm341-settings-reset-ach-a, vbb 227 → 228. Button removable post-trial pipe verification. No new §23 rule.)

## 19. Current status — 25 May 2026 PM-352 (Picker thumb tap-to-preview rewritten to robust pattern after PM-346 wasn't working in production. Dean's diagnostic screenshot showed play icons on Box Jump (video_url NULL) which should have been dimmed — meaning PM-346's no-video path was running for everyone, or video_url was always falsy. Two probable causes both addressed: (1) iOS WKWebView intermittently doesn't bubble taps from lazy-loaded `<img>` children to parent `<div>` inline `onclick` handlers — fixed by replacing inline onclick strings with post-render `addEventListener` wiring + `pointer-events:none` on inner img/svg so taps fire on the thumb div itself, never on the child; (2) `ex.video_url` direct read missed catalogue rows with en-dash vs hyphen mismatches — fixed by routing through `getVideoUrl(ex.exercise_name)` which uses `normaliseExName` (the same lookup the working in-session-card path uses via `loadSessionVideos`). Also fixed dataset round-trip escaping (`"` → `&quot;` instead of stale `'` → `\'` from inline-onclick days). Net: copies the in-session-card binding mechanism verbatim instead of inventing a parallel variant. Retro: PM-346 shipped working code with a fragile binding pattern when a working pattern existed three functions away — should have copied that one. vyve-site `3ae13c71`, sw cache `pm351-fullscreen-video-x-safe-area-a` → `pm352-picker-thumb-click-robust-a`, vbb 238 → 239 both marker files. §23.41 first-100 verify all 4 files clean. No new §23 hard rule promoted — "copy existing working mechanism rather than design fresh variant" banked as candidate pending second occurrence.)

 clears iOS safe-area inset. Dean after testing PM-346: "the X is too high in the corner that I can't actually click it." Button at `top:16px` sat under iPhone status bar / notch (~47-50px chrome before content area) — untappable. Fix: `top: calc(env(safe-area-inset-top, 0px) + 12px)` + `right: calc(env(safe-area-inset-right, 0px) + 16px)` — resolves to iOS-reported inset on notched devices, 0px fallback elsewhere. While there: hit target 40x40 → 48x48 (above Apple 44pt thumb-reach guideline, was below), × glyph 1.5rem → 1.75rem, bg `rgba(0,0,0,0.6)` → `0.7`, added flex centring + `padding:0` + `line-height:1` for mathematical × centring (was off-centre due to default button padding). Single-file behavioural change in `workouts-programme.js` `openVideoFullscreen()`. Misread on my part early in turn: chased "exercise library" as a separate surface — there isn't one. `exercise.html` is Body hub (Movement/Workouts/Cardio streams), workouts.html "Browse Library" tab is *programme* catalogue. The picker IS where PM-346 wiring lives and Dean's screenshot confirmed it working. vyve-site `9bef8bcd`, sw cache `pm350-ach-focus-pillar-live-a` → `pm351-fullscreen-video-x-safe-area-a`, vbb 237 → 238 on both marker files. §23.66/§23.70 fired once — claim PM-349 → PM-350 → PM-351 cascade as parallel session shipped twice during my build; clean sed-rebase. §23.41 first-100 verify all 4 files clean. **New §23 hard rule earned (see §23.72):** any fixed-position modal control within top 48px or bottom 48px of viewport must use `env(safe-area-inset-*)` insets, not bare px values. Second occurrence of this class of bug.)

 now shows live destination slot. Dean's feedback: drag feels off, can't tell where the row will land. Considered up/down buttons as alternative — landed on fixing the drag instead since drag is the iOS-native pattern members expect. Three changes in `workouts-exercise-menu.js` `pointerdown`/`rOnMove`/`rOnUp`: (1) source row switches to `position:fixed` with `scale(1.02)` + shadow + 0.96 opacity = floating tile that tracks pointer cleanly above the stable list; (2) dashed teal placeholder inserted where source was, repositioned every `pointermove` via midpoint detection (pointer-y < otherRow midpoint = insert before) using `insertBefore`/`appendChild` → other rows reflow naturally; (3) `pointerup` trusts `rTargetIdx` (placeholder's current slot), splices source into `reorderExercises[rTargetIdx]` directly, no more closest-midpoint re-search. Index conversion verified for top/middle/bottom/end cases — otherRows excludes source so splicing source out makes otherRows match reorderExercises order, rTargetIdx = direct insertion index. Added `pointercancel` listener routing to cleanup for iOS interrupts (call/background mid-drag). Limitation noted: no auto-scroll of `.reorder-list` when dragging near top/bottom — fine for typical 4-6 exercise PPL sessions, backlog candidate for ≥8-exercise sessions. vyve-site `3c142262`, sw cache `pm347-ach-toast-deeplink-a` → `pm348-reorder-live-slot-a`, vbb 234 → 235 on both marker files. §23.66/§23.70 fired once — initial PM-347 claim collided with parallel session's ach-toast-deeplink, clean sed-rebase to PM-348 + re-fetch of live marker files to rebase from 233 → 235. §23.41 first-100 verify all 4 files clean. No new §23 hard rule.)

 Dean: "they sit behind the thumbnail and if they click it the video loads." Picker rows (swap modal + builder multi-select modal) had inert thumbnails despite `workout_plans.video_url` being populated for 168/297 rows. In-session card thumbnails already opened the demos via `openVideoFullscreen` (workouts-programme.js:335-346); picker rows missed the wire. Single-file patch in `workouts-exercise-menu.js` `renderExerciseList`: thumb gets `onclick="event.stopPropagation(); openVideoFullscreen('${url}')"` + `cursor:pointer` when `video_url` exists; `stopPropagation` is the key bit since row outer onclick is `selectExercise` (swap) or multi-select toggle (builder) — preview must ≠ select. No-video thumbs dimmed to `opacity:0.55` so the play-icon `::after` and image both fade together = clear visual cue "no demo here". No new CSS/JSON/modal — fully reuses existing fullscreen player. Data gap surfaced: 129/297 workout_plans rows have NULL video_url, backlog candidate for video coverage fill. vyve-site `2b1a3db8`, sw cache `pm345-swap-preserves-progress-a` → `pm346-picker-thumb-tap-preview-a`, vbb 232 → 233 on both index.html + settings.html. §23.41 first-100 verify clean. No new §23 earned — behavioural addition on existing rails.)

 Dean's feedback: mid-session swap was resetting the workout. Three independent bugs in `workouts-exercise-menu.js` `selectExercise` swap branch, all fixed in one patch. (1) Cross-card DOM wipe — `renderSessionBody()` re-rendered every set card from `sessionExercises`, vapourising any kg/reps typed-but-unticked on every card, not just the swapped slot. Fix: snapshot domState by exercise_name (same shape as `saveSessionState` workouts-session.js:67-103) BEFORE the mutation, replay post-render for every slot except the swapped one (matched by index, captured before nulling swapExerciseIdx). (2) Template clobber — replacement was forced to `sets: 3, reps: '8-12', rest_seconds: 90` regardless of original targets; 5x5 strength blocks silently became 3x8-12 on swap. Fix: carry sets/reps/rest_seconds/letter_badge from original slot. (3) Orphan sessionLog + completedSetsCount drift — old-name `sessionLog` entry would never save (slot no longer in sessionExercises), and the in-session ticked-set counter wasn't reconciled when ticked sets on the swapped slot were dropped. Fix: `delete sessionLog[original]` + `completedSetsCount = max(0, completedSetsCount - droppedTicks)`. Final step: `saveSessionState()` after replay so localStorage `vyve_active_session` matches post-swap DOM for background→foreground restore. Limitation noted but not fixed: name-keyed snapshot can collide if the same exercise appears in two slots — pre-existing in `saveSessionState`, candidate for slot-index+name composite key in a future tightening pass. vyve-site `d296abf4`, sw cache `pm344-nav-icons-lucide-a` → `pm345-swap-preserves-progress-a`, vbb 231 → 232 on both index.html and settings.html. PAT-direct path via Git Data API (memory #11 — Composio not loading via tool_search this session). §23.41 first-100 verify on all 4 files at commit SHA matched. No new §23 earned — behavioural fix, not procedural; the cross-card wipe was a missed test case at the time of the original swap implementation, not a recurring class.)

 Bug: Dean reviewed PM-333 on device, certificate stars firing on Mind/Body/Connect despite low actual activity. Cause: PM-331 used raw row counts; multi-row-per-day tables (Mind/Body/Connect at 2/day cap, plus Connect spreading across 4 tables) blew past 30 quickly. Fix: strict mirror of engagement-v2.html renderProgressTab (lines 970-1078) — Habits = distinct activity_date days; Mind/Body/Connect = Σ min(2, dayCount) with appropriate table merges; Check-ins = distinct ISO weeks via wellbeing_checkins. Same 10 tables engagement-v2 pulls; same dayKey extraction; same ISO week key (Jan 4 anchor). Table-pulled handler extended to also include session_views + replay_video_views + session_live_views since the new Connect maths reads those three. Doctrine: home and engagement-v2 must never disagree on counts; maths duplicated inline for now, candidate for `pillar-math.js` shared helper refactor. Where Dean's stated model diverges (he wants Check-ins to also count live session weekly check-in + monthly_checkins; engagement-v2 currently only counts wellbeing_checkins weeks): home matches canonical surface, not ahead of it — extension is a future ship via shared helper. vyve-site `2f0762d1`, sw cache `pm333-pills-bigger-reorder-carousel-sessionsdata-a` → `pm334-pills-engagement-v2-maths-a`, vbb 220 → 221, settings-vbb 220 → 221. §23.41 fresh-HEAD clean twice, no drift. §23.52(a/c) honoured. §23.52(b) files[].status `{modified: 3}`. §23.53 first-100 re-fetch matched. 9 inline scripts node --check clean. Tag balance 29/29. No new §23 hard rules earned. Lesson worth banking: row count is never activity count — always match the canonical surface's dedupe/cap maths. Second occurrence of this class of bug (first was PM-159). One more recurrence and it earns a §23.)

## 19. Current status — 25 May 2026 PM-333 (Three home follow-ups: pills bigger + Mind-before-Body + carousel switched to VYVE_SESSIONS. (1) Ring `56→64px`, stroke `4→4.5`, ring-val font `0.86→0.95rem` — the `N/30` reads at arm's length now. (2) PROGRESS_TRACKS reordered to **Habits → Mind → Body → Connect → Check-ins**. Will propagate to other surfaces as they get touched (engagement-v2, achievements page, hub nav). (3) Live Sessions This Week carousel switched from `calendar_occurrences` (Dexie-synced, async, requires sync.js pull) to `window.VYVE_SESSIONS` from `sessions-data.js` (hardcoded recurring catalogue, synchronous, always-present). Dean's device showed nothing on home while Connect's `renderLiveThisWeek` painted full — PM-190.d's lesson re-applied: hardcoded sessions-data.js is the right source for the fixed-recurrence catalogue, Dexie is unnecessary plumbing for this shape of data. Verbatim port of connect.html's renderLiveThisWeek into home's paintLiveCarousel. Dropped LIVE_CAT_SHORT, fmtLiveWhen, DAY_SHORT helpers, and the `calendar_occurrences` branch in vyve-localdb-table-pulled. Added `<script src="/sessions-data.js" defer>` to head (sw.js already precached it). calendar_occurrences table stays in Dexie schema — connect-calendar.html still uses it. vyve-site `65038984`, sw cache `pm332-yt-ready-referrerpolicy-a` → `pm333-pills-bigger-reorder-carousel-sessionsdata-a`, vbb 219 → 220, settings-vbb 219 → 220. §23.41 fresh-HEAD fired once (parallel PM-332 marker-only collision, clean sed-rebase PM-332→PM-333 + Update 219→220 + cache key swap). 9 inline scripts node --check clean. Tag balance 29/29. files[].status `{modified: 3}` per §23.52(b). No new §23 hard rules earned.)

## 19. Current status — 25 May 2026 PM-331 (Home progress pills cert badges + live carousel connect-parity. Two surfaces touched in `index.html`: My Progress rings switched from legacy 5 labels with hardcoded zeros to canonical engagement-v2 five — **Habits/Body/Mind/Connect/Check-ins** — with real lifetime counts from Dexie. Ring shows `value % 30` toward next certificate (resetting sprint loop, opposite to the PM-322 Achievements page's escalating tier ladder by design — home is daily-driver, Achievements is longer game). Gold Lucide filled-star + count badge below label = `floor(value/30)` certificates earned, hidden when 0 but slot reserved so layout doesn't shift. Pillar palette via `.pill[data-pillar="X"]` selectors mirrors engagement-v2 canonical colours. `loadPillarCounts()` does 7-parallel Dexie reads matching engagement-v2.html exactly (daily_habits / workouts+cardio+movement merged / mind_activities / connect_checkins / wellbeing_checkins); hooked into boot, `vyve-localdb-table-pulled`, and `_rerenderHome` so optimistic writes refresh rings instantly. Live Sessions This Week carousel: replaced hero-style full-bleed `.live-card` pattern with connect.html's `.scroll-card` pattern lifted verbatim from lines 480-504. Data path unchanged (calendar_occurrences from Dexie, PM-256). `image_url` now rendered (was ignored, every card painted as generic placeholder because all 193 active rows have null session_title/host_name in current backfill). Short category label map drops "Yoga, Pilates & Stretch" → "Yoga & Stretch". Host line dropped entirely when null — no more "VYVE team" filler. Live-now broadcast gets green eyebrow + ● LIVE NOW. Empty state shows nothing per §23.46 — LIVE_PLACEHOLDERS retired. vyve-site `af32355a`, sw cache `pm330-live-iframe-restore-explicit-origin-a` → `pm331-home-pills-cert-carousel-parity-a`, vbb 217 → 218, settings-vbb 217 → 218. §23.41 fresh-HEAD fired once mid-build (parallel session shipped their PM-330 live-iframe restore touching same three marker files — marker-only collision, full sed-rebase clean from PM-330 → PM-331 + Update 217 → 218 + cache-key swap). §23.70 PM-claim-at-commit-time also fired correctly. 9 inline scripts node --check clean. Tag balance 28/28. files[].status `{modified: 3, added: 0, removed: 0}` per §23.52(b). First-100 re-fetch verification on all three files at committed SHA matched. No new §23 hard rules earned.)

## 19. Current status — 25 May 2026 PM-310 → PM-315 (Live tracker device-walk chain. PM-304 ship landed end-to-end code but YT IFrame API never attached on device. Five-commit diagnostic chain followed by root-cause fix. **PM-310** `4c8b93e8`: debug strip ALWAYS-ON (was `?debug=tracker` URL-gated in PM-304 — broken in Capacitor native app, no address bar; banked memory #24). Strip expanded to show page state alongside tracker state, higher-contrast styling. 4 files. **session-publish EF v2** deployed same window: `enableAutoStop: true → false` so broadcasts no longer auto-kill at scheduled_end_time (bad UX if instructor runs over) — they now end only when instructor stops Riverside (RTMP ingest stops → YouTube transitions broadcast to complete after ~30s idle); banked memory #25. **PM-311** `c165743d`: YT.Player init-path diagnostics — session fields populated at each step (`ytLoaded`, `playerConstructedAt`, `ytLoadError`, `ytConstructError`, `ytError` via new onError handler), strip shows each in red on failure. 5 files. **PM-312** `59a44eee`: "Force refresh app" button on Settings under Build row (three-step nuclear: caches.keys() → delete all, navigator.serviceWorker.getRegistrations() → unregister all, location.reload(true)). Dean force-quit 18+ times, JS still served PM-310 — needed an explicit nuke. Parallel-session PM-312 (Home padding lift) collision; tree merge additive. 3 files. **PM-313** `85d02d06`: TEMP propagation test — LIVE pill text "Live now" → "PROPAGATION TEST 313" to determine whether GitHub Pages CDN was lagging. Dean confirmed pill changed visually AND strip's new init-path fields appeared with `ytLoadError: Failed to load YT IFrame API script` in red — propagation healthy, the strip header showing "PM-310" was static text never bumped per ship, cosmetic only. 4 files. **PM-315** `15b3a431`: ROOT CAUSE FIX. CSP on all 8 *-live.html shells (inherited from PM-251 template) had strict `script-src 'self' 'unsafe-inline' jsdelivr/cdnjs/unpkg/supabase/posthog` — `https://www.youtube.com` not whitelisted. `loadYTApi()` injected `<script src="https://www.youtube.com/iframe_api">`, CSP blocked it silently, promise rejected, player never attached. Added `https://www.youtube.com https://s.ytimg.com` to script-src on all 8 shells (s.ytimg.com chains for player.js / www-widgetapi.js). Replays work because replays.html has no CSP at all — different shell, no policy inherited. Also: PROPAGATION TEST 313 pill reverted, strip header bumped PM-310 → PM-315 (cosmetic), settings-vbb drift catch-up. 12 files atomic via Git Data API. sw cache `pm314-home-hero-shrink-140px-a` → `pm315-csp-youtube-allow-a`, vbb 203 → 205, settings-vbb 204 → 205. **§23.66 fresh-HEAD audit fired five times across the chain** — caught parallel PM-312 (Home padding) and PM-314 (Home hero shrink); both additive merges, no overwrites. **State-machine bug logged for PM-316+** (NOT fixed this chain): LIVE state currently gates on clock-time only (`row._start <= now < row._end`), so YouTube `enableAutoStart=true` broadcasts going live before scheduled `starts_at` don't flip page state until clock arrives. Banked memory #23 with the test-time workaround (schedule `starts_at = now()` or `now+30s`). Architectural fix needs a YT broadcast status probe overriding state when broadcast is actually live ahead of schedule. **Not yet validated** — Dean ended session before PM-315 walk; CSP fix is hypothesis-confirmed-by-PM-311-diagnostic (strip showed exactly the failure mode CSP produces). Next session resumes the walk with a fresh test row. No new §23 hard rule earned — lessons banked as memories #23/24/25.)

## 19. Current status — 25 May 2026 PM-307 (Movement first-class — `movement_activities` table replaces two-table routing. The big structural change: Dean asked for Movement/Workouts/Cardio as three separate tracks under Body, all rolling up to progress. Movement was implemented since 19 April as a "virtual" track that wrote walks into `cardio` with `logged_via='movement'` and stretch/yoga/pilates/etc into `workouts` with `plan_name='Movement'`. That two-table routing produced PM-159, PM-169, and PM-303-class bugs across `movement.html` + `movement-history.html` + the home/engagement readers. PM-307 collapses to one table: `movement_activities`, mirror of `mind_activities`, `kind` ∈ walk/stretch/yoga/mobility/pilates/other as the only discriminator. 24 historical rows migrated id-preserving with charity/counter triggers DISABLED around the source-table deletes to preserve aggregates. Client surfaces (8 files): db.js SCHEMA_V15 + sync.js hydrate config + movement.html full rewrite (single payload, single Dexie store, single bus event `movement:logged`/`movement:failed`) + movement-history.html single-store collectLogs + engagement-v2.html BODY metric workouts+cardio+movement cap-2/day with Movement getting its own pill in the activity log + index.html subscribes + settings.html vbb catch-up + sw.js cache key. Bus taxonomy locked: `movement:logged` fires only from movement.html; `cardio:logged` and `workout:logged` continue from their owning pages. Dead filters in cardio.html/cardio-history.html/workout-history.html on the old discriminators left in place as no-ops post-migration; cleanup deferred to PM-308 (cardio walking-removal + per-member custom cardio kinds, Dean's next ask). vyve-site `ba172e94`, sw cache `pm307-movement-activities-table-a`, vbb 197. Supabase migration `pm_304_create_movement_activities` (filename PM-304 is historical artefact — vyve-site PM-304 was session_live_views from the other session; migration names are append-only and irrelevant for forensics, the brain entry PM-307 is the canonical reference). §23.14 fresh-HEAD recheck fired THREE times (PM-304+305 between session start and first commit attempt; PM-306 mid-rebase; final commit clean) — each recovery used per-file re-fetch + surgical re-apply on the three vbb-marker/sw files, the five untouched data/wiring files verified byte-equal via md5sum-on-file (echo-piped md5 strips trailing newlines and false-positives — only md5sum on a written file is safe). Code-comment inconsistency note: inline comments inside the five rewritten files still say "PM-306" because that was the working number through most of the rewrite; commit message + brain entry both PM-307, accepted minor inconsistency. No new §23 hard rule earned.)

**PM-303 (25 May 2026, vyve-site `6e83fbba`).** Race-fix mechanic in movement.html, 4 files. Dean's device test surfaced: log walk on movement.html → see in Recent Movement → navigate away → return → empty state. Root cause: the optimistic Dexie upsert is fire-and-forget; the subsequent `renderMovementLog()` opens a fresh Dexie tx that can open before the put tx commits. IDB serialises rw tx on the same store, but Dexie's `put` promise resolves on operation-queue, not on `tx.complete` — `_mvCollectLogs()` returns pre-write state, `writeMvLogCache(stale)` overwrites the cache, next cold-paint reads stale and renders empty. Fix: synchronous cache prepend — synthesise the cache row from the same payload we just wrote, prepend to existing cache, slice to 5, `writeMvLogCache + _mvRenderRows`. The async Dexie reconciliation still runs as belt-and-braces second pass. Files: movement.html + sw.js cache key bump pm302→pm303 + index.html vbb 192→193 + settings.html drift catch-up 189→193. No new §23 rule earned — race-fix mechanic mirroring the cardio.html readCache pattern. **Pattern now superseded by PM-307's single-table shape — the cache prepend logic was adapted but the underlying race no longer applies the same way (one Dexie store instead of two).**

## 19. Current status — 25 May 2026 PM-306 (Home hero -10px tighten — single-surface visual refinement on `index.html`. Hero height + main padding-top both dropped from `max(250px, 35vh)` to `max(240px, calc(35vh - 10px))` per §8 paired-values invariant; per-pillar hubs (Body/Mind/Connect) untouched at canonical 35vh from PM-247. Dean's spec: photo location and composition correct, just bottom of band should sit 10px higher so content below floats up. Mind initially mis-diagnosed in-chat as the offender but source-confirmed already at 35vh; only Home asked for the tighten. 3-file atomic vyve-site commit `9c561e98`. sw cache `pm305-engagement-redirect-a` → `pm306-home-hero-tighten-a`. vbb-marker 195 → 196. settings-vbb-marker 194 → 195. Composio still 401 ~5 days; Vault PAT direct via Supabase MCP throughout per §23.45. §23.41 fresh-HEAD checked twice, no drift. §23.52(a)/(c) and §23.53 honoured. No new §23 rule earned — pure visual tighten using established doctrine.)

## 19. Current status — 25 May 2026 PM-304 (Live-session per-broadcast watch-time attribution shipped. Unified `player-tracker.js` polymorphic on `mode='replay'|'live'` replaces `replay-tracker.js` with back-compat alias preserved. New `session_live_views` Supabase table mirrors `replay_video_views` shape with FK to `calendar_occurrences(id)` instead of `replay_videos`. `session-live.js` `syncPlayerTracker(state, ctx)` lifecycle inits tracker on entry to LIVE / destroys on exit / no-op on same-broadcast re-render; iframe URL gains `enablejsapi=1` + `origin` for IFrame API attach; stable `id="sl-live-iframe"` for tracker pickup. `?debug=tracker` on-page debug strip mirrors PM-296 replay strip shape. Both `engagement.html` Variety and `engagement-v2.html` Connect metric read `session_live_views` additively. Threshold 30s (parity with replays). Completion stamped on YT_ENDED for live regardless of pct_watched (broadcast over = watched). vyve-site `7764e863`, 20 files atomic. sw cache `pm303-movement-cache-race-a` → `pm304-live-tracker-a` (twice-rebased per §23.66 double-fire). vbb 193 → 194. settings-vbb 193 → 194. Supabase migration `pm299_create_session_live_views` (filename carries early-session PM number; the migration is PM-304's table — Supabase migration names append-only, no rename). **`§23.66` fired TWICE in one ship cycle** (parallel-session PM-302 connect.html script-tag fix, then PM-303 movement.html cache prepend race); both rebases recovered cleanly with per-file content re-fetch + surgical re-apply + sed sweep of PM number across 7 files. **`§23.67` codified** — script-tag INCLUSION auditing during new-feature ship (inverse of PM-294's deletion auditing; PM-302's connect.html `home-state-local.js` miss + PM-294's tracking.js drop are two occurrences of the same root-shape bug in one night). **What's still owed:** live device walk (Path C — manual session-publish invocation for near-now `calendar_occurrences` row, Riverside stream against Yoga RTMP key, walk PRE_ROLL → LIVE → tracker attach → 30s threshold → upsert cadence → broadcast end). Delete orphan `replay-tracker.js` from repo + sw.js precache after device walk confirms. PM-294 follow-up #4 (per-category cumulative watch-time achievements) banked in backlog — design tier thresholds after ~1 week of trial data.)

## 19. Current status — 24 May 2026 PM-285 → PM-293 (Habits-on-home polish + cross-page sync — six-commit arc closing the home habit surface to production form. PM-285 mood panel in-flow card (was `position:fixed` glass overlay on hero base, now scrolls with content as relative in-flow element inside `<main>`; existing `.hidden`/`.collapsing` show-hide states unchanged; light-mode background override added). PM-286 Lucide habit icons — 36 SVG `<symbol>` defs inlined into body, `iconForHabit(title, pot)` rewritten with 4-level lookup (exact title → keyword fallback → pot fallback → generic `circle-dashed`), all 34 active `habit_library` titles name-matched against Lucide names verified live before drafting; same commit added uncompleted-first sort, always-visible "See all ›" → /habits.html, and `habits:set-changed` bus subscriber so settings edits propagate to home instantly. PM-287 pot-coloured tiles matching progress-ring palette (mindfulness purple `#B186C9` / movement green `#8FC470` / nutrition deep amber `#D89760` / sleep teal-light `#4DAAAA` / social orange `#F0A075`), Lucide `currentColor` inheritance picks up the tint automatically per glyph. PM-288 done-state teal with theme-conditional split (teal-light `#4DAAAA` on dark, deep VYVE teal `#1B7878` on light) — gold retired from done indicator entirely, reserved for future "earned" signals. PM-291 home↔habits.html cross-page sync — `togglePill` on home now publishes `habit:logged` envelope matching habits.html schema (`kind:'canonical'`, `habit_id`, `is_yes`, `logged_at`, `synthetic_key`, `sign`), home subscribes to its own `habit:logged` with a second `loadHabits()` handler alongside the legacy `_rerenderHome` (legacy rebuilds dashboard via `buildHomeFromDexie` but doesn't touch the new `__habits` vertical list); same commit added overflow expander dashed pill `+ {N} more ▾` / `Show less ▴` when `__habits.length > 5`, state in-memory only resets on page boot. PM-293 envelope-aware untick race fix — habits.html undo runs Dexie delete fire-and-forget then publishes, subscriber's `loadHabits()` read stale Dexie; fix mutates `__habits` synchronously from envelope's `sign` + `habit_id` then renders, then second-passes `loadHabits()` for completeness. Earns §23.65 — envelope-trusted subscriber pattern. Final state: vyve-site `58c01d3c`, sw cache `pm293-habit-untick-race-a`, vbb-marker 183. Six commits all mine; other Claude session (engagement-v2 + connect-hydration campaign) interleaved four commits in the same window — §23.14 fresh-HEAD-recheck fired four times and recovered cleanly each time; other session shipped §23.14 violation in PM-290 that overwrote PM-291, their PM-292 was a recovery commit restoring the erased content within ~90s. Note: my PM-289 → renumbered to PM-291 to avoid collision with other session's PM-289 (Elite hero) that landed in the same window. PM-numbering across the two sessions is consistent now.)
...
### §23.65 — Envelope-trusted subscriber when publishing surface uses fire-and-forget Dexie writes (PM-293, 24 May 2026)

**Earned PM-293.** Habits.html undo flow (`undoHabit()` at L772) runs `VYVELocalDB.daily_habits.delete()` fire-and-forget — the promise is **not** awaited — then immediately publishes `VYVEBus.publish('habit:logged', { sign:-1 })`. The fire-and-forget ordering is correct for the publisher's own UX (no `await` blocks the user-perceived tap), but it creates a race for cross-page subscribers: they arrive in a window where Dexie still holds the old state. Subscribers that re-read Dexie as their only state-update path produce stale paints. Dean's device test: untick on /habits.html, nav to home, tile stays in done state for ~50ms until Dexie completes + a second event/timer triggers re-render. Visibly broken.

**Rule.** Cross-page bus subscribers MUST NOT trust Dexie as the sole source of truth for the state change the event describes, when the publishing surface is known to use fire-and-forget Dexie writes (the §23.39 default for premium-feel UX). The subscriber applies the envelope's signal (sign / primary key / discriminator) to the active surface's in-memory state **synchronously**, renders, AND THEN re-reads Dexie as a second pass for state not in the envelope (assignment-set changes, related rows, anything else that might have moved).
...
VYVEBus.subscribe('<event>:logged', function (envelope) {
  if (envelope && envelope.<primary_key>) {
    // 1. Synchronous in-memory mutation from envelope — render immediately
    var change = decodeChangeFromEnvelope(envelope);  // sign:-1 → remove, sign:+1 → add, etc.
    applyToActiveState(change);
    if (typeof renderActiveSurface === 'function') renderActiveSurface();
  }
  // 2. Second pass — re-read Dexie for state not in envelope (assignment changes etc.)
  if (typeof reloadActiveData === 'function') reloadActiveData(email);
});
```

The two passes serve different purposes: pass 1 discharges the user-perceived state-change contract (the thing the envelope explicitly describes); pass 2 catches anything the envelope didn't carry. By the time pass 2's Dexie read fires, the fire-and-forget delete has usually completed.

**Audit signal.** `grep -nE "VYVEBus\.subscribe\(['\"][a-z_]+:logged" *.html *.js`. For each match, check whether the subscriber calls only a `loadX()` / `fetchX()` style helper (Dexie-reread-as-only-path). If yes, AND the publishing surface for that event uses fire-and-forget Dexie writes (default), the subscriber needs the envelope-aware pattern. PM-293 fixed the home subscriber for `habit:logged`; surfaces still to audit: engagement.html, mind.html, connect.html, exercise.html (all have hub-surface subscribers).

**Distinct from §23.7.6.** §23.7.6 covers the *active surface* mutating its own state synchronously on local tap (the publisher fixing its own UX). §23.65 covers *cross-page subscribers* reacting to remote publishes (the subscriber fixing its own paint because the publisher's Dexie write isn't awaitable from the subscriber's perspective). Both close the same race class, in mirror positions.

**The envelope contract this depends on.** Publishers MUST include the primary key and a signal of the change direction (sign / is_yes / kind discriminator) in the envelope. The habit-event schema (`{kind:'canonical', habit_id, is_yes, logged_at, synthetic_key, sign}`) is the canonical example — subscribers can decode change direction from `sign` or `is_yes` alone. Future event schemas should preserve this property: any event that subscribers might re-paint on must carry enough payload that subscribers don't need to re-read storage to know what changed.
...
### §23.67 — Script-tag inclusion auditing during new-feature ship (PM-304, 25 May 2026)

**Status: HARD RULE.** Earned across two occurrences in one night.

**The pattern.** A new feature (or a refactor that introduces a new client-side module dependency) lands runtime code on one page (the publishing surface or the new module) but the script tag that loads that module is missing from a CONSUMING page. The consumer page's call into `window.VYVEModuleName.foo()` silently bails because `window.VYVEModuleName` is undefined; the function's own input guard catches the undefined and returns cleanly; no error is thrown; the feature appears to be working but renders/computes nothing meaningful. Sibling of `§23` candidate from PM-294 around script-tag DELETION during refactors. Both shapes — adding code that needs a script tag, removing code that had one — have the same failure mode: silent.

**Two occurrences in one night (25 May 2026):**
1. **PM-302 (parallel-session, `a977c749`).** `connect.html` called `VYVEHomeStateLocal.computeHomeStateFromDexie(memberEmail)` but never loaded `/home-state-local.js`. Window object undefined → input guard at function head returned without setting state → Elite hero dot strip rendered empty + streak counter stayed at HTML default `0` despite server-side streak of 5. Bug existed from PM-198 (22 May 2026) — three days undetected because nobody'd reproduced the cold-paint path with an existing-row member on connect.html.
2. **PM-294 the-live-tracking-restoration (`537c73b1`, 24 May 2026).** PM-251's state-machine refactor dropped `<script src="/tracking.js">` from all 8 `*-live.html` shells. Manual "Mark as completed" CTA in the new state machine looked like a working replacement during PM-251 review; the silent loss of automatic session-view attribution on every live page from PM-251 ship onward was invisible until Dean cross-checked Supabase for missing `session_views` rows.

**The audit pattern.**

```
# Files that REFERENCE a global but don't INCLUDE its script
grep -l "VYVEHomeStateLocal\b" *.html | while read f; do
  grep -q "home-state-local.js" "$f" || echo "MISSING include: $f"
done

# Reverse — files that INCLUDE a script but never reference its global
# (rarer, less critical — usually just dead include weight)
grep -l "home-state-local.js" *.html | while read f; do
  grep -q "VYVEHomeStateLocal\b" "$f" || echo "DEAD include: $f"
done
```
...
### §23.72 — Fixed-position modal controls in iOS chrome zones must use safe-area-inset (PM-351, 25 May 2026)

**Rule.** Any fixed-position interactive element within the top 48px or bottom 48px of viewport on a fullscreen modal/overlay must use `env(safe-area-inset-top, 0px)` / `env(safe-area-inset-bottom, 0px)` / `env(safe-area-inset-left, 0px)` / `env(safe-area-inset-right, 0px)` in its positioning, not bare pixel values. Fallback parameter (`, 0px`) is mandatory for browsers that don't support `env()`.

**Why.** iPhones from XS forward have a status-bar / Dynamic Island / notch zone (~47-50pt) at the top, and a home-indicator zone (~34pt) at the bottom. Landscape adds left/right insets too. A control rendered at `top: 16px` sits *underneath* iOS chrome on every modern iPhone — visually hidden by the status bar overlay and untappable because iOS captures touches in that zone. Bare-px positioning works in browser dev tools (no safe-area) but fails on every real device.
...
The `+ Npx` is the gap *between the inset boundary and the control* — pick 10-16px for visual breathing room. Don't omit the fallback parameter — Chrome desktop and older Safari versions need it.

**Also: hit-target minimums.** Apple HIG says 44pt minimum for tappable controls; we ship 48px to give 2px margin against the rounded button corner clipping the visual edge of the hit area. Don't ship 40x40 buttons; that's below the guideline and members with larger fingers (or in motion) miss them.

**Detection at review time.** Search for `position:absolute|fixed` paired with `top:|bottom:|left:|right:` containing a bare px value within the first or last 48px of viewport. CSS audit script could flag these automatically in future; for now, eyeball every modal close/back/CTA control during code review.

**The cost of not having this rule.** PM-346 shipped the picker tap-to-preview wiring as a positive change, but the player it opened had a pre-existing `top:16px` close button that put the × under the iPhone notch. Dean discovered the × was uncloseable only after PM-346 made the player more commonly opened. Two-strike pattern (this and an earlier sv-close issue) — promoting to a hard rule before it happens a third time.

### §23.73 — Audit bridge-load coverage before claiming any per-surface wire complete (PM-359 → PM-364, 25 May 2026)

**Rule.** When wiring a cross-cutting capability behind a JavaScript bridge (window-namespaced shim, single `*.js` file, defensive `if (window.X)` guards at every call site), the per-surface call-site wire is only half the work. The other half is verifying the bridge `<script src="/...">` tag is loaded on every target surface that can fire the wire. The defensive guard at the call site means missing bridge-load is silent — `if (window.VYVEHaptics)` is false, the call is a no-op, no error, no log, no symptom visible to the developer running browser DevTools on the one page where the bridge IS loaded. Failure mode is invisible until tested on a surface the original wiring PM didn't think to test.

**Trigger conditions.** This rule applies when ALL of the following are true:
1. The capability is exposed via a window-attached namespace (`window.VYVEHaptics`, `window.VYVEData`, `window.VYVEBus`, `window.VYVELocalDB`, etc.)
2. Call sites guard the namespace with a defensive `if (window.X)` or `try/catch` wrapper
3. The capability is intended to work across multiple page surfaces (not page-local)

If any of those three is missing, this rule doesn't apply — page-local helpers don't need the audit, and unguarded namespace access throws loudly which surfaces the gap immediately.

**The audit pattern.** Before claiming any PM that adds a wire of this shape complete:
1. Identify the bridge file (`haptics.js`, `bus.js`, `db.js`, etc.)
2. Grep across all root-level HTML for `bridge_filename` — count load sites
3. Cross-reference against the set of surfaces that could fire the wire (every page that could trigger the capability — for achievements that's every activity surface; for the bus that's every page; for hydration helpers that's every page that does Dexie writes)
4. If the bridge-load set is a proper subset of the trigger set, the wire is incomplete — sweep the bridge to the missing surfaces in the same PM OR ship a follow-up PM before claiming the wire is platform-live

**The pattern that earned this rule.** PM-359 wired `VYVEHaptics.success()` to `habits.html` daily-habit log success. PM-360 added the home-page renderer (sibling-renderer audit per §23.69 caught the missing surface). Both PMs claimed "haptics wired" and went to changelog. Then PM-363 wired `VYVEHaptics.heavy()` at the `achievements.js` `showNext()` chokepoint — a single wire intended to fire from every activity surface (32 metrics × 327 tiers). Discovery during PM-364 prep: `haptics.js` was loaded on 3 pages total. PM-363's wire was silently no-op'ing on 38 surfaces. PM-364 swept the bridge platform-wide and retroactively unblocked PM-363.

**Why this is distinct from §23.67 (script-tag inclusion auditing).** §23.67 catches missing dependent script tags when shipping a NEW feature on a NEW page (the new page didn't load the supporting library). §23.73 is the reverse direction: an EXISTING capability behind a bridge, where the bridge has limited surface coverage and a new wire assumes platform-wide coverage. The fix shape is also different: §23.67 = add the missing tag on the new page; §23.73 = sweep the bridge to every surface the capability needs to fire on.

**Detection during code review.** When reviewing a PM that adds a `if (window.X) window.X.method()` call site, ask: is this surface guaranteed to load `<bridge>.js`? If the reviewer can't immediately answer, the PR author hasn't done the audit and the review should request it.

**The cost of not having this rule.** PM-363 looked clean on every metric — node check clean, post-commit verify clean, deployed to main, build marker bumped. The achievement toast appeared correctly on every page (it's pure DOM). Only the haptic was silently missing on 38/41 surfaces. The discovery was incidental — PM-364 prep audited haptics.js coverage as part of "wire workouts.html set-tick" prep work. Without that audit, PM-363's silent-failure state could have persisted until Dean tried earning an achievement on (say) the cardio page and noticed no haptic, then bisected back through the haptics PRs trying to figure out why.
...
### §23.76 — iPad orientation arrays are non-negotiable all-four (PM-413, 26 May 2026)

**Rule.** Any iOS Info.plist orientation work — whether intentional, the result of a sed sweep, or stash-conflict resolution — must independently inspect and preserve `UISupportedInterfaceOrientations~iphone` and `UISupportedInterfaceOrientations~ipad` arrays. The iPad array is non-negotiable: it must declare all four orientations (`UIInterfaceOrientationPortrait`, `UIInterfaceOrientationPortraitUpsideDown`, `UIInterfaceOrientationLandscapeLeft`, `UIInterfaceOrientationLandscapeRight`). The iPhone array may be any subset including portrait-only; iPad has no such latitude.

**Why.** App Store validation Code 90474 fires when iPad orientations are incomplete. Apple's stated reason: iPadOS multitasking (Stage Manager, Split View, Slide Over) requires the app to render correctly in both orientations on both halves of the screen. An app that declares iPad-portrait-only cannot participate in landscape-side Split View, which Apple treats as a hard failure of the platform contract — not a warning, a validation block.

**How this surfaced.** PM-413 iOS 1.4 Archive cycle 1. Stash-conflict resolution earlier in the same session had collapsed iPad orientations to portrait-only matching iPhone, via `sed -i '' '/UIInterfaceOrientationLandscapeLeft/d' ios/App/App/Info.plist`. That sed pattern is array-agnostic — it deleted the LandscapeLeft string from BOTH the iphone and ipad arrays without distinguishing them. PM-250 portrait-lock work earlier in May had been scoped via `UISupportedInterfaceOrientations~iphone` specifically, leaving iPad alone. This session's blanket sed undid that surgical scoping silently. Validation caught it, not the build.

**Audit signal at edit time.** Any change to Info.plist orientation arrays must be diffed against both `~iphone` and `~ipad` keys before commit. Specifically: `grep -A 6 'UISupportedInterfaceOrientations' Info.plist` should show two distinct array blocks; iPad block must contain all four orientation strings. Surgical fixes target one or the other via the specific key, never the generic key.

**Audit signal at session-end.** Any session that touched ios/App/App/Info.plist must include a pre-Archive grep check: both arrays present, iPad has all 4, iPhone has whatever is intended. This is a 5-second check that saves an Archive + Validate + diagnose cycle (~10-15 min on Dean's Mac).

**Recovery pattern.** If validation Code 90474 fires post-Archive, the fix is a Python heredoc edit (sed in-place is too blunt for the array-aware fix):

```python
# Restore all 4 iPad orientations
import re
with open('ios/App/App/Info.plist', 'r') as f:
    content = f.read()
ipad_block = '''<key>UISupportedInterfaceOrientations~ipad</key>
	<array>
...
# substitute the existing ~ipad block — pattern needs to match whatever the current state is
# specific regex depends on current state; the safe pattern is to find the key and replace through the closing </array>
```

Then re-Archive, re-Validate, re-Upload. ~5 min recovery cost.

**Application history.**

- PM-250 (May 2026) shipped iPhone-scoped portrait lock via `UISupportedInterfaceOrientations~iphone` — surgical, correct, did not touch iPad
- PM-413 (26 May 2026) blanket sed during stash-conflict resolution collapsed iPad to portrait-only. Validation Code 90474 caught it on Archive cycle 1. Recovered via Python heredoc in-Archive cycle 2

**Sibling to §23.41 (pre-commit fresh-HEAD discipline).** Both are pre-action audit gates that save a much more expensive rollback. §23.41 prevents stale-tree commits; §23.76 prevents stale-array validation failures.
