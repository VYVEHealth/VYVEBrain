...
<!--CURRENT_FRONT_START-->
**PM-844/848/849 (2026-08-06): PARTNER FUNNEL P0 STACK SHIPPED — supersedes the PM-843 "awaiting go" line. (Renumbered from 844/845/846 same-day: a parallel CC session took PM-845–847; CC commit messages keep their historical labels — `4700aab2` says PM-846 but is the PM-849 mint modal — changelog holds the mapping. PM-850 = CC comment sweep `b69d640e`.)** Wizard is now a 5-step application (video optional, Launch step post-approval; on-screen resume-link card; Test-Site `9a3af440`; Dean device check ?z=844 pending). Nightly cron (partner-draft-purge **v2**) sends 48h + 7d stalled-draft nudges (7d names the honest deletion date incl. photo ID) and onboarding/declined stage emails — candidates via SECURITY DEFINER RPCs `partner_drafts_nudge_candidates`/`partner_stage_email_candidates` (submitted-only for stages, so the 9 backfilled live partners are excluded); one-shot flags in credentials._system, written via credentials-only updates so the purge clock never resets; live-proven end-to-end, current candidates 0/0. Pre-filled invite links: partner-onboarding **v13** `mint_invite` (admin-gated inside a public EF, partner-file-url v3 gate; CORS + admin.vyvehealth.co.uk) mints a draft from Lewis's call notes + sends the Lewis-voiced invite with the personal resume link; duplicate guard returns existing links; `_system.invited` cohort = the invite→submit conversion metric; CC invite modal `4700aab2` leads with "Mint & send invite link" (result panel, copy button, honest send status), manual path secondary. MINT SHAPE RULE: wizard resume reads credentials.formFields + TOP-LEVEL credentials.pillar in wizard tokens (physical/mental/social) — any server-side draft creation must write both shapes or the form lands empty. **§23.164:** backdating partner test rows = §23.156 trigger-disable pattern on trg_sync_partner_onboarding_pct + explicit status_changed_at UPDATE (defaults now() on insert). Lewis (non-gating): copy pass on the 5 new emails; decisions (a) photo-ID policy, (b) quiz coherence owed.**
...
| `vyve-partner-draft-purge-daily` | `30 3 * * *` | partner-draft-purge (jobid 54, PM-747 v1 purge + PM-848 v2 nudges/stage emails — abandoned partner drafts 30-day retention, 48h/7d nudges, onboarding/declined stage emails, rate-window prune) |
