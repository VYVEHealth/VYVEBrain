...
## 19. Current status — 16 May 2026 PM-133 (Updates 1–6: cardio cold-paint + instant logging fixed; movement local-first root fix shipped, awaiting device confirmation)
...
**§23.29 — The `server.url` dev-loop build has a caching layer; do not assume reloads are live.** Surfaced PM-128–133. Updates took 3-4 cold reopens to land; the build banner showed a stale "Update N" on first open and the fresh value only after a page navigation — on the same page. This is stale-while-revalidate-shaped behaviour (WKWebView HTTP cache or Capacitor bundle — not yet definitively identified). The banner's `sw` field reads `no-sw`, so it is not a service worker. Consequence: never trust a single cold reopen to reflect a just-shipped change; the build banner ("Update N") is the only reliable confirmation a change actually reached the device. Identifying this cache layer is an open item.
