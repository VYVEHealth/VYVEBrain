...
| Haptics client | `haptics.js` v1 (PM-278, 25 May 2026) — `window.VYVEHaptics` Capacitor-Haptics-shaped API: `light/medium/heavy()` impact, `success/warning/error()` notification, `selection()` tick. Native iOS/Android route through `window.Capacitor.Plugins.Haptics` (plugin bundled in vyve-capacitor since 1.2); Android web uses `navigator.vibrate(ms)` fallback; iOS Safari + desktop silent no-op. Platform detected once at load, cached. All call sites try/catch — never throws. **Loaded by `index.html` only at ship — other pages adopt incrementally as tap handlers get touched.** First migration target: PM-259 long-press V-logo reset (currently `navigator.vibrate(35)`, iOS Safari no-op) → `VYVEHaptics.medium()`. |
...
**Recurrence — PM-277 → PM-278 (25 May 2026).** Exact same failure mode, two days later. PM-277 haptics-bridge session started with HEAD at `fa2cd908`. A parallel Claude session shipped PM-277 focus-tick (`950a726f`) between session-start file fetch and commit time, adding 126 lines of `.is-completed` work to `index.html`. My commit script ran `§23.41` fresh-HEAD-fetch on the ref correctly, **noticed HEAD had moved**, **logged a warning, and proceeded anyway** because it only re-fetched the ref — not the file bodies. Net: my session-start `index.html` blob wholesale-replaced the sibling's just-shipped 17-occurrence focus-tick work. Detected within minutes via post-commit grep for `.is-completed` on live main. Recovered by re-fetching `index.html` from the sibling commit SHA, re-applying haptics edits on top, and re-shipping as `db8cea41` (cache key bumped to `pm278-haptics-bridge-recovery-a` to disambiguate from the PM-277 number collision). Sibling work fully preserved in the recovery.

**Sharpened discipline (effective immediately).** The §23.58 rule already covered this — rule 4 explicitly says "applies to EVERY file in the commit batch". Tonight's failure was a discipline lapse, not a missing rule. **Hard requirement:** when the §23.41 fresh-HEAD-fetch shows HEAD has moved from the expected parent, the commit script MUST abort and re-fetch the bodies of every file in the commit batch before proceeding. Logging a warning and continuing is the trap. Any future `commit.py` that detects HEAD drift mid-script must either (a) refetch all named file bodies + re-apply local edits on top (preferred for surgical edits like script-tag adds) or (b) abort and surface to Dean for a merge decision (preferred for substantive edits where re-applying isn't trivial). **The "⚠ parent SHA has moved" warning is not informational — it is a stop sign.** Update `commit.py` templates accordingly the next time one is written.
