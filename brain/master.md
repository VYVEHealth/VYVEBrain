...
### Coaching platform (PM-955, 1 Sep 2026)
Kahunas-style PT client management. Coaches are partners (`partner_partners.partner_type='coach'`). **Tables:** `coach_clients` (partner_id+member_email unique; status invited→active→archived; `assignments` jsonb {onboarding_form_id, checkin_form_id, habits_template_id, workout_template_id, nutrition_template_id, supplements_template_id}; `invite_code` 12-char unique — permanent pretty-link handle; consent_accepted_at + coach_terms_version/accepted_at stamped by the first-login gate), `coach_terms` (versioned T&Cs per coach), `coach_forms` (kind onboarding|checkin, questions jsonb, multiple per kind allowed), `coach_form_responses` (unique form/member/week), `coach_templates` (kind habits|workout|nutrition|supplements, payload jsonb, UNIQUE partner_id+kind+name, soft-delete via active), `coach_supplements`. **Access model:** `is_coach_of(email)` SECURITY DEFINER — true only while the caller's partner (get_my_partner_id) has that client status='active' AND consent_accepted_at NOT NULL. coach_read RLS on exactly: members, workouts, exercise_logs, exercise_swaps, custom_workouts, daily_habits, member_habits, nutrition_logs, weight_logs, cardio, workout_plan_cache, weekly_goals. **HARD BOUNDARY: no policy may ever grant coach access to wellbeing_checkins, weekly_scores, ai_interactions, or any Mind-pillar table** — clinical oversight (Phil) stays out of the coaching path by construction. Coach writes to member tables go through EFs only; isolation is Postgres-enforced (proven with rolled-back fixtures — the Sage-DPO answer). **Money:** client pays coach directly outside VYVE; member row billing_tier='coaching', 7-day trial then £10/mo (trial-wall routing UNBUILT); partner_memberships row referred/account_type='coaching'/'trialing'→ subscription_value 10 at conversion; run_partner_payouts counts b2c+coaching → coach £5/mo for membership life, survives coaching end + archive. **Invite chain:** portal/email use permanent `www.vyvehealth.co.uk/start/<invite_code>` → Test-Site 404.html /start matcher → `coach-join` EF (verify_jwt false) mints a FRESH type='recovery' link per click (never expires, scanner-proof, resend-stable) → set-password.html PASSWORD_RECOVERY form; code goes dead once activated (redirects login). **Surfaces:** standalone `coach-portal.html` in vyve-command-centre (kept deliberately separate from partner-portal.html — Dean call, "intersect later"): roster, add-client w/ six assignment dropdowns, per-coach generated .xlsx bulk template (hidden Lists sheet = data-validation dropdowns of the coach's saved plans), plan builders for all six kinds, client detail w/ live assignments editor + activity timeline. **EFs:** coach-provision-client v4 (verify_jwt on), coach-join v2 (verify_jwt false BY DESIGN). **Shell (PM-955j):** Kahunas-style left sidebar — Profile/Messages/Notifications/Dashboard/Clients/Nutrition/Workouts/Calendar/Content Library/Automations/Forms/Settings; Dashboard is the landing view; Forms/Workouts/Nutrition are section-filtered lenses over the one builder engine; Messages/Notifications/Calendar/Content/Automations are declared coming-soon surfaces (the IA for those builds). **Gate + apply (PM-956, 2 Sep 2026):** coach-welcome.html — standalone 4-step first-login gate for billing_tier='coaching' (VYVE terms → coach terms [self-skips if coach has none] → explicit data-sharing consent [shared-list + database-enforced never-list card] → coach onboarding questionnaire, all 8 question types; photo/video answers upload member-side to the private coach-checkins bucket at <email>/onboarding/<qid>-<ts>.<ext>). Both auth.js gates (consent fires first, then onboarding) branch coaching members here; the gate stamps members terms/privacy/health-consent, coach_clients coach_terms_version+consent_accepted_at, flips invited→active (arms is_coach_of), writes coach_form_responses (form/member/ISO-Monday week), sets onboarding_complete, then fires **coach_apply_assignments(partner_id, member_email)** — SECURITY DEFINER RPC, service-role only, the single materialisation path shared with coach-provision-client v5's update_assignments action (portal saves auto-apply when active+consented). Materialisation: habits → habit_library rows **active=false, created_by='coach:<pid>'** (embedded joins + Dexie member-habit hydrate ignore library active, so they render/log/autotick; active=false keeps them out of every catalogue hydrate and the custom-habit picker — zero pollution; habit_pot='movement', dedupe by created_by+title) + member_habits assigned_by='coach:<pid>' (CHECK extended), removed titles deactivated, member/AI rows untouched; workout → live programme_json contract exactly (surface='workouts', 8 repeating weeks, session_name/session_number/warm_up/cool_down strings, exercises exercise_name/sets/reps/rest_seconds/notes/letter_badge) source='coach'/source_id=template, prior workout actives deactivated — AI regen never clobbers; nutrition → tdee_target=calories + members.macro_override jsonb (coach explicit macros; nutrition.html renders them verbatim, shows guidance + supplement-stack cards, hides the recalculator) + nutrition_guidance; supplements → replace-set in coach_supplements. Coach terms: versioned editor in portal Settings (retire active → insert max+1; example 5-section template one click away). **Conversion + feedback (PM-956b):** stripe-webhook v14 rail 0 — an existing account_type='coaching' partner_memberships row is UPDATEd at customer.subscription.created (net value, active, stripe ids) before the metadata/coupon/slug rails run, no partner-status gate (relationship explicit in coach_clients); trial-end-email v2 sends coaching members coach-framed £10 copy on the same continue.html?cid= rail. Post-session feedback: workouts.difficulty_rating (1-5) + member_note, coaching-only block on the completion screen (active coach_clients check, sessionStorage-cached), PATCH by client_id with a 3s race-retry against the background workout POST; surfaces in the portal client-detail timeline. **Messaging (PM-960, 2 Sep 2026):** `coach_messages` — one implicit thread per (partner, member), text-only 1–4k, RLS-direct writes (coach via is_coach_of + sender='coach'; member only to their active consented coach), read_at marking both sides. Portal Messages view (thread list, unread badge, 15s poll); member coach-messages.html + coach-nudge.js second block (vyve_cmsg_state 2-min cache, .nav-msg-item menu rows + unread pill + home card); push on coach send: trg_coach_message_push → coach-message-push EF v2 (vault-key relay — VYVE_INTERNAL_KEY is vault-only via vyve_internal_key() RPC, NEVER Deno.env) → send-push (in-app row + APNs, route /coach-messages.html, no daily dedupe). Not a scored activity. Notifications feed / automations (email, PM-958g) / calendar v1 / lead forms shipped PM-958e→959. **Still open:** automations channel='message' wiring, content library, coach profile editing, typed daily habit forms (Kahunas model — mockup queued), nutrition-adherence portal view, client self-booking (wire the existing booking engine). **Coach payment/invoicing tooling deliberately OUT, permanent** (Dean, 2 Sep 2026): clients pay the coach outside VYVE and the £5/mo rev-share is the commercial answer — never build packages/invoicing. **Overhaul (PM-982 map → PM-983 waves, 3 Sep 2026):** 61-item Kahunas gap map at `tasks/coach-portal-gap-map.md`; 7-wave build order in backlog. **Wave 1 LIVE (CC `9da8c820`):** nested sidebar submenus + `kindsel:<kind>` deep-link router + footer; dashboard cockpit (quick actions, latest clients/check-ins/messages); clients stats strip + searchable status-filtered table w/ list-grid toggle + 25-row paging + per-row actions dropdown; 4-step add/edit wizard — phone + weight unit + welcome-pack PDF (coach-content `p-<pid>/pack-*`, 7-day signed link in the invite) + scheduled invite send + check-in day/frequency; bell popover; broadcast + new-message picker; roster Check-ins page; Daily check-ins grid (habits logged vs assigned, Mon-based weeks). **Assignments jsonb extended (zero DDL):** template slots + `checkin_day` (int 0=Mon..6=Sun) + `checkin_frequency` (weekly|fortnightly|monthly) + `_meta` {phone, weight_unit, welcome_pack_path} + transient `_invite_send_at` (cleared by the send sweep). `coach_forms.is_default` (partial unique: one default per coach per kind among active) preselects wizard defaults. **coach-provision-client v9:** update_assignments MERGE-preserves the non-slot keys (v8 wholesale-replace would wipe them — any future writer of assignments must preserve them too); `update_client` edits invited_* always, members row only pre-activation; cron branch `send_scheduled_invites` (x-vyve-cron-key = VYVE_CRON_KEY, pg_cron job 65 hourly :15) sends held invites. **Portal code pattern (PM-983):** Wave-1 JS lives at the bottom of the main IIFE and SHADOWS go/loadClients/renderRoster/renderDashboard by same-scope redeclaration (last-wins hoisting); legacy #cl-editor is CSS-hidden but kept in the DOM so the original top-level listeners keep their targets — edit the bottom versions, never delete the legacy ids. **Wave 2 LIVE (PM-984, CC `fc073da7` + site `c1e33f6d` vbb 584 + migration `pm984_wave2_habit_note_and_apply`):** staged 3-step wizards w/ stock question banks for onboarding (13 Q)/check-in (7 Q + 10-metric collapsed Progress Tracking bank → `question.section='progress'`, grouped member-side in coach-checkin.html and in the review timeline)/habits (starter bank + library picker); wizard intercepts plOpen via `w2WizOpen` hook (other kinds keep the legacy editor), clears #pl-body on open (duplicate-id race for #q-rows/#h-rows); plLoad v3 shadow = DEFAULT pill + clear-then-set default; T&C `ct-boiler` full 10-section boilerplate w/ coach-name substitution (payment wording never implies VYVE processes payment). Habit daily notes (#43): `daily_habits.note` (≤600) + habits-template `payload.allow_note` → coach_apply_assignments merges `assignments.habit_note` (jsonb || — any assignments writer must keep MERGE-preserving); member habits.html shows a per-habit “note for your coach” gated on that flag (fail-silent otherwise), notes surface in the portal Daily grid (✎ + title). `assignments.checkin_day` (0=Mon) now surfaced member-side on the nudge card + coach-checkin header (#4 closed). Template edits do NOT re-apply to assigned clients — re-save assignments (or gate) to propagate allow_note.
