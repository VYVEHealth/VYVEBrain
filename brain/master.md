...
**Phase 2 (PM-641) SHIPPED:** `lib/auth.js` role-aware (broadcasts `window.VYVE_USER.role`). `pages/tasks.html` role-scoped controls (New Task + Delete → admin only). Attachment section in editor: upload/download/delete via `cc-task-docs-url` EF v1. `cc-task-docs` private bucket + Storage RLS live. **Remaining:** 3-source calendar UI (Phase 3); scheduler → `calendar_occurrences` (Phase 4).
