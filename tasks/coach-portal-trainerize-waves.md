...
**Part 2 remaining:** `coach_exercises.video_url_alt` + `alt_label` are LIVE and empty; build the CC bulk-attach tool under the VYVE library (paste CSV / URL list → per-row result), the "missing video" filter on the exercise grid, and the member player's alt-video toggle (default off `members.gender` where set, remembered in localStorage). 866 stock rows unfilmed.
