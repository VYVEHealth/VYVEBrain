...
**Status:** **Waves 0, 1 and 2 shipped 13 September 2026; Wave 3 part 1 (the plan surface) shipped PM-1252, corrected PM-1253.** *(Historic: Waves 0/1 PM-1239, PM-1240, corrected PM-1241; Wave 2 part 1 PM-1244; Wave 2 part 2 — the wizard itself — PM-1247→1250, vbb 644→647.)* Older status line follows: — schema, pace engine, template catalogue and variable plan length are live, server only, nothing member-facing. Plan lengths now: 5k 6–20 weeks, 10k 8–24, general training 4–26, start-running fixed at 9. **Wave 2 part 2 mockup v2 shipped PM-1246** (availability, volume, intensity, surface, second race time; engine `run-engine@0.6.0` takes `available_dows`). **Mockup shipped PM-1245** (`docs/mockups/running-wizard-mockup.html` in vyve-command-centre, twelve screens, nothing built) — the build is gated on Dean's reaction, Phil's injury wording and Lewis's strings.
...
### Wave 3 — Plan surfaces — **PART 1 SHIPPED PM-1252/PM-1253; part 2 is the catalogue (PM-1243)**

**Part 1, shipped 13 Sep 2026 (vyve-site `8374594e` + `ffe51b6c`, vbb 648→649, NOT OTA'd):** `running.html` — plan header, projection band, training calendar of week tiles, seven day cards a week, and the session detail (blocks, numbered steps, repeat groups, limit-not-a-target, outdoor/treadmill). Running is a Body stream gated on an active plan, PM-1213's pattern. Race day is rendered on the calendar, the week tile and the header. Km/miles toggle on the surface (`run_member_profile.units`, display only). The page makes no engine call; §23.349 is enforced structurally so nothing can render "0 km". **Still owed on part 1:** Dean's device walk, then the single commit that makes Running visible — soft-kill `running-plan.html`, drop `run: true`, remove the PM-1248 dev-panel row — which waits on Lewis's strings.

**Part 2 is the catalogue, below.**
**Dean's bar: it must feel close to Runna or another premium running product; the route there is Claude's call.** The catalogue expands here, not earlier — the member never sees how many plans exist until this surface renders, and anything authored before it is another grid to re-cut if the surface wants a different shape. Add templates **only where the training genuinely differs**: half marathon (the real gap), marathon, a first-5k/parkrun bridge between Start Running and 5k Improver, **Return to Running** (conservative reload, pairs with the physio product — Runna has no equivalent), and 10 mile. About ten templates, presented as thirty-odd browsable entries — **confirmed by Dean PM-1246, to be authored straight after the wizard ships, with half and marathon gated on a qualified (CiRF) reviewer before a member can start one** via length × level × days-per-week, because most of Runna's apparent breadth is merchandising rather than programming (parkrun Improvement / Get Fit / New To Running are all 5km, 6–26 weeks, separated by level and volume). **Not building: Hyrox, triathlon, Functional Fitness** — outside what VYVE is and where Runna defends a different market. The binding constraint on catalogue size is the sign-off surface, not compute: a plan is a prescription, and fifty of them is a document no coach finishes.
...
**ADDED PM-1253 (Dean): race week has to know there is a race.** The engine places sessions without reading `race_date`, so on Dean's own 10k an Easy Run was scheduled ON race day — invisible until PM-1253 drew the race card beside it. Race day should be clear, race week should taper into it rather than past it, and the race itself should be the plan's last dated thing. This is engine + template work (`buildPlan` session placement), not a surface fix, and it gets sharper the moment half and marathon land in the catalogue, where a misplaced race week is a genuinely bad prescription rather than an oddity.
