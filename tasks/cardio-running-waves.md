...
**Status:** **Waves 0 and 1 shipped 13 September 2026 (PM-1239, PM-1240, corrected PM-1241)** — schema, pace engine and the template catalogue are live, server only, nothing member-facing. Wave 2 (wizard, recap, projection) is the next build session.
**Owner gates:** Lewis — all member-facing strings + trial/entitlement framing. Phil — the injury-history question, its consent wording and anything that adapts training on an injury answer. **A qualified running coach — an England Athletics Coach in Running Fitness (CiRF) or equivalent — signs off the catalogue before it is member-facing.** *Corrected PM-1241: this was originally Calum's gate. He is a PT, not an endurance coach; CiRF is the off-track award that actually covers planning and mesocycles. Calum owns the strength work that runs alongside, where the evidence is strongest (~50% reduction in overuse running injury from strength training, 26-RCT meta-analysis) and where he is qualified.*
