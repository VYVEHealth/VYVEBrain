...
- 📋 **OPEN (P0) — PM-38: Layer 1c-9 — `settings.html` persona switch → `bus.publish('persona:switched', ...)`.** Ninth 1c migration. Taxonomy ADD — `persona:switched` event doesn't exist today. Pre-flight at next session start (HEAD will be `c1c731a1` = the PM-37 ship): identify the exact persona-switch publish site in settings.html (likely a `savePersona` or similar handler that writes `members.persona` via PATCH and may also write the `persona_switches` audit table). Settings.html is 100,328 bytes — second-largest portal page after index.html — so the publish site may be one of several save handlers in the same file (theme toggle, notification prefs, goal focus, privacy toggle are all separate save surfaces; only persona-switch is in scope for 1c-9). Whole-tree audit needed: existing primitives at the publish site, current cross-page consumers of persona changes (the persona is referenced on home dashboard greeting, possibly on workouts/check-in pages for tone modulation), and whether settings.html owns any achievement track (probably not — persona switching isn't an achievement). Schema: `bus.publish('persona:switched', { from_persona, to_persona })` is the natural shape; confirm both fields are available at the publish site. Likely consumers: index.html (home greeting refresh), nutrition.html (TDEE persona-modulated copy if any), wellbeing-checkin.html (AI rec persona). Engagement scoring is unaffected by persona — engagement.html non-touch likely. SYMMETRIC fallback expected if invalidate + record-recent-activity present pre-bus; ASYMMETRIC if zero primitives (which is plausible since persona switch isn't a "logged activity"). settings.html may NOT load bus.js today — confirm at pre-flight; first new bus.js wiring since PM-37 (nutrition.html). Use the PM-37 self-subscribe pattern (NEW §23 sub-rule) ONLY if settings.html owns an achievement journey for persona switches; otherwise the publishing-page-only pattern applies. SW cache bump same atomic commit. Pre-flight will resolve.
