...
> **[2026-08-10 · MEMBER DISCOUNTS FOLLOW-ONS (PM-893 shipped)]** (1) **Seed the Newcastle businesses** — zero build: one INSERT per business into `member_perks` (business_name, headline e.g. "20% off food", redemption_type code|show_app|link, code/instructions, optional logo → perk-logos bucket, website). Awaiting Dean's list. (2) **Lewis copy pass** on perks.html member-facing copy incl. the third-party-offers disclaimer (shipped placeholder-honest, non-gating). (3) **CC perks admin surface (~0.5 session, optional):** add/edit/deactivate perks + per-business reveal/view counts from perk_events — the "47 members pulled up your offer this month" line for the businesses. SQL inserts suffice until then. (4) Later: per-member unique codes if a business wants true redemption verification.
