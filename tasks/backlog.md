> **[PM-843 · 2026-08-06 · PARTNER FUNNEL P0 STACK — approved in principle, awaiting Dean's go]** From the funnel deep-dive (doc with Dean; warm-lead model: ~90% Lewis-screened before the link — the wizard is formalisation, not application). **(1) Wizard re-sequencing (~1 session):** mandatory welcome-video + launch-date steps OUT of the application (post-approval portal tasks; optional video slot stays); landing-page time honesty ("apply in ~10 min"); on-screen resume link + spam hint. EF already tolerates videoless submits — wizard + copy change. **(2) Draft nudges + stage emails (~1 session):** 48h/7d stalled-draft emails incl. honest 30-day-deletion warning (purge currently deletes applicants' uploaded IDs silently), minimal stage-change emails (Lewis is the channel under the warm model — keep light), riding the nightly cron; copy → Lewis pass non-gating. **(3) Pre-filled invite links (~0.5 session):** post-screen, CC action or Claude mints a draft from Lewis's call notes → personal resume link + Lewis-voiced invite email; warm lead skips to signature+uploads. Metric: invite→submit ≥80%. **DECISIONS OWED (Dean+Lewis, block nothing above):** (a) photo-ID policy — wizard requires a passport under "VYVE verifies your identity" but NO verification process exists and submit auto-stamps identity_verified:true (GDPR minimisation; PM-634 recorded ID as removed, live wizard disagrees) — either an explicit admin "ID checked" pipeline action feeds the flag, or collection moves to contract stage; (b) quiz coherence — 3-question safeguarding/GDPR gate quizzes on hidden training modules — restore modules (Lewis/Phil content) or reframe as declaration. **NOW, zero build:** Lewis chases Vanessa with ?resume=13f61ee9-a991-4a2d-8b34-a096c914cb5a; once (1) ships, invite her to submit without the video.
