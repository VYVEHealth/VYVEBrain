> **[2026-09-03 · PM-982 COACH-PORTAL OVERHAUL — GAP MAP BANKED, BUILD PENDING]** The full 61-item Kahunas gap map is at `tasks/coach-portal-gap-map.md` (58 buildable + 3 exclusions; headline builds, schema-touch consolidation, and parked product calls inside). Next session on this thread: read the gap map FIRST, then produce build order + Claude-assisted estimates, then implement in one go. §23.23 applies (scan last 15 vyve-command-centre + vyve-site commits for already-shipped overlap before design talk). Supersedes the scattered coaching stretch items below where they overlap (content library, check-in metric charts, client self-booking, supplements text/PDF, coach profile editing — all now have numbered specs in the map).
