...
**DO FIRST:** read-only backfill — count members ALREADY broken by the workouts bug (no plan / no habits / never signed in) and remediate that backlog, not just stop future cases.
  ...
  - [ ] **Consent-version stamping gap (PM-615 obs):** new member rows have `privacy_version`/`health_consent_version` NULL — consent gate not writing the PM-603 version columns. Enterprise/Sage consent-audit relevance. Fix when next touching the consent gate.
