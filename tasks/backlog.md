...
> **[2026-09-03 · PM-983/984 COACH-PORTAL OVERHAUL — WAVES 1-2 SHIPPED, WAVES 3-7 QUEUED]** Gap map at `tasks/coach-portal-gap-map.md` (PM-982). Wave 1 LIVE (CC `9da8c820` + coach-provision-client v9 + coach_forms.is_default + cron 65): sidebar IA/footer, dashboard cockpit, clients stats+table+search+grid, 4-step wizard (phone/weight-unit/welcome-pack/scheduled-invite/check-in day+frequency), actions dropdown, bell popover, broadcast+new-message, roster Check-ins page, Daily grid. Approved order, Claude-assisted estimates: **Wave 2 SHIPPED (PM-984, one session):** #39/40/43/44 + #38 completion + #4 member read — CC `fc073da7`, site OTA `c1e33f6d` (vbb 584), migration `pm984_wave2_habit_note_and_apply`. NOTE: allow_note reaches existing clients only on an assignments re-save or gate pass (apply-time merge); #41 confirmed already-satisfied by the PM-955 habits lens + new wizard. **Wave 3 SHIPPED (PM-985, one session):** #19-23/#25-31 workout depth — 977-row muscle_volumes enrichment (fed: one-shot EF + vyve: name rules), Total Volume Sets live everywhere, duration-type end-to-end (authoring → programme_json v2 → member player duration_secs), alternatives/guided swaps, library card grid, plans-list upgrade (counts/quick-view/preview/assign-to/duplicate), drag-reorder + default prefill. CC `76f735d4`, site `602888a6` vbb 585, OTA `60249cef`, migrations `pm985_wave3_workout_depth` + `pm985_program_json_v2_duration_alternatives`. Dean passes owed (portal grid/TVS/assign + member 585 timed-set/swap-strip); `pm985-enrich-exercises` EF is dead — deletion list. #24 circuits still PARKED. **Wave 4 SHIPPED (PM-986, one session):** #13-18 nutrition — CC `844af645`, site OTA `7f7bcd29` (vbb 586, deployment `9ae35033`), migrations `pm986_wave4_nutrition` + `pm986_apply_v5_days_and_supp_docs`. Day-tabbed ingredient builder (off-proxy v10 + new coach_foods), meals library kind=meal (snapshot-by-value), Foods kindsel lens, shopping list, supplements list/doc modes (attachment_path + member storage policy), plans-list rollups/preview/assign-to; apply v5 days[] canonical + supp doc row, fixture-proven. #16 meals-vs-days left open by construction (days folded into plan editor; days library = one-line kind widen if ever wanted). NOTE: template edits still do NOT re-apply to assigned clients (re-save assignments or gate). Dean passes owed: portal ingredient-plan chain (apostrophe-brand search, grams+serving maths, shopping list, assign→member card) + member 586 (day sections, supplement doc opens). Capawesome CLI 4.18.4: apps:bundles:create DEPRECATED → apps:liveupdates:upload. **Wave 5 SHIPPED (PM-987, one session):** #50-61 client-detail restructure + slim #46 — CC `ef324d3d` (viewClient SHADOWED at IIFE tail — edit the bottom version from now on), site OTA `b4e2a0a2` (vbb 587, deployment `d8ec08ad`), migration `pm987_wave5_client_detail`, coach-provision-client v10 (META_KEYS += water_goal/gates — v9 would have wiped habit_note on update_assignments; GATE_KEYS = load_calc + exercise_library only). Tabbed workspace (10 tabs incl. metric-columned check-ins w/ column gear + progress modal, Q&A edit/print, week-paged logs, goals w/ member countdown, plans + water goal + gates), check-in-day toast, chat popover. Member 587: home goal-countdown card, coach water goal (challenge override still wins upward), e1RM behind load_calc, swap picker coach-strip-only when exercise_library off w/ alternatives. PROTECTED assignments keys now incl. water_goal + gates. Calendar tab + Reset-questionnaire deferred to Wave 6 / gate rework. Dean passes owed: portal 10-tab chain incl. Plans save feedback + chat round-trip; member 587 countdown/water/e1RM/swap-gate (needs a slot w/ alternatives + exercise_library off). Capawesome gotchas: --channel = NAME, --git-ref fails on private repos (use --custom-property), --private-key replaces --private-key-path. **Wave 6 SHIPPED (PM-989, one session):** #32-36 + the Wave-5 Calendar-tab deferral — CC `adef98a2`, site OTA `4cfc4138` (vbb 588, build `b1a45bc1`), migration `pmxxx_wave6_calendar_content_drip`. coach_events + push-sync trigger (notify → scheduled_pushes −1h, cancel withdraws), month/week/list grid, event editor, requests inbox on new bk_partner_* RLS (Confirm paid → booking-paid-confirm EF; EMPTY until coaches create booking_services rows), content library file/editor/link w/ folders + audience + drip (cci_member_read enforces consent-anchored drip server-side; storage read-through policy; bucket 200MB +video/audio/docx), member coach-content.html + 'From Your Coach' nav rows (coach-nudge probe). Copy placeholder — Lewis revise non-gating. Drip not live-observable until a client consents (roster = 1 invited row). Dean passes owed: portal calendar/event-push/book-a-call/inbox/content chain + client-detail Calendar tab; member 588 menu row, drip hide/release, signed file open. Polish (non-urgent, fold into a later wave): inline video/audio playback + NEW-release pill on coach-content.html. **Wave 7** (~1.5) #45/47-49 settings + compliance; Data Consent page copy is Lewis-GATING for visibility. One migration per wave, batched per gap-map schema list. §23.23 sweep done this session — do NOT re-run for waves 2-7 unless parallel sessions land coach work.
...
> **[2026-09-03 · AMBASSADORS FOLLOW-ONS (PM-975 shipped)]** (1) **Lewis: ambassador agreement wording** (owed since PM-868) — the CC go-live tap assumes a signed agreement exists offline. (2) **Lewis: chase agreements for the 4 pre-gate live ambassadors** (antonia/april/david/ivan — flagged in-UI “no signed agreement on record”). (3) Lewis copy pass on both new CC pages’ member-adjacent strings (share text esp.) — non-gating. (4) human_promo_code wiring (validate_code accepts code OR slug) remains the Piece 1 remnant — today the spoken code IS the slug, which works.
