> **[SPRINT WINDOW: Dean wants a HUGE dent in the coaching platform before his usage reset THURSDAY 3 Sep — sessions resume Tue 2 Sep. Order of attack REVISED (Dean, 1 Sep late): gate → apply pipeline → **WORKOUT BUILDER v2 — the main event**: per-coach exercise library (coach_exercises table) w/ unlisted-YouTube video URLs embedded in the member workout player, day templates as reusable units, multi-week programme assembly w/ per-week variation/progression, full prescription fields (sets/reps/tempo/rest/RIR, supersets/circuits) → trial-wall £10 routing → workout summaries → check-in bucket → Calum provisioning; messaging/automations only if core lands early. Workout depth outranks everything after apply — Dean: “this needs to be super in-depth”.]**
