...
- **Security commit 1B — hygiene completion. NEW.** Three items, all minutes-each, all defensive. Deferred from commit 1 due to context budget on EF redeploys.
  - **CORS fallback fix on `wellbeing-checkin` v27 + `log-activity` v28.** Same pattern as `member-dashboard` v59: drop the `*` fallback for empty/null Origin, fall through to `https://online.vyvehealth.co.uk`. Lower-impact than `member-dashboard` (POST-only, real browsers always send Origin) but worth tidying for posture.
  - **`ai_interactions` audit logging in 3 EFs.** Currently 21 rows, all `triggered_by='onboarding'`. Add inserts in `wellbeing-checkin` (after the Anthropic call, before returning), `anthropic-proxy` (transparent passthrough — log the request shape + response.content[0].text), `re-engagement-scheduler` (inside `aiLine()` helper). Schema: `member_email`, `triggered_by` (`'wellbeing-checkin'`/`'running-plan'`/`'re-engagement'`), `persona`, `prompt_summary`, `recommendation`, `decision_log` (jsonb with model, max_tokens, response_status). Mirror the existing onboarding write shape exactly.
  - **100KB payload cap helper in `_shared/security.ts`.** Top-of-handler check: `if (req.headers.get('content-length') && Number(req.headers.get('content-length')) > 102400) return 413`. Roll into every EF that reads `req.json()`: `member-dashboard` (no body but worth the guard), `wellbeing-checkin`, `monthly-checkin`, `log-activity`, `onboarding`, `register-push-token`, `share-workout`, `edit-habit`, all admin EFs.

- **Security commit 2 — CSP + XSS render-time sanitisation. NEW.** Member-facing, requires SW cache bump and incognito live-URL test. Strict `<meta http-equiv="Content-Security-Policy">` block in every portal HTML head: `default-src 'self'`, `script-src 'self' https://cdn.jsdelivr.net https://*.supabase.co`, `connect-src 'self' https://*.supabase.co https://api.brevo.com https://api.openfoodfacts.org`, `style-src 'self' 'unsafe-inline'`, `img-src 'self' data: https:`, `frame-ancestors 'none'`. Plus render-time sanitiser for stored content flowing into `innerHTML` — start with `custom_workouts.workout_name` and `exercise_notes.exercise_name` on `workouts.html`/`exercise.html`, audit beyond. Replace `el.innerHTML = userText` with `el.textContent = userText` where structure isn't needed; tiny escape helper for the cases that do need HTML. Cache bump pattern: `vyve-cache-v2026-05-NNa-csp` or whatever date the work lands. Mandatory incognito test on the live URL because cached service workers will mask CSP violations on returning members.

- **Security commit 3 — `gdpr-export` EF. NEW.** Procurement blocker. Mockup-first per Dean's rule. Single signed-URL JSON download via Supabase Storage `gdpr-exports/{email}/{timestamp}.json`, 7-day expiry. Walks ~28 tables: 16 member-scoped sources (`members`, `daily_habits`, `workouts`, `cardio`, `session_views`, `replay_views`, `wellbeing_checkins`, `weekly_scores`, `weekly_goals`, `monthly_checkins`, `nutrition_logs`, `nutrition_my_foods`, `weight_logs`, `exercise_logs`, `exercise_notes`, `custom_workouts`) + derived (`member_home_state`, `member_stats`, `certificates`, `member_achievements`, `member_health_connections`, `member_health_daily`, `member_health_samples`, `running_plan_cache` member-relevant rows, `member_running_plans`, `persona_switches`, `engagement_emails`, `notifications`, `member_notifications`). `verify_jwt:true`. Member can only export own data; admin endpoint same EF behind admin-role guard for procurement "export this member's data" use case. Receipt to `admin_audit_log` action `gdpr_export`. Audit estimated 6h, realistic.

- **Security commit 4 — `gdpr-erase-request` + `gdpr-erase-execute` EFs. NEW.** Procurement blocker. Mockup-first. Two-phase to allow accidental-request recovery and match industry practice. `gdpr-erase-request` writes a row to a new `gdpr_erasure_requests` table with 30-day grace + receipt. `gdpr-erase-execute` cron-triggered after grace expiry: deletes from same ~28 tables in dependency order (kill `tg_refresh_member_home_state` triggers on `member_home_state` first, then walk children, then parent `members` and `auth.users`). Receipt to `admin_audit_log` at both phases. Audit estimated 4h, probably 5 with the request/execute split.

- **Re-role 5 cosmetic `public`-role policies to `authenticated`. Hygiene.** Found while verifying audit findings: `monthly_checkins_member_insert`, `scheduled_pushes_self_insert`, `members can insert chat`, `Members can insert own shares`, `members_insert_own_custom_habits`. All have proper `WITH CHECK (auth.email() = member_email)` (or `created_by`/`shared_by` equivalent) so `auth.email()` returning null on anon requests already blocks them. Re-roling is cosmetic for procurement reviewers who flag the `public` label; documented in `security_questionnaire.md`. Bundle with commit 1B.
