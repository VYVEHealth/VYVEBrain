> **[2026-09-04 · PM-991 PARTNER PORTAL OVERHAUL — WAVE 1 SHIPPED, W2-W6 queued]** Method = PM-982 redux, benchmark our own coach portal; map at `tasks/partner-portal-gap-map.md`. W1 ✅ (CC `ba038b87`) · W2 ✅ PM-992 (CC `bf83f48d`, partner-notify v1 + cron 67, prefs jsonb + notify ledger; Account area = completion bar / launch checklist / prefs / password change). REMAINING: **W2b** admin Go Live in CC gated on portal minimums (welcome video + profile completion — PM-880 posture, Lewis deferred-assessments nod = switch not build; welcome-video 48h-skip already satisfied by PM-900 auto-push-on-approval) · **W3** calendar unification (sessions+diary+bookings → month/week/list grid) + requests inbox + content library lanes/folders/stats · **W4** money (earnings deep-dive, in-portal referral funnel, monthly statements + finance payout run, human promo codes Piece 1) · **W5** reach (leads generalised beyond coaches, ratings list surface, community engagement stats, Checkout Session Piece 2, CC manual attribution Piece 5) · **W6** wizard at-source (photos→thumbnails + cropper port, honest upload errors, wizard reportError) + partners.vyvehealth.co.uk + PORTAL_BRAND_URL + one-shot EF deletions. Dean W1 pass owed: partner login → dashboard tiles real numbers, all 7 legacy views route (bookings lazy-loads), notif badge clears on open, #bookings deep link.
