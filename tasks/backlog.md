...
**Debug strip gating (P1, pre-next-binary).** PM-310 always-on live-tracker debug strip across the 8 `*-live.html` shells (via `session-live.js`) + `?debug=tracker` mind-video strip (via `mind.html`). Gate behind `localStorage.getItem('vyve_debug_tracker') === '1'`. PM-315 CSP fix for YT IFrame API still not validated on device walk — either validate first and remove strip, or keep flag-gated until next walk. Settings Reset Achievements + Force Refresh App buttons stay (intentional trial-period support tools).
...
**PM-251c — Chat + Q&A unlock (v1.1 feature flag flip).** Locked behind `COMING_SOON_TABS = true` in `session-live.js`. Tabs render with lock icon + "Coming soon". Flip flag + port legacy chat code (was in pre-PM-251 engine; not retained in new engine to keep v1 surface clean). Chat-only: ~2 hour session (port + wire + device test). Q&A with Supabase persistence: +1-2 hours (new `session_qa` table — currently legacy code stores Q&A client-side only).
...
**Certificate re-pillaring (post-launch).** Movement as its own activity track shipped via PM-307. Open piece: certificate tracks (currently Architect/Warrior/Relentless/Elite/Explorer per activity) restructured onto pillar-level tracks (Mind/Body/Connect). Lewis-gated. Old earned certs grandfather as `pillar='legacy'`. See `Certificates re-pillaring` item in Infra / Data — same scope.
