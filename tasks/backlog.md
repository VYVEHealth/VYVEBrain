...
- 📋 **OPEN (P0) — PM-32: Layer 1c-3 — `workouts-session.js` `saveExerciseLog` → `bus.publish('set:logged', ...)`.** Third 1c migration per the taxonomy plan. saveExerciseLog today calls L405 invalidate + L406 evaluate (writeQueued path) + L412 evaluate (legacy fallback) — three primitive sites collapse to one `bus.publish('set:logged', { exercise_log_id, exercise_name, sets, reps, weight_kg })`. Migration label REFACTOR (decouple) — no defect underneath, but the inline coupling between the save handler and the achievements evaluator is the kind of cross-concern bleed the bus migration is fixing. Subscribers per taxonomy: exercise.html (PR strip refresh on each set), workouts-session.js (next-set hint, currently inline), achievements.js (eval). Add `<script src="/bus.js" defer>` to exercise.html. Same option-(a) discipline as PM-30/PM-31. SW cache bump same atomic commit.

- 📋 **NEW (P3) — Pre-PM-32 spike: `vyve_exercise_cache_v2` staleness on `set:logged`.** Open question for the 1c-3 patch: when an exercise log is saved, should the exercise.html cache (`vyve_exercise_cache_v2`) be marked stale? Today the cache is uninvalidated by saveExerciseLog (only the `programme:updated` ADD migration would touch it per the taxonomy). Pre-flight 1c-3 with a 5-min read of exercise.html's PR strip + the cache shape to decide before patching. If yes, fold into 1c-3; if no, leave for `programme:updated`.
