...
- **P0-2 — `body:logged` aggregator.** Three Body publishers (`workouts-session.js`, `cardio.html`, `movement.html`) need to publish a `body:logged` aggregator event alongside their existing per-table publish, so the 3 Connect surfaces (`connect.html`, `connect-feed.html`, `connect-challenge.html`) currently subscribing to `body:logged` (with 0 publishers) actually receive Body-activity signals. Picks the discriminator shape during build. Next ship per Dean's "Chat 8" framing.
...
- **Tier 2.3 `checkin_questions` catalogue.** Versioned slider questions for weekly + monthly check-ins. Schema + Dexie read path can ship without Lewis copy; he populates wording when ready. ~1 hour. Sole remaining unblocked item in the content-surface audit queue from PM-372.
