...
## Shipped 23 May 2026 — PM-212 + 212.1 + 212.2 — Podcast hub MVP + same-session follow-ups (nav.js, logo fallback, in-app mailto modal, episode-add playbook, V-glyph fallback, Louis EXIF NULL) [vyve-site `bff78cb4` → `9badb2cb` → `38c64631`, brain `0ce93681` → next]

**PM-212.2 follow-up.** Two fixes after device test. (1) Logo fallback PM-212.1 shipped (`<img src="/logo.png">` + `filter: brightness(0) invert(1)`) wasn't rendering visibly at 78px tile size — swapped to Playfair "V" character (no asset, no filter, matches nav.js .mph-logo-v shape). (2) Louis Watkins thumbnail rotated 90° at source (EXIF Orientation not applied by Drive thumbnail service) — Supabase UPDATE set thumbnail_url=NULL so it falls back to the V-glyph; proper fix lands at v1.1 Storage migration where we can pre-process orientation. 3-file atomic commit, sw cache `pm211-podcast-b` → `pm211-podcast-c`, vbb-marker 82 → 83.
