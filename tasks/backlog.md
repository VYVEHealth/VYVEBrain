...
- 📋 **OPEN (P2) — Investigate `vyve_dashboard_cache` localStorage key.** Appears in vyve-site grep namespace inventory (cache-bus-taxonomy.md line ~110) but no clear read path in any of the 73 source files at HEAD `df41d7cb`. Either: (a) residue from a never-cleaned migration that should be removed in `auth:signed-out` cleanup, (b) written by dynamic key construction with template strings the grep missed, (c) read by a code path I'm misreading. 30-min spike before bus.js (PM-29) commits — keep/remove decision needs to be made before the `auth:signed-out` bus handler is wired, otherwise the handler either misses a real cache or evicts a key that's actually live.

- 📋 **OPEN (P0) — PM-27: Email-key the outbox.** `vyve_outbox` and `vyve_outbox_dead` not email-keyed today. Shared device or sign-out / sign-in-as-different-member can flush queued writes from the previous member against the current session's JWT. RLS catches server-side (member_email mismatch on body), but client-side UX is wrong — queued writes silently fail, breadcrumbs in `vyve_recent_activity_v1` show phantom logged-X entries that never lands. Migrate to `vyve_outbox_<email>` + `vyve_outbox_dead_<email>` with a one-shot reader on first sign-in that adopts any old `vyve_outbox` rows whose `member_email` matches current session and discards the rest. SW cache bump in same commit. Touch points: `vyve-offline.js` (key constants + adopt-on-first-load logic), every page that reads `localStorage.getItem('vyve_outbox')` directly (audit needed — most reads go through VYVEData but a few are inline). Estimated 1 commit, ~1.5h.

- 📋 **OPEN (P0) — PM-28: Commit `playbooks/cache-bus-taxonomy.md` to VYVEBrain.** Sign-off path: PM-27 ships and verifies → re-pre-flight nothing else has shifted in vyve-site → commit the taxonomy as PM-28 (`brain` directory) plus a changelog entry. Brain-only.
