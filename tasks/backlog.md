...
- 📋 **OPEN (P0) — PM-31: Layer 1c-2 — `workouts-session.js` complete handler → `bus.publish('workout:logged', source:'programme'|'custom', ...)`.** Next 1c migration per the taxonomy plan. workouts-session.js complete handler today calls invalidate + record + evaluate (3 primitives, 2 publish sites — `completeProgrammeSession` + `completeCustomWorkout`); collapses to one `bus.publish('workout:logged', { workout_id, completed:true, duration_min, source:'programme'|'custom' })`. Subscribers per taxonomy: workouts.html, exercise.html, index.html, achievements.js. Same option-(a) discipline as PM-30 — publishing site emits bus event only; subscribers call the existing primitives internally. Add `<script src="/bus.js" defer>` to workouts.html. SW cache bump same atomic commit.
