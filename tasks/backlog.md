> **[2026-09-04 · PM-1001 CORS SWEEP — survey DONE, 2/16 fixed, 14 remain (mechanical)]** Store binaries = capacitor://localhost (iOS) / https://localhost (Android) — ship config has no server block. FIXED+verified: notifications v25, log-activity v46. **REMAINING 14 (add capacitor://localhost, + https://localhost where absent; un-pin the pinned two):** leaderboard, wellbeing-checkin*, member-achievements*, log-perf, gdpr-export-request, gdpr-erase-request, gdpr-erase-cancel, gdpr-erase-status, submit-complaint*, create-booking*, confirm-booking*, partner-file-url*, monthly-checkin (PINNED online), get-activity-feed (PINNED online); anthropic-proxy (missing BOTH native origins). *=verify_jwt TRUE — preserve. Multi-file EFs redeploy ALL files (log-activity precedent). Verify per fn = Management-API eszip grep for capacitor://localhost (MGMT_PAT, Composio curl). apply-trial deliberately left pinned www (web-only). Handoff prompt delivered to Dean.
