...
> **[2026-08-11 · CC MOBILE AUDIT + NATIVE APP (Dean direction, PM-920)]** CC is heading for a phone app for account management. PM-920 fixed the shell-level overflow (topnav toolbar) + meetings rows; a page-by-page mobile fit pass is now a PRECONDITION of the wrap — fold it into Streamline Phase 2/3 since partners.html (165KB) and admin-console.html (132KB) are the biggest offenders and are being ported anyway. Then the wrap itself (Capacitor, reuse vyve-capacitor patterns; CC has no sw/vbb ritual — Cloudflare deploys straight). Sequence: Phase 2 port + mobile pass → Phase 3 → wrap.
