...
- 📋 **OPEN (P0) — PM-40: Layer 1c-11 — TBD specific row.** Eleventh 1c migration. Four remaining 1c surfaces post-PM-39. Taxonomy ordering needs fresh pre-flight at next session start (HEAD will be `1a5d9ef8` = the PM-39 ship). Candidates by surface: (a) **monthly-checkin.html** — already loads bus.js + has `habit:logged` subscriber at L935; needs its own publish surface for monthly check-in submission. The page has TWO `VYVEAchievements.evaluate()` sites (L760, L952) and zero invalidate/record. Likely a NEW event `monthly_checkin:submitted` (taxonomy ADD), evaluate-only pre-bus → ASYMMETRIC if monthly_checkin has no engagement-cache impact, SYMMETRIC if it does. Pre-flight to confirm. (b) **shared-workout.html** — workout sharing flow, zero primitives currently, likely emits `workout:shared` (ADD; ASYMMETRIC). (c) **certificate.html / certificates.html** — certificate generate / view flows, zero primitives, may emit `certificate:earned` or `certificate:viewed` (ADD; achievement-adjacent — would need self-subscribe pattern if achievements track exists for cert milestones). (d) **live-session pages** (yoga-live, mindfulness-live, therapy-live, events-live, education-live, podcast-live, workouts-live, checkin-live) — eight live-session pages that all share session-live.js. Likely emit `session:joined` or `session:viewed` (ADD). Recommendation: monthly-checkin.html is the biggest-blast-radius option remaining, has clean existing primitives, and the bus.js wiring is already done — fastest path. Pre-flight at session start will resolve.
