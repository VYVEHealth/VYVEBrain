...
**Housekeeping item from PM-378 ship.** Cross-repo PM-378 collision: parallel `VYVEHealth/VYVEBrain` commit `451f849` titled "PM-378 — Architecture map (staging)" claimed PM-378 for a brain-only staging doc (`brain/staging/architecture-map-pm378.md`) ~13 minutes before my vyve-site `abcf3d3e` POST /git/commits. vyve-site `abcf3d3e` retained PM-378 as canonical because it's permanent in the commit message, 6 source files, the Supabase migration name, the sw cache key, and the `CATALOGUE_INVALIDATION_KEY` value — renumbering would have generated permanent footnote churn across all of those. Proposed at next housekeeping pass: rename `brain/staging/architecture-map-pm378.md` → `architecture-map-pm378b.md` (or whatever non-clashing label) to disambiguate. No downstream references in the brain yet — single grep + single file rename + single brain commit. Sibling item: §23.70 cross-repo extension candidate banked — current rule scans the ship target repo only; cross-repo PM-N namespace cases (where brain and vyve-site PM numbers can collide silently) are out of grep window. Not promoted on first occurrence; if this recurs, promote.
