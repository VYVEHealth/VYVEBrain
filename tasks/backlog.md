...
- 📋 **OPEN (P0) — PM-36: Layer 1c-7 — `log-food.html` insert + delete → `bus.publish('food:logged' / 'food:deleted', ...)`.** Seventh 1c migration. **First 1c migration with two distinct event names from one publishing surface** — both events are taxonomy ADDs (neither exists today; food:logged and food:deleted are new event names introduced by 1c-7). Taxonomy migration label is REFACTOR + race-fix. Pre-flight: log-food.html currently has `inline cache writes + 2× evaluate` per the taxonomy 1c-7 row — confirm exact line numbers and primitive shape against live source at next pre-flight. NO new subscribers needed for the food:* events unless engagement_cache or home_cache wants to react (probably yes — home dashboard's nutrition strip and engagement variety component). Pre-flight should confirm: (1) what cache keys log-food.html writes on insert/delete and whether they're page-local or cross-page; (2) whether index.html home subscriber should react to food:logged for the variety component (currently the home dashboard's nutrition strip refreshes on its own cycle, not on log events); (3) whether engagement.html should fold food:* into _markEngagementStale; (4) where the `<script src="/bus.js" defer>` tag goes on log-food.html. Bus.js is NOT yet loaded on log-food.html — first new bus.js script-tag addition since PM-34 (movement.html). SW cache bump same atomic commit. **First-time wiring decisions** (food:logged + food:deleted as two distinct events vs one event with `kind:'insert'|'delete'` discriminator) deserve a spike at pre-flight time — taxonomy currently lists them as two events but the discriminator-vs-event-pair decision was made at PM-26 audit time without live source check; revisit.
