> **[2026-09-04 · PM-991 PARTNER PORTAL OVERHAUL — WAVE 1 SHIPPED, W2-W6 queued]** Method = PM-982 redux, benchmark our own coach portal; map at `tasks/partner-portal-gap-map.md`. W1 ✅ (CC `ba038b87`: sidebar IA, dashboard cockpit, notifications feed, hash routing — client-only, RLS pre-covered, jsdom 18/18). REMAINING: **W2** Account area (tabbed profile + completion bar, notification prefs jsonb, post-approval checklist card, welcome-video slot skipping 48h publish_at, admin Go Live in CC on portal minimums — PM-880 posture, Lewis deferred-assessments nod = switch not build) · **W3** calendar unification (sessions+diary+bookings → month/week/list grid) + requests inbox + content library lanes/folders/stats · **W4** money (earnings deep-dive, in-portal referral funnel, monthly statements + finance payout run, human promo codes Piece 1) · **W5** reach (leads generalised beyond coaches, ratings list surface, community engagement stats, Checkout Session Piece 2, CC manual attribution Piece 5) · **W6** wizard at-source (photos→thumbnails + cropper port, honest upload errors, wizard reportError) + partners.vyvehealth.co.uk + PORTAL_BRAND_URL + one-shot EF deletions. Dean W1 pass owed: partner login → dashboard tiles real numbers, all 7 legacy views route (bookings lazy-loads), notif badge clears on open, #bookings deep link.
