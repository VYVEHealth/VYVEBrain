> **[PM-843 · 2026-08-06 · PARTNER FUNNEL P0 STACK — SHIPPED as PM-844/848/849, this session]** All three items live: wizard re-sequenced (5 steps, video optional, resume-link card — `9a3af440`), 48h/7d nudges + stage emails riding cron 54 (partner-draft-purge v2), pre-filled invite links (partner-onboarding v13 `mint_invite` + CC modal `4700aab2`). **SURVIVING OPEN ITEMS:** **DECISIONS OWED (Dean+Lewis, block nothing):** (a) photo-ID policy — wizard requires a passport under "VYVE verifies your identity" but NO verification process exists and submit auto-stamps identity_verified:true (GDPR minimisation; PM-634 recorded ID as removed, live wizard disagrees) — either an explicit admin "ID checked" pipeline action feeds the flag, or collection moves to contract stage; (b) quiz coherence — 3-question safeguarding/GDPR gate quizzes on hidden training modules — restore modules (Lewis/Phil content) or reframe as declaration. **LEWIS COPY PASS (non-gating):** 48h + 7d nudge emails, onboarding + declined stage emails, Lewis-voiced invite email. **NOW, zero build:** Lewis chases Vanessa with ?resume=13f61ee9-a991-4a2d-8b34-a096c914cb5a — she can now submit WITHOUT the video; metric: invite→submit ≥80%, trackable via the `_system.invited` cohort. Dean device checks pending: wizard ?z=844, CC mint walk.
