...
- 📋 **OPEN (P0) — PM-45: Layer 2 prep (Supabase Realtime + storage event integration).** Layer 2 is cross-tab/cross-device cache coherence: Realtime row events translate to bus events of the same name a local write would publish. Existing storage-event cross-tab transport (already in bus.js) handles within-browser-instance fanout; Realtime adds cross-device fanout (member logs habit on phone, sees the change reflected on desktop within seconds). Pre-flight scope:

  1. **Realtime channel topology.** One channel per active table (`daily_habits`, `workouts`, `cardio`, `nutrition_logs`, `weight_logs`, `wellbeing_checkins`, `monthly_checkins`, `session_views`, `replay_views`, `custom_workouts`, `shared_workouts`?). RLS-aware: each member subscribes only to rows where `member_email = auth.email()`. Test that Supabase Realtime respects the existing RLS policies (it does by default, but verify).

  2. **Realtime → bus mapping.** A Realtime INSERT on `daily_habits` should publish `habit:logged` with `origin: 'realtime'` (new origin value alongside `'local'` and `'remote'`). A DELETE on `nutrition_logs` should publish `food:deleted`. The mapping is per-table-per-event-type; codify in `playbooks/realtime-bus-bridge.md` (new playbook to create).

  3. **Idempotency boundary.** Same member, same row, same write → fires `local` then `remote` (storage event from another tab) then `realtime` (cross-device echo). Subscribers must be idempotent. They already are for cache-stale (idempotent by definition), but achievements eval is debounced 1.5s — verify the debounce handles 3-fold fan-in correctly.

  4. **Connection lifecycle.** Realtime channels need to subscribe on auth-ready, unsubscribe on auth-signed-out. The existing `auth:ready` and `auth:signed-out` bus events are the right hook points (codified in bus.js header comment).

  5. **Reconciliation gap.** Remote-origin events that fire while the local tab is offline (Realtime missed) need a catch-up sweep on next online event. The catch-up sweep is Layer 3 territory but the design needs to leave room for it (e.g. each subscriber should be tolerant of missed events — most are because they just bust caches, but achievement-track-completing events may need a periodic re-evaluate sweep).

  Layer 2 is bigger than any single Layer 1c migration. Estimate: 4-8 commits over 2-3 sessions. First commit (PM-45) is the bus.js patch + the realtime-bus-bridge.md playbook, no member-facing wiring yet — purely infrastructure prep with self-tests against a mock Realtime channel.
...
- 📋 **CARRIED FORWARD (P3) — Certificate cross-tab/cross-device cache coherence.** Punt to Layer 2 — actually a natural fit there. When the certificate-checker EF v9 cron inserts a new `certificates` row at 09:00 UTC, the Realtime channel for `certificates` should fire `certificate:earned` on whichever client tab is active. Member sees the cert appear without manual refresh.
