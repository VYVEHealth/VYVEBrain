...
> **[2026-09-02 · P0 — COACHING SPRINT REMAINDER (gate + apply pipeline + bucket SHIPPED in PM-956; vyve-site d40b5876 vbb 565, CC 8e77ea60)]** **(1) Trial-wall £10 routing:** coaching-tier day-7 wall → £10/mo Checkout Session (extend partner-join-redirect pattern); stripe-webhook sets partner_memberships subscription_value=10 → £5/mo flows through run_partner_payouts. **(2) Workout-completion summary to coach** (Kahunas: difficulty slider + note on finish) surfacing in coach client-detail view. **(3) Provision Calum** (auth user + admin_users partner row + set-password link — 2-min job, say go). **Device test first:** log in as deanonbrown12@hotmail.com — auth.js routes to coach-welcome, walk all 4 steps (fixture: 5 real templates + terms v1 under the Dean Brown test coach, status invited/unconsented), finish → programme/habits/nutrition/supplements in-app; then portal client-detail save → verify auto-apply feedback line. **Then (sidebar slots live, PM-955j):** coach messaging (portal↔app threads); automations (triggered client emails); content library (partner_content_items + YouTube unlisted); calendar wiring (booking engine exists); notifications feed; coach profile editing; Simple/PDF programme variants (later).
