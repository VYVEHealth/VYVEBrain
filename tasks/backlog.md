...
- 📋 **OPEN (P0) — PM-39: Layer 1c-10 — TBD specific row.** Tenth 1c migration. Taxonomy ordering needs a fresh review at next session start (HEAD will be `a0b98f17` = the PM-38 ship). Five remaining 1c migrations across {shared-workout, monthly-checkin, certificate generate/download, live-session pages, and any other publish surface still on direct-primitive pattern}. Pre-flight: re-read the original Layer 1c taxonomy and pick the next row by smallest-blast-radius / clearest semantics first. Candidates by surface: (a) **shared-workout.html** — workout sharing flow, likely emits `workout:shared` (taxonomy ADD); (b) **monthly-checkin.html** — already has bus subscriber wiring for habit:logged at L935, may need its own publish for `monthly_checkin:submitted`; (c) **certificate.html** / certificate generation — may emit `certificate:earned` (taxonomy ADD; achievement-adjacent); (d) **live-session pages** (yoga-live, mindfulness-live, etc.) — emit `session:joined` or `session:viewed`. Pre-flight will surface which has the cleanest primitives shape and prioritise. Likely SYMMETRIC fallback for any page that already has invalidate/record/eval primitives at the publish site; ASYMMETRIC for any with zero primitives (apply the PM-38 §23 hard rule). SW cache bump same atomic commit. Pre-flight at session start will resolve specific 1c row.
