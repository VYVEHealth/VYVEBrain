> **[2026-09-02 · PM-966 REVEAL-FLOW — follow-ons]** (1) **Dean device pass on 575** (force-quit ×2 or fresh install w/ patience): video keeps playing at plan-ready + View-my-plan button, rv2/rv3 centred, hub videos muted-autoplay w/ sound chip, lands on Home, Body hub video appears via nav. (2) **First-run OTA race — design call (Dean):** block first launch on sync vs 60s release (§23.188); either way next binaries bake `d69e64c6`+ (supersedes the PM-964 bake line). (3) **Store-first customer funnel (P0 tomorrow, pre-dates tonight's fixes):** login screen needs 'New to VYVE? Create your account' + honest reset-email copy — first organic App Store customer dead-ended tonight (no account, silent no-op reset); Dean to forward their email address for account check + reply. (4) Lewis copy pass owed on new strings: 'Your plan is ready', loading subtext, 'View my plan →', 'Tap for sound'.
