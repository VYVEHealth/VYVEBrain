> **[2026-09-03 · GATE PASSED ON 581 — BAKE IN FLIGHT]** Dean confirmed all fixed on 581. Bake `a0d24ce1` → iOS 2.0.1 build 6 / Android 1.0.10 code 56, expedited review requested. Post-approval: OTA continuity check, LU key rotation, fixture sweep + comps (test1, test6, a@a.com, d@d.cok + other tonight signups). PM-973 shipped: join-date promotion clamp (sync-health-data v26) — verify behaviourally on next fresh signup (expect promoted.skipped_prejoin > 0, Body 0/30 day one). Still queued: store-first customer funnel P0 (spec in backlog 3f4f26df — awaiting customer email from Dean), EF CORS sweep, achievements.ts hk_connected inversion, Lewis copy pass on new onboarding strings, HAVEN/Phil unchanged.
