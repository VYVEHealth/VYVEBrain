...
- **5 member_email FKs are NO ACTION.** `custom_workouts`, `exercise_logs`, `exercise_swaps`, `shared_workouts.shared_by`, `workout_plan_cache`. Future migration: `ALTER TABLE ... DROP CONSTRAINT ...; ALTER TABLE ... ADD CONSTRAINT ... FOREIGN KEY (member_email) REFERENCES members(email) ON DELETE CASCADE;` for each. Not blocking — `gdpr_erasure_purge` works around with explicit-deletes-in-order — but future writers might assume CASCADE behaviour.
- **Cron auth via Vault not hardcoded literal.** Tonight's `cron.alter_job` calls bake the CRON_SECRET into the SQL command. Migrate to a Vault lookup or DB-level GUC so secret rotation is one place not many.
- **Visual QA pass on commit 4.** Open `https://online.vyvehealth.co.uk/settings.html` in production after GitHub Pages rebuild propagates (~30-90s post-commit), check the Danger Zone card renders correctly in light + dark theme, and that the modal typed-email gate enables/disables the confirm button as expected. Don't trigger the actual delete on a real account.
- **Test member auth.users seeding.** Tonight's E2E used a test member with no auth.users row, so the `auth.admin.deleteUser` codepath returned `no_auth_user_found` instead of exercising the actual auth delete. Either update the test-member seeding pattern to create both rows, or build a dedicated GDPR test fixture.
- **Email pipeline tag for erasure events.** Tonight's emails are tagged `gdpr_erase_request`, `gdpr_erase_cancel`, `gdpr_erase_executed`. Worth adding these tags to whatever Brevo report Lewis sees so the activity is visible without grep'ing the audit log.
- **`gdpr_erase_request` admin path UI.** EF supports admin-on-behalf-of erasure (audit-logged), but no admin UI yet. Low priority — manual SQL is fine for the rare case where staff need to trigger erasure on a member's behalf.
- **`platform_alerts` from erasure path** should surface in the daily ops digest. Currently they're written to the table but not yet pulled into any dashboard; the `daily-report` EF should be extended to include `gdpr_erase_*` types in its alert-summary section.

## Added 07 May 2026 PM-4 (Backup & DR session 1)

- **Item 3 — `vyve-ef-source-backup` EF + cron — NEW (next session priority).** EF that uses the Supabase Management API (auth via `SUPABASE_MGMT_PAT` secret, generated 07 May 2026 PM-4 by Dean) to walk all KEEP-listed Edge Functions, fetch each EF's `files[]` array via `https://api.supabase.com/v1/projects/{ref}/functions/{slug}/body`, and commit `{name}/{file}` for each multi-file function and `{name}/index.ts` for single-file into `VYVEBrain/staging/edge-functions/{slug}/` via `GITHUB_PAT_BRAIN`. Manifest at `VYVEBrain/staging/edge-functions/MANIFEST.json` with `snapshot_at`, `supabase_project`, `ef_count`, `keep_list`, `platform_versions`, `ezbr_sha256` per EF. Single `GITHUB_COMMIT_MULTIPLE_FILES` per run. Cron `vyve-ef-source-backup-weekly` Sundays 02:00 UTC, 5-min timeout. Logs to `vyve_job_runs`. Email-watchdog (jobid 16) catches failures. KEEP list per filtering done 07 May 2026 PM-4: 61 EFs (out of 94 active on platform), DELETE list excludes one-shot patchers (`ban-user-anthony`, `send-stuart-reset`, `generate-stuart-plan`, `trigger-owen-workout`, `trigger-callum-workout`, `create-ai-decisions-table`, `setup-ai-decisions`, `setup-member-units`, `create-test-member`, `inspect-members-schema`, `run-monthly-checkins-migration`, `run-migration-monthly-checkins`, `monthly-checkin-test`, `debug-exercise-search`, `debug-cert-content`, `debug-show-file`, `test-html-render`, `add-exercise-stream`, `force-cache-refresh`, `update-brain-changelog`, `thumbnail-audit`, `thumbnail-upload`, `thumbnail-batch-upload`, `seed-library-1`, `seed-library-2`, `seed-b1`, `smoketest-ach-push`, `fire-test-push`, `send-test-push`, `delete-housekeeping`, `send-test-welcome`, `resend-welcome`, `send-password-reset`). Build estimated 30-40 min. Includes manual first-run invocation to populate the initial staging dir + verification re-fetch. **Pre-build checklist**: (a) Dean rotates `SUPABASE_MGMT_PAT` since the original was visible in chat — set fresh value as project secret, name unchanged. (b) Confirm `GITHUB_PAT_BRAIN` still active (per §24, expires 18 April 2027 — fine). (c) Confirm KEEP list still accurate by re-running `Supabase:list_edge_functions` and diffing against the 07 May filter — new EFs since today should be added; deletions if any would be removed.
