...
- 📋 **OPEN (P0) — PM-37: Layer 1c-8 — `nutrition.html` weight log → `bus.publish('weight:logged', ...)`.** Eighth 1c migration. Taxonomy migration label is REFACTOR + scope-fix on `members + wb_last`. Pre-flight: nutrition.html has `invalidate + evaluate` per the taxonomy 1c-8 row — confirm exact line numbers and primitive shape against live source at next pre-flight (HEAD `640c9d69` = the PM-36 ship). Whole-tree audit needed at session start. Schema: `bus.publish('weight:logged', { weight_kg, logged_date })`. Cross-page consumers expected: index.html (home dashboard's weight strip), engagement.html (no current consumer for weight, but may want to add — pre-flight decides). Probably ASYMMETRIC fallback if the scope-fix gap is real (members table writes might not currently invalidate cache); SYMMETRIC if pre-bus already had invalidate + evaluate. nutrition.html does NOT load bus.js today — first new bus.js script-tag wiring needed since PM-36 (log-food.html). SW cache bump same atomic commit. Spike at pre-flight: confirm whether `wb_last` is a real cache key (whole-tree grep) and what `members` table writes actually need invalidated downstream — the taxonomy mention may be stale. Also: weight log is a single-publish-surface migration unlike PM-36 — should be a smaller commit, similar shape to PM-35.
