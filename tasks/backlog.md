...
**capacitor.config.json doctrine — RESOLVED (PM-569, 9 Jun 2026): bundled mode (option a) chosen and SHIPPED.** iOS 1.7 is LIVE with `server.url` REMOVED → iOS members are bundled/frozen, updatable only via a Capawesome OTA (app `f9961f66-eb66-4102-b1c5-f9b2c7baeebf`, prod channel `89e12796`). Android bundled 1.0.6 vc51 still in review (Android members stay server.url-live until it lands). **P0 NATIVE PRIORITY — verify ONE Capawesome OTA push end-to-end (`--rollout 0.1` canary first):** until proven, iOS members have NO working update path short of a full store resubmit (§23.106). **Root cause pinned (PM-571 / §23.107): LiveUpdate (8.2.2) is installed + configured in Mac-local config but has NO `sync()`/`ready()` calls in the web shell — 1.7 literally cannot receive an OTA. Fix is JS-only (wire `sync()` on launch + `ready()` after first paint + channel `89e12796`, native-guarded) but requires shipping a NEW bundled binary (1.8); the canary OTA must target 1.8, not 1.7.** Hard pre-Sage gate, ahead of new features. Also outstanding: vyve-capacitor REMOTE is BEHIND Mac-local — the 1.5/1.6/1.7 ship-state is uncommitted on remote (latest remote commit PM-560 splash); curate + atomic-commit Mac-local → remote per PM-413 Pending #2.
