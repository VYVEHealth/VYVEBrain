...
> **[2026-08-11 · CC → PHONE APP BY END OF WEEK (Dean, PM-920/923) — AUDIT DONE, wrap gate = playbooks/cc-mobile-audit.md checklist]** Systemic fixes SHIPPED PM-920/921/923 (topnav, checkboxes, all tables self-scroll, .tab-bar swipes, tasks board snap-scrolls, media clamp). REMAINING before wrap: (1) Phase 2 partners.html port + mobile pass (standalone doc — outside the global .page rules, 8 tables, worst surface); (2) Phase 3 admin-console port + mobile pass (5 tables); (3) partner-portal same class (partners' own surface — decide wrap-scope); (4) calendar.html mobile month view (agenda list ≤640); (5) chart canvas device sweep; (6) Dean full-sidebar iPhone walk logging against the playbook. THEN Capacitor wrap pointing at admin.vyvehealth.co.uk (no sw/vbb ritual; reuse vyve-capacitor patterns — note vyve-capacitor git-init debt). Conventions for all new/ported pages are §Conventions in the playbook.
