...
- 📋 **OPEN (P0) — PM-35: Layer 1c-6 — `workouts-builder.js` custom workout creation → `bus.publish('workout:logged', source:'builder', ...)`.** Sixth 1c migration. Taxonomy migration label is REFACTOR + scope-fix (no current invalidation today = real bug). Pre-flight: workouts-builder.js currently fires `VYVEAchievements.evaluate()` after a successful custom workout INSERT but does NOT call `invalidateHomeCache` or `recordRecentActivity`. The bus migration auto-fixes this — publishing `workout:logged source:'builder'` routes through the existing index.html home-stale + engagement.html engagement-stale + workouts.html eval subscribers, all already wired from PM-31 / PM-33 / PM-33-bonus. The home-stale gap on builder-creates is a small but real bug — when a member creates a custom workout, the home dashboard's "today's workouts" count doesn't refresh until next sign-in. Workouts.html subscriber's source-discriminated programme_cache stale will correctly bypass for `source:'builder'` (only fires on `source:'programme'`). Schema: `bus.publish('workout:logged', { workout_id?, completed:true, duration_min?, source:'builder' })`. Pre-flight against live source — confirm the exact line numbers, whether duration_min is available at the publish site, whether there's a writeQueued path or just a direct fetch (the discriminator for "is the bus-fallback else-branch needed?" is whether the existing primitive triplet is post-await; if it's pre-await we'd need a different shape). NO new subscribers needed. Adds `<script src="/bus.js" defer>` to workouts-builder.js's host page if it's not already present (workouts-builder.js is a JS module loaded by some host page — pre-flight to identify which page hosts it and whether bus.js is already wired there, e.g. workouts.html already has bus.js from PM-31). SW cache bump same atomic commit.
