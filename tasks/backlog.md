...
> **[2026-09-02 · P0 — COACHING SPRINT: gate/apply/bucket (PM-956) + £10 routing/workout feedback (PM-956b) ALL SHIPPED — vyve-site c659d006 vbb 566, CC 3dd0a537, stripe-webhook v14, trial-end-email v2]** **(1) Device test:** log in as deanonbrown12@hotmail.com — walk the 4-step gate (fixture ready: 5 templates + terms v1, status invited/unconsented) → programme/habits/nutrition/supplements land; complete a workout → difficulty+note block appears → check the portal client-detail timeline shows it; portal assignments save → auto-apply feedback line. **(2) First £10 coaching conversion verifies the webhook rail live** (sandbox can't sign Stripe events) — deanonbrown12's partner_memberships row sits at 0/trialing, should flip to 10/active. **(3) Provision Calum** — staged, on Dean's explicit go (auth user + admin_users partner row + set-password link; his partner row is live). **Then (sidebar slots live, PM-955j):** coach messaging (portal↔app threads); automations (triggered client emails); content library (partner_content_items + YouTube unlisted); calendar wiring (booking engine exists); notifications feed; coach profile editing; Simple/PDF programme variants (later).
