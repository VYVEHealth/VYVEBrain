...
const ALERT_CC  = ["lewisvines@hotmail.com", "team@vyvehealth.co.uk"];
...
serve(async (req) => {
  ...
  if (!BREVO) return new Response(JSON.stringify({error:"BREVO_API_KEY not set"}), {status:500});
  ...
  try {
    ...
    issues.push({severity:"warn",code:"check1_error",title:"Daily-report check failed",detail:e.message});
  }
  ...
  try {
    ...
    issues.push({severity:"warn",code:"check2_error",title:"Bounce check failed",detail:e.message});
  }
  ...
  try {
    ...
    issues.push({severity:"warn",code:"check3_error",title:"Blocklist check failed",detail:e.message});
  }

  // ============== CHECK 4: Cron job failures in last 6h ==============
  try {
    const { data: failures } = await sb.rpc("watchdog_cron_failures", { hours_back: 6 });
    if (failures && failures.length > 0) {
      const summary = (failures as any[]).slice(0,5).map((f:any) => `${f.jobname}: ${f.return_message?.slice(0,80)}`).join("; ");
      issues.push({
        severity: "critical",
        code: "cron_failures",
  ...
  try {
    ...
    if (recent.length >= 5) {
      issues.push({
        ...
        detail: `Sample recipients: ${[...new Set(recent.slice(0,8).map(e=>e.email))].join(', ')}`,
      });
    }
  } catch (e:any) {
    issues.push({severity:"warn",code:"check5_error",title:"Bounce-spike check failed",detail:e.message});
  }

  // No issues? Done.
  if (issues.length === 0) {
    return new Response(JSON.stringify({success:true,checked_at:now.toISOString(),issues:0}), {status:200, headers:{...corsHeaders,"Content-Type":"application/json"}});
  }
  ...
  const recentCodes = new Set((recent || []).map((r:any)=>r.code));
  const newIssues = issues.filter(i => !recentCodes.has(i.code));

  if (newIssues.length === 0) {
    return new Response(JSON.stringify({success:true,issues:issues.length,suppressed:issues.length,sent:0}), {status:200, headers:{...corsHeaders,"Content-Type":"application/json"}});
  }
  ...
  const sent = await fetch("https://api.brevo.com/v3/smtp/email", {
    ...
    body: JSON.stringify({
      sender:{name:"VYVE Watchdog",email:"team@vyvehealth.co.uk"},
      to:[{email:ALERT_TO}],
      cc: ALERT_CC.map(e => ({email: e})),
  ...
  for (const i of newIssues) {
    await sb.from("watchdog_alerts").insert({code: i.code, severity: i.severity, title: i.title, detail: i.detail, fired_at: now.toISOString()});
  }
...
function buildAlertHtml(issues: Issue[], now: Date): string {
