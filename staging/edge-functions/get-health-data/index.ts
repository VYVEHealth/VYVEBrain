...
function emptyDailyBuckets(start: Date, end: Date): Record<string, Record<string, number>> {
...
Deno.serve(async (req) => {
  ...
  try {
    const { data: connRows } = await svc
      ...
      .select("platform, last_sync_at, last_sync_status, granted_scopes, total_synced, revoked_at")
    ...
    const samples = [...workoutSamples, ...hrSamples, ...weightSamples, ...sleepSamples];

    // Fetch daily aggregates (steps, distance, active_energy)
    const { data: dailyRaw, error: dailyErr } = await svc
    ...
    // ─── Workouts card ───
    const workouts = workoutSamples.map((s: any) => {
      const start = new Date(s.start_at);
      const end = new Date(s.end_at);
      const durationMin = Math.max(1, Math.round((end.getTime() - start.getTime()) / 60000));
      const distM = (s.metadata && typeof s.metadata === "object") ? Number((s.metadata as any).distance_m) || null : null;
      return {
        date: s.start_at,
    ...
    const hrAgg: Record<string, { min: number; max: number; sum: number; n: number }> = {};
    for (const s of hrSamples) {
      const day = isoDate(new Date(s.start_at));
      const v = Number(s.value || 0);
      if (!hrAgg[day]) hrAgg[day] = { min: v, max: v, sum: v, n: 1 };
      else {
    ...
    // ─── Sleep nightly ───
    const sleepNightly: Record<string, { asleep: number; rem: number; deep: number; light: number; awake: number }> = {};
    for (const s of sleepSamples) {
      ...
      if (!sleepNightly[day]) sleepNightly[day] = { asleep: 0, rem: 0, deep: 0, light: 0, awake: 0 };
      const state = (s.metadata && typeof s.metadata === "object") ? String((s.metadata as any).sleep_state || "").toLowerCase() : "";
    ...
    // ─── Weight ───
    const weightHkSamples = weightSamples.map((s: any) => ({
    ...
    const { data: wlRows } = await svc
      .from("weight_logs")
      .select("logged_date, logged_at, weight_kg, native_uuid")
      .eq("member_email", memberEmail)
      .gte("logged_date", isoDate(startDate))
      .order("logged_date", { ascending: false });

    const weightLogs = (wlRows || []).map((r: any) => ({
      date: r.logged_date,
      at: r.logged_at,
    ...
    const counts: Record<string, number> = {
      ...
      steps: 0, active_energy: 0, distance: 0,
    };
    // For aggregated types: count = distinct days with data
    for (const d of daily) {
      const t = d.sample_type as string;
      if (t in counts) counts[t]++;
    }

    return jsonResponse({
      ok: true,
      window: { start: startDate.toISOString(), end: endDate.toISOString(), days: WINDOW_DAYS },
      connection,
      counts,
      total_samples_in_window: samples.length + daily.length,
      workouts,
      steps_daily: Object.entries(stepsDaily)
        .map(([date, v]) => ({ date, total: v.total || 0 }))
        .sort((a, b) => b.date.localeCompare(a.date)),
      heart_rate_daily: Object.entries(hrAgg)
        .map(([date, v]) => ({ date, min: v.min, max: v.max, avg: Math.round(v.sum / v.n) }))
        .sort((a, b) => b.date.localeCompare(a.date)),
      active_energy_daily: Object.entries(kcalDaily)
        .map(([date, v]) => ({ date, total: Math.round(v.total || 0) }))
        .sort((a, b) => b.date.localeCompare(a.date)),
      sleep_nightly: Object.entries(sleepNightly)
        .map(([date, v]) => ({
          date,
        ...
        .sort((a, b) => b.date.localeCompare(a.date)),
      distance_daily: Object.entries(distDaily)
        .map(([date, v]) => ({ date, total_km: Number((v.total_km || 0).toFixed(2)) }))
        .sort((a, b) => b.date.localeCompare(a.date)),
    ...
    return jsonResponse({ error: "internal_error", message: String((err as any)?.message || err) }, 500);
  }
});
