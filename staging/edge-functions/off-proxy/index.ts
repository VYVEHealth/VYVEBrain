...
function scoreOFF(p: any): number {
  let s = 0;
  const countries: string[] = p.countries_tags || [];
  const isUK = countries.some((c: string) => c.includes('united-kingdom'));
...
async function searchCommonFoods(q: string): Promise<any[]> {
  ...
  const [r1, r2] = await Promise.all([
    fetch(`${base}&food_name=ilike.*${encodeURIComponent(lower)}*`, { headers }),
    fetch(`${base}&search_terms=ilike.*${encodeURIComponent(lower)}*`, { headers }),
  ]);

  const rows1: any[] = r1.ok ? await r1.json() : [];
  const rows2: any[] = r2.ok ? await r2.json() : [];

  const seen = new Set<string>();
  const merged: any[] = [];
  for (const r of [...rows1, ...rows2]) {
    if (!seen.has(r.food_name)) { seen.add(r.food_name); merged.push(r); }
  }
  return merged.slice(0, 12);
}

function formatCommonFood(r: any) {
  return {
  ...
  try {
    if (type === 'search') {
      ...
      const [commonRows, offData] = await Promise.all([
        searchCommonFoods(q),
        fetch(
          `${OFF_BASE}/cgi/search.pl?search_terms=${encodeURIComponent(q)}&json=1&page_size=${PAGE_SIZE}&fields=${OFF_FIELDS}&sort_by=unique_scans_n`,
          { headers: { 'User-Agent': 'VYVEHealth/1.0 (team@vyvehealth.co.uk)' } }
      ...
      const offProducts = ((offData.products || []) as any[])
        .filter((p: any) => p.product_name && p.product_name.trim().length > 0)
        .map((p: any, i: number) => ({ ...p, _score: scoreOFF(p), _rank: i }))
        .sort((a: any, b: any) =>
          b._score !== a._score ? b._score - a._score : a._rank - b._rank
        )
        .slice(0, 20)
        .map(({ _score, _rank, ...p }: any) => p);

      console.log(`[off-proxy] q="${q}" common=${commonProducts.length} off=${offProducts.length}`);

      const merged = [...commonProducts, ...offProducts].slice(0, 24);
      return jsonOk({ products: merged });
      ...
      const res    = await fetch(offUrl, { headers: { 'User-Agent': 'VYVEHealth/1.0 (team@vyvehealth.co.uk)' } });
      if (!res.ok) return json({ error: `OFF returned ${res.status}` }, 502);
      return jsonOk(await res.json());

    } else {
      return json({ error: 'type must be search or barcode' }, 400);
    }
