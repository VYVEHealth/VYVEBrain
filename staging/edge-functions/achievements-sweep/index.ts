...
async function pushAchievementEarned(email: string, earns: MemberEarn[]): Promise<boolean> {
  ...
  try {
    const res = await fetch(`${SUPABASE_URL}/functions/v1/achievement-earned-push`, {
      ...
      body: JSON.stringify({ member_email: email, earns }),
    });
    if (!res.ok) {
    ...
    console.warn("[achievements-sweep] push error", email, (e as Error).message);
    return false;
  }
}

async function sweepMemberDays(supabase: any): Promise<SweepResult> {
  const result: SweepResult = {
    metric: "member_days", rows_inserted: 0, members_processed: 0,
    push_attempted: 0, push_succeeded: 0, push_failed: 0, errors: [],
  };

  // v2: pull title + body alongside tier_index + threshold for push payload
  const { data: tiers, error: tErr } = await supabase
    .from("achievement_tiers").select("tier_index, threshold, title, body")
    .eq("metric_slug", "member_days").order("tier_index");
  if (tErr) { result.errors.push(`tiers: ${tErr.message}`); return result; }
  if (!tiers?.length) { result.errors.push("no tiers"); return result; }
  ...
  if (mErr) { result.errors.push(`members: ${mErr.message}`); return result; }
  if (eErr) { result.errors.push(`earned: ${eErr.message}`); return result; }

  const earnedSet = new Set<string>();
  for (const r of earned ?? []) earnedSet.add(`${(r.member_email as string).toLowerCase()}:${r.tier_index}`);

  const tierMap = new Map<number, { title: string; body: string | null }>();
  for (const t of tiers) tierMap.set(t.tier_index, { title: t.title, body: t.body });

  const now = Date.now();
  const newRows: any[] = [];
  // v2: track freshly-earned tiers per member for push fan-out
  const earnsByMember = new Map<string, MemberEarn[]>();
  ...
  for (const m of members ?? []) {
    ...
    for (const t of tiers) {
      ...
      newRows.push({ member_email: email, metric_slug: "member_days", tier_index: t.tier_index });
...
serve(async (_req) => {
  try {
    ...
    return new Response(JSON.stringify({
      success: true,
      elapsed_ms: elapsed,
      results: [memberDaysRes],
      phase2_deferred: [
        "lifetime_steps", "lifetime_distance_hk", "lifetime_active_energy", "nights_slept_7h",
        "full_five_weeks", "charity_tips", "personal_charity_contribution",
        "tour_complete", "healthkit_connected",
      ],
