...
Deno.serve(async (req)=>{
  ...
  if (req.method === 'POST' && action === 'add_programme') {
    ...
    await adminClient.from('workout_plan_cache').update({
      is_active: false,
      paused_at: new Date().toISOString()
    }).eq('member_email', email).eq('is_active', true).eq('programme_json->>surface', surface);
    ...
    const newProgrammeJson = {
      ...share.full_programme_json || {},
      surface
    };
    const { error: insertError } = await adminClient.from('workout_plan_cache').insert({
      member_email: email,
