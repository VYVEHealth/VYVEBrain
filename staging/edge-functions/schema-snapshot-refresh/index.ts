...
async function logAlert(severity: 'critical'|'high'|'info', type: string, details: string) {
  try {
    await fetch(`${SUPABASE_URL}/rest/v1/platform_alerts`, {
      ...
      body: JSON.stringify({ severity, type, source: 'schema-snapshot-refresh', details: details.slice(0, 4000) }),
    });
  } catch (_) { /* best-effort */ }
}
...
async function buildSnapshot(): Promise<string> {
  ...
  try {
    const tablesQ = await client.queryObject<{tbl: string; col: string; dtype: string; nullable: string; dflt: string|null; is_unique: boolean; is_pk: boolean}>`
      SELECT
        c.table_name AS tbl,
    ...
    const checksQ = await client.queryObject<{tbl: string; name: string; expr: string}>`
    ...
    const fksQ = await client.queryObject<{src: string; src_col: string; tgt: string; tgt_col: string; name: string}>`
      SELECT
    ...
    const triggersQ = await client.queryObject<{tbl: string; name: string; timing: string; event: string; fn: string}>`
    ...
    const fnsQ = await client.queryObject<{name: string; args: string; kind: string}>`
    ...
    const policiesQ = await client.queryObject<{tbl: string; name: string; cmd: string; roles: string; expr: string|null; chk: string|null}>`
    ...
    const rlsTablesQ = await client.queryObject<{tbl: string; rls: boolean}>`
    ...
    const indexesQ = await client.queryObject<{tbl: string; name: string; defn: string}>`
      SELECT tablename AS tbl, indexname AS name, indexdef AS defn
      FROM pg_indexes WHERE schemaname='public'
      ORDER BY tablename, indexname;
    `;

    const cronQ = await client.queryObject<{name: string; schedule: string; cmd: string; active: boolean}>`
    ...
    const colsByTable: Record<string, typeof tablesQ.rows> = {};
    for (const r of tablesQ.rows) { (colsByTable[r.tbl] ||= []).push(r); }
    const checksByTable: Record<string, typeof checksQ.rows> = {};
    for (const r of checksQ.rows) { (checksByTable[r.tbl] ||= []).push(r); }
    const fksBySrc: Record<string, typeof fksQ.rows> = {};
    for (const r of fksQ.rows) { (fksBySrc[r.src] ||= []).push(r); }
    const triggersByTable: Record<string, typeof triggersQ.rows> = {};
    for (const r of triggersQ.rows) { (triggersByTable[r.tbl] ||= []).push(r); }
    const policiesByTable: Record<string, typeof policiesQ.rows> = {};
    for (const r of policiesQ.rows) { (policiesByTable[r.tbl] ||= []).push(r); }
    const indexesByTable: Record<string, typeof indexesQ.rows> = {};
    for (const r of indexesQ.rows) { (indexesByTable[r.tbl] ||= []).push(r); }
...
async function commitIfChanged(content: string): Promise<{committed: boolean; reason: string; commit_sha?: string; blob_sha?: string}> {
  const apiUrl = `https://api.github.com/repos/${OWNER}/${REPO}/contents/${FILE_PATH}?ref=${BRANCH}`;
  const getRes = await fetch(apiUrl, { headers: GH_HEADERS });
  ...
  if (currentContent && normalise(currentContent) === normalise(content)) {
    return { committed: false, reason: 'No structural changes since last snapshot' };
  }
  ...
  const putRes = await fetch(apiUrl, {
    ...
    body: JSON.stringify({
      ...
      sha: currentSha,
      branch: BRANCH,
      committer: { name: 'VYVE Schema Bot', email: 'team@vyvehealth.co.uk' },
    }),
  });
...
serve(async (req) => {
  ...
  try {
    ...
    return new Response(JSON.stringify({ ...result, ms, chars: md.length }), {
