...
async function getStats() {
  ...
  const [membersRes, habitsRes, sessionsRes, checkinsRes] = await Promise.all([
    ...
    fetch(`${SUPABASE_URL}/rest/v1/daily_habits?select=id&created_at=gte.${todayStr}T00:00:00`, { headers }),
    fetch(`${SUPABASE_URL}/rest/v1/session_views?select=id&created_at=gte.${todayStr}T00:00:00`, { headers }),
    fetch(`${SUPABASE_URL}/rest/v1/kahunas_checkins?select=id&week_start=gte.${weekStart}`, { headers }),
  ]);
...
async function generateBrief(stats: Record<string, number>): Promise<string> {
  const context = `
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (action === 'brief') {
      ...
      return new Response(JSON.stringify({ brief, stats }), {
        headers: { ...corsHeaders, 'Content-Type': 'application/json' }
      });
    }

    return new Response(JSON.stringify({ error: 'Unknown action. Use ?action=stats or ?action=brief' }), {
      status: 400,
      headers: { ...corsHeaders, 'Content-Type': 'application/json' }
    });
