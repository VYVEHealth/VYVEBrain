...
function isoMondayUtc(now = new Date()): string {
...
function authorise(req: Request): { ok: true } | { ok: false; status: number; body: any } {
  const auth = req.headers.get('authorization') || '';
  const bearer = auth.startsWith('Bearer ') ? auth.slice(7) : '';
  if (SERVICE_ROLE_KEY && bearer === SERVICE_ROLE_KEY) return { ok: true };
  if (LEGACY_JWT && bearer === LEGACY_JWT) return { ok: true };
  if (SHARED_SECRET && req.headers.get('x-seed-secret') === SHARED_SECRET) return { ok: true };
  return { ok: false, status: 401, body: { error: 'unauthorized' } };
}

async function listActiveMembers(): Promise<string[]> {
  ...
  return (rows as { email: string }[]).map((m) => m.email.toLowerCase().trim()).filter(Boolean);
}

async function upsertGoals(emails: string[], weekStart: string): Promise<{ written: number; failed: number; error?: string }> {
  if (emails.length === 0) return { written: 0, failed: 0 };
  ...
  if (!r.ok) {
    ...
    return { written: 0, failed: emails.length, error: `${r.status}: ${errText.slice(0, 300)}` };
  }
  ...
  return { written: emails.length, failed: 0 };
}
...
serve(async (req) => {
  ...
  try {
    ...
    return new Response(JSON.stringify({ ok: false, week_start: weekStart, error: String(e) }), {
      status: 500,
      headers: { 'Content-Type': 'application/json' },
    });
  }
