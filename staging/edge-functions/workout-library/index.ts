...
async function enforcePausedCap(admin, email) {
  const { data: paused } = await admin.from('workout_plan_cache').select('id, paused_at').eq('member_email', email).eq('is_active', false).eq('source', 'library').order('paused_at', {
    ascending: false
  });
...
Deno.serve(async (req)=>{
  ...
  if (req.method === 'POST') {
    ...
    if (body.action === 'resume' && body.plan_id) {
      ...
      }).eq('member_email', email).eq('is_active', true).eq('programme_json->>surface', resumeSurface);
    ...
    const { data: pausedRows } = await admin.from('workout_plan_cache').update({
      is_active: false,
      paused_at: new Date().toISOString()
    }).eq('member_email', email).eq('is_active', true).eq('programme_json->>surface', surface).select('id');
    ...
    const durationWeeks = programme.duration_weeks ?? ((newProgrammeJson.weekly_targets || []).length || 0);
