...
serve(async (req) => {
  ...
  try {
    ...
    if (req.method === "DELETE") {
      ...
      if (!habitType) {
        return new Response(
          JSON.stringify({ success: false, error: "habit_type parameter required for DELETE" }),
    ...
    if (req.method === "POST") {
      ...
      if (!habit_type || completed === undefined) {
        return new Response(
          JSON.stringify({ success: false, error: "habit_type and completed are required" }),
          { status: 400, headers: { ...corsHeaders, "Content-Type": "application/json" } }
        );
      }
      ...
      if (existing) {
        ...
        const { error: updateErr } = await supabase
          ...
          .update({ 
            completed, 
            notes: notes || null,
            logged_at  // Update timestamp to reflect the edit
          })
          .eq('member_email', member_email)
          .eq('activity_date', today)
          .eq('habit_type', habit_type);
