...
function gateMutation(role: string, origin: string | null): Response | null {
  if (role === 'viewer') {
    return json({ success: false, error: 'Viewer role cannot duplicate programmes' }, 403, origin);
  }
  return null;
}
...
async function handleListProgrammes(req: Request, _admin: { email: string; role: string }, body: any) {
  const origin = req.headers.get('origin');
  const includeInactive = body?.include_inactive !== false; // default true

  let q = service.from('programme_library')
    .select('id, programme_name, description, category, difficulty, equipment, days_per_week, duration_weeks, sessions_per_week, tags, is_active, sort_order, created_at')
  ...
  if (error) {
    ...
    return json({ success: false, error: 'Failed to load programmes', details: error.message }, 500, origin);
  }
  return json({ success: true, programmes: data ?? [] }, 200, origin);
}
...
async function handleGetProgramme(req: Request, _admin: { email: string; role: string }, body: any) {
  ...
  if (!isValidUuid(programme_id)) {
    return json({ success: false, error: 'programme_id must be a valid UUID' }, 400, origin);
  }

  const { data, error } = await service.from('programme_library')
    .select('id, programme_name, description, category, difficulty, equipment, days_per_week, duration_weeks, sessions_per_week, programme_json, tags, preview_sessions, is_active, sort_order, created_at')
    .eq('id', programme_id)
    .maybeSingle();
  ...
  if (!data) return json({ success: false, error: 'Programme not found' }, 404, origin);
  ...
  const shape = {
    ...
    has_warmup_cooldown: firstWeekSessions.some((s: any) => s?.warm_up || s?.cool_down),
    total_exercise_slots: weeksArr.reduce((total: number, wk: any) => {
      if (!Array.isArray(wk?.sessions)) return total;
      return total + wk.sessions.reduce((t: number, s: any) => t + (Array.isArray(s?.exercises) ? s.exercises.length : 0), 0);
    }, 0)
  };

  return json({ success: true, programme: data, shape }, 200, origin);
}

// duplicate_programme: clone an existing row as a new UUID; is_active=false, name suffixed " (copy)".
// body: { programme_id, reason? }
async function handleDuplicateProgramme(req: Request, admin: { email: string; role: string }, body: any) {
  const origin = req.headers.get('origin');
  ...
  const { data: source, error: srcErr } = await service.from('programme_library')
    .select('programme_name, description, category, difficulty, equipment, days_per_week, duration_weeks, sessions_per_week, programme_json, tags, preview_sessions, sort_order')
    .eq('id', programme_id)
    .maybeSingle();
  if (srcErr) {
    console.error('duplicate source lookup failed:', srcErr);
    return json({ success: false, error: 'Source lookup failed', details: srcErr.message }, 500, origin);
  }
  if (!source) return json({ success: false, error: 'Source programme not found' }, 404, origin);
  ...
  for (let i = 2; i <= 20; i++) {
    const { data: clash } = await service.from('programme_library')
      .select('id').eq('programme_name', newName).maybeSingle();
    if (!clash) break;
    newName = `${baseName} ${i}`;
  }
  ...
  const insertRow = {
    ...
    is_active:         false,  // per product decision — duplicates are drafts
    sort_order:        null    // don't inherit sort position
  };

  const { data: inserted, error: insErr } = await service.from('programme_library')
    .insert(insertRow)
    .select('id, programme_name, is_active, created_at')
    .maybeSingle();

  if (insErr) {
    console.error('duplicate insert failed:', insErr);
    return json({ success: false, error: 'Duplicate failed', details: insErr.message }, 500, origin);
  }
  ...
  const audit_logged = await writeAudit({
    ...
    column_name: '__row__',
    old_value: { source_id: programme_id, source_name: source.programme_name },
    new_value: { new_id: inserted?.id, new_name: newName, is_active: false },
    reason, ip, userAgent
  });
...
Deno.serve(async (req: Request) => {
  ...
  catch { body = {}; } // list_programmes works with no body
  ...
  try {
    switch (action) {
      case 'list_programmes':      return await handleListProgrammes(req, authResult, body);
      case 'get_programme':        return await handleGetProgramme(req, authResult, body);
      case 'duplicate_programme':  return await handleDuplicateProgramme(req, authResult, body);
      default:
        return json({
          success: false,
          error: `Unknown action: ${action}`,
          available_actions: ['list_programmes', 'get_programme', 'duplicate_programme']
        }, 400, origin);
    }
  } catch (e) {
