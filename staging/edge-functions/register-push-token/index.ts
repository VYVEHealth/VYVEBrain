...
serve(async (req) => {
  ...
  if (!['ios', 'android'].includes(platform)) return badRequest('platform must be ios or android', CORS);
  if (!['development', 'production'].includes(environment)) return badRequest('environment must be development or production', CORS);
  ...
  return new Response(
    JSON.stringify({ ok: true, id: rows[0]?.id, member_email: email }),
