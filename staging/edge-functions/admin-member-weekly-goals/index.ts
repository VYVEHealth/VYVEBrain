...
const TARGET_FIELDS = ['habits_target', 'workouts_target', 'cardio_target', 'sessions_target', 'checkin_target'] as const;
...
function ukTodayIsoDate(): string {
...
function validateTargetValue(field: TargetField, raw: any): { valid: true; value: number } | { valid: false; error: string } {
  const n = typeof raw === 'string' ? parseInt(raw, 10) : Number(raw);
  if (!Number.isFinite(n) || !Number.isInteger(n)) {
    return { valid: false, error: `${field} must be a whole number` };
  }
  if (n < TARGET_MIN || n > TARGET_MAX) {
    return { valid: false, error: `${field} must be between ${TARGET_MIN} and ${TARGET_MAX}` };
  }
...
async function handleGetWeeklyGoals(req: Request, _admin: { email: string; role: string }, body: any) {
  const origin = req.headers.get('origin');
  ...
  if (week_start === undefined || week_start === null || week_start === '') {
    ...
    if (!isValidIsoDate(week_start)) {
      return json({ success: false, error: 'week_start must be YYYY-MM-DD' }, 400, origin);
    }
    if (!isMonday(week_start)) {
      return json({ success: false, error: 'week_start must be an ISO Monday' }, 400, origin);
    }
    ws = week_start;
  }
  ...
  const { data, error } = await service
    .from('weekly_goals')
    .select('id, member_email, week_start, habits_target, workouts_target, cardio_target, sessions_target, checkin_target, created_at')
    .eq('member_email', email)
    .eq('week_start', ws)
    .maybeSingle();
  if (error) {
    console.error('get_weekly_goals failed:', error);
    return json({ success: false, error: 'Failed to load weekly goals', details: error.message }, 500, origin);
  }
  return json({ success: true, week_start: ws, goals: data ?? null }, 200, origin);
}

async function handleUpsertWeeklyGoals(req: Request, admin: { email: string; role: string }, body: any) {
  const origin = req.headers.get('origin');
  ...
  for (const field of TARGET_FIELDS) {
    if (!(field in body)) {
      return json({ success: false, error: `Missing required field: ${field}` }, 400, origin);
    }
    const v = validateTargetValue(field, (body as any)[field]);
  ...
  const { data: existing } = await service
    .from('weekly_goals')
    .select('id, habits_target, workouts_target, cardio_target, sessions_target, checkin_target')
    .eq('member_email', email)
    .eq('week_start', week_start)
    .maybeSingle();

  if (existing) {
    const allSame = TARGET_FIELDS.every(f => (existing as any)[f] === parsedTargets[f]);
    if (allSame) {
      return json({
        success: true,
        no_op: true,
  ...
  if (upErr) {
    ...
    return json({ success: false, error: 'Upsert failed', details: upErr.message }, 500, origin);
  }
...
Deno.serve(async (req: Request) => {
  ...
  try {
    switch (action) {
      case 'get_weekly_goals':    return await handleGetWeeklyGoals(req, authResult, body);
      case 'upsert_weekly_goals': return await handleUpsertWeeklyGoals(req, authResult, body);
      default:
