...
const EXCLUDED = ["test@test.com","maketest@vyvehealth.co.uk","team@vyvehealth.co.uk"];
...
serve(async (req) => {
  ...
  if (!BREVO) return new Response(JSON.stringify({ success:false, error:"BREVO_API_KEY not set" }), { status:500, headers:corsHeaders });

  let overrideDate: string | null = null;
  let overrideTo: string | null = null;
  let overrideCc: string | null = null;
  try {
  ...
  if (overrideDate && !/^\d{4}-\d{2}-\d{2}$/.test(overrideDate)) {
    return new Response(JSON.stringify({success:false,error:"date must be YYYY-MM-DD"}),{status:400,headers:corsHeaders});
  }
  ...
  const dfm = (dow + 6) % 7;
  const wMon = new Date(yest); wMon.setUTCDate(yest.getUTCDate() - dfm);
  const wSun = new Date(wMon); wSun.setUTCDate(wMon.getUTCDate() + 6);
  const dateLabel = yest.toLocaleDateString("en-GB", { weekday:"long", day:"numeric", month:"long", year:"numeric", timeZone:"UTC" });
  ...
  try {
    const [h,w,c,s,ci,newM,totalM,reeng,aiCheckins,aiOnboarding,hDetail,wDetail,cDetail,sDetail] = await Promise.all([
      ...
      sb.from("members").select("email",{count:"exact",head:true}).eq("subscription_status","active").not("email","in",`(${EXCLUDED.map(e=>`\"${e}\"`).join(",")})`),
    ...
    const cap = (rows:{member_email:string}[], n:number) => { const m:Record<string,number>={}; for(const r of rows) m[r.member_email]=(m[r.member_email]||0)+1; return Object.values(m).reduce((s,c)=>s+Math.min(c,n),0); };
    ...
    const checkinCount = new Set((ci.data||[]).map((r:any)=>r.member_email)).size;
    const totalActs    = habitCount+workoutCount+cardioCount+sessionCount+checkinCount;
    const uniqueActive = new Set([...h.data||[],...w.data||[],...c.data||[],...s.data||[]].map((r:any)=>r.member_email)).size;
    const totalMembers = totalM.count || 0;
    const newMembers   = (newM.data||[]) as {email:string;first_name:string|null;last_name:string|null;company:string|null}[];
    const reengaged    = (reeng.data||[]) as {member_email:string;stream:string;email_key:string}[];
    const checkinAI    = (aiCheckins.data||[]) as {member_email:string;score_wellbeing:number;flow_type:string;ai_persona:string;ai_recommendation:string;logged_at:string}[];
    const onboardAI    = (aiOnboarding.data||[]) as {member_email:string;persona:string;recommendation:string;created_at:string}[];

    const allEmails = [...new Set([
      ...(hDetail.data||[]).map((r:any)=>r.member_email),
      ...(wDetail.data||[]).map((r:any)=>r.member_email),
      ...(cDetail.data||[]).map((r:any)=>r.member_email),
      ...(sDetail.data||[]).map((r:any)=>r.member_email),
      ...checkinAI.map(r=>r.member_email),
      ...onboardAI.map(r=>r.member_email),
      ...reengaged.map(r=>r.member_email),
    ])];
    const { data: nameRows } = allEmails.length ? await sb.from("members").select("email,first_name,last_name,company").in("email",allEmails) : {data:[]};
    ...
    type ActRow = {email:string;type:string;detail:string;time:string;company:string};
    const actRows:ActRow[] = [];
    for(const r of hDetail.data||[]) actRows.push({email:r.member_email,type:'Habit',detail:'Daily habit logged',time:r.logged_at,company:companyMap[r.member_email]||''});
    for(const r of wDetail.data||[]) actRows.push({email:r.member_email,type:'Workout',detail:r.workout_name||(r.plan_name||'Workout'),time:r.logged_at,company:companyMap[r.member_email]||''});
    for(const r of cDetail.data||[]) {
      const parts = [r.activity_type||'Cardio'];
      if(r.duration_minutes) parts.push(`${r.duration_minutes} min`);
      if(r.distance_km) parts.push(`${r.distance_km} km`);
      actRows.push({email:r.member_email,type:'Cardio',detail:parts.join(' · '),time:r.logged_at,company:companyMap[r.member_email]||''});
    }
    for(const r of sDetail.data||[]) actRows.push({email:r.member_email,type:'Session',detail:r.session_title||'Live session viewed',time:r.logged_at,company:companyMap[r.member_email]||''});
    actRows.sort((a,b)=>a.time.localeCompare(b.time));

    const html = buildDaily(dateLabel, { habitCount, workoutCount, cardioCount, sessionCount, checkinCount, totalActs, uniqueActive, totalMembers }, newMembers, reengaged, nameMap, checkinAI, onboardAI, actRows);
    ...
    if (overrideCc) brevoBody.cc = [{ email: overrideCc }];
    ...
    return new Response(JSON.stringify({success:true,date:dateLabel,backfill:!!overrideDate,to:toEmail,cc:overrideCc,totalActs,newMembers:newMembers.length,activityRows:actRows.length}),{status:200,headers:{...corsHeaders,"Content-Type":"application/json"}});
  } catch(err:any) {
...
const TYPE_COL:Record<string,string>={Habit:'#1B7878',Workout:'#2A6BB0',Cardio:'#C65D00',Session:'#6B4EA8'};

function statRow(items:{label:string;val:number|string;sub:string}[]):string {
...
function aiBlock(name:string,subtitle:string,scoreHtml:string,persona:string,text:string):string {
...
function buildDaily(date:string,a:{habitCount:number;workoutCount:number;cardioCount:number;sessionCount:number;checkinCount:number;totalActs:number;uniqueActive:number;totalMembers:number},newM:{email:string;first_name:string|null;last_name:string|null;company:string|null}[],reeng:{member_email:string;stream:string;email_key:string}[],nameMap:Record<string,string>,checkinAI:{member_email:string;score_wellbeing:number;flow_type:string;ai_persona:string;ai_recommendation:string;logged_at:string}[],onboardAI:{member_email:string;persona:string;recommendation:string;created_at:string}[],actRows:ActRow[]):string{
  const streamCol:Record<string,string>={A:"#C65D00",B:"#1B7878",C1:"#2A9D8F",C2:"#6B9E9E",C3:"#3D5A5A"};
  const stepLbl:Record<string,string>={A_48h:"48h",A_96h:"96h",A_7d:"7d",A_14d:"14d",B_3d:"3d",B_7d:"7d",B_14d:"14d",B_30d:"30d",C1_7d:"7d",C1_14d:"14d",C1_30d:"30d",C2_7d:"7d",C2_14d:"14d",C2_30d:"30d",C3_7d:"7d",C3_14d:"14d",C3_30d:"30d"};
  const streamDesc:Record<string,string>={A:"App not opened",B:"No activity",C1:"App dormant",C2:"Portal dormant",C3:"Both dormant"};
  ...
  const actDetail=actRows.length===0
    ?`<tr><td colspan="4" style="padding:14px 0;text-align:center;color:#7A9A9A;font-size:13px;">No activities logged yesterday.</td></tr>`
    :actRows.map(r=>{
        const col=TYPE_COL[r.type]||'#1B7878';
        const t=new Date(r.time).toLocaleTimeString('en-GB',{hour:'2-digit',minute:'2-digit',timeZone:'UTC'})+' UTC';
  ...
  const checkinBlocks=checkinAI.map(r=>{const name=(nameMap[r.member_email]||r.member_email).trim();const time=new Date(r.logged_at).toLocaleTimeString('en-GB',{hour:'2-digit',minute:'2-digit',timeZone:'UTC'})+' UTC';return aiBlock(name,`Weekly check-in · ${r.flow_type} flow · ${time}`,scoreChip(r.score_wellbeing),r.ai_persona||'',r.ai_recommendation||'');}).join('');
  const onboardBlocks=onboardAI.map(r=>{const name=(nameMap[r.member_email]||r.member_email).trim();const time=new Date(r.created_at).toLocaleTimeString('en-GB',{hour:'2-digit',minute:'2-digit',timeZone:'UTC'})+' UTC';return aiBlock(name,`Onboarding recommendations · ${time}`,'',r.persona||'',r.recommendation||'');}).join('');
  ...
  return W([HDR("Daily Report",date),SEC("Membership"),statRow([{label:"Total members",val:a.totalMembers,sub:"active subscriptions"},{label:"New yesterday",val:newM.length,sub:"joined"},{label:"Active yesterday",val:a.uniqueActive,sub:"logged activity"}]),`<tr><td style="padding:4px 28px 12px;"><table width="100%" style="border:1px solid #C8E4E4;border-radius:8px;overflow:hidden;"><thead><tr style="background:#EAF5F5;"><th style="padding:8px 10px;text-align:left;font-size:10px;color:#1B7878;font-weight:600;text-transform:uppercase;">Name</th><th style="padding:8px 10px;text-align:left;font-size:10px;color:#1B7878;font-weight:600;text-transform:uppercase;">Email</th><th style="padding:8px 10px;text-align:left;font-size:10px;color:#1B7878;font-weight:600;text-transform:uppercase;">Company</th></tr></thead><tbody>${newRows}</tbody></table></td></tr>`,DIV,SEC("Activities yesterday"),statRow([{label:"Habits",val:a.habitCount,sub:"max 1/member"},{label:"Workouts",val:a.workoutCount,sub:"max 2/member"},{label:"Cardio",val:a.cardioCount,sub:"max 2/member"},{label:"Sessions",val:a.sessionCount,sub:"max 2/member"},{label:"Check-ins",val:a.checkinCount,sub:"1/week"}]),`<tr><td style="padding:4px 28px 12px;"><table width="100%" style="background:#0D2B2B;border-radius:8px;"><tr><td style="text-align:center;padding:14px;"><div style="font-size:28px;font-weight:700;color:#7AB8B8;font-family:Georgia,serif;">${a.totalActs}</div><div style="font-size:10px;color:#4A8A8A;text-transform:uppercase;letter-spacing:1px;margin-top:2px;">Total activities</div></td></tr></table></td></tr>`,`<tr><td style="padding:4px 28px 12px;"><table width="100%" style="border:1px solid #C8E4E4;border-radius:8px;overflow:hidden;"><thead><tr style="background:#EAF5F5;"><th style="padding:8px 10px;text-align:left;font-size:10px;color:#1B7878;font-weight:600;text-transform:uppercase;">Member</th><th style="padding:8px 10px;text-align:left;font-size:10px;color:#1B7878;font-weight:600;text-transform:uppercase;">Type</th><th style="padding:8px 10px;text-align:left;font-size:10px;color:#1B7878;font-weight:600;text-transform:uppercase;">Activity</th><th style="padding:8px 10px;text-align:left;font-size:10px;color:#1B7878;font-weight:600;text-transform:uppercase;">Time</th></tr></thead><tbody>${actDetail}</tbody></table></td></tr>`,DIV,SEC("AI Responses yesterday"),aiSection,DIV,SEC("Re-engagement"),`<tr><td style="padding:4px 28px 12px;"><p style="font-size:12px;color:#7A9A9A;margin:0 0 8px;">${reeng.length} email${reeng.length!==1?'s':''} sent yesterday</p><table width="100%" style="border:1px solid #C8E4E4;border-radius:8px;overflow:hidden;"><thead><tr style="background:#EAF5F5;"><th style="padding:8px 10px;text-align:left;font-size:10px;color:#1B7878;font-weight:600;text-transform:uppercase;">Name</th><th style="padding:8px 10px;text-align:left;font-size:10px;color:#1B7878;font-weight:600;text-transform:uppercase;">Stream</th><th style="padding:8px 10px;text-align:left;font-size:10px;color:#1B7878;font-weight:600;text-transform:uppercase;">Reason</th><th style="padding:8px 10px;text-align:left;font-size:10px;color:#1B7878;font-weight:600;text-transform:uppercase;">Step</th></tr></thead><tbody>${reRows}</tbody></table></td></tr>`,FTR].join(''));
}
