...
function buildCancelEmailHTML(firstName: string): string {
...
serve(async (req: Request) => {
  ...
  if (tokenRaw) {
    ...
    const { data } = await supabaseSvc
      ...
      .select("id, member_email, scheduled_for, cancelled_at, executed_at")
      .eq("cancel_token", tokenRaw)
      .maybeSingle();
    row = data;
  ...
  if (updateErr) {
    ...
    return new Response(JSON.stringify({ error: "Failed to cancel erasure" }), {
  ...
  try {
    ...
    console.error("[gdpr-erase-cancel v2] Brevo send failed:", (e as Error).message);
    try {
      await supabaseSvc.from("platform_alerts").insert({
        ...
        details: `Cancellation succeeded for ${row.id} but confirmation email failed: ${(e as Error).message}`,
      });
    } catch (e2) {
