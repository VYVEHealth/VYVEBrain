...
const VALID_TABLES = new Set([
  'cc_clients','cc_leads','cc_investors','cc_partners','cc_tasks',
  'cc_decisions','cc_okrs','cc_finance','cc_revenue','cc_grants',
  'cc_posts','cc_invoices','cc_sessions','cc_intel','cc_knowledge',
  'cc_documents','cc_swot','cc_episodes'
]);
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (email !== 'team@vyvehealth.co.uk') return json({ error: 'Unauthorised' }, 403);
    ...
    if (table === 'signed-url' && id) {
      ...
      if (urlError) return json({ error: urlError.message }, 500);
      return json({ url: urlData.signedUrl, name: doc.name });
    }

    // ── Table validation ─────────────────────────────────────────────────────
    if (!table || !VALID_TABLES.has(table)) {
      return json({ error: `Invalid table: ${table}. rawPath=${rawPath}, subPath=${subPath}, segments=${JSON.stringify(segments)}` }, 400);
    }
    ...
    // ── PATCH ────────────────────────────────────────────────────────────────
    if (req.method === 'PATCH') {
      if (!id) return json({ error: 'ID required for PATCH' }, 400);
      const body = await req.json();
    ...
    if (req.method === 'DELETE') {
      ...
      if (table === 'cc_documents') {
        ...
        if (doc?.storage_path) {
          await sb.storage.from('cc-documents').remove([doc.storage_path]);
        }
      }
