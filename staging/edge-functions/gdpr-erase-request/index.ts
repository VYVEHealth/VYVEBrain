...
async function getAuthEmail(req: Request, supabaseUrl: string, anonKey: string): Promise<string | null> {
...
async function isAdmin(supabaseSvc: any, email: string): Promise<boolean> {
...
const callout = (t: string) => `<div style="background:#FFF8E5;border-left:4px solid #C9A84C;padding:16px 20px;border-radius:6px;margin:20px 0;"><p style="margin:0;font-size:14px;color:#5A4A1A;line-height:1.6;">${t}</p></div>`;

function buildRequestEmailHTML(firstName: string, cancelUrl: string, scheduledFor: string, kind: "member_self" | "admin", requestedBy: string): string {
  const name = firstName || "there";
  const scheduledDate = new Date(scheduledFor).toLocaleDateString("en-GB", {
    weekday: "long", year: "numeric", month: "long", day: "numeric",
  });
  const adminBlurb = kind === "admin"
    ? pp(`This deletion was scheduled by a VYVE administrator (<strong>${requestedBy}</strong>) on your behalf, in line with your right of erasure. If this wasn't requested by you, click the cancel link below immediately.`)
    : pp("This deletion was requested from inside your VYVE account.");
...
async function sendBrevo(brevoKey: string, toEmail: string, toName: string, html: string, subject: string): Promise<string | null> {
...
serve(async (req: Request) => {
  ...
  let kind: "member_self" | "admin";
  let subject: string;

  if (adminRequest) {
    const requesterIsAdmin = await isAdmin(supabaseSvc, requesterEmail);
    if (!requesterIsAdmin) {
      return new Response(JSON.stringify({ error: "Admin access required for cross-member erasure" }), {
  ...
  const { data: member } = await supabaseSvc
    ...
    .maybeSingle();
  if (!member) {
    return new Response(JSON.stringify({ error: "Subject email not found" }), {
      status: 404, headers: { ...cors, "Content-Type": "application/json" },
    });
  }

  // Already-pending check (also enforced by DB partial unique index, but we want a clean 409)
  const { data: pending } = await supabaseSvc
  ...
  const { data: row, error: insertErr } = await supabaseSvc
    .from("gdpr_erasure_requests")
    .insert({
      member_email: subject,
    ...
    .select("id, member_email, request_kind, requested_at, scheduled_for, cancel_token")
    .single();
  ...
  if (insertErr || !row) {
    ...
    return new Response(JSON.stringify({ error: "Failed to schedule erasure" }), {
  ...
  try {
    ...
    console.error("[gdpr-erase-request] Brevo send failed:", (e as Error).message);
    try {
      await supabaseSvc.from("platform_alerts").insert({
        ...
        member_email: subject,
        details: `Erasure ${row.id} scheduled but confirmation email failed: ${(e as Error).message}`,
      });
    } catch (e2) {
      console.error("[platform_alerts insert]", (e2 as Error).message);
    }
  }
  ...
  return new Response(JSON.stringify({
    ...
    email_sent: !!messageId,
    message: kind === "admin"
      ? `Erasure scheduled for ${subject}. Confirmation email sent with cancel link.`
      : "We've scheduled your account for deletion. Check your email for confirmation and a cancel link.",
  }), {
    status: 202,
