...
function humanize(raw: string, kind: "workout" | "cardio"): string {
  const norm = String(raw || "").toLowerCase().replace(/[^a-z0-9]/g, "");
  const map: Record<string, string> = {
    running: "Run", walking: "Walk", cycling: "Ride", hiking: "Hike",
    rowing: "Row", swimming: "Swim",
    strengthtraining: "Strength", functionalstrengthtraining: "Strength",
    traditionalstrengthtraining: "Strength", coretraining: "Core",
    pilates: "Pilates", yoga: "Yoga",
    hiit: "HIIT", highintensityintervaltraining: "HIIT",
    elliptical: "Elliptical", stairclimbing: "Stair climb",
  };
...
Deno.serve(async (req) => {
  ...
  if (isNaN(before.getTime())) return json({ error: "invalid_before" }, 400);
  ...
  try {
    ...
    const [wRes, cRes] = await Promise.all([
      svc.from("workouts")
        .select("id, activity_date, workout_name, plan_name, duration_minutes, logged_at")
        .eq("member_email", memberEmail)
        .lt("logged_at", before.toISOString())
        .order("logged_at", { ascending: false })
        .limit(fetchLimit),
      svc.from("cardio")
        .select("id, activity_date, cardio_type, duration_minutes, distance_km, logged_at")
        .eq("member_email", memberEmail)
        .lt("logged_at", before.toISOString())
        .order("logged_at", { ascending: false })
        .limit(fetchLimit),
    ...
    const wRows = (wRes.data || []).map((r: any) => ({ ...r, _src: "workouts" }));
    const cRows = (cRes.data || []).map((r: any) => ({ ...r, _src: "cardio" }));
    const merged = [...wRows, ...cRows]
      .filter((r: any) => r.logged_at)
      .sort((a: any, b: any) => new Date(b.logged_at).getTime() - new Date(a.logged_at).getTime())
      .slice(0, limit);

    const promotedIds = merged.map((r: any) => r.id);
    const samplesByPromotedId = new Map<string, any>();
    if (promotedIds.length > 0) {
      const { data: samplesData } = await svc
        .from("member_health_samples")
        .select("id, promoted_to, promoted_id, app_source, native_uuid, workout_type, start_at, end_at")
        .eq("member_email", memberEmail)
        .in("promoted_id", promotedIds);
      for (const s of samplesData || []) {
        if (s.promoted_id) samplesByPromotedId.set(s.promoted_id, s);
      }
    }

    const hrCandidates = merged
      .map((r: any, idx: number) => ({ idx, sample: samplesByPromotedId.get(r.id) }))
      .filter((c) => c.sample && c.sample.start_at && c.sample.end_at);

    const hrByRowIdx = new Map<number, { avg: number; max: number }>();
    if (hrCandidates.length > 0) {
      ...
      if (isFinite(minStart) && isFinite(maxEnd)) {
        const { data: hrRows } = await svc
          .from("member_health_samples")
          .select("start_at, end_at, value")
          .eq("member_email", memberEmail)
          .eq("sample_type", "heart_rate")
          .gte("start_at", new Date(minStart).toISOString())
          .lte("end_at", new Date(maxEnd).toISOString())
          .limit(5000);
        ...
        for (const c of hrCandidates) {
          ...
          const matches = hr.filter((h: any) => {
            const hs = new Date(h.start_at).getTime();
            return hs >= wStart && hs <= wEnd;
          });
          if (matches.length >= 3) {
            const values = matches.map((h: any) => Number(h.value || 0)).filter((v) => v > 0);
            if (values.length === 0) continue;
            const avg = values.reduce((a, b) => a + b, 0) / values.length;
            const max = Math.max(...values);
            hrByRowIdx.set(c.idx, { avg: Math.round(avg), max: Math.round(max) });
          }
        }
      }
    }

    const rows = merged.map((r: any, idx: number) => {
      ...
      return {
        ...
        pace_per_km: (r._src === "cardio" && r.distance_km && r.duration_minutes)
          ? Math.round((r.duration_minutes / r.distance_km) * 60)
          : null,
    ...
    return json({ ok: true, rows, next_before: nextBefore });
  } catch (err) {
    console.error("get-activity-feed error", err);
    return json({ error: "internal_error", message: String((err as any)?.message || err) }, 500);
  }
});
