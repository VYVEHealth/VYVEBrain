...
serve(async (req)=>{
  ...
  try {
    ...
    const [member, homeStateRpc, weeklyGoalsRow, habitsRecent, workoutsRecent, cardioRecent, sessionsRecent, replaysRecent, certificates, healthConnections, memberHabits, dailyToday, sleepLastNight, workoutsToday, cardioToday, habitsThisWeek, mindActivities, movementActivities, connectCheckins, certBuckets, achievementsPayload] = await Promise.all([
      ...
      rpc('get_certificate_buckets_for', {
        p_email: user.email
    ...
    if (!member) return new Response(JSON.stringify({
    ...
    const bucketsPayload = certBuckets ? {
      habits: certBuckets.habits || 0,
      mind: certBuckets.mind || 0,
      body: certBuckets.body || 0,
      connect: certBuckets.connect || 0,
      checkins: certBuckets.checkins || 0
    } : null;
    return new Response(JSON.stringify({
      member: {
        ...
        join_date: joinDate,
        tour_completed_at: member.tour_completed_at ?? null
      },
      engagement: {
