...
const ASLEEP_STATES = new Set(['asleep','light','rem','deep']);
...
function ukToday() { return ukLocalDateISO(); }
...
function evaluateHealthRule(rule, snap) {
  if (!rule) return { satisfied: null, progress: null };
  if (!snap.hasHealthkitConnection) return { satisfied: null, progress: null };
  try {
    if (rule.source === 'daily') {
      const col = dailyMetricColumn(rule.metric);
      if (!col) return { satisfied: null, progress: null };
      const row = snap.dailyByType.get(col);
      if (!row) return { satisfied: null, progress: null };
      ...
      return { satisfied: applyOp(rule.op, value, target), progress: { value: Number(value.toFixed(2)), target, unit: dailyUnitFor(rule.metric, unitIn) } };
    }
    if (rule.source === 'samples_sleep') {
      if (snap.sleepLastNightAsleepMin === null) return { satisfied: null, progress: null };
      const value = snap.sleepLastNightAsleepMin;
      const target = rule.value ?? 0;
      return { satisfied: applyOp(rule.op, value, target), progress: { value: Math.round(value), target, unit: 'minutes' } };
    }
    if (rule.source === 'activity_tables') {
      if (rule.metric === 'workout_any' && rule.agg === 'exists') {
        ...
        return { satisfied: count > 0, progress: { value: count, target: 1, unit: 'workouts' } };
      }
      if (rule.metric === 'cardio_duration_minutes' && rule.agg === 'sum') {
        const value = snap.cardioTodayMinutes;
        const target = rule.value ?? 0;
        return { satisfied: applyOp(rule.op, value, target), progress: { value, target, unit: 'minutes' } };
      }
      return { satisfied: null, progress: null };
    }
  } catch (_) { return { satisfied: null, progress: null }; }
  return { satisfied: null, progress: null };
}
serve(async (req)=>{
  ...
  try {
    const supabase = createClient(SUPABASE_URL, SUPABASE_ANON, { global: { headers: { Authorization: req.headers.get('Authorization') ?? '' } } });
    const supabaseSr = createClient(SUPABASE_URL, SUPABASE_KEY, { auth: { persistSession: false } });
    ...
    const [member, homeStateRpc, weeklyGoalsRow, habitsRecent, workoutsRecent, cardioRecent, sessionsRecent, replaysRecent, certificates, healthConnections, memberHabits, dailyToday, sleepLastNight, workoutsToday, cardioToday, habitsThisWeek, mindActivities, movementActivities, connectCheckins, certBuckets, achievementsPayload] = await Promise.all([
      q('members', `email=eq.${enc}&select=*`).then((r)=>r[0]),
      rpc('member_home_state_get_fresh', { p_email: user.email }),
      ...
      rpc('get_certificate_buckets_for', { p_email: user.email }).then((r) => Array.isArray(r) ? r[0] : r),
      getMemberAchievementsPayload(supabaseSr, user.email.toLowerCase(), { inflightLimit: 3, recentLimit: 8 }).catch((e)=>{ console.warn('[member-dashboard] achievements payload err:', e.message); return { unseen: [], inflight: [], recent: [], earned_count: 0, hk_connected: false }; })
    ]);
    ...
    for (const row of dailyToday || [])dailyByType.set(row.sample_type, { value: Number(row.value), unit: row.unit || '' });
    let sleepLastNightAsleepMin = null;
    const sleepRows = sleepLastNight || [];
    if (sleepRows.length > 0) {
      let total = 0;
      for (const s of sleepRows){ const sstate = String(s.metadata?.sleep_state ?? '').toLowerCase(); if (!ASLEEP_STATES.has(sstate)) continue; total += Number(s.value || 0); }
    ...
    const snap = { todayLocal, dailyByType, sleepLastNightAsleepMin, workoutsTodayCount: workoutsTodayArr.length, cardioTodayCount: cardioTodayArr.length, cardioTodayMinutes: cardioTodayArr.reduce((s, r)=>s + Number(r.duration_minutes || 0), 0), hasHealthkitConnection };
    const habits = (memberHabits || []).filter((mh)=>mh.habit_library).map((mh)=>{
      ...
      return { habit_id: lib.id, habit_pot: lib.habit_pot, habit_title: lib.habit_title, habit_description: lib.habit_description, habit_prompt: lib.habit_prompt, difficulty: lib.difficulty, has_rule: !!rule, health_auto_satisfied: evaluation.satisfied, health_progress: evaluation.progress };
    });
    const sessionsAll = [...sessionsRecent.map((s)=>s.activity_date),...replaysRecent.map((r)=>r.activity_date)];
    ...
    const goalsPayload = { week_start: currentWeekStart, targets: { habits_target: Number(tgt.habits_target ?? 3), exercise_target: Number(tgt.exercise_target ?? 3), sessions_target: Number(tgt.sessions_target ?? 2), checkin_target: Number(tgt.checkin_target ?? 1) }, progress: { habits: habitDaysThisWeek, exercise: Number(state.workouts_this_week ?? 0) + Number(state.cardio_this_week ?? 0), sessions: Number(state.sessions_this_week ?? 0), checkin: Number(state.checkins_this_week ?? 0) } };
    const bucketsPayload = certBuckets ? { habits: certBuckets.habits||0, mind: certBuckets.mind||0, body: certBuckets.body||0, connect: certBuckets.connect||0, checkins: certBuckets.checkins||0 } : null;
    return new Response(JSON.stringify({
      member: { first_name: member.first_name, last_name: member.last_name, email: member.email, persona: member.persona, onboarding_complete: member.onboarding_complete, subscription_status: member.subscription_status, join_date: joinDate, tour_completed_at: member.tour_completed_at ?? null },
      engagement: { score: state.engagement_score ?? 50, components: { recency: Number(state.engagement_recency ?? 0), consistency: Number(state.engagement_consistency ?? 0), variety: Number(state.engagement_variety ?? 0), wellbeing: Number(state.engagement_wellbeing ?? 0) }, streak: overallStreak, streak_by_type: { habits: habitStreak, workouts: workoutStreak, cardio: cardioStreak, sessions: sessionStreak, checkins: checkinStreak }, active_days_30: state.active_days_30 ?? 0 },
      progress: { habits: { count: habitsTotal, target: 30, best_streak: habitStreak.best }, workouts: { count: workoutsTotal, target: 30, best_streak: workoutStreak.best }, cardio: { count: cardioTotal, target: 30, best_streak: cardioStreak.best }, sessions: { count: sessionsTotal, target: 30, best_streak: sessionStreak.best }, checkins: { count: checkinsTotal, target: 30, best_streak: checkinStreak.best } },
      recent: { habits: state.recent_habits_30d ?? 0, workouts: state.recent_workouts_30d ?? 0, cardio: state.recent_cardio_30d ?? 0, sessions: state.recent_sessions_30d ?? 0, checkins: checkinsTotal },
      activity_log: activityLog,
      _buckets: bucketsPayload,
      ...
      wellbeing: { current_score: state.wellbeing_latest_score ?? null, last_checkin: state.wellbeing_latest_iso_week ?? null },
      charity_total: charityTotal,
      certificates: certificates,
      habits: habits,
      health_feature_allowed: true,
      health_connection_state: hasHealthkitConnection ? 'connected' : 'none',
      health_connections: (healthConnections || []).map((c)=>({ platform: c.platform, granted_scopes: c.granted_scopes || [], connected_at: c.connected_at, last_sync_at: c.last_sync_at, last_sync_status: c.last_sync_status, total_synced: c.total_synced || 0, is_revoked: !!c.revoked_at })),
