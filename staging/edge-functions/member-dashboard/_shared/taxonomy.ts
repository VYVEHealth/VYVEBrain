...
export const STRENGTH_CANON = new Set(["functionalstrengthtraining","traditionalstrengthtraining","strengthtraining","coretraining","pilates","crosstraining","calisthenics"]);
export const CARDIO_CANON = new Set(["running","cycling","walking","hiking","rowing","swimming","highintensityintervaltraining","hiit","elliptical","stairclimbing","mixedcardio"]);
export const IGNORED_CANON = new Set(["mindandbody","dance","golf","baseball","basketball","americanfootball","soccer","tennis","racquetball","tabletennis","badminton","squash","cricket","bowling","climbing","surfing","waterfitness","waterpolo","watersports","gymnastics","martialarts","trackandfield","volleyball","skating","snowboarding","skiing","downhillskiing","crosscountryskiing","hockey","handball","rugby","archery","equestrian","fishing","hunting","lacrosse","mountainbiking","paddlesports","play","preparationandrecovery","sailing","wrestling","other"]);
...
export function classifyWorkout(workoutTypeRaw, durationMin) {
  ...
  if (canon === YOGA_CANON) { return durationMin >= YOGA_STRENGTH_MIN_MINUTES ? "strength" : "ignored"; }
  if (CARDIO_CANON.has(canon)) return "cardio";
  return "unknown";
}
export const ALLOWED_DAILY_TYPES = new Set(["steps","distance","active_energy"]);
export function applyOp(op, value, target) {
  switch(op){ case "gte": return value >= target; case "lte": return value <= target; case "eq": return value === target; case "exists": return value > 0; default: return false; }
}
function isBST(d) { const m = d.getUTCMonth(); return m >= 3 && m <= 9; }
