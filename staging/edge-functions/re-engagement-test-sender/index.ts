...
// HTML helpers
const wrap = (body: string, stream: string, key: string) =>
...
const btn = (label: string, href: string) =>
  `<div style="text-align:center;margin:28px 0;"><a href="${href}" style="background:#0D2B2B;color:#fff;text-decoration:none;padding:14px 32px;border-radius:8px;font-size:15px;font-weight:600;display:inline-block;">${label} &rarr;</a></div>`;
...
async function ai(instruction: string, maxTokens = 90): Promise<string> {
  ...
  try {
    const res = await fetch("https://api.anthropic.com/v1/messages", {
      ...
      body: JSON.stringify({model:"claude-sonnet-4-5",max_tokens:maxTokens,system:RIVER_PROMPT,
...
async function sendEmail(subject: string, html: string, stream: string, key: string): Promise<void> {
  ...
  const r = await fetch("https://api.brevo.com/v3/smtp/email", {
    ...
    body: JSON.stringify({
      ...
      htmlContent: html,
      tags: ["re-engagement-test", stream, key],
    }),
  });
  if (!r.ok) {
...
async function sendAll() {
  type E = {stream:string;key:string;subject:string;build:()=>Promise<string>};
  const emails: E[] = [
    // STREAM A
    { stream:"A", key:"A_48h", subject:"Your VYVE Health app is ready, Dean",
      build: async () => {
    ...
    { stream:"A", key:"A_96h", subject:"Habits. Workouts. Cardio. It\u2019s all in there, Dean.",
      build: async () => {
    ...
    { stream:"A", key:"A_7d", subject:"A week in \u2014 your app is still waiting, Dean",
      build: async () => {
    ...
    { stream:"A", key:"A_14d", subject:"Last nudge from us, Dean",
      build: async () =>
        `${h2("One last nudge, Dean.")}${pp("This is our last reminder. We don\u2019t want to keep landing in your inbox.")}${pp("Your habits, workouts and cardio are set up and ready. When you\u2019re ready, they\u2019ll be there.")}${pp(CHARITY_LINE)}${appBtn}`
    },
    // STREAM B
    { stream:"B", key:"B_3d", subject:"Dean, your programme is live \u2014 let\u2019s use it",
      build: async () => {
    ...
    { stream:"B", key:"B_7d", subject:"A week in \u2014 nothing logged yet, Dean",
      build: async () => {
    ...
    { stream:"B", key:"B_14d", subject:"Dean, is VYVE actually working for you?",
      build: async () => {
    ...
    { stream:"B", key:"B_30d", subject:"One last thing before we step back, Dean",
      build: async () => {
    ...
    // STREAM C1
    { stream:"C1", key:"C1_7d", subject:"You\u2019re showing up \u2014 are you tracking it, Dean?",
      build: async () => {
    ...
    { stream:"C1", key:"C1_14d", subject:"The habits make the sessions count, Dean",
      build: async () => {
    ...
    { stream:"C1", key:"C1_30d", subject:"A month of sessions, Dean \u2014 now let\u2019s add the rest",
      build: async () => {
    ...
    // STREAM C2
    { stream:"C2", key:"C2_7d", subject:"The app\u2019s only half the picture, Dean",
      build: async () => {
    ...
    { stream:"C2", key:"C2_14d", subject:"Your AI coach has been waiting, Dean",
      build: async () => {
    ...
    { stream:"C2", key:"C2_30d", subject:"A month of habits, Dean \u2014 here\u2019s what you\u2019re missing",
      build: async () => {
    ...
    // STREAM C3
    { stream:"C3", key:"C3_7d", subject:"We\u2019ve missed you, Dean",
      build: async () => {
    ...
    { stream:"C3", key:"C3_14d", subject:"Still here, Dean",
      build: async () => {
    ...
    { stream:"C3", key:"C3_30d", subject:"One last check-in, Dean",
      build: async () => {
...
serve(async (req) => {
  ...
  return new Response(
    JSON.stringify({success:true,message:"Sending 16 emails in background. Check team@vyvehealth.co.uk in ~90 seconds.",emails:16}),
