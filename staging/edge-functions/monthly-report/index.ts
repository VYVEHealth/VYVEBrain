...
serve(async (req) => {
  ...
  if (!BREVO) return new Response(JSON.stringify({ success:false, error:"BREVO_API_KEY not set" }), { status:500 });

  let overrideMonthStart: string | null = null;
  ...
  const pmEnd  = mStart;
  const monthLabel = mStart.toLocaleDateString("en-GB",{month:"long",year:"numeric",timeZone:"UTC"});
  ...
  try {
    const [
      ...
      newThisM, newPrevM,
      appOpened,
      habM, wrkM, carM, sesM, ciM,
      habPM, wrkPM, carPM, sesPM,
      reengM, reengPM,
      certsTotal, charityTotal, certsM,
      byCompany,
      streamDist,
      avgScoresM, avgScoresPM,
    ] = await Promise.all([
      sb.from("members").select("email",{count:"exact",head:true}).eq("subscription_status","active").not("email","in",exFilter),
      sb.from("members").select("email",{count:"exact",head:true}).lt("created_at",mStart.toISOString()).eq("subscription_status","active").not("email","in",exFilter),
      ...
      sb.from("members").select("email",{count:"exact",head:true}).gte("created_at",pmStart.toISOString()).lt("created_at",pmEnd.toISOString()).not("email","in",exFilter),
      sb.from("members").select("email",{count:"exact",head:true}).eq("kahunas_qa_complete",true).eq("subscription_status","active").not("email","in",exFilter),
      ...
      sb.from("certificates").select("id",{count:"exact",head:true}).eq("charity_moment_triggered",true),
      sb.from("certificates").select("id",{count:"exact",head:true}).gte("earned_at",mStart.toISOString()).lt("earned_at",mEnd.toISOString()),
    ...
    const uniq=(rows:{member_email:string}[])=>new Set(rows.map(r=>r.member_email)).size;
    const avg=(rows:{wellbeing_score?:number;engagement_score?:number}[],key:"wellbeing_score"|"engagement_score")=>rows.length?+(rows.reduce((s,r)=>s+(r[key]||0),0)/rows.length).toFixed(1):null;
    ...
    const activeMCount  = new Set([...habM.data||[],...wrkM.data||[],...sesM.data||[],...ciM.data||[]].map((r:any)=>r.member_email)).size;
    const activePMCount = new Set([...habPM.data||[],...wrkPM.data||[],...sesPM.data||[]].map((r:any)=>r.member_email)).size;
    ...
    const newMList = (newThisM.data||[]) as {email:string;first_name:string|null;company:string|null}[];
    const newMRows = newMList.length===0
      ? `<tr><td colspan="2" style="padding:10px;text-align:center;color:#7A9A9A;font-size:13px;">No new members this month.</td></tr>`
      : newMList.map(m=>`<tr style="border-bottom:1px solid #EAF5F5;"><td style="padding:7px 10px;font-size:13px;color:#0D2B2B;">${m.first_name||m.email}</td><td style="padding:7px 10px;font-size:12px;color:#5A7A7A;">${m.company||""}</td></tr>`).join("");

    const latestStream:Record<string,string>={};
    ...
    const streamNames:Record<string,string>={A:"App not opened",B:"No activity yet",C1:"App dormant, portal active",C2:"Portal dormant, app active",C3:"Both dormant"};
    ...
    const d=(a:number,b:number)=>a>b?`<span style="color:#1B7878;"> +${a-b}</span>`:(a<b?`<span style="color:#C65D00;"> ${a-b}</span>`:"");

    const html = W([
      HDR("Monthly Report",monthLabel),
      SEC("Growth"),
      statRow([
        {label:"Total members",val:total+(growthPct!==null?`<span style="font-size:12px;color:#1B7878;"> ${growthPct>0?"+":""}${growthPct}%</span>`:""as any),sub:"active subscriptions"},
        {label:"New this month",val:(newMList.length)+(d(newMList.length,newPrevM.count||0) as any),sub:`${newPrevM.count||0} prev month`},
        {label:"App opened rate",val:`${appRate}%`,sub:`${appOpened.count||0} members`},
      ]),
      ...
      statRow([
        {label:"Active members",val:activeMCount+(d(activeMCount,activePMCount) as any),sub:`${retentionRate}% retention`},
        {label:"Total activities",val:actsM+(d(actsM,actsPM) as any),sub:`${actsPM} prev month`},
        {label:"Per active member",val:actsPerMember,sub:"avg activities"},
        {label:"Avg wellbeing",val:wellM!==null?`${wellM} ${wellTrend}`:"n/a",sub:wellPM!==null?`${wellPM} prev month`:"first month"},
      ]),
      DIV,
      SEC("Re-engagement health"),
      statRow([
        {label:"Emails sent",val:(reengM.data||[]).length+(d((reengM.data||[]).length,(reengPM.data||[]).length) as any),sub:`${(reengPM.data||[]).length} prev month`},
        {label:"Members in streams",val:Object.values(streamCount).reduce((s,n)=>s+n,0),sub:"currently in re-engagement"},
      ]),
      ...
      statRow([
        {label:"Certificates earned",val:certsTotal.count||0,sub:"all time"},
        {label:"This month",val:certsM.count||0,sub:"earned"},
        {label:"Donated months",val:charityTotal.count||0,sub:"total donated"},
      ]),
      FTR,
    ].join(""));
    ...
    const brevoBody:any = {
      ...
      to:[{email:toEmail,name:"VYVE Team"}],
    ...
    return new Response(JSON.stringify({success:true,month:monthLabel,backfill:!!overrideMonthStart,to:toEmail,cc:overrideCc,total,actsM,activeMCount}),{status:200,headers:{...corsHeaders,"Content-Type":"application/json"}});
  } catch(err) {
