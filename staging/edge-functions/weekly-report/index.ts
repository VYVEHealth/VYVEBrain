...
serve(async (req) => {
  ...
  let overrideWeekStart: string | null = null;
  ...
  if (overrideWeekStart && !/^\d{4}-\d{2}-\d{2}$/.test(overrideWeekStart)) {
    return new Response(JSON.stringify({success:false,error:"week_start must be YYYY-MM-DD"}),{status:400,headers:corsHeaders});
  }
  ...
  const dateRange = `${lastMon.toLocaleDateString("en-GB",{day:"numeric",month:"short",timeZone:"UTC"})} – ${lastSun.toLocaleDateString("en-GB",{day:"numeric",month:"short",year:"numeric",timeZone:"UTC"})}`;
  ...
  try {
    ...
    const [
      totalM, newThisW, newPrevW,
      appOpened,
      habW, wrkW, carW, sesW, ciW,
      habPW, wrkPW, carPW, sesPW,
      reengW, reengPW,
      certsTotal, charityTotal,
      avgScores,
    ] = await Promise.all([
      sb.from("members").select("email",{count:"exact",head:true}).eq("subscription_status","active").not("email","in",exFilter),
      sb.from("members").select("email",{count:"exact",head:true}).gte("created_at",wStart).lt("created_at",wEnd).not("email","in",exFilter),
      sb.from("members").select("email",{count:"exact",head:true}).gte("created_at",pwStart).lt("created_at",pwEnd).not("email","in",exFilter),
    ...
    const activeWCount  = new Set([...habW.data||[],...wrkW.data||[],...sesW.data||[],...ciW.data||[]].map((r:any)=>r.member_email)).size;
    const activePWCount = new Set([...habPW.data||[],...wrkPW.data||[],...sesPW.data||[]].map((r:any)=>r.member_email)).size;

    const delta=(a:number,b:number)=>b===0?"":a>b?`<span style="color:#1B7878;font-size:11px;"> +${a-b}</span>`:(a<b?`<span style="color:#C65D00;font-size:11px;"> ${a-b}</span>`:` <span style="color:#7A9A9A;font-size:11px;">&#8213;</span>`);

    const scores = avgScores.data||[];
    const avgWell = scores.length ? (scores.reduce((s:number,r:any)=>s+(r.wellbeing_score||0),0)/scores.length).toFixed(1) : "n/a";
    const avgEng  = scores.length ? Math.round(scores.reduce((s:number,r:any)=>s+(r.engagement_score||0),0)/scores.length) : "n/a";

    const reengByStream:Record<string,number> = {};
    for(const r of reengW.data||[]) reengByStream[r.stream]=(reengByStream[r.stream]||0)+1;
    const streamNames:Record<string,string> = {A:"App not opened",B:"No activity",C1:"App dormant",C2:"Portal dormant",C3:"Both dormant"};
    ...
    const html = W([
      ...
      SEC("Membership"),
      statRow([
        {label:"Total members",val:totalM.count||0,sub:"active"},
        {label:"New this week",val:(newThisW.count||0)+(delta(newThisW.count||0,newPrevW.count||0) as any),sub:`${newPrevW.count||0} prev week`},
        {label:"App opened",val:`${appRate}%`,sub:`${appOpened.count||0} of ${totalM.count||0}`},
      ]),
      DIV,
      SEC("Engagement this week"),
      statRow([
        {label:"Active members",val:activeWCount+(delta(activeWCount,activePWCount) as any),sub:`${activePWCount} prev week`},
        {label:"Total activities",val:actsW+(delta(actsW,actsPW) as any),sub:`${actsPW} prev week`},
        {label:"Check-in rate",val:`${checkinRate}%`,sub:`${uniq(ciW.data||[])} members`},
        {label:"Avg wellbeing",val:avgWell,sub:`Avg engagement: ${avgEng}`},
      ]),
      DIV,
      ...
      statRow([
        ...
        {label:"Donated months",val:charityTotal.count||0,sub:"triggered"},
      ]),
      FTR,
    ].join(""));
    ...
    return new Response(JSON.stringify({success:true,dateRange,backfill:!!overrideWeekStart,to:toEmail,cc:overrideCc,actsW,activeWCount,reengTotal}),{status:200,headers:{...corsHeaders,"Content-Type":"application/json"}});
  } catch(err) {
