...
export const IGNORED_CANON = new Set([
  ...
  "gymnastics", "martialarts", "trackandfield", "volleyball", "skating", "snowboarding",
...
export type WorkoutClass = "strength" | "cardio" | "ignored" | "unknown";

export function classifyWorkout(workoutTypeRaw: unknown, durationMin: number): WorkoutClass {
...
export type HealthRuleSource = "daily" | "samples_sleep" | "activity_tables";
export type HealthRuleAgg = "sum" | "max" | "exists" | "duration_sum_minutes" | "exists_row_gte";
export type HealthRuleWindow = "today_local" | "last_night" | "last_24h";
export type HealthRuleOp = "gte" | "lte" | "eq" | "exists";

export interface HealthRule {
  source: HealthRuleSource;
  metric: string;
  agg: HealthRuleAgg;
  window: HealthRuleWindow;
  op: HealthRuleOp;
  value?: number;
}
...
export function applyOp(op: HealthRuleOp, value: number, target: number): boolean {
  switch (op) {
    case "gte": return value >= target;
    case "lte": return value <= target;
    case "eq":  return value === target;
    case "exists": return value > 0;
...
export function ukLocalDateISO(now: Date = new Date()): string {
  const offsetHours = isBST(now) ? 1 : 0;
  const local = new Date(now.getTime() + offsetHours * 3600_000);
  return local.toISOString().slice(0, 10);
}

export function lastNightWindow(now: Date = new Date()): { windowStart: Date; windowEnd: Date } {
  const offsetHours = isBST(now) ? 1 : 0;
...
export function dailyMetricColumn(metric: string): string | null {
...
export function dailyUnitFor(metric: string, storedUnit: string): string {
