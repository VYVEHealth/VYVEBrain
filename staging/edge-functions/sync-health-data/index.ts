...
function dayOfWeek(d: Date): string { return ["Sunday","Monday","Tuesday","Wednesday","Thursday","Friday","Saturday"][d.getUTCDay()]; }
function timeOfDay(d: Date): string { const h = d.getUTCHours(); if (h < 5) return "night"; if (h < 12) return "morning"; if (h < 17) return "afternoon"; if (h < 21) return "evening"; return "night"; }
function isoDate(d: Date): string { return d.toISOString().slice(0, 10); }

function platformSourceTag(platform: string): "healthkit" | "health_connect" {
...
function diagnosticsShowAuthBlocked(diag: any): boolean {
  ...
  return probes.every((p: any) =>
    p && p.ok === false && /Authorization not determined/i.test(String(p.error || ""))
  );
}

async function passesOutlierCheck(sample: Record<string, unknown>, memberEmail: string, svc: ReturnType<typeof createClient>): Promise<boolean> {
  ...
  if (reason) {
    try {
      await svc.from("platform_alerts").insert({ member_email: memberEmail, severity: "warning", source: "sync-health-data", type: "outlier_sample_rejected", details: `${reason}; native_uuid=${sample.native_uuid} | sample=${JSON.stringify(sample).slice(0, 800)}` });
    } catch (_) { }
    return false;
  }
  return true;
...
function promoteMapping(sample: Record<string, unknown>, memberEmail: string, sourceTag: "healthkit" | "health_connect"): { target: "workouts" | "cardio" | "weight_logs"; row: Record<string, unknown>; } | null {
  const t = sample.sample_type as string;
  const start = new Date(String(sample.start_at));
  const end = new Date(String(sample.end_at));
  ...
  if (t === "workout") {
    const wtRaw = (sample.workout_type as string) ?? "";
    ...
    if (isStrength) {
      return { target: "workouts", row: { member_email: memberEmail, activity_date: isoDate(start), day_of_week: dayOfWeek(start), time_of_day: timeOfDay(start), session_number: 1, plan_name: null, workout_name: wtRaw || "Workout", duration_minutes: durationMin, logged_at: start.toISOString(), source: sourceTag } };
    }
    if (isCardio) {
      const cardioType = wtCanon;
      const distanceM = Number(sample.metadata && (sample.metadata as any).distance_m) || null;
      return { target: "cardio", row: { member_email: memberEmail, activity_date: isoDate(start), day_of_week: dayOfWeek(start), time_of_day: timeOfDay(start), session_number: 1, cardio_type: cardioType, duration_minutes: durationMin, distance_km: distanceM ? Number((distanceM / 1000).toFixed(2)) : null, logged_at: start.toISOString(), source: sourceTag } };
    }
    return null;
  }
  return null;
...
function promotedConflictTarget(target: "workouts" | "cardio"): string {
  return target === "cardio"
    ? "member_email,source,cardio_type,logged_at"
    : "member_email,source,logged_at";
}
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    try {
      await svc.from("platform_alerts").insert({ member_email: memberEmail, severity: "error", source: "sync-health-data", type: "handler_exception", details: String((err as any)?.message || err).slice(0, 500) });
    } catch (_) { }
    return jsonResponse({ error: "internal_error" }, 500);
  }
});

async function handlePushDaily(body: Record<string, unknown>, memberEmail: string, svc: ReturnType<typeof createClient>): Promise<Response> {
  const platform = String(body.platform || "");
  if (!["healthkit","health_connect"].includes(platform)) return jsonResponse({ error: "invalid_platform" }, 400);

  const dailyIn = Array.isArray(body.daily) ? body.daily as Record<string, unknown>[] : [];
  if (dailyIn.length > MAX_DAILY_PER_CALL) return jsonResponse({ error: "batch_too_large", limit: MAX_DAILY_PER_CALL }, 413);

  const cutoff = new Date(Date.now() - MAX_SAMPLE_AGE_DAYS * 86400_000);
  const toUpsert: Record<string, unknown>[] = [];
  const skipped = { invalid: 0, too_old: 0, bad_type: 0, bad_value: 0 };

  for (const d of dailyIn) {
    if (!d || !d.sample_type || !d.date || d.value == null) { skipped.invalid++; continue; }
    const t = String(d.sample_type);
    if (!ALLOWED_DAILY_TYPES.has(t)) { skipped.bad_type++; continue; }
    const date = String(d.date);
    const dObj = new Date(date + "T00:00:00Z");
    if (isNaN(dObj.getTime())) { skipped.invalid++; continue; }
    if (dObj < cutoff) { skipped.too_old++; continue; }
    const v = Number(d.value);
    if (!isFinite(v) || v < 0) { skipped.bad_value++; continue; }
    if (t === "steps" && v > OUTLIER.steps_per_day) { skipped.bad_value++; continue; }
    toUpsert.push({
      member_email: memberEmail,
      source: platform,
      sample_type: t,
  ...
  if (toUpsert.length > 0) {
    const { data, error } = await svc
      .from("member_health_daily")
      .upsert(toUpsert, { onConflict: "member_email,source,sample_type,date" })
      .select("member_email");
...
async function handlePullSamples(body: Record<string, unknown>, memberEmail: string, svc: ReturnType<typeof createClient>): Promise<Response> {
  const platform = String(body.platform || "");
  if (!["healthkit","health_connect"].includes(platform)) return jsonResponse({ error: "invalid_platform" }, 400);
  ...
  if (samplesIn.length > MAX_SAMPLES_PER_CALL) return jsonResponse({ error: "batch_too_large", limit: MAX_SAMPLES_PER_CALL }, 413);
  ...
  for (const s of samplesIn) {
    try {
      if (!s.native_uuid || !s.sample_type || !s.start_at || !s.end_at) { skipped.invalid++; continue; }
      ...
      toInsert.push({ member_email: memberEmail, source: platform, sample_type: s.sample_type, native_uuid: s.native_uuid, start_at: s.start_at, end_at: s.end_at, value: s.value ?? null, unit: s.unit ?? null, workout_type: s.workout_type ?? null, metadata: s.metadata ?? {}, app_source: s.app_source ?? null });
    } catch (_) { skipped.invalid++; }
  }
  let insertedSamples: Record<string, unknown>[] = [];
  if (toInsert.length > 0) {
    const { data, error } = await svc.from("member_health_samples").upsert(toInsert, { onConflict: "member_email,source,native_uuid", ignoreDuplicates: true }).select("id, sample_type, workout_type, start_at, end_at, value, unit, metadata, native_uuid");
    if (error) throw error;
  ...
  for (const s of insertedSamples) {
    ...
    if (m.target === "weight_logs") {
      const { data, error } = await svc.from("weight_logs").upsert(m.row, { onConflict: "member_email,logged_date" }).select("id");
      if (!error && data && data.length > 0) {
        promoted.weight_logs++;
        await svc.from("member_health_samples").update({ promoted_to: "weight_logs", promoted_id: data[0].id }).eq("id", (s as any).id);
      }
    } else {
      ...
      const { data, error } = await svc.from(m.target).upsert(m.row, { onConflict: promotedConflictTarget(m.target as "workouts" | "cardio"), ignoreDuplicates: true }).select("id");
      if (error) promoted.skipped_cap++;
      else if (data && data.length > 0) {
        promoted[m.target]++;
        await svc.from("member_health_samples").update({ promoted_to: m.target, promoted_id: data[0].id }).eq("id", (s as any).id);
      }
    }
  }
  const { data: pendingWrites } = await svc.from("member_health_write_ledger").select("id, vyve_source_table, vyve_source_id, queued_at").eq("member_email", memberEmail).eq("platform", platform).eq("write_status", "queued").order("queued_at", { ascending: true }).limit(50);
  ...
  for (const row of pendingWrites || []) {
    ...
    if (src) pendingPayloads.push({ ledger_id: row.id, vyve_source_table: tgt, payload: src });
  }
  await svc.from("member_health_connections").update({ total_synced: (insertedSamples.length || 0) + 0 }).eq("member_email", memberEmail).eq("platform", platform);
  return jsonResponse({ ok: true, inserted_raw: insertedSamples.length, skipped, promoted, pending_writes: pendingPayloads, server_last_sync_at: isAuthBlocked ? null : nowIso, auth_blocked: isAuthBlocked });
}

async function handleConfirmWrite(body: Record<string, unknown>, memberEmail: string, svc: ReturnType<typeof createClient>): Promise<Response> {
  ...
  const { data, error } = await svc.from("member_health_write_ledger").update({ native_uuid: nativeUuid, write_status: status, confirmed_at: new Date().toISOString(), error_message: errorMsg }).eq("id", ledgerId).eq("member_email", memberEmail).select("id");
  if (error) throw error;
...
async function handleMarkRevoked(body: Record<string, unknown>, memberEmail: string, svc: ReturnType<typeof createClient>): Promise<Response> {
  const platform = String(body.platform || "");
  if (!["healthkit","health_connect"].includes(platform)) return jsonResponse({ error: "invalid_platform" }, 400);
  const { error } = await svc.from("member_health_connections").update({ revoked_at: new Date().toISOString(), last_sync_status: "revoked" }).eq("member_email", memberEmail).eq("platform", platform);
  if (error) throw error;
