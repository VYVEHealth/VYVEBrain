...
serve(async (req) => {
  ...
  try {
    ...
    let q = supabase.from("member_achievements")
      .update({ seen_at: now })
    ...
    if (mark_all === true) {
      ...
      return new Response(JSON.stringify({ success: false, error: "specify {mark_all:true} OR {metric_slug,tier_index}" }),
    ...
    return new Response(JSON.stringify({ success: true, marked: data?.length ?? 0, rows: data ?? [] }),
