...
function validateAuth(req: Request): { valid: boolean; error?: string; identifier?: string } {
  if (!EMPLOYER_API_KEY) {
    ...
    return { valid: false, error: 'Server misconfiguration — contact support.' };
  }
  ...
  if (!providedKey) {
    return { valid: false, error: 'Missing API key. Provide x-api-key header or api_key query param.' };
  }
  if (providedKey !== EMPLOYER_API_KEY) {
    return { valid: false, error: 'Invalid API key.' };
  }
  return { valid: true, identifier: 'api-key-auth' };
}
...
serve(async (req) => {
  ...
  try {
    ...
    let memberEmails: string[] | null = null;
    const allMembers = await query('members', 'select=email,company,created_at');

    const companies: string[] = [...new Set(
      allMembers.map((m: any) => m.company).filter((c: any) => !!c)
    )] as string[];

    if (company !== 'ALL') {
      memberEmails = allMembers
        .filter((m: any) => m.company === company)
        .map((m: any) => m.email);
    }

    const totalMembers = company === 'ALL'
      ? allMembers.length
      : (memberEmails?.length ?? 0);

    async function fetchActivity(table: string, select = 'member_email,activity_date,logged_at') {
    ...
    const allActivity = [...habits, ...workouts, ...cardio, ...kahCheckins, ...wbCheckins, ...sessions, ...replays];
    ...
    const scopeEmails = memberEmails ?? allMembers.map((m: any) => m.email);
    let activeCount = 0, quietCount = 0, inactiveCount = 0;
    for (const email of scopeEmails) {
      const last = lastActiveMap[email];
      if (!last)         { inactiveCount++; continue; }
      if (last >= day7)  { activeCount++;   continue; }
      if (last >= day30) { quietCount++;    continue; }
      inactiveCount++;
    }

    const engagementRate = totalMembers > 0
      ? Math.round(((activeCount + quietCount) / totalMembers) * 100) : 0;
    ...
    const trend = Object.keys(monthCounts).sort().slice(-10).map(ym => {
      const [y, m] = ym.split('-');
      const label = new Date(parseInt(y), parseInt(m)-1, 1).toLocaleDateString('en-GB', {month:'short', year:'2-digit'});
      return { label, count: monthCounts[ym] };
    });
    ...
    const catMap: Record<string, {liveViews:number,liveMins:number,replayViews:number,replayMins:number}> = {};
    const SKIP_CATS = new Set(['Sessions Hub', 'Unknown', '']);
    for (const s of sessions) {
      const cat = s.category || 'Unknown';
      if (SKIP_CATS.has(cat)) continue;
      if (!catMap[cat]) catMap[cat] = {liveViews:0,liveMins:0,replayViews:0,replayMins:0};
    ...
    const totalLiveMins   = sessions.reduce((s:number,r:any) => s + (Math.round(parseFloat(r.minutes_watched)||0)), 0);
    const totalReplayMins = replays.reduce((s:number,r:any)  => s + (Math.round(parseFloat(r.minutes_watched)||0)), 0);

    return new Response(JSON.stringify({
      companies,
      members: { total: totalMembers, active: activeCount, quiet: quietCount, inactive: inactiveCount, engagementRate },
      activities: { total: totalActivities, thisMonth, avgPerMember, habits: totalHabits, checkins: totalCheckins, workouts: totalWorkouts, cardio: totalCardio },
      sessions: { totalLiveViews, totalReplayViews, totalLiveMins, totalReplayMins, byCategory },
      charity: { monthsDonated, remainder, progressPct, nextMilestone: { remaining: 30 - remainder } },
      trend,
    }), { headers: { ...CORS, 'Content-Type': 'application/json' } });
