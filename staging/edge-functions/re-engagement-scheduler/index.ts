...
const CADENCE = {
  A: [
    { key: "A_48h", delayHours: 48 },
    { key: "A_96h", delayHours: 96 },
    { key: "A_7d",  delayHours: 168 },
    { key: "A_14d", delayHours: 336 }
  ],
  B: [
    { key: "B_3d",  delayHours: 72 },
    { key: "B_7d",  delayHours: 168 },
    { key: "B_14d", delayHours: 336 },
    { key: "B_30d", delayHours: 720 }
  ]
};
...
function hasUsefulProfile(m: any) { return !!(m.persona || m.goal_focus); }
function isOverwhelmed(m: any) {
  const v = m.overwhelm_response || "";
  return v.includes("Fewer notifications") || v.includes("I'll manage myself");
}

function classifyGoal(goalFocus: string | null) {
  const g = (goalFocus || "").toLowerCase();
  const rules = [
    { keys: ["weight","lose","fat","slim","tone","calorie","diet"], primary: "habits",   secondary: "cardio",   tertiary: "workouts" },
    { keys: ["muscle","strength","lift","build","bulk","power"],     primary: "workouts", secondary: "habits",   tertiary: "cardio" },
    { keys: ["run","race","5k","10k","marathon","cardio","endurance","fitness"], primary: "cardio", secondary: "workouts", tertiary: "habits" },
    { keys: ["stress","anxiety","mental","mind","calm","relax","balance"], primary: "habits", secondary: "cardio", tertiary: "workouts" },
    { keys: ["sleep","rest","recovery","energy","fatigue","tired"],  primary: "habits",   secondary: "cardio",   tertiary: "workouts" },
    { keys: ["health","wellbeing","lifestyle","general","overall","better"], primary: "habits", secondary: "cardio", tertiary: "workouts" }
  ];
  for (const r of rules) if (r.keys.some((k) => g.includes(k))) return { primary: r.primary, secondary: r.secondary, tertiary: r.tertiary };
  return { primary: "habits", secondary: "workouts", tertiary: "cardio" };
}
...
async function aiLine(
  ...
  try {
    ...
    if (!r.ok) { console.error("[ai] error:", d); return null; }
    ...
    EdgeRuntime.waitUntil(
      writeAiInteraction(
        email,
        persona,
        `re-engagement ${streamKey} (${maxTokens} max_tokens) | system: ${systemPrompt.slice(0, 200)}`,
        txt,
        {
          model: "claude-haiku-4-5-20251001",
          max_tokens: maxTokens,
    ...
    console.error("[ai] fetch failed:", (e as Error).message);
    return null;
  }
}

async function buildA(m: any, key: string) {
...
${appBtn}`
  ...
  return { subject: subjects[key], html: wrap(body), ai_used: useAI && key !== "A_14d" };
}

async function buildB(m: any, key: string) {
  const n = m.first_name || "there";
  const useAI = hasUsefulProfile(m) && !!m.persona_system_prompt;
  const priority = classifyGoal(m.goal_focus);
...
${backBtn}`
  ...
  let body = staticBodies[key];
  if (useAI) {
    const instructions: Record<string, { inst: string; maxTokens: number; insertBefore: string }> = {
      B_3d:  { inst: `Write 2 sentences. Reference the member's goal ("${m.goal_focus}") and lead with ${priority.primary} as the specific first action today. Reference welcome_rec_1 if available. Concrete and small. Do not greet or sign off.`, maxTokens: 100, insertBefore: hubBtn },
      B_7d:  { inst: `Write 2 sentences. Connect the member's goal ("${m.goal_focus}") to ${priority.primary} as the one thing they can do today. Warm accountability, no guilt. Do not greet or sign off.`, maxTokens: 100, insertBefore: hubBtn },
      B_14d: { inst: `Write 1 sentence. Honest, warm. Acknowledge it might not feel right yet. If life context includes time pressure (${lc}), reference that 5 minutes counts. Do not greet or sign off.`, maxTokens: 70, insertBefore: hubBtn },
      B_30d: { inst: `Write 2 sentences in persona voice. Warm no-pressure close. Reference the goal ("${m.goal_focus}") naturally. Do not repeat the charity line \u2014 it is already in the email. Do not greet or sign off.`, maxTokens: 100, insertBefore: pp(CHARITY_LINE) + backBtn }
    };
  ...
  return { subject: subjects[key], html: wrap(body), ai_used: useAI };
}

async function sendBrevo(to: string, name: string, subject: string, html: string, tags: string[]) {
...
serve(async (req) => {
  ...
  try {
    ...
    const { data: homeStateRows } = emails.length
      ...
      : { data: [] };
    const homeStateMap: Record<string, { last_habit_at: string | null; last_workout_at: string | null; last_session_at: string | null; last_checkin_at: string | null }> = {};
    for (const r of homeStateRows || []) homeStateMap[r.member_email] = r;

    const members = (membersRaw || []).map((m) => {
      const hs = homeStateMap[m.email] || { last_habit_at: null, last_workout_at: null, last_session_at: null, last_checkin_at: null };
      return {
        ...m,
    ...
    const sentMap: Record<string, Record<string, Set<string>>> = {};
    const suppMap: Record<string, Set<string>> = {};
    for (const r of sent || []) {
    ...
    for (const m of members) {
      if (EXCLUDED_EMAILS.has(m.email.toLowerCase())) {
        results.push({ email: m.email, stream: "none", email_key: "", status: "excluded" });
        continue;
      }
...
async function processMember(m: any, supabase: any, now: Date, sentMap: any, suppMap: any, dryRun: boolean) {
  ...
  const lastAny = [lH, lW, lS, lC].filter(Boolean).reduce((a: Date | null, b: Date | null) => !a || (b && b > a) ? b : a, null as Date | null);
  const hasAnyActivity = !!lastAny;
  const consentDone = !!m.privacy_accepted_at;

  let stream: string | null = null;
  let trigger: Date = created;
  ...
  if (!stream) return { email: m.email, stream: "none", email_key: "", status: "skipped", reason: "active \u2014 no stream" };
  if (suppMap[m.email]?.has(stream)) return { email: m.email, stream, email_key: "", status: "suppressed" };

  const sentForStream: Set<string> = sentMap[m.email]?.[stream] || new Set();
  if (isOverwhelmed(m) && sentForStream.size >= 1) return { email: m.email, stream, email_key: "", status: "skipped", reason: "overwhelm preference" };

  const cadence = CADENCE[stream as "A" | "B"];
  let nextStep: { key: string; delayHours: number } | null = null;
  for (const step of cadence) {
    if (sentForStream.has(step.key)) continue;
    if (now >= new Date(trigger.getTime() + step.delayHours * 3600000)) { nextStep = step; break; }
  }
  if (!nextStep) return { email: m.email, stream, email_key: "", status: "skipped", reason: "not yet due or cadence complete" };

  const result = stream === "A" ? await buildA(m, nextStep.key) : await buildB(m, nextStep.key);
  if (!result) return { email: m.email, stream, email_key: nextStep.key, status: "error", reason: "no content built" };
  if (dryRun) return { email: m.email, stream, email_key: nextStep.key, status: "sent", reason: "DRY RUN", subject: result.subject, ai_used: result.ai_used };

  try {
    const msgId = await sendBrevo(m.email, m.first_name || "there", result.subject, result.html, [stream, nextStep.key]);
    await supabase.from("engagement_emails").insert({
      member_email: m.email, stream, email_key: nextStep.key,
      sent_at: now.toISOString(), brevo_message_id: msgId, suppressed: false
    });
