...
const storeBtn = (label, href)=>`<a href="${href}" style="background:#0D2B2B;color:#fff;text-decoration:none;padding:14px 24px;border-radius:8px;font-size:15px;font-weight:600;display:inline-block;margin:6px;">${label} &rarr;</a>`;
const storeRow = (iosLabel, androidLabel)=>`<div style="text-align:center;margin:28px 0;">${storeBtn(iosLabel, IOS_STORE_URL)}${storeBtn(androidLabel, ANDROID_STORE_URL)}</div>`;
