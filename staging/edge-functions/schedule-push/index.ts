...
serve(async (req: Request) => {
  ...
  if (!Number.isFinite(fireInSecondsRaw)) {
    return new Response(JSON.stringify({ error: 'fire_in_seconds must be a number' }), {
      status: 400,
      headers: { ...CORS, 'Content-Type': 'application/json' }
    });
  }
  ...
  if (!res.ok) {
    ...
    return new Response(JSON.stringify({ error: 'db error', status: res.status, detail: txt.slice(0, 500) }), {
      status: 500,
      headers: { ...CORS, 'Content-Type': 'application/json' }
    });
  }
