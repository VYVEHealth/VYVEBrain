...
async function buildExport(supabaseSvc, subject) {
  ...
  for (const batch of batches){
    const results = await Promise.all(batch.map((t)=>supabaseSvc.from(t.table_name).select("*").eq(t.subject_column, subject)));
    batch.forEach((t, i)=>{
      const r = results[i];
      if (r.error) {
        ...
        out._meta = {
          ...out._meta,
          [`error_${t.table_name}`]: r.error.message
        };
      } else {
