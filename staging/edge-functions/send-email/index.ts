...
export function classifyGoal(goalFocus: string | null): {
  primary: string; secondary: string; tertiary: string; category: string;
} {
  const g = (goalFocus || "").toLowerCase();
  const rules: { keys: string[]; category: string; primary: string; secondary: string; tertiary: string }[] = [
    { keys: ["weight","lose","fat","slim","tone","calorie","diet"],           category: "body_composition",   primary: "habits",   secondary: "cardio",   tertiary: "workouts" },
    { keys: ["muscle","strength","lift","build","bulk","power"],              category: "strength",           primary: "workouts", secondary: "habits",   tertiary: "cardio"   },
    { keys: ["run","race","5k","10k","marathon","cardio","endurance","fitness"], category: "endurance",        primary: "cardio",   secondary: "workouts", tertiary: "habits"   },
    { keys: ["stress","anxiety","mental","mind","calm","relax","balance"],   category: "mental_wellbeing",  primary: "habits",   secondary: "cardio",   tertiary: "workouts" },
    { keys: ["sleep","rest","recovery","energy","fatigue","tired"],          category: "recovery",           primary: "habits",   secondary: "cardio",   tertiary: "workouts" },
    { keys: ["health","wellbeing","lifestyle","general","overall","better"], category: "general_health",    primary: "habits",   secondary: "cardio",   tertiary: "workouts" },
  ];
  for (const rule of rules) {
    if (rule.keys.some(k => g.includes(k))) {
      return { primary: rule.primary, secondary: rule.secondary, tertiary: rule.tertiary, category: rule.category };
    }
  }
  return { primary: "habits", secondary: "workouts", tertiary: "cardio", category: "unknown" };
}

// ── Persona-voiced AI line generator ─────────────────────────────────────────────────
export async function generatePersonaLine(
  systemPrompt: string,
  userMessage: string,
  maxTokens = 150
): Promise<string> {
...
export async function sendBrevoEmail(
  to: string, name: string, subject: string, html: string, tags: string[]
): Promise<string | null> {
...
export const p  = (t: string) => `<p style="margin:0 0 16px;font-size:15px;color:#3A5A5A;line-height:1.7;">${t}</p>`;
export const h2 = (t: string) => `<h2 style="margin:0 0 20px;font-size:22px;font-family:Georgia,serif;color:#0D2B2B;font-weight:400;">${t}</h2>`;
...
serve(async (req) => {
  ...
  try {
    ...
    const tags: string[] = body.tags || ["test"];
    ...
    return new Response(JSON.stringify({ success: true, messageId: msgId, to: toEmail, subject }),
