...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    // Surface any errors
    for (const [label, result] of Object.entries({ members, habits, workouts, cardio, sessions, checkins, weekly, emails, certs })) {
      if ((result as any).error) throw new Error(`${label}: ${(result as any).error.message}`);
    }
