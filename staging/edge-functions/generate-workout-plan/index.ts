...
const PLAN_TYPE_SPLITS: Record<string,string> = {
  PPL: 'Push / Pull / Legs',
...
function selectPlanType(data: Record<string,unknown>): { planType: string; splitType: string } {
  ...
  const goals = ((data.trainingGoals as string[]) || []).map(g => g.toLowerCase()).join(' ');
  const lifeContext = (data.lifeContext as string[]) || [];
  const hasSensitive = lifeContext.some((c: string) => SENSITIVE_CONTEXT.includes(c));
  ...
  return { planType, splitType: PLAN_TYPE_SPLITS[planType] };
}

async function callAnthropic(system: string|null, user: string, maxTokens: number): Promise<{text: string; stopReason: string}> {
...
async function generateProgrammeOverview(
  data: Record<string,unknown>,
  splitType: string,
  planType: string
): Promise<{ programme_name: string; rationale: string }> {
  const scores = (data.scores as Record<string,string>) || {};
  const injuries = (data.injuries as string[]||[]).join(', ') || 'none';
  ...
  const goals = (data.trainingGoals as string[]||[]).join(', ');
  const specificGoal = String(data.specificGoal || '');
  const equipment = (data.equipment as string[]||[]).join(', ') || 'standard gym';
...
function parseWeeksArray(raw: string): unknown[] {
...
serve(async (req) => {
  ...
  try {
    ...
    const libRes = await fetch(
      `${SUPABASE_URL}/rest/v1/workout_plans?select=exercise_name,video_url,thumbnail_url&video_url=not.is.null&order=exercise_name`,
      { headers: { 'apikey': SUPABASE_KEY, 'Authorization': `Bearer ${SUPABASE_KEY}` } }
    );
    const libraryRaw: {exercise_name: string; video_url: string; thumbnail_url: string}[] = libRes.ok ? await libRes.json() : [];
    // Deduplicate by exercise_name
    const seen = new Set<string>();
    const library = libraryRaw.filter(r => { if (seen.has(r.exercise_name)) return false; seen.add(r.exercise_name); return true; });
    ...
    const videoMap: Record<string, {video_url: string; thumbnail_url: string}> = {};
    for (const ex of library) {
      videoMap[ex.exercise_name] = { video_url: ex.video_url, thumbnail_url: ex.thumbnail_url };
    }
...
[{
  ...
  "sessions": [{
    ...
    const [result1, result2] = await Promise.all([
      callAnthropic(
        systemBase,
        `Generate weeks 1-4 only (sessions 1–${trainDays * 4}). Foundation and build phases. Return ONLY the JSON array.`,
        16000
      ),
      callAnthropic(
        systemBase,
        `Generate weeks 5-8 only (sessions ${trainDays * 4 + 1}–${trainDays * 8}). Volume phase (weeks 5-6: 4 sets 8-10 reps), peak (week 7: 4-5 sets 6-8 reps), deload (week 8: 2-3 sets 12-15 reps). Return ONLY the JSON array.`,
        16000
      ),
    ]);
    ...
    const allWeeks = [...weeks1to4, ...weeks5to8];
    ...
    for (const week of allWeeks as Array<{sessions: Array<{exercises: Array<Record<string,unknown>>}>}>) {
    ...
    const totalSessions = allWeeks.reduce((a: number, w: {sessions: unknown[]}) => a + (w.sessions?.length || 0), 0);
