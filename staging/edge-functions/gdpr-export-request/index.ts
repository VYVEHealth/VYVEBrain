...
serve(async (req: Request) => {
  ...
  if (adminRequest) {
    ...
    if (!requesterIsAdmin) {
      return new Response(JSON.stringify({ error: "Admin access required for cross-member exports" }), {
  ...
  if (kind === "member_self") {
    const { data: recent } = await supabaseSvc
      ...
      .select("id, requested_at, delivered_at")
  ...
  const { data: row, error: insertErr } = await supabaseSvc
    ...
    .select("id, member_email, request_kind, requested_at")
    .single();

  if (insertErr || !row) {
    console.error("[gdpr-export-request] insert failed:", insertErr);
    return new Response(JSON.stringify({ error: "Failed to queue export request" }), {
  ...
  return new Response(JSON.stringify({
    ...
    message: kind === "admin"
      ? `Export queued for ${subject}. They will receive an email when it's ready.`
      : "We'll email you within an hour with a download link. The link will work for 7 days.",
  }), {
    status: 202,
