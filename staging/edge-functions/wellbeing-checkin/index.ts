...
async function scanForCrisis(email, name, source, fields) {
  try {
...
async function handleNewCheckin(email, body, CORS) {
  const { mood, dimension_energy, dimension_sleep, dimension_stress, dimension_body, drivers, improvement_focus, free_text } = body;
  ...
  const today = ukToday();
  const [memberRows, homeStateRows, recentCheckins, habits, workouts, cardio, movement, mind, dailyMoods] = await Promise.all([
    q('members', `email=eq.${enc}&select=persona,first_name,specific_goal,success_vision,past_barriers,experience_level,training_days_per_week,training_location,injuries,sleep_issues,life_context,sensitive_context,training_goals,barriers&limit=1`),
    q('member_home_state', `member_email=eq.${enc}&select=habits_this_week,workouts_this_week,cardio_this_week,sessions_this_week,checkins_this_week,last_habit_at,last_workout_at,last_cardio_at,last_session_at&limit=1`),
    q('wellbeing_checkins', `member_email=eq.${enc}&select=score_wellbeing,logged_at,dimension_energy,dimension_sleep,dimension_stress,dimension_body,drivers,improvement_focus&order=logged_at.desc&limit=4`),
    q('daily_habits', `member_email=eq.${enc}&activity_date=gte.${weekAgo}&activity_date=lte.${today}&select=activity_date`),
    q('workouts', `member_email=eq.${enc}&activity_date=gte.${weekAgo}&activity_date=lte.${today}&select=activity_date,workout_name`),
    q('cardio', `member_email=eq.${enc}&activity_date=gte.${weekAgo}&activity_date=lte.${today}&select=activity_date,cardio_type`),
    q('movement_activities', `member_email=eq.${enc}&activity_date=gte.${weekAgo}&activity_date=lte.${today}&select=activity_date,kind`),
    q('mind_activities', `member_email=eq.${enc}&activity_date=gte.${weekAgo}&activity_date=lte.${today}&select=activity_date,kind`),
    q('daily_mood_checkins', `member_email=eq.${enc}&mood_date=gte.${weekAgo}&select=mood_date,mood_value&order=mood_date.asc`)
  ]);
  const member = memberRows[0] ?? {};
  const homeState = homeStateRows[0] ?? {};
  const persona = member.persona?.toUpperCase() || 'RIVER';
  const firstName = member.first_name || 'there';
  const personaLine = PERSONA_PROMPTS[persona] || PERSONA_PROMPTS.RIVER;
  const memberGoal = member.specific_goal || member.success_vision || null;
  const prevScores = recentCheckins.map((r)=>r.score_wellbeing).filter(Boolean);
  const previousScore = prevScores[0] ?? null;
  ...
  const dimensionsBlock = dimLines.length > 0 ? `Dimension breakdown: ${dimLines.join(', ')}.` : 'No dimension breakdown provided.';
  const habitDays = new Set(habits.map((h)=>h.activity_date)).size;
  const workoutFmt = workouts.length > 0 ? `${workouts.length} workout${workouts.length !== 1 ? 's' : ''} (${[
    ...new Set(workouts.map((w)=>w.workout_name).filter(Boolean))
  ].slice(0, 3).join(', ') || 'logged'})` : null;
  ...
  const hsSessions = homeState.sessions_this_week ?? 0;
  const activityLines = [];
  ...
  const prevFocusLine = lastCheckin?.improvement_focus ? `Last week they wanted to improve: ${lastCheckin.improvement_focus}.` : '';
  ...
  for(let i = 6; i >= 0; i--){
    ...
    const match = dailyMoods.find((m)=>m.mood_date === dateStr);
  ...
  const dailyMoodPromptLine = moodEntries.length > 0 ? `Daily mood logged this week (${moodEntries.length} day${moodEntries.length !== 1 ? 's' : ''}): ${moodEntries.join(', ')}.` : '';
  const profileParts = [];
  if (memberGoal) profileParts.push(`Goal: ${memberGoal}`);
  if (member.experience_level) profileParts.push(`Experience: ${member.experience_level}`);
  if (member.training_days_per_week) profileParts.push(`Training days/week target: ${member.training_days_per_week}`);
  if (member.past_barriers) profileParts.push(`Past barriers: ${member.past_barriers}`);
  if (member.injuries) profileParts.push(`Injuries: ${member.injuries}`);
  if (member.sleep_issues) profileParts.push(`Sleep issues: ${member.sleep_issues}`);
  ...
  const driversLine = drivers && drivers.length > 0 ? `What shaped the week (member-selected): ${drivers.join(', ')}.` : 'No specific drivers selected.';
  const focusLine = improvement_focus ? `Member wants to focus on: ${improvement_focus}.` : '';
  ...
  const userMessage = free_text ? `Mood score: ${mood}/10. In my own words: "${free_text}"` : `Mood score: ${mood}/10.`;
  ...
  try {
    const wcRes = await fetch(`${SUPABASE_URL}/rest/v1/wellbeing_checkins?on_conflict=member_email,iso_week,iso_year`, {
      ...
      body: JSON.stringify({
        ...
        iso_week,
        iso_year,
        score_wellbeing: mood,
        flow_type: 'active',
        ai_recommendation: rawText,
        ai_persona: persona,
        logged_at: nowISO,
  ...
  EdgeRuntime.waitUntil(writeAiInteraction(email, persona, `Weekly check-in v37 (mood=${mood}, habits=${habitDays}d, workouts=${workouts.length}, cardio=${cardio.length}, movement=${movement.length}, mind=${mind.length}, drivers=${drivers?.join(',') || 'none'}, focus=${improvement_focus || 'none'})`, rawText, {
    model: 'claude-sonnet-4-5',
    max_tokens: 1200,
    mood,
    dimensions: {
  ...
  const notifBody = mood >= 7 ? `Score ${mood}/10 — a strong week. Your ${persona} reflection is ready.` : mood >= 5 ? `Score ${mood}/10. Your ${persona} reflection is ready.` : `Score ${mood}/10 noted. Your ${persona} reflection is ready.`;
...
async function handleLegacyCheckin(req, email, body, CORS) {
...
serve(async (req)=>{
  ...
  try {
    ...
    try {
      ...
      if (typeof body.answer === 'string' && body.answer.trim()) scanFields.answer = body.answer;
      if (Object.keys(scanFields).length > 0) {
        const nameGuess = String(body.member?.firstName ?? '');
