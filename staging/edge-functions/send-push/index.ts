...
let _vapidPrivKey: CryptoKey | null = null;
...
async function makeVapidJwt(audience) {
  ...
  const header = b64u(enc.encode(JSON.stringify({ typ: 'JWT', alg: 'ES256' })).buffer);
...
async function encryptPayload(payload, p256dhB64, authB64) {
  ...
  const recipientKey = await crypto.subtle.importKey('raw', recipientPubKey, { name: 'ECDH', namedCurve: 'P-256' }, false, []);
  ...
  const ikmKey = await crypto.subtle.importKey('raw', new Uint8Array(ikmBits), 'HKDF', false, ['deriveBits']);
  ...
  const encrypted = await crypto.subtle.encrypt({
    name: 'AES-GCM',
    iv: nonceBits
  }, aesKey, concat(enc.encode(payload), new Uint8Array([2])));
  const recordSize = new DataView(new ArrayBuffer(4));
  recordSize.setUint32(0, 4096);
  return concat(salt, new Uint8Array(recordSize.buffer), new Uint8Array([65]), serverPubKeyRaw, new Uint8Array(encrypted));
}
...
async function sendNativeBatch(memberEmails, title, body, data) {
  try {
    const res = await fetch(`${SUPABASE_URL}/functions/v1/push-send-native`, {
      ...
      body: JSON.stringify({ member_emails: memberEmails, title, body, data })
    });
    if (!res.ok) {
      const txt = await res.text();
      console.warn('[send-push] native batch failed:', res.status, txt);
      return { ok: false, sent: 0, revoked: 0, attempted: 0, error: txt };
    }
    ...
    return { ok: false, sent: 0, revoked: 0, attempted: 0, error: String(e) };
  }
}
...
serve(async (req) => {
  ...
  const memberEmails = Array.isArray(input.member_emails)
    ? input.member_emails.map((e) => String(e).toLowerCase().trim()).filter(Boolean)
    : [];
  ...
  if (!memberEmails.length) {
    return new Response(JSON.stringify({ error: 'member_emails required' }), {
      status: 400,
      headers: { ...CORS, 'Content-Type': 'application/json' }
    });
  }
  if (!type || !title || !body) {
    return new Response(JSON.stringify({ error: 'type, title, body required' }), {
      status: 400,
      headers: { ...CORS, 'Content-Type': 'application/json' }
    });
  }
  ...
  for (const email of memberEmails) {
    processed++;
    const detail = { email, deduped: false, notification_id: null, web: { attempted: 0, sent: 0, revoked: 0 } };

    if (dedupeSameDay) {
      const dedupeRes = await db(
        `member_notifications?member_email=eq.${encodeURIComponent(email)}&type=eq.${encodeURIComponent(type)}&created_at=gte.${today}T00:00:00Z&select=id&limit=1`
      );
    ...
    if (!skipInapp) {
      ...
      const insertPayload: Record<string, unknown> = { member_email: email, type, title, body };
    ...
    if (!skipWeb) {
      const subRes = await db(
        `push_subscriptions?member_email=eq.${encodeURIComponent(email)}&select=endpoint,p256dh,auth_key`
      );
      if (subRes.ok) {
        ...
        for (const sub of subs) {
          ...
          webAttempted++;
          const payload = JSON.stringify({ title, body, data: customData });
  ...
  let nativeResult = { sent: 0, revoked: 0, attempted: 0, skipped: 0 };
  if (nativeRecipients.length > 0) {
    nativeResult = await sendNativeBatch(nativeRecipients, title, body, { ...customData, type });
  }
