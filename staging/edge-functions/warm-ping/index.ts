...
async function pingFunction(name: string): Promise<{ name: string; status: number; ms: number }> {
  ...
  try {
    ...
    return { name, status: 0, ms: Date.now() - start };
  }
}
...
serve(async (_req) => {
  ...
  const summary = results.map(r =>
    `${r.name}: ${r.status >= 200 && r.status < 400 ? '✓' : '✗'} ${r.status} (${r.ms}ms)`
  ).join(', ');

  console.log(`warm-ping v4: ${summary}`);

  return new Response(
    JSON.stringify({ ok: true, version: 4, count: results.length, pinged: results, ts: new Date().toISOString() }),
