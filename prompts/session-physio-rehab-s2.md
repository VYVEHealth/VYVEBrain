...
Two things ride with Wave 0 because the demo can't happen without them. First, inline play in the exercise library, both portals: the play button on a card should open the video right there — a native player for our own files, a YouTube embed for links. Right now only direct mp4s play; a YouTube link just shows a thumbnail, so the RMP rows can't be watched at all yet. Second, keep the libraries separate: a `library` column on `coach_exercises`, `strength` for everything that exists today and `rehab` for the physio set, each portal reading its own plus the partner's own rows. A coach can switch the rehab set on in settings if they want it; nobody gets it by default. Tag the RMP demo rows `rehab` in the same migration. Talk me through the schema before it runs.
