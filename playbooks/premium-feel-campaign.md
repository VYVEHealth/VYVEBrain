...
- **Status:** SHIPPED to `local-first-spike` commit `197a0d89` (PM-86 brain-restore narrative; build chat shipped between PM-82.5 and PM-86, brain commit recovered after scope-chat drift). Three Supabase reads flipped to Dexie-first in `cardio.html`: `weekly_goals` cardio-week count + workouts week count in `fetchWeek`, and `cardio` history slice 10 in `fetchHistory`. Canonical non-empty-gate three-way branch. Script chain matches canonical position; SW cache key bumped to `pm78-pf9-cardio-a`. **PF-4b Part 2 finding:** cardio's POST is a direct `fetch()`, not `VYVEData.writeQueued`, so PF-4's shadow drainer doesn't see it. Workaround: per-page optimistic `VYVELocalDB.cardio.upsert` at POST-success handler (matches PF-1 `daily_habits` pattern). `movement.html` not touched in this commit (cardio.html only) — movement quick-add path remains queued for a follow-up if/when it surfaces. Awaiting Dean's real-device verification: `localStorage.setItem('vyve_lf_spike','1')` → reload → throttle to Offline → `/cardio.html` → week count + history render from Dexie; plan hero card blank (PF-30); logging a session shows the optimistic write working with instant history update.
...
- **Estimated length:** 2-3 hours.
- **Status:** SHIPPED to `local-first-spike` commit `a248bfef` (PM-89 narrative). New `home-state-local.js` module (~25KB / 700 LOC) ports `refresh_member_home_state` (50-field row computation) + `compute_engagement_components` (4-component scoring formula) SQL functions to JS. Streak algorithm: gaps-and-islands over distinct activity_dates (per type + overall) and over iso_year*53+iso_week indices (weekly check-in streak). Capped daily count `SUM(LEAST(2, daily_count))` for sessions/workouts/cardio aggregates matches SQL. BST handling via local-date-components. `buildHomeFromDexie` upgraded to return full EF response shape with computed engagement.score, components, streak, streak_by_type, recent.* counts, weekly_goals.targets+progress. `charity_total` reads last-cached value from `vyve_home_v3_<email>` before falling to 0. `certificates` reads last-cached from `vyve_engagement_cache`. `loadDashboard()` still fires to upgrade charity_total + achievements + certificates + HK state from EF — these are genuinely server-only. TTL-cache skip-EF-when-fresh is the post-launch iteration. SW cache key bumped to `pm78-pf11b-home-b`; home-state-local.js added to SW precache. node --check passed on all 8 inline scripts in patched index.html + sw.js. Verification path: `localStorage.setItem('vyve_lf_spike','1')` → reload → throttle Offline → home renders full dashboard with real engagement score + streaks from Dexie.
...
- **Needs Dean:** verification.
- **Estimated length:** 2 hours.
- **Status:** SHIPPED 13 May 2026 PM-91 to `local-first-spike` (commit `302087de`). settings.html only — 2 reads flipped (`habit_library` catalogue with public-read cache pattern, member habits hydrate 2-up) + 6 optimistic upsert sites added at write paths (profile name, persona switch, theme, notification prefs, goal focus, privacy toggle). `certificates.html` + `engagement.html` carved out to **PF-12b** as a separate follow-up — both are read-mostly EF-backed pages (certificate-serve EF + engagement summary) and the Dexie carve-out for them is lower-impact than getting PF-13 scaffolding in. PF-12b estimated 1-2 hours. SW cache key bumped `pm90-pf30-telem-a` → `pm78-pf12-settings-a`.
...
- **Status:** SCAFFOLDING SHIPPED 13 May 2026 PM-92 to `local-first-spike` (commit `11abad83`). New file `/hydration.js` (19KB, 444 LOC) — full orchestration: `VYVEHydration.maybeShow()` invoked from index.html bootstrap before any other paint logic; COPY_TABLE with 5 personas × generic + goal-echo variants (23 entries total, all tagged `// COPY: DEAN TO FINALISE` so a regex sweep of `COPY_DEAN_FINAL` finds every line needing review); HAVEN never-echo hard rule codified three layers deep (header docstring, COPY_TABLE comment, runtime short-circuit at `goal = (persona === 'HAVEN') ? null : resolveGoal()`); SAFE_ECHO_GOALS whitelist hardcoded with 7 entries (`build strength`, `improve performance`, `find calm`, `better sleep`, `build a habit`, `understand my body`, `build a routine` — `lose weight` deliberately absent and falls through to SPARK.generic); 1500ms minimum display via `Promise.all([hydrateWait, minWait])`; once-per-device via `localStorage.vyve_hydration_shown`; 5 persona-matched animations as inline CSS (reusable by PF-27 — NOVA pulsing dot, RIVER breathing circle, SPARK ripple, SAGE rotation, HAVEN gentle wave); telemetry emits `welcome_hydration_shown` + `welcome_hydration_skipped` to PostHog. **COPY GATE OPEN — Dean to finalise** the 23 entries in COPY_TABLE before launch (~30-45 min). The current draft lines are member-displayable real sentences (no `TODO:` strings) so the scaffolding is safe to merge if launch hits before copy is finalised — but the persona voice ROI is locked away until Dean's editing pass lands. **Onboarding stash follow-up** still pending: `onboarding_v8.html` + onboarding EF response handler need to write `localStorage.vyve_persona` + `localStorage.vyve_primary_goal` alongside the auth token stash on signup-completion. Without it, the fresh-device-on-returning-member path uses the 50-100ms Dexie fallback (acceptable, below human perceptual threshold, swap is imperceptible). Animations are shared with PF-27 (2 features of build for 1 — assets reused). SW cache key bumped `pm78-pf12-settings-a` → `pm92-pf13-hydration-a`.
...
- **What:** Strip the now-meaningless perf-v2 metrics (server round-trip timing that's no longer the bottleneck) and add the metrics that actually matter post-migration. (Originally also planned to wire PostHog identity — discovered during shipping that identity was already wired in `auth.js / vyveCapturePageView` calling `posthog.identify(email)` — see PM-90 narrative + PM-93 audit; PostHog project key `phc_8gekeZglc1HBDu3d9kMuqOuRWn6HIChhnaiQi6uvonl`.)
...
- **New metrics to capture:**
  ...
  7. **PostHog identity wiring.** ~~Currently pending per memory.~~ ALREADY WIRED — discovered during PF-30 shipping (PM-90). `posthog.identify(email)` fires in `vyveCapturePageView` at auth.js L524, called adjacent to `vyveSignalAuthReady` in the bootstrap. The memory entry on "pending" was stale from the perf-v2 era; PF-30 left identity untouched.
...
- **Status:** SHIPPED 13 May 2026 PM-90 to `local-first-spike` (commit `707aa3af`). perf.js v3 strips dead metrics (the `cache_first` axis post-local-first noise, the `/functions/v1/log-perf` POST target), redirects to PostHog `capture()`. New events emitted: `perf_first_paint` + `perf_cross_nav` (sessionStorage handoff for tap→next-screen timing) + `perf_auth_ready` + `perf_paint_done` + `perf_navigation_timings` (pagehide rollup) + `perf_sync_queue_high` (fires when `_sync_queue` depth >5 on pagehide) + `dexie_hydrate_completed` (listens for `vyve-localdb-hydrated`, includes ok/failed table counts and ms). index.html telemetry hooks on PF-11a/PF-11b paint paths: `dexie_cold_paint` + `dexie_full_paint` + `dexie_engagement_drift` (stashes Dexie engagement.score in `window._vyveDexiePaint` at paint time, compares against EF's engagement.score on loadDashboard return, tagged with `known_divergence_sources: ['weekly_scores_v_wellbeing_checkins']` so we don't chase the schema gap as a bug). db.js wraps the getDB() open-failed catch with `dexie_idb_lost` (when the error message contains "Connection to Indexed Database" — the WKWebView crash-wipe shape from §3.1 mitigation A) and `dexie_open_failed` (other open errors). auth.js `posthog.init` config gains `session_recording: { maskAllInputs: true, maskInputOptions: { password: true, email: false }, recordCrossOriginIframes: false }` — 100% sampling for soft-launch week, drop to 10% +1 week after hard-launch. SW cache key bumped `pm78-pf11b-home-b` → `pm90-pf30-telem-a`. Files committed: perf.js (v2→v3 13KB→15KB), auth.js (init config), db.js (crash recovery), sw.js (cache key), index.html (paint+drift hooks). node --check passed on all JS files + all 8 index.html inline scripts. Byte-equal post-commit verification (md5 staged vs Contents API base64-decode pinned to commit SHA) confirmed all 5 files. **PostHog identity wiring confirmed already done** — `posthog.identify(email)` fires in `vyveCapturePageView` from `vyveSignalAuthReady` (auth.js L25431). The memory entry / playbook line on "identity pending" was stale. PF-30 leaves identity untouched. **Sentry deferred to PF-30b** — needs DSN from Dean (free-tier Sentry project, ~3 min). Once DSN provided, PF-30b lands the Sentry JS SDK loader in auth.js with `Sentry.setUser({email})` in the same `vyveCapturePageView` hook (post `posthog.identify`).
- **Observability stack — full picture (added PM-85):** PostHog handles product analytics — every metric in the list above plus per-member behaviour patterns (funnels, retention cohorts, feature adoption), once `posthog.identify(member_email)` is wired post-`vyveAuthReady`. Add **Sentry** alongside it as the error/crash channel — PostHog isn't a crash reporter, and the WKWebView IndexedDB "Connection to Indexed Database server lost" errors + sync drainer failures + any unhandled exception need somewhere to land. Sentry free tier covers our scale comfortably; JS SDK wiring is ~30 minutes on top of PF-30's existing scope. Plus **session replay turned up to 100% sampling for the soft-launch week** — PostHog session replay is included in the free tier up to 5,000 recordings/month, which is effectively unlimited for the 15-member cohort. Watching real sessions during the launch window is the single fastest way to catch "subtle bug no member would describe to support" issues. Drop replay sampling to 10% after hard-launch +1 week for cost discipline. **Supabase Logs** (already in the dashboard) covers backend EF errors, RLS denials, slow queries — no integration work, just remember to check it during the soft-launch week. Combined stack: PostHog for product, Sentry for crashes/errors, PostHog session replay turned up for the launch window, Supabase Logs for backend. Industry-standard early-stage SaaS observability — no exotic tooling, no per-month cost above what's already paid.
