...
|---|-----------|---------|------------------------|-------------------|----------------|--------------------|------------|
| 1 | **Supabase Inc** (on AWS) | Primary datastore, authentication, edge compute, file storage | All member records: identity (name, email, DOB, phone), auth credentials, activity/engagement logs, health and wellbeing data, AI interaction history | **Yes** — wellbeing scores, mental-health check-ins, HAVEN conversations, health questionnaire answers | **EU (AWS eu-west-1, Ireland)** — project pinned to West EU | None required (EU processing) | Supabase DPA available via dashboard — **ACTION: Lewis to countersign and file**; AWS is Supabase's listed sub-processor |
...
- All primary member data, including all special-category health data, is stored and processed in the EU (Ireland), with RLS enforced on every member-scoped table.
