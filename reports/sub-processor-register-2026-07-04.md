...
- PostHog DPA: EXECUTED + countersigned 12 Jul 2026 (PandaDoc CEIKI-EW7QC-O8PB5-RRC7F, Dean Brown Director & CTO / Charles Cook VP Operations). **OPERATING CONSTRAINT (Annex I: sensitive categories N/A): no health VALUES in PostHog event payloads — enforced in code at vyve-site `0a616ca4` (PM-771 allowlist trim: score/feeling/energy/weight/weight_kg/food_kcal removed).** Events themselves (engagement signals) still mirror.
- Anthropic: DPA incorporated by Commercial Terms acceptance (position note filed — SCCs Module 2 + UK addendum, no-training, 7-day log auto-delete, 30-day deletion, DPF participant).
- **DPA PILE COMPLETE — all 5 supplier lines closed 12 Jul 2026: Supabase (executed), Brevo (by acceptance), PostHog (executed+countersigned), Capawesome (by acceptance), Anthropic (by acceptance).** All evidence in reports/compliance/ + CC → Documents → Compliance.
